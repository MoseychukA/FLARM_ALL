...
#include "EEPROMRF.h"
 ...
 // Этот файл частично основан на https://github.com/watson/libmodes/

 // у нас есть 112-битное (14 байт) сообщение в виде шестнадцатеричных цифр, по 2 на байт
 // (28 символов) с предшествующим '*' и завершающимся ';' + <CR><LF>
 // например: *8D4B1621994420C18804887668F9;
 // (более короткие сообщения режима S имеют меньше (14) символов,
 // например: *02E198BFAF8676;
 // Или в режиме 2+ с префиксом RSSI:
 // начинается с «+», 2 символа RSSI, остальное как указано выше:
 // например: +1A8DC03ABC9901939CA00706079C17;

 // Сообщения ES транспондера (DF17) структурированы следующим образом:
 // DF: 5 бит
 // CA: 3 бита
 // ICAO ID: 24 бита
 // ME (тело сообщения): 56 бит
 // PI (CRC и т. д.): 24 бита — можно игнорировать?
...
// расшифровать высоту, закодированную по методу Гиллхэма («Gray»)
// - based on http://www.ccsinfo.com/forum/viewtopic.php?p=140960
// - требуются входные биты, подготовленные как:
...
// Декодировать 12-битное поле высоты AC (в DF 17 и других, но не в режиме S).
// Возвращает высоту или 0, если ее невозможно декодировать.
static uint32_t decode_ac12_field() {
    ...
    uint32_t alt;
    if ((msg[5] & 0x01) == 0) {    // q_bit не установлен, высота закодирована в коде Гиллхэма
/*
        byte 0-based:  5         6
