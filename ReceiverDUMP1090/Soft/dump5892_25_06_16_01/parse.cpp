...
#include "EEPROMRF.h"
 ...
 // Ётот файл частично основан на https://github.com/watson/libmodes/

 // у нас есть 112-битное (14 байт) сообщение в виде шестнадцатеричных цифр, по 2 на байт
 // (28 символов) с предшествующим '*' и завершающимс€ ';' + <CR><LF>
 // например: *8D4B1621994420C18804887668F9;
 // (более короткие сообщени€ режима S имеют меньше (14) символов,
 // например: *02E198BFAF8676;
 // »ли в режиме 2+ с префиксом RSSI:
 // начинаетс€ с Ђ+ї, 2 символа RSSI, остальное как указано выше:
 // например: +1A8DC03ABC9901939CA00706079C17;

 // —ообщени€ ES транспондера (DF17) структурированы следующим образом:
 // DF: 5 бит
 // CA: 3 бита
 // ICAO ID: 24 бита
 // ME (тело сообщени€): 56 бит
 // PI (CRC и т. д.): 24 бита Ч можно игнорировать?

#define byte2hex_msb(b) hex[((b)>>4)&0x0F]
#define byte2hex_lsb(b) hex[(b)&0x0F]

// предполагать, что данные всегда действительны и заглавные
//int hex2bin(char c) {
...
// расшифровать высоту, закодированную по методу √иллхэма (ЂGrayї)
// - based on http://www.ccsinfo.com/forum/viewtopic.php?p=140960
// - требуютс€ входные биты, подготовленные как:
...
// ƒекодировать 12-битное поле высоты AC (в DF 17 и других, но не в режиме S).
// ¬озвращает высоту или 0, если ее невозможно декодировать.
static uint32_t decode_ac12_field() {
    if (msg[5] == 0 && (msg[6]&0xF0) == 0) {
        ++msg_by_alt_cat[0];
        ++gray_count[3];
        return 0;            // все биты 0 = высота неизвестна
    }
    uint32_t alt;
    if ((msg[5] & 0x01) == 0) {    // q_bit не установлен, высота закодирована в коде √иллхэма
/*
        byte 0-based:  5         6
...
//    i += 2;      // 2 hex chars converted into one binary byte
    ...
    else if (dfs == DF20 || dfs == DFSALL || dfs == DFNOTL) 
    {
        ...
        else if (mm.frame == 0) 
        {
            ...
            Serial.println("return7");
            return false;
        }
        else 
        {                // other frames (e.g., 21)
            Serial.println("return8");
            return false;
        }
    }
    else 
    {                // dfs == D17,D18,D78 and frame is not 17 nor 18
        Serial.println("return9");
        return false;
    }
...
/*
* „тобы определить, получаете ли вы сообщение ADS-B или сообщение TIS-B,
вам следует начать с просмотра формата нисход€щей линии св€зи (DF, первые 5 бит сообщени€),
если DF = 17, то это сообщение ADS-B. ≈сли DF = 18, то вы смотрите на поле управлени€ (CF, биты 6-8).
CF = 0 и CF = 1 Ч это сообщени€ ADS-B. CF = 2,3 и 5 Ч это сообщени€ TIS-B. 
CF = 6 Ч это сообщени€ ADS-R. CF = 4 Ч это сообщени€ о состо€нии системы TIS-B / ADS-R.
