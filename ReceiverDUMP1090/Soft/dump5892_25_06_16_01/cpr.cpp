...
// 
// Адаптировано из dump1090, декодера сообщений Mode S для устройств RTLSDR.
// Оригинальный код авторских прав (C) 2012 Сальваторе Санфилиппо <antirez@gmail.com>
// Значительно изменен для эффективности - Авторские права (C) 2024 Моше Бранер <moshe.braner@gmail.com>
...
// 1090-WP29-07-Draft_CPR101 (который также определяет decodeCPR() )
//
// Несмотря на то, что говорилось в предыдущем комментарии, нам *не* следует использовать trunc().
// См. рисунок 5-5 / 5-6 и обратите внимание, что floor применяется к (0.5 + fRP - fEP), а не
// напрямую к (fRP - fEP). Уравнение 38 верно.
//
