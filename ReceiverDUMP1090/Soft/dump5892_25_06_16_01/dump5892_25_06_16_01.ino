...
void setup()
{
  ...
  //else 
  //{
     // Serial.setRxBufferSize(INPUT_BUF_SIZE);//!!
    //  Serial.begin(SERIAL_OUT_BR, SERIAL_8N1, 16, 17);//!!
     // Serial.begin(SERIAL_IN_BR, SERIAL_8N1, 16, 17);//!!
...
void input_loop()
{
    ...
    {                // ждем начала нового предложения
        if (Serial.available() > (INPUT_BUF_SIZE - 256)) //!!
        {
            // входной буфер заполняется, очистите его
            Serial.readBytes(buf, 256);  // отбрасываем некоторые входные данные//!!
            in_discard();
        }
    }
...
void parse_loop()
{
    ...
    if (inputchars > 0/* && settings->parsed != RAWFMT*/) 
    {
        if (buf[0] == '*' || buf[0] == '+') 
        {        // ADS-B data received
           // Serial.println("parsing_success");//!!
