...
(function(){
  ...
  form.addEventListener('submit', function(e){
    ...
    function postText(url, body){
      return fetch(url, {method:'POST', body:body, cache:'no-store'}).then(function(r){
    ...
    function postChunk(blob, offset, retry){
      var data=new FormData();
      data.append('chunk', blob, 'firmware.part');
      return postText('/update_chunk?offset='+encodeURIComponent(offset), data).catch(function(err){
        if(retry>0){
          setStatus('&#1055;&#1086;&#1074;&#1090;&#1086;&#1088; &#1087;&#1072;&#1082;&#1077;&#1090;&#1072;...');
          return new Promise(function(resolve){ setTimeout(resolve, 500); }).then(function(){ return postChunk(blob, offset, retry-1); });
        }
        throw err;
      });
    }
    (async function(){
      try{
        await postText('/update_begin?size='+encodeURIComponent(file.size), null);
        var chunkSize=8192;
        var sent=0;
        while(sent<file.size){
          var end=Math.min(sent+chunkSize,file.size);
          setStatus('&#1055;&#1077;&#1088;&#1077;&#1076;&#1072;&#1095;&#1072; '+Math.round((sent*100)/file.size)+'%...');
          await postChunk(file.slice(sent,end), sent, 3);
          sent=end;
          setProgress(Math.round((sent*100)/file.size));
        }
        setStatus('&#1047;&#1072;&#1074;&#1077;&#1088;&#1096;&#1077;&#1085;&#1080;&#1077; &#1079;&#1072;&#1087;&#1080;&#1089;&#1080;...');
        await postText('/update_end', null);
        setProgress(100);
...
static void handleUpdateUpload()
{
    ...
    else if (upload.status == UPLOAD_FILE_ABORTED)
    {
        ...
        if (g_otaExpectedSize != 0U && g_otaWritten >= g_otaExpectedSize && !Update.hasError())
        {
            g_otaTotal = g_otaExpectedSize;
            if (Update.end(true))
            {
