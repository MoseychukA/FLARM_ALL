...
static void RF_ApplySelectedProtocol()
{
    ...
    case RF_PROTOCOL_MAVLINK:
        /*
         * MAVLink передаем поверх того же стабильного физического LoRa-профиля,
         * который уже использовался для OGNTP. Меняем только кодек полезной
         * нагрузки, а не внутренний дескриптор радиопротокола LMIC.
         * Это исключает аварии при работе низкоуровневого драйвера SX127x.
         */
    ...
    /* Частотный план для MAVLink over LoRa используем тот же, что и для OGNTP. */
    RF_FreqPlan.setPlan(settings->band, RF_PROTOCOL_OGNTP);
...
uint8_t RF_Payload_Size(uint8_t protocol)
{
    ...
    case RF_PROTOCOL_MAVLINK:   return MAVLINK_LORA_PAYLOAD_SIZE;
    default:                    return 0;
    }
}
