...
static String hardwarePage()
{
    ...
    html += F("<div class='settingsBlock'><div class='blockTitle'>WiFi</div><table>");
    html += F("<tr><th>WiFi</th><td><select class='wLong hardwareCtl30 compactSelect' style='width:210px;min-width:210px;max-width:210px;' name='wifi_mode'>");
    html += String("<option ") + sel((settings ? settings->wifi_mode : WIFI_CONTROL_ON) == WIFI_CONTROL_OFF) + " value='0'>Off</option>";
    html += String("<option ") + sel((settings ? settings->wifi_mode : WIFI_CONTROL_ON) == WIFI_CONTROL_AP) + " value='1'>AP</option>";
    html += String("<option ") + sel((settings ? settings->wifi_mode : WIFI_CONTROL_ON) == WIFI_CONTROL_ON) + " value='2'>On</option>";
    html += F("</select></td></tr>");
    html += F("<tr><th>SSID</th><td>");
    html += WiFi_ssid();
    ...
    html += LAN_dhcpLeaseActive() ? F(" / DHCP") : F(" / Static");
    html += F("</td></tr></table></div>");
    html += F("<div class='settingsBlock'><div class='blockTitle'>Bluetooth</div><table>");
    ...
    html += String("<option ") + sel((settings ? settings->bluetooth : BLUETOOTH_OFF) == BLUETOOTH_OFF) + " value='0'>Service only</option>";
    html += String("<option ") + sel((settings ? settings->bluetooth : BLUETOOTH_OFF) == BLUETOOTH_LE) + " value='1'>BLE UART</option>";
    html += String("<option ") + sel((settings ? settings->bluetooth : BLUETOOTH_OFF) == BLUETOOTH_AUTO) + " value='2'>Auto (BLE)</option>";
    html += F("</select></td></tr>");
    html += F("<tr><th>Name</th><td>");
    html += Bluetooth_name();
    html += F("</td></tr>");
    html += F("<tr><th>State</th><td>");
    html += Bluetooth_active() ? (Bluetooth_connected() ? F("Connected") : F("Advertising")) : F("Off");
    html += F("</td></tr>");
    html += F("<tr><th>NMEA stream</th><td>");
    html += (settings && settings->nmea_out == NMEA_OUTPUT_BLUETOOTH && settings->bluetooth != BLUETOOTH_OFF) ? F("Enabled by NMEA output") : F("Off, commands only");
    html += F("</td></tr></table></div>");
...
static String hardwareSavedPage(bool defaultsLoaded)
{
    ...
    html += F("<tr><th>WiFi</th><td>"); html += WiFi_controlModeName(settings ? settings->wifi_mode : WIFI_CONTROL_ON); html += F("</td></tr>");
    ...
    html += F("<tr><th>Bluetooth</th><td>"); html += bluetoothModeName(settings ? settings->bluetooth : BLUETOOTH_OFF); html += F("</td></tr>");
    html += F("<tr><th>Bluetooth NMEA stream</th><td>"); html += (settings && settings->nmea_out == NMEA_OUTPUT_BLUETOOTH && settings->bluetooth != BLUETOOTH_OFF) ? F("On") : F("Off"); html += F("</td></tr>");
...
static String outputsSavedPage()
{
    ...
    html += F("<tr><th>Bluetooth</th><td>"); html += bluetoothModeName(settings ? settings->bluetooth : BLUETOOTH_OFF); if (settings) { html += F(" / "); html += Bluetooth_name(); } html += F("</td></tr>");
...
static void handleHardwareLive()
{
    ...
    else
    {
        ...
        if (server.hasArg("wifi_mode")) settings->wifi_mode = (uint8_t)clampInt(server.arg("wifi_mode").toInt(), WIFI_CONTROL_OFF, WIFI_CONTROL_ON);
        ...
        if (server.hasArg("bluetooth")) settings->bluetooth = (uint8_t)clampInt(server.arg("bluetooth").toInt(), BLUETOOTH_OFF, BLUETOOTH_AUTO);
