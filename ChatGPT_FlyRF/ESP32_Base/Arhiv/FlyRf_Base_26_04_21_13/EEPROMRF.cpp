...
static void setDefaultsBody()
{
    ...
    eeprom_block.field.settings.nmea_out = 1;
    ...
    eeprom_block.field.settings.g_localIP[3] = 100;
    ...
    eeprom_block.field.settings.raw_lora_tft = 0;
    eeprom_block.field.settings.raw_lora_serial = 0;
...
void EEPROM_setup(void)
{
    ...
    if (settings != nullptr)
    {
        ...
        if (settings->raw_lora_tft > 1)
        {
            settings->raw_lora_tft = 0;
            changed = true;
        }
        if (settings->raw_lora_serial > 1)
        {
            settings->raw_lora_serial = 0;
            changed = true;
        }
        if (settings->block_addr > 0xFFFFFFUL)
        {
            settings->block_addr = 0;
            changed = true;
        }
        if (settings->block_addr2 > 0xFFFFFFUL)
        {
            settings->block_addr2 = 0;
            changed = true;
        }
        if (settings->block_addr3 > 0xFFFFFFUL)
        {
            settings->block_addr3 = 0;
            changed = true;
        }
