...
namespace
{
    ...
    static char g_activeTextMessage[BUFFER_SIZE] = {};
...
#ifndef TRACKER_CONFIRM_BUTTON_PIN
#define TRACKER_CONFIRM_BUTTON_PIN 45
#endif
#ifndef TRACKER_CONFIRM_BUTTON_ACTIVE_LEVEL
#define TRACKER_CONFIRM_BUTTON_ACTIVE_LEVEL LOW
#endif
#ifndef TRACKER_CONFIRM_BUTTON_DEBOUNCE_MS
#define TRACKER_CONFIRM_BUTTON_DEBOUNCE_MS 30UL
#endif
    static bool g_confirmButtonStable = false;
    static bool g_confirmButtonLastSample = false;
    static uint32_t g_confirmButtonLastChangeMs = 0U;
    ...
    static void trackerReplyMessageReceipt(unsigned long messageId)
    {
        char buffer[20];
    ...
    static void trackerStoreTextMessage(unsigned long messageId, const char* text)
    {
        uint8_t hourNow = 0U;
        uint8_t minuteNow = 0U;
        trackerCurrentTime(hourNow, minuteNow);

        char prefix[12];
        snprintf(prefix, sizeof(prefix), "%02u:%02u ", (unsigned)hourNow, (unsigned)minuteNow);

        memset(g_activeTextMessage, 0, sizeof(g_activeTextMessage));
        strncpy(g_activeTextMessage, prefix, sizeof(g_activeTextMessage) - 1U);
        if (text != nullptr && *text != '\0')
        {
    ...
    static void trackerPollConfirmButton()
    {
        const uint32_t nowMs = millis();
        const bool samplePressed = (digitalRead(TRACKER_CONFIRM_BUTTON_PIN) == TRACKER_CONFIRM_BUTTON_ACTIVE_LEVEL);

        if (samplePressed != g_confirmButtonLastSample)
        {
            g_confirmButtonLastSample = samplePressed;
            g_confirmButtonLastChangeMs = nowMs;
        }

        if ((uint32_t)(nowMs - g_confirmButtonLastChangeMs) < TRACKER_CONFIRM_BUTTON_DEBOUNCE_MS)
        {
            return;
        }

        if (samplePressed != g_confirmButtonStable)
        {
            g_confirmButtonStable = samplePressed;
            if (g_confirmButtonStable && g_hasActiveTextMessage && g_allowReadConfirm)
            {
    ...
    static bool handleSetTxt(unsigned long messageId, char* params)
    {
        ...
        if (timePart != nullptr && *timePart != '\0')
        {
            uint8_t msgHour = 0U;
        ...
        trackerStoreTextMessage(messageId, textPart);
        trackerReplyMessageReceipt(messageId);
        return true;
    }
...
void Tracker_setup()
{
    ...
    g_rxBuffer[0] = '\0';
    g_lastRxMs = millis();
    pinMode(TRACKER_CONFIRM_BUTTON_PIN, INPUT_PULLUP);
    g_confirmButtonLastSample = (digitalRead(TRACKER_CONFIRM_BUTTON_PIN) == TRACKER_CONFIRM_BUTTON_ACTIVE_LEVEL);
    g_confirmButtonStable = g_confirmButtonLastSample;
    g_confirmButtonLastChangeMs = g_lastRxMs;
