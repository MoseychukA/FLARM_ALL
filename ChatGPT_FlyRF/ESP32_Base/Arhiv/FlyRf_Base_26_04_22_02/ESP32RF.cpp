...
static void drawTrackerTextMessageBlock()
{
    ...
    char lineBuf[128];
    for (int line = 0; line < 4 && *src != '\0'; ++line)
    {
        while (*src == ' ')
        {
            ++src;
        ...
        size_t bytesUsed = copyUtf8PrefixByChars(src, charsPerLine, lineBuf, sizeof(lineBuf));
        if (bytesUsed == 0)
        {
            break;
        }
        ...
        if (src[bytesUsed] != '\0')
        {
            ...
            if (breakPos != nullptr)
            {
                lineBuf[breakPos - lineBuf] = '\0';
                bytesUsed = (size_t)(breakPos - lineBuf) + 1U;
            }
        }

        back.drawString(lineBuf, boxX + 6, boxY + 5 + line * lineHeight, 2);
        src += bytesUsed;
    }
