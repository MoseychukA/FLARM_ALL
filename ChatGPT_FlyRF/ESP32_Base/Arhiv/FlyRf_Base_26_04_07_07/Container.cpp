...
static uint32_t normalizeTrafficAddr(uint32_t addr)
{
    return addr & 0xFFFFFFUL;
}

static bool isBlockedTrafficAddr(uint32_t addr)
{
    if (settings == nullptr)
    {
        return false;
    ...
    const uint32_t blockedAddr = normalizeTrafficAddr(settings->block_addr);
    if (blockedAddr == 0U)
    {
        return false;
    }

    return normalizeTrafficAddr(addr) == blockedAddr;
}
...
bool Traffic_Add(ufo_t *fop)
{
    ...
    if (isBlockedTrafficAddr(fop->addr))
    {
        return false;
    }

    fop->addr = normalizeTrafficAddr(fop->addr);

    if (fop->addr == normalizeTrafficAddr(ThisAircraft.addr))
    {
        return false;
    }
...
void RemoveBlockedTrafficFromContainer()
{
    const uint32_t blockedAddr = (settings != nullptr) ? normalizeTrafficAddr(settings->block_addr) : 0U;
    if (blockedAddr == 0U)
    {
        return;
    }
    ...
    for (int i = 0; i < MAX_TRACKING_OBJECTS; ++i)
    {
        if (normalizeTrafficAddr(Container[i].addr) == blockedAddr)
        {
            Container[i] = EmptyFO;
        }
    }
