...
{
    if (!g_displayReady || settings == nullptr || !settings->raw_lora_tft)
    {
        return;
    }
...
void displayAllPlanes() // Вывести текстовую информацию по самолетам на дисплей
{
    ...
    if (settings != nullptr && settings->raw_lora_tft)
...
static bool shouldDrawTestCoordinatesBlock()
{
    ...
    return settings->view_test_coord == VIEW_COORD_ON;
}
...
static void drawBottomRightGnssBlock()
{
    ...
    const int yTop = shouldDrawTestCoordinatesBlock() ? 238 : 270;
    const int dy = 15;
    char lineBuf[64];
