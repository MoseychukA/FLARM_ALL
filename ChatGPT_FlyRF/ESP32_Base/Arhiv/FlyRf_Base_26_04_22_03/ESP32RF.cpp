...
static void drawTrackerTextMessageBlock()
{
    ...
    back.fillRoundRect(boxX, boxY, boxW, boxH, 4, TFT_BLACK);
    back.setTextDatum(TL_DATUM);
    back.setTextColor(TFT_YELLOW, TFT_BLACK);
    back.setAttribute(UTF8_SWITCH, true);
    back.loadFont(AA_FONT_TIME24);

    char drawBuf[BUFFER_SIZE];
    strncpy(drawBuf, message, sizeof(drawBuf) - 1U);
    drawBuf[sizeof(drawBuf) - 1U] = '\0';

    while (drawBuf[0] != '\0' && back.textWidth(drawBuf) > (boxW - 6))
    {
        size_t len = strlen(drawBuf);
        if (len == 0U)
        {
            break;
        }
        ...
        size_t cut = len;
        while (cut > 0U && (static_cast<uint8_t>(drawBuf[cut - 1U]) & 0xC0U) == 0x80U)
        {
            --cut;
        }
        if (cut > 0U)
    ...
    back.drawString(drawBuf, boxX + 2, boxY + 2);
