...
static void drawStatusInfoBlock()
{
    ...
    back.setTextColor(TFT_DARKGREY, TFT_BLACK);
    back.drawString(String("AP ") + (WiFi_ready() ? String("On") : String("Off")), xRight, y0);
    back.drawString(String("IP ") + WiFi_apIP().toString(), xRight, y0 + dy);

    const String ssidLabel = String("SSID ") + String(WiFi_ssid());
    const int maxSsidChars = 22;
    String ssidText = ssidLabel;
    if ((int)ssidText.length() > maxSsidChars)
    {
        ssidText = ssidText.substring(0, maxSsidChars - 1) + String("…");
    }

    back.setTextColor(TFT_DARKGREY, TFT_BLACK);
    back.drawString(ssidText, xRight, y0 + dy * 2);

    uint32_t txPackets = 0;
    uint32_t rxPackets = 0;
    RF_GetPacketCounters(txPackets, rxPackets);
    back.drawString(String("LoRa Tx ") + String(txPackets) + String("  Rx ") + String(rxPackets), xRight, y0 + dy * 3);
}
