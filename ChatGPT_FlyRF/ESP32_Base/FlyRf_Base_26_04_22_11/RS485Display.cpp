...
static aux_t g_auxTx = {};  // Параметр геометрии, координаты, размера или угла.
static aux_t g_auxRx = {};  // Параметр геометрии, координаты, размера или угла.
static uint8_t g_btn1 = 0;  // Объект внешнего интерфейса, экрана, порта или канала связи.
static uint8_t g_btn2 = 0;  // Объект внешнего интерфейса, экрана, порта или канала связи.
static volatile uint8_t g_pendingButton = 0;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
static volatile uint32_t g_lastRxTickMs = 0;  // Временная отметка, интервал или значение тайм-аута.
static uint32_t previousMillis = 0;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
static const uint32_t interval = 1000UL;  // Временная отметка, интервал или значение тайм-аута.
static SemaphoreHandle_t rs485Mutex = nullptr;  // Параметр геометрии, координаты, размера или угла.
static SemaphoreHandle_t auxMutex = nullptr;  // Параметр геометрии, координаты, размера или угла.
static TaskHandle_t g_rs485RxTask = nullptr;  // Параметр геометрии, координаты, размера или угла.
...
{
    uint16_t crc = 0x0000;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
{
    uint8_t v = g_pendingButton;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    g_pendingButton = 0;
    return v;
}
...
bool receivePacket_RS485(full_packet_net_t* pkt, uint8_t* btn1, uint8_t* btn2)
{
    ...
    static size_t idx = 0;  // Счетчик, индекс, позиция или номер элемента.

    const int READ_BUDGET = 256;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    int read_left = READ_BUDGET;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    if (rs485Mutex) xSemaphoreTake(rs485Mutex, portMAX_DELAY);
    ...
    size_t start = 0;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    for (;;)
    {
        if (idx - start < 4)
        {
...
void rxTask(void* param)
{
    ...
    static full_packet_net_t recpkt;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    uint8_t btn1 = 0, btn2 = 0;  // Объект внешнего интерфейса, экрана, порта или канала связи.
    for (;;)
    {
        esp_task_wdt_reset();
...
void RS485Display_loop()
{
    ...
    static full_packet_net_t packet = {};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    memset(&packet, 0, sizeof(packet));
    ufo_t self = EmptyFO;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    ...
    aux_t auxCopy = {};  // Параметр геометрии, координаты, размера или угла.
