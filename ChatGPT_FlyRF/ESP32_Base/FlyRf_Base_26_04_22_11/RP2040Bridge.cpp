...
static RP2040BridgeDiag g_diag = {};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
static uint8_t g_rxBuffer[RP2040BRIDGE_PACKET_SIZE] = {0};  // Буфер, текстовая строка или рабочее сообщение.
static size_t g_rxIndex = 0;  // Счетчик, индекс, позиция или номер элемента.
static uint16_t g_gainValues[RP2040BRIDGE_GAIN_VALUES_MAX] = {0U};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
static uint8_t g_gainCount = 1U;  // Счетчик, индекс, позиция или номер элемента.
static TaskHandle_t g_rp2040Task = nullptr;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
{
    uint32_t temp = 0;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    memcpy(&temp, &val, sizeof(temp));
    temp = swap32(temp);
    float out = 0.0f;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
bool RP2040Bridge_processPacket(const uint8_t* data, size_t len)
{
    ...
    ToDUMP1090_RAW inRaw = {};  // Параметр геометрии, координаты, размера или угла.
    ToDUMP1090 packet = {};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
{
    static uint32_t lastGainMs = 0;  // Временная отметка, интервал или значение тайм-аута.
