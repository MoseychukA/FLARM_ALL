...
static uint32_t g_flarmPackets = 0;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
static FlarmRawPacket g_lastPacket = {};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
unsigned long TxTimeMarker = 0;  // Временная отметка, интервал или значение тайм-аута.
byte TxBuffer[MAX_PKT_SIZE] __attribute__((aligned(sizeof(uint32_t))));

static portMUX_TYPE g_packetCountersMux = portMUX_INITIALIZER_UNLOCKED;  // Счетчик, индекс, позиция или номер элемента.
volatile uint32_t tx_packets_counter = 0;  // Счетчик, индекс, позиция или номер элемента.
volatile uint32_t rx_packets_counter = 0;  // Счетчик, индекс, позиция или номер элемента.

int8_t RF_last_rssi = 0;  // Числовой параметр навигации, радиообмена, геометрии или измерения.

FreqPlan RF_FreqPlan;  // Параметр конфигурации интерфейса, адресации или выбранного режима.

static size_t RF_tx_size = 0;  // Параметр геометрии, координаты, размера или угла.
static volatile bool g_rxPacketPending = false;  // Параметр геометрии, координаты, размера или угла.
static size_t g_lastRxSize = 0;  // Буфер, текстовая строка или рабочее сообщение.
static uint32_t g_lastRfPacketMs = 0;  // Временная отметка, интервал или значение тайм-аута.
static uint32_t g_lastRfRefreshMs = 0;  // Временная отметка, интервал или значение тайм-аута.
...
static const uint32_t kRfRxRefreshMs = 120000UL;  // Временная отметка, интервал или значение тайм-аута.
static const uint32_t kRfNoPacketRestartMs = 900000UL;  // Временная отметка, интервал или значение тайм-аута.

static uint8_t g_lastAppliedLoRaProfile = 0xFF;  // Параметр конфигурации интерфейса, адресации или выбранного режима.
static uint8_t g_lastAppliedLoRaFixedMode = 0xFF;  // Параметр конфигурации интерфейса, адресации или выбранного режима.
static uint8_t g_lastAppliedLoRaFixedFreq = 0xFF;  // Параметр конфигурации интерфейса, адресации или выбранного режима.
static uint8_t g_lastAppliedRfProtocol = 0xFF;  // Счетчик, индекс, позиция или номер элемента.
static bool g_rfRuntimeReconfigurePending = false;  // Временная отметка, интервал или значение тайм-аута.
...
static uint8_t getLoRaProfile()
{
    ...
    uint8_t profile = settings->lora_profile;  // Параметр конфигурации интерфейса, адресации или выбранного режима.
    if (profile > 3U)
    {
        profile = 0U;
    }
...
{
    uint8_t idx = settings ? settings->lora_fixed_freq : 0;  // Счетчик, индекс, позиция или номер элемента.
    if (idx >= (sizeof(kLoRaFixedFreqHz) / sizeof(kLoRaFixedFreqHz[0]))) idx = 0;
    return idx;
}

const rfchip_ops_t* rf_chip = NULL;  // Параметр геометрии, координаты, размера или угла.
bool RF_SX12XX_RST_is_connected = true;  // Параметр геометрии, координаты, размера или угла.

size_t(*protocol_encode)(void*, ufo_t*);
bool   (*protocol_decode)(void*, ufo_t*, ufo_t*);

static Slots_descr_t Time_Slots, * ts;
static uint8_t       RF_timing = RF_TIMING_INTERVAL;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
extern bool sx12xx_receive_active;  // Логический флаг состояния, разрешения или наличия данных.
extern int8_t sx12xx_channel_prev;  // Параметр конфигурации интерфейса, адресации или выбранного режима.
...
{
    char buf[24];  // Буфер, текстовая строка или рабочее сообщение.
...
bool RF_GetLoraRuntimeInfo(RF_LoraRuntimeInfo &info)
{
    ...
    if (!g_rfRuntimeReconfigurePending && rf_chip->type == RF_IC_SX1276)
    {
        ...
        const uint8_t outputPower = regPaConfig & 0x0F;  // Параметр геометрии, координаты, размера или угла.
...
String RF_GetLoraProfileDetailsText(void)
{
    ...
    char buf[128];  // Буфер, текстовая строка или рабочее сообщение.
...
static void printDecodedLoRaPacketToSerial(const ufo_t *pkt, int16_t rawRssi, size_t rxSize)
{
    ...
    char callsign[9] = {};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
{
    String str = "";  // Буфер, текстовая строка или рабочее сообщение.
    for (int i = 0; i < size; i++) {
        byte c = buffer[i];  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
uint8_t parity(uint32_t x) {
    uint8_t parity = 0;  // Параметр геометрии, координаты, размера или угла.
    while (x > 0) {
        if (x & 0x1) {
            parity++;
        }
...
byte RF_setup()
{
    ...
    if (rf_chip)
    {
        ...
        const rf_proto_desc_t* p;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        ...
        uint16_t duration = ts->s0.duration + ts->s1.duration;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
static void RF_ApplySelectedProtocol()
{
    ...
    const rf_proto_desc_t* p = LMIC.protocol ? LMIC.protocol : &ogntp_proto_desc;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    RF_timing = p->tm_type;
    ts = &Time_Slots;
    ts->air_time = p->air_time;
    ts->interval_min = p->tx_interval_min;
    ...
    ts->s0.duration = p->slot0.end - p->slot0.begin;
    ts->s1.duration = p->slot1.end - p->slot1.begin;
    const uint16_t duration = ts->s0.duration + ts->s1.duration;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    ts->adj = duration > ts->interval_mid ? 0 : (ts->interval_mid - duration) / 2;
}
...
static void RF_ServiceRuntimeSettingsChange()
{
    ...
    const uint8_t fixedMode = settings->lora_fixed_channel;  // Параметр конфигурации интерфейса, адресации или выбранного режима.
    const uint8_t fixedFreq = getLoRaFixedFreqIndex();
    const uint8_t rfProtocol = settings->rf_protocol;  // Счетчик, индекс, позиция или номер элемента.
...
{
    size_t size = 0;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
int count_test = 0;  // Счетчик, индекс, позиция или номер элемента.
...
{
    ufo_t selfAircraft = EmptyFO;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
{
    bool rval = false;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
osjob_t sx12xx_txjob;  // Параметр геометрии, координаты, размера или угла.
osjob_t sx12xx_timeoutjob;  // Временная отметка, интервал или значение тайм-аута.

static void sx12xx_tx_func(osjob_t* job);
static void sx12xx_rx_func(osjob_t* job);
static void sx12xx_rx(osjobcb_t func);

static bool sx12xx_receive_complete = false;  // Параметр геометрии, координаты, размера или угла.
bool sx12xx_receive_active = false;  // Логический флаг состояния, разрешения или наличия данных.
static bool sx12xx_transmit_complete = false;  // Параметр геометрии, координаты, размера или угла.
...
{
    uint8_t val;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
static void sx12xx_channel(int8_t channel)
{
    if (channel != -1 && channel != sx12xx_channel_prev)
    {
        ...
        int8_t fc = 0;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
{
    bool success = false;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    ...
    if (sx12xx_receive_complete == true)
    {
        ...
        u1_t size = LMIC.dataLen - LMIC.protocol->payload_offset - LMIC.protocol->crc_size;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
static void sx12xx_transmit()
{
    ...
    unsigned long tx_timeout = 60;  // Временная отметка, интервал или значение тайм-аута.
    if (LMIC.protocol)
    {
...
static void sx12xx_rx_func(osjob_t* job)
{
    ...
    u1_t i;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
static void sx12xx_tx(unsigned char* buf, size_t size, osjobcb_t func) {
    ...
    u1_t crc8;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    u2_t crc16;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
void ParseData()
{
    ...
    fo = EmptyFO;
    const uint8_t protocol = settings ? settings->rf_protocol : RF_PROTOCOL_OGNTP;  // Счетчик, индекс, позиция или номер элемента.
