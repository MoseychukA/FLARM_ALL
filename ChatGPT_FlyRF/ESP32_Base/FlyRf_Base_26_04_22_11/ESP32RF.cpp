...
TFT_eSprite state_SOS_button = TFT_eSprite(&tft);   // Спрайт индикации состояния/статуса кнопки SOS.
...
static uint8_t takeDisplayButtonEvent()
{
    ...
    const uint8_t event = g_pendingDisplayButton;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    g_pendingDisplayButton = 0;
    return event;
}
...
void Display_setup()
{
   ...
   // byte rval = DISPLAY_NONE;
    ...
    uint16_t tbw1;  // Параметр геометрии, координаты, размера или угла.
    ...
    bool spriteInitOk = true;  // Объект внешнего интерфейса, экрана, порта или канала связи.
    ...
    int a = 270;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    for (int i = 0; i < 360; i++)
    {
...
static bool block_plane_display = false;  // Параметр геометрии, координаты, размера или угла.
int line_Sprite = 0;                      // текущая линия для вывода (строк без дыр)
static int prev_lines_count = 0;  // Счетчик, индекс, позиция или номер элемента.
...
static void formatRawPacketHexLines(const FlarmRawPacket& packet, char* line1, size_t line1Size, char* line2, size_t line2Size)
{
    ...
    static const char hexDigits[] = "0123456789ABCDEF";  // Параметр геометрии, координаты, размера или угла.
    char fullHex[(FLARM_RFM95_MAX_PACKET * 2U) + 1U];
    size_t outPos = 0;  // Счетчик, индекс, позиция или номер элемента.
    ...
    {
        const uint8_t b = packet.data[i];  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        fullHex[outPos++] = hexDigits[(b >> 4) & 0x0F];
        fullHex[outPos++] = hexDigits[b & 0x0F];
    }
    fullHex[outPos] = 0;

    const size_t splitPos = 28U;  // Счетчик, индекс, позиция или номер элемента.
    if (line1 && line1Size)
    {
...
//------------------------------------------------------------------------------
// Назначение функции: Формирует и выводит данные на экран или во внешний интерфейс.
// Локальные переменные:
...
static void drawRawLoRaPacketBlock()
{
    ...
    const int boxX = 2;  // Параметр геометрии, координаты, размера или угла.
    const int boxY = 22;  // Параметр геометрии, координаты, размера или угла.
    const int boxW = 476;  // Параметр геометрии, координаты, размера или угла.
    const int boxH = 38;  // Параметр геометрии, координаты, размера или угла.
    ...
    FlarmRawPacket packet = {};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    ...
    char headBuf[48];  // Буфер, текстовая строка или рабочее сообщение.
    ...
    char line1[40];  // Счетчик, индекс, позиция или номер элемента.
    char line2[40];  // Счетчик, индекс, позиция или номер элемента.
...
void displayAllPlanes() // Вывести текстовую информацию по самолетам на дисплей
{
    ...
    line_Sprite = 0;
    bool has_data_to_display = false;  // Параметр геометрии, координаты, размера или угла.
    ...
    char buf[32];  // Буфер, текстовая строка или рабочее сообщение.
    char fltBuf[24];  // Буфер, текстовая строка или рабочее сообщение.
...
{
    int koeff_line = 10;  // Счетчик, индекс, позиция или номер элемента.
...
unsigned long lastToggle = 0;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
const unsigned long showPeriod = 4000;     // 4 секунды показа
const unsigned long hidePeriod = 1000;     // 1 секунда скрытия
bool isMessageVisible = false;  // Логический флаг состояния, разрешения или наличия данных.
bool isMessageDeleted = false;  // Логический флаг состояния, разрешения или наличия данных.
...
{
    int32_t ringMeters;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    int32_t maxVisibleMeters;  // Логический флаг состояния, разрешения или наличия данных.
    const char* label;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
};
...
static float radarDistanceToPixels(const RadarScaleDef& scale, float distanceMeters)
{
    ...
    const float outerRadiusSpan = outerRadiusPx - ringRadiusPx;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
...
{
    int16_t labelRelX;  // Параметр геометрии, координаты, размера или угла.
    int16_t labelRelY;  // Параметр геометрии, координаты, размера или угла.
    bool labelOnLeft;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
};

struct RadarPlacedSymbol
{
    int16_t absX;  // Параметр геометрии, координаты, размера или угла.
    int16_t absY;  // Параметр геометрии, координаты, размера или угла.
...
struct RadarPlacedLabel
{
    int16_t absX;  // Параметр геометрии, координаты, размера или угла.
    int16_t absY;  // Параметр геометрии, координаты, размера или угла.
    int16_t width;  // Параметр геометрии, координаты, размера или угла.
    int16_t height;  // Параметр геометрии, координаты, размера или угла.
    bool valid;  // Логический флаг состояния, разрешения или наличия данных.
};
...
static constexpr int16_t kRadarLabelSpriteWidth = 55;  // Объект внешнего интерфейса, экрана, порта или канала связи.
static constexpr int16_t kRadarLabelSpriteHeight = 15;  // Объект внешнего интерфейса, экрана, порта или канала связи.
static constexpr int16_t kRadarTrendArrowGlyphWidth = 7;  // Счетчик, индекс, позиция или номер элемента.
static constexpr int16_t kRadarTrendArrowGlyphHeight = 9;  // Счетчик, индекс, позиция или номер элемента.
static constexpr int16_t kRadarTrendArrowGlyphGapPx = 0;  // Счетчик, индекс, позиция или номер элемента.
static constexpr int16_t kRadarTargetDotRadiusPx = 6;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
static constexpr int16_t kRadarLabelGapPx = 0;  // Параметр геометрии, координаты, размера или угла.
static constexpr int16_t kRadarOverlayScreenMinX = 0;  // Параметр геометрии, координаты, размера или угла.
static constexpr int16_t kRadarOverlayScreenMaxX = 480;  // Параметр геометрии, координаты, размера или угла.
static constexpr int16_t kRadarOverlayScreenMinY = 0;  // Параметр геометрии, координаты, размера или угла.
static constexpr int16_t kRadarOverlayScreenMaxY = 320;  // Параметр геометрии, координаты, размера или угла.
static constexpr int16_t kRadarOverlayBlockWidth = kRadarLabelSpriteWidth;  // Параметр геометрии, координаты, размера или угла.
static constexpr int16_t kRadarOverlayBlockHeight = kRadarLabelSpriteHeight;  // Параметр геометрии, координаты, размера или угла.
static constexpr int16_t kRadarTargetCollisionRadiusPx = 18;  // Счетчик, индекс, позиция или номер элемента.
static constexpr int16_t kRadarLabelCollisionPaddingPx = 2;  // Счетчик, индекс, позиция или номер элемента.
static constexpr uint8_t kRadarOverlayGapVariantsCount = 6;  // Счетчик, индекс, позиция или номер элемента.
static const int16_t kRadarOverlayGapVariants[kRadarOverlayGapVariantsCount] = {0, 4, 8, 12, 16, 20};  // Параметр геометрии, координаты, размера или угла.
...
{
    int16_t value = angleDeg % 360;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    if (value < 0)
    {
        value += 360;
    }
...
static RadarOverlayLayout radarBuildOverlayLayout(int16_t targetRelX, int16_t targetRelY, int16_t courseDeg,
                                                  ...
                                                  const RadarPlacedLabel* placedLabels, int placedLabelCount)
{
    RadarOverlayLayout bestLayout = {};  // Параметр геометрии, координаты, размера или угла.
    const int16_t targetAbsX = kRadarPivotX + targetRelX;  // Параметр геометрии, координаты, размера или угла.
    const int16_t targetAbsY = kRadarPivotY - targetRelY;  // Параметр геометрии, координаты, размера или угла.
    ...
    int bestPenalty = 32767;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
...
static void radarDrawTargetDirect(TFT_eSprite& sprite, int16_t centerX, int16_t centerY, int16_t courseDeg, int16_t speedView, uint16_t color)
{
    ...
    const float startDist = 5.0f;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
static void radarDrawLabelDirect(TFT_eSprite& sprite, int16_t x, int16_t y, const char* text, uint8_t trendArrow, uint16_t color, bool labelNearTargetOnRight)
{
    ...
    int16_t textX = x + 2;  // Буфер, текстовая строка или рабочее сообщение.
...
{
    const int x = kRadarOwnshipCenterX;  // Параметр геометрии, координаты, размера или угла.
    const int y = kRadarOwnshipCenterY;  // Параметр геометрии, координаты, размера или угла.
...
static void drawGnssStatusOverlay()
{
    ...
    const bool waitingGps = GNSS_waitingForInitialFix() || GNSS_waitingForRecovery();
    const bool noGpsData = GNSS_noDataTimeout();
    if (!waitingGps && !noGpsData)
    {
        return;
    }

    const int boxX = 54;  // Параметр геометрии, координаты, размера или угла.
    const int boxY = 110;  // Параметр геометрии, координаты, размера или угла.
    const int boxW = 372;  // Параметр геометрии, координаты, размера или угла.
    const int boxH = 92;  // Параметр геометрии, координаты, размера или угла.

    back.fillRoundRect(boxX, boxY, boxW, boxH, 8, TFT_BLACK);
    back.drawRoundRect(boxX, boxY, boxW, boxH, 8, TFT_DARKGREY);
    back.setTextColor(TFT_YELLOW, TFT_BLACK);
    back.setTextDatum(MC_DATUM);
    back.loadFont(AA_FONT_TIME18);

    if (waitingGps)
    {
        back.drawString("ОПРЕДЕЛЕНИЕ МЕСТОПОЛОЖЕНИЯ", 240, 142);
        back.drawString("ОЖИДАЙТЕ ....", 240, 172);
    }
    else
    {
        back.drawString("ДАННЫЕ GPS НЕ ПОЛУЧЕНЫ", 240, 157);
    }
...
static void drawTopLeftTimeBlock()
{
    ...
    char timeBuf[24];  // Временная отметка, интервал или значение тайм-аута.
    if (settings != nullptr && FlyRfMode_usesLocalCoordinates(settings->mode))
    {
...
static size_t copyUtf8PrefixByChars(const char* src, size_t maxChars, char* dst, size_t dstSize)
{
    ...
    size_t bytesOut = 0;  // Параметр геометрии, координаты, размера или угла.
    size_t charsOut = 0;  // Параметр геометрии, координаты, размера или угла.

    while (*p != 0 && charsOut < maxChars)
    {
        size_t cpLen = 1;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
static size_t wrapTrackerMessageLines(TFT_eSprite& spr, const char* text, char lines[][160], size_t maxLines, int maxWidth)
{
    ...
    size_t lineIndex = 0U;  // Счетчик, индекс, позиция или номер элемента.
    const char* p = text;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.

    while (*p != '\0' && lineIndex < maxLines)
    {
        char current[160] = {};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        size_t currentLen = 0U;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        const char* lastSpaceSrc = nullptr;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        size_t lastSpaceLen = 0U;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    ...
    if (*p != '\0' && lineIndex > 0U)
    {
        char* last = lines[lineIndex - 1U];  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
{
    static char lastMessage[BUFFER_SIZE] = {};  // Временная отметка, интервал или значение тайм-аута.
    static bool lastActive = false;  // Логический флаг состояния, разрешения или наличия данных.
    ...
    const uint32_t period = isMessageVisible ? showPeriod : hidePeriod;  // Временная отметка, интервал или значение тайм-аута.
...
static void drawTrackerTextMessageBlock()
{
    ...
    const int boxX = 0;  // Параметр геометрии, координаты, размера или угла.
    const int boxY = 25;  // Параметр геометрии, координаты, размера или угла.
    const int boxW = 480;  // Параметр геометрии, координаты, размера или угла.
    const int boxH = 160;  // Параметр геометрии, координаты, размера или угла.
    ...
    char lines[5][160] = {};
    const int maxWidth = boxW - 6;  // Параметр геометрии, координаты, размера или угла.
...
{
    int bestRssi = 0;  // Буфер, текстовая строка или рабочее сообщение.
    bool found = false;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    time_t freshestTs = 0;  // Параметр геометрии, координаты, размера или угла.
...
static void drawBottomLeftLoRaBlock()
{
    ...
    const int x = 2;  // Параметр геометрии, координаты, размера или угла.
    const int y0 = 257;  // Параметр геометрии, координаты, размера или угла.
    const int dy = 15;  // Параметр геометрии, координаты, размера или угла.
    char lineBuf[64];  // Счетчик, индекс, позиция или номер элемента.

    uint32_t txPackets = 0;  // Параметр геометрии, координаты, размера или угла.
    uint32_t rxPackets = 0;  // Параметр геометрии, координаты, размера или угла.
    ...
    if (rfFreqHz > 0UL)
    {
        const uint32_t mhz = rfFreqHz / 1000000UL;  // Параметр геометрии, координаты, размера или угла.
...
static bool shouldDrawGnssBlock()
{
    ...
    if (settings != nullptr && settings->lan_state_view != 0)
    {
        return true;
    }
...
static void drawBottomRightGnssBlock()
{
    ...
    const int xRight = 478;  // Параметр геометрии, координаты, размера или угла.
    // Блок GPS смещен ниже экрана, так как в нормальном режиме он выводится один.
    const int yTop = 274;  // Параметр геометрии, координаты, размера или угла.
    const int dy = 14;  // Параметр геометрии, координаты, размера или угла.
    char lineBuf[64];  // Счетчик, индекс, позиция или номер элемента.
...
static void drawBottomRightTestCoordinatesBlock()
{
    ...
    const float testLat = settings ? settings->local_latitude : ThisAircraft.local_latitude;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    const float testLon = settings ? settings->local_longitude : ThisAircraft.local_longitude;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    ...
    const int yTop = 289;  // Параметр геометрии, координаты, размера или угла.
    const int dy = 14;  // Параметр геометрии, координаты, размера или угла.
    char lineBuf[64];  // Счетчик, индекс, позиция или номер элемента.
...
static void drawRadarBaseAndStatus()
{
    ...
    if (divider <= 32767)
    {
        ...
        const int data_KM_x = kRadarRangeLabelX;  // Параметр геометрии, координаты, размера или угла.
        const int data_KM_y = kRadarRangeLabelY;  // Параметр геометрии, координаты, размера или угла.

        const uint8_t dividerIndex = clampRadarScaleIndex((uint8_t)(divider_num - 1));
        const RadarScaleDef& scaleForText = kRadarScaleDefs[dividerIndex];  // Буфер, текстовая строка или рабочее сообщение.
...
void Display_loop()
{
    ...
    char buf[16];  // Буфер, текстовая строка или рабочее сообщение.
    uint32_t disp_value;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.

    uint16_t tbw;  // Параметр геометрии, координаты, размера или угла.
    uint16_t tbh;  // Параметр геометрии, координаты, размера или угла.
