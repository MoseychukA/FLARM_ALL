...
namespace
{
    ...
    {
        bool active;  // Логический флаг состояния, разрешения или наличия данных.
        ...
        double latitude;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
        double longitude;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
        int16_t offsetSteps;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        int8_t direction;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        uint32_t lastStepMs;  // Временная отметка, интервал или значение тайм-аута.
        uint32_t startDelayMs;  // Временная отметка, интервал или значение тайм-аута.
    };

    static uint32_t g_lastOwnshipTxMs = 0;  // Временная отметка, интервал или значение тайм-аута.
    static uint8_t g_lastModeApplied = 0xFFU;  // Параметр конфигурации интерфейса, адресации или выбранного режима.
    static float g_lastModeBaseLat = 9999.0f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    static float g_lastModeBaseLon = 9999.0f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    static int16_t g_testFlightOffsetSteps = 0;  // Параметр геометрии, координаты, размера или угла.
    static int8_t g_testFlightDirection = 1;  // Параметр геометрии, координаты, размера или угла.
    static uint32_t g_lastTestFlightStepMs = 0;  // Временная отметка, интервал или значение тайм-аута.
    static SimTrafficState g_simTraffic[5] = {};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    static uint32_t g_testTrafficEpochMs = 0;  // Временная отметка, интервал или значение тайм-аута.
    static bool g_simTrafficWasActive = false;  // Логический флаг состояния, разрешения или наличия данных.
    static uint16_t g_testDisplayCourseDeg = 0U;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    static uint32_t g_lastTestDisplayCourseStepMs = 0U;  // Временная отметка, интервал или значение тайм-аута.

    constexpr double kEarthRadiusMeters = 6371000.0;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    constexpr float kTestTrackOwnForwardCourseDeg = 70.0f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    constexpr float kTestTrackStepMeters = 500.0f;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    constexpr int16_t kTestTrackLimitSteps = 40;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    constexpr float kTestTrackSpeedKnots = ((kTestTrackStepMeters / 1852.0f) * 3600.0f) * 0.5f;
    constexpr uint32_t kMotionStepPeriodMs = 2000UL;  // Временная отметка, интервал или значение тайм-аута.
    constexpr float kSimTrafficInitialCourseDeg = 40.0f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    constexpr float kSimTrafficCourseStepDeg = 20.0f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    constexpr uint32_t kSimTrafficAsyncDelayMs = 5000UL;  // Временная отметка, интервал или значение тайм-аута.
    constexpr uint32_t kTestDisplayCourseStepPeriodMs = 3000UL;  // Временная отметка, интервал или значение тайм-аута.
    constexpr uint16_t kTestDisplayCourseStepDeg = 5U;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    constexpr float kSimAltitudeMinMeters = 1000.0f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    constexpr float kSimAltitudeMaxMeters = 8000.0f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    constexpr float kTestOwnshipAltitudeMeters = 3000.0f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    constexpr float kDefaultTestBaseLat = 50.0000f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    constexpr float kDefaultTestBaseLon = 8.0000f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    constexpr uint32_t kSimAltitudeHalfPeriodMs = 700000UL;  // Временная отметка, интервал или значение тайм-аута.
    constexpr uint32_t kSimAltitudeFullPeriodMs = kSimAltitudeHalfPeriodMs * 2UL;  // Временная отметка, интервал или значение тайм-аута.
    constexpr size_t kSimTrafficCount = sizeof(g_simTraffic) / sizeof(g_simTraffic[0]);
    constexpr size_t kSimAltitudeProfileCount = kSimTrafficCount + 1U;  // Счетчик, индекс, позиция или номер элемента.
...
//------------------------------------------------------------------------------
                                  ...
                                  float& outLatDeg, float& outLonDeg)
    {
        ...
        const double angularDistance = distanceMeters / kEarthRadiusMeters;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
...
//------------------------------------------------------------------------------
    ...
    {
        bool changed = false;  // Параметр геометрии, координаты, размера или угла.
...
//------------------------------------------------------------------------------
    ...
    {
        const float altitudeRange = kSimAltitudeMaxMeters - kSimAltitudeMinMeters;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
        ...
        const bool climbing = cycleMs < kSimAltitudeHalfPeriodMs;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        ...
        const float signedVerticalSpeedMps = climbing ? verticalSpeedMps : -verticalSpeedMps;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
...
//------------------------------------------------------------------------------
    static void applyLocalCoordinateMode()
    {
        ...
        const uint8_t mode = settings->mode;  // Параметр конфигурации интерфейса, адресации или выбранного режима.
        float baseLat = settings->local_latitude;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
        float baseLon = settings->local_longitude;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
        ...
        float outLat = baseLat;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
        float outLon = baseLon;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
        float outCourse = 0.0f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
        float outSpeed = 0.0f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
        float outAltitude = kSimAltitudeMinMeters;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
        float outPressureAltitude = kSimAltitudeMinMeters;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
        float outVs = 0.0f;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        int outVertRate = 0;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        const uint32_t nowMs = millis();
...
void SystemLoop()
{
    static uint32_t lastDisplayLoopMs = 0;  // Временная отметка, интервал или значение тайм-аута.
