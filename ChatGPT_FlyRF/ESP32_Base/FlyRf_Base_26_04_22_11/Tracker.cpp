...
namespace
{
    constexpr uint8_t TRACKER_ADDR_TYPE_ICAO = 1U;  // Параметр конфигурации интерфейса, адресации или выбранного режима.

    EspSoftwareSerial::UART SERIAL_TRACKER;  // Объект внешнего интерфейса, экрана, порта или канала связи.
    constexpr auto TRACKER_SERIAL_CONFIG = EspSoftwareSerial::SWSERIAL_8N1;  // Объект внешнего интерфейса, экрана, порта или канала связи.
    static char g_rxBuffer[TRACKER_INPUT_BUFFER_SIZE + 1U] = {};  // Буфер, текстовая строка или рабочее сообщение.
    static size_t g_rxLen = 0U;  // Параметр геометрии, координаты, размера или угла.
    static uint32_t g_lastRxMs = 0U;  // Временная отметка, интервал или значение тайм-аута.
    constexpr uint32_t TRACKER_IDLE_FLUSH_MS = 30U;  // Временная отметка, интервал или значение тайм-аута.
    constexpr uint16_t TRACKER_TEXT_MAX_CHARS = 172U;  // Буфер, текстовая строка или рабочее сообщение.
    constexpr uint16_t TRACKER_FLY_TTL_MINUTES = 20U;  // Временная отметка, интервал или значение тайм-аута.
    constexpr uint16_t TRACKER_TEXT_TTL_MINUTES = 10U;  // Временная отметка, интервал или значение тайм-аута.
    constexpr uint32_t TRACKER_TEXT_AUTO_CLEAR_MS = 10UL * 60UL * 1000UL;  // Временная отметка, интервал или значение тайм-аута.
    constexpr uint32_t TRACKER_SIM_DURATION_MS = 60000UL;  // Временная отметка, интервал или значение тайм-аута.
    constexpr uint32_t TRACKER_SIM_STEP_MS = 1000UL;  // Временная отметка, интервал или значение тайм-аута.
    constexpr size_t TRACKER_SIM_MAX_TARGETS = MAX_TRACKING_OBJECTS;  // Параметр геометрии, координаты, размера или угла.
    constexpr double TRACKER_EARTH_RADIUS_METERS = 6371000.0;  // Числовой параметр навигации, радиообмена, геометрии или измерения.

    struct TrackerSimTarget
    {
        bool active = false;  // Логический флаг состояния, разрешения или наличия данных.
        ufo_t traffic = {};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        uint32_t startMs = 0U;  // Временная отметка, интервал или значение тайм-аута.
        uint32_t lastUpdateMs = 0U;  // Временная отметка, интервал или значение тайм-аута.
    };

    static TrackerSimTarget g_simTargets[TRACKER_SIM_MAX_TARGETS] = {};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    static char g_activeTextMessage[BUFFER_SIZE] = {};  // Логический флаг состояния, разрешения или наличия данных.
    static bool g_hasActiveTextMessage = false;  // Логический флаг состояния, разрешения или наличия данных.
    static bool g_newTextMessageFlag = false;  // Логический флаг состояния, разрешения или наличия данных.
    static bool g_allowReadConfirm = false;  // Логический флаг состояния, разрешения или наличия данных.
    static bool g_messageReceivedFlag = false;  // Логический флаг состояния, разрешения или наличия данных.
    static uint8_t g_activeTextMessageId = 0U;  // Логический флаг состояния, разрешения или наличия данных.
    static uint32_t g_activeTextMessageStoredMs = 0U;  // Логический флаг состояния, разрешения или наличия данных.
...
//------------------------------------------------------------------------------
    ...
    {
        char buffer[32];  // Буфер, текстовая строка или рабочее сообщение.
...
//------------------------------------------------------------------------------
    ...
    {
        char buffer[48];  // Буфер, текстовая строка или рабочее сообщение.
        if (cmd == nullptr || cmd[0] == '\0')
        {
...
//------------------------------------------------------------------------------
    static bool trackerParseClockText(const char* text, uint8_t& outHour, uint8_t& outMinute)
    {
        ...
        char hourBuf[4] = {};  // Временная отметка, интервал или значение тайм-аута.
        char minBuf[4] = {};  // Буфер, текстовая строка или рабочее сообщение.
        ...
        unsigned long hourVal = 0UL;  // Временная отметка, интервал или значение тайм-аута.
        unsigned long minVal = 0UL;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
//------------------------------------------------------------------------------
    ...
    {
        char buffer[20];  // Буфер, текстовая строка или рабочее сообщение.
        snprintf(buffer, sizeof(buffer), "#%lu", messageId);
        trackerWriteLine(buffer);

        char countBuffer[20];  // Счетчик, индекс, позиция или номер элемента.
...
//------------------------------------------------------------------------------
    static void trimAscii(char* text)
    {
        ...
        char* start = text;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
//------------------------------------------------------------------------------
    static size_t utf8Length(const char* text)
    {
        ...
        size_t count = 0U;  // Счетчик, индекс, позиция или номер элемента.
...
//------------------------------------------------------------------------------
    ...
    {
        uint8_t checksum = 0U;  // Параметр геометрии, координаты, размера или угла.
...
//------------------------------------------------------------------------------
    static void trackerWriteNmeaBody(const char* body)
    {
        ...
        char line[196];  // Счетчик, индекс, позиция или номер элемента.
...
//------------------------------------------------------------------------------
    static void fillCallsignTracker(char* dst, size_t dstSize, const char* src)
    {
        ...
        size_t out = 0U;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
//------------------------------------------------------------------------------
    static bool parseCsvFields(char* text, char* fields[], size_t expectedCount)
    {
        ...
        size_t count = 0U;  // Счетчик, индекс, позиция или номер элемента.
        char* savePtr = nullptr;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
//------------------------------------------------------------------------------
                                         ...
                                         float& outLatDeg, float& outLonDeg)
    {
        ...
        const double angularDistance = distanceMeters / TRACKER_EARTH_RADIUS_METERS;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
...
//------------------------------------------------------------------------------
    static TrackerSimTarget* trackerAcquireSimTarget(uint32_t addr)
    {
        ...
        size_t oldestIndex = 0U;  // Счетчик, индекс, позиция или номер элемента.
        uint32_t oldestStart = g_simTargets[0].startMs;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
//------------------------------------------------------------------------------
    static bool handleSetTxt(unsigned long messageId, char* params)
    {
        ...
        char local[TRACKER_INPUT_BUFFER_SIZE + 1U];  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        strncpy(local, params, sizeof(local) - 1U);
        local[sizeof(local) - 1U] = '\0';
        trimAscii(local);

        char* textPart = local;  // Буфер, текстовая строка или рабочее сообщение.
        ...
        uint8_t msgHour = 0U;  // Временная отметка, интервал или значение тайм-аута.
        uint8_t msgMinute = 0U;  // Временная отметка, интервал или значение тайм-аута.
...
//------------------------------------------------------------------------------
    static bool handleSetFly(char* params)
    {
        ...
        char work[TRACKER_INPUT_BUFFER_SIZE + 1U];  // Параметр геометрии, координаты, размера или угла.
        strncpy(work, params, sizeof(work) - 1U);
        work[sizeof(work) - 1U] = '\0';

        char* fields[13] = {};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        ...
        unsigned long addr = 0UL;  // Параметр конфигурации интерфейса, адресации или выбранного режима.
        unsigned long squawk = 0UL;  // Параметр геометрии, координаты, размера или угла.
        unsigned long aircraftType = 0UL;  // Параметр геометрии, координаты, размера или угла.
        unsigned long hourMsg = 0UL;  // Временная отметка, интервал или значение тайм-аута.
        unsigned long minuteMsg = 0UL;  // Временная отметка, интервал или значение тайм-аута.
        float altitude = 0.0f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
        float pressureAltitude = 0.0f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
        float speed = 0.0f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
        float course = 0.0f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
        float latitude = 0.0f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
        float longitude = 0.0f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
        long vertRate = 0L;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        ...
        char* endVert = nullptr;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        ...
        ufo_t traffic = EmptyFO;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
//------------------------------------------------------------------------------
    ...
    {
        char body[160];  // Параметр геометрии, координаты, размера или угла.
...
//------------------------------------------------------------------------------
    static void processTrackerMessage(char* message)
    {
        ...
        char* fields[4] = {};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        size_t fieldCount = 0U;  // Счетчик, индекс, позиция или номер элемента.
        char* p = message + 1;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        fields[fieldCount++] = p;
        ...
        unsigned long messageId = 0UL;  // Временная отметка, интервал или значение тайм-аута.
...
bool Tracker_confirmActiveTextMessage()
{
    ...
    const uint8_t messageIdToConfirm = g_activeTextMessageId;  // Временная отметка, интервал или значение тайм-аута.
