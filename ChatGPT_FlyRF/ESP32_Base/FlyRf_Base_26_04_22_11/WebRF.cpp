...
String TxDataTemplate;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
static WebServer server(80);
static bool g_rebootPending = false;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
static uint32_t g_rebootAtMs = 0;  // Временная отметка, интервал или значение тайм-аута.
static bool g_updateOk = false;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
static String g_updateMessage;  // Временная отметка, интервал или значение тайм-аута.
static bool g_otaInProgress = false;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
static uint32_t g_otaWritten = 0;  // Параметр геометрии, координаты, размера или угла.
static uint32_t g_otaTotal = 0;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
{
    char buf[48];  // Буфер, текстовая строка или рабочее сообщение.
    if (Baro_available())
    {
...
static String loraRuntimeModeText()
{
    ...
    const char* runtimeProfile = "OGN compatible / custom";  // Временная отметка, интервал или значение тайм-аута.
    ...
    char syncBuf[16];  // Буфер, текстовая строка или рабочее сообщение.
    snprintf(syncBuf, sizeof(syncBuf), "%02X", (unsigned)info.syncWord);

    String html;  // Параметр геометрии, координаты, размера или угла.
    html.reserve(900);
...
static void parseIpArg(const String& name, uint8_t out[4])
{
    ...
    int a = 0, b = 0, c = 0, d = 0;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
{
    String out;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    out.reserve(src.length() + 8);
    for (size_t i = 0; i < src.length(); ++i)
    {
        const char c = src[i];  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        if (c == '\\' || c == '"') out += '\\';
        if (c == '\r' || c == '\n') out += ' ';
        else out += c;
    }
...
{
    const float DEG_TO_RAD_F = 0.017453292519943295769f;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    const float R = 6371.0f;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.

    const float p1 = lat1 * DEG_TO_RAD_F;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    const float p2 = lat2 * DEG_TO_RAD_F;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
{
    String s;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    s.reserve(5000);
...
{
    String normalized;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
{
    char addr1[16];  // Параметр конфигурации интерфейса, адресации или выбранного режима.
    char addr2[16];  // Параметр конфигурации интерфейса, адресации или выбранного режима.
    char addr3[16];  // Параметр конфигурации интерфейса, адресации или выбранного режима.
...
(function(){
  ...
  var busy=false;  // Параметр геометрии, координаты, размера или угла.
...
static String hardwarePage()
{
    ...
    html += F("</select></td></tr>");
    char blockAddr1[16];  // Параметр конфигурации интерфейса, адресации или выбранного режима.
    char blockAddr2[16];  // Параметр конфигурации интерфейса, адресации или выбранного режима.
    char blockAddr3[16];  // Параметр конфигурации интерфейса, адресации или выбранного режима.
...
{
    String json;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    json.reserve(6144);
    json += "{\"count\":";
    ...
    int flarmCount = 0;  // Счетчик, индекс, позиция или номер элемента.
    int adsbCount = 0;  // Счетчик, индекс, позиция или номер элемента.
    ...
    bool first = true;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    const float rxLat = ThisAircraft.local_latitude;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    const float rxLon = ThisAircraft.local_longitude;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
...
static void handleHardwareLive()
{
    ...
    bool resetDefaults = false;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
{
    bool defaultsLoaded = false;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    if (settings)
    {
        if (server.hasArg("default_settings") && server.arg("default_settings").toInt() == 1)
        {
...
static void handleOtaStatus()
{
    ...
    json += F(",\"message\":\"");
    String msg = g_updateMessage;  // Временная отметка, интервал или значение тайм-аута.
    msg.replace("\\", "\\\\");
    msg.replace("\"", "\\\"");
    msg.replace("\r", " ");
    msg.replace("\n", " ");
