...
namespace
{
    constexpr uint8_t MAVLINK_STX_V1 = 0xFEU;  // Параметр геометрии, координаты, размера или угла.
    constexpr uint8_t MAV_SYS_ID = 1U;  // Параметр геометрии, координаты, размера или угла.
    constexpr uint8_t MAV_COMP_ID = 1U;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    constexpr uint32_t MAVLINK_PERIOD_MS = 1000UL;  // Временная отметка, интервал или значение тайм-аута.

    constexpr uint8_t MAVLINK_MSG_ID_HEARTBEAT = 0U;  // Временная отметка, интервал или значение тайм-аута.
    constexpr uint8_t MAVLINK_MSG_ID_GPS_RAW_INT = 24U;  // Временная отметка, интервал или значение тайм-аута.
    constexpr uint8_t MAVLINK_MSG_ID_GLOBAL_POSITION_INT = 33U;  // Временная отметка, интервал или значение тайм-аута.
    constexpr uint8_t MAVLINK_MSG_ID_VFR_HUD = 74U;  // Временная отметка, интервал или значение тайм-аута.
    constexpr uint8_t MAVLINK_MSG_ID_ADSB_VEHICLE = 246U;  // Временная отметка, интервал или значение тайм-аута.

    constexpr uint8_t MAVLINK_CRC_HEARTBEAT = 50U;  // Параметр геометрии, координаты, размера или угла.
    constexpr uint8_t MAVLINK_CRC_GPS_RAW_INT = 24U;  // Параметр геометрии, координаты, размера или угла.
    constexpr uint8_t MAVLINK_CRC_GLOBAL_POSITION_INT = 104U;  // Счетчик, индекс, позиция или номер элемента.
    constexpr uint8_t MAVLINK_CRC_VFR_HUD = 20U;  // Параметр геометрии, координаты, размера или угла.
    constexpr uint8_t MAVLINK_CRC_ADSB_VEHICLE = 184U;  // Параметр геометрии, координаты, размера или угла.

    constexpr uint8_t MAV_TYPE_FIXED_WING = 1U;  // Параметр геометрии, координаты, размера или угла.
    constexpr uint8_t MAV_AUTOPILOT_INVALID = 8U;  // Логический флаг состояния, разрешения или наличия данных.
    constexpr uint8_t MAV_MODE_FLAG_CUSTOM_MODE_ENABLED = 1U;  // Логический флаг состояния, разрешения или наличия данных.
    constexpr uint8_t MAV_STATE_ACTIVE = 4U;  // Логический флаг состояния, разрешения или наличия данных.
    constexpr uint8_t GPS_FIX_TYPE_NO_GPS = 0U;  // Параметр геометрии, координаты, размера или угла.
    constexpr uint8_t GPS_FIX_TYPE_3D_FIX = 3U;  // Параметр геометрии, координаты, размера или угла.
    constexpr uint8_t ADSB_EMITTER_TYPE_NO_INFO = 0U;  // Параметр геометрии, координаты, размера или угла.
    constexpr uint16_t ADSB_FLAGS_VALID_COORDS = 1U << 0;  // Логический флаг состояния, разрешения или наличия данных.
    constexpr uint16_t ADSB_FLAGS_VALID_ALTITUDE = 1U << 1;  // Логический флаг состояния, разрешения или наличия данных.
    constexpr uint16_t ADSB_FLAGS_VALID_HEADING = 1U << 2;  // Логический флаг состояния, разрешения или наличия данных.
    constexpr uint16_t ADSB_FLAGS_VALID_VELOCITY = 1U << 3;  // Логический флаг состояния, разрешения или наличия данных.
    constexpr uint16_t ADSB_FLAGS_VALID_CALLSIGN = 1U << 4;  // Логический флаг состояния, разрешения или наличия данных.
    constexpr uint16_t ADSB_FLAGS_VALID_SQUAWK = 1U << 5;  // Логический флаг состояния, разрешения или наличия данных.

    static uint8_t g_seq = 0U;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    static uint32_t g_lastSendMs = 0U;  // Временная отметка, интервал или значение тайм-аута.
...
//------------------------------------------------------------------------------
    ...
    {
        float cmps = value * 100.0f;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
//------------------------------------------------------------------------------
    ...
    {
        uint16_t crc = 0xFFFFU;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        if (data != nullptr)
        {
...
//------------------------------------------------------------------------------
    ...
    {
        uint32_t raw = 0U;  // Параметр геометрии, координаты, размера или угла.
...
//------------------------------------------------------------------------------
    static void sendFrame(uint8_t msgId, uint8_t crcExtra, const uint8_t* payload, uint8_t payloadLen)
    {
        ...
        uint8_t frame[8U + 255U + 2U] = {0};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
        size_t totalLen = 0U;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
//------------------------------------------------------------------------------
    static void extractCallsign(const char* src, char* dst, size_t dstSize)
    {
        ...
        size_t j = 0U;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
//------------------------------------------------------------------------------
    static void writeCallsignField(const char* src, uint8_t* dst9)
    {
        ...
        memset(dst9, 0, 9U);
        char callsign[9] = {0};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
//------------------------------------------------------------------------------
    static bool buildOwnshipAdsbPayload(uint8_t* payload, size_t payloadSize)
    {
        ...
        ufo_t own = {};  // Параметр геометрии, координаты, размера или угла.
        ...
        float altMeters = 0.0f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
...
//------------------------------------------------------------------------------
    ...
    {
        uint8_t payload[9] = {0};  // Параметр геометрии, координаты, размера или угла.
...
//------------------------------------------------------------------------------
    static void sendOwnshipGpsRaw()
    {
        ...
        uint8_t payload[30] = {0};  // Параметр геометрии, координаты, размера или угла.
...
//------------------------------------------------------------------------------
    static void sendOwnshipGlobalPosition()
    {
        ...
        const float courseRad = ThisAircraft.course * DEG_TO_RAD;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
        ...
        uint8_t payload[28] = {0};  // Параметр геометрии, координаты, размера или угла.
...
//------------------------------------------------------------------------------
    ...
    {
        uint8_t payload[20] = {0};  // Параметр геометрии, координаты, размера или угла.
...
//------------------------------------------------------------------------------
    ...
    {
        uint8_t payload[38] = {0};  // Параметр геометрии, координаты, размера или угла.
...
size_t mavlink_lora_encode(void* pkt, ufo_t* this_aircraft)
{
    ...
    uint8_t payload[38] = {0};  // Параметр геометрии, координаты, размера или угла.
    uint8_t frame[MAVLINK_LORA_PAYLOAD_SIZE] = {0};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    size_t outLen = 0U;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.

    bool payloadReady = false;  // Логический флаг состояния, разрешения или наличия данных.
    if (this_aircraft != nullptr)
    {
        ufo_t own = *this_aircraft;  // Параметр геометрии, координаты, размера или угла.
...
bool mavlink_lora_decode(void* pkt, ufo_t* this_aircraft, ufo_t* fop)
{
    ...
    const uint8_t* payload = nullptr;  // Параметр геометрии, координаты, размера или угла.
