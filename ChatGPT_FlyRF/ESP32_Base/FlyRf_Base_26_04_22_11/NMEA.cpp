...
static bool g_rs485Ready = false;  // Логический флаг состояния, разрешения или наличия данных.
static NMEADiag g_diag = {};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
static uint8_t g_lastAnnouncedSerialMode = 0xFFU;  // Параметр конфигурации интерфейса, адресации или выбранного режима.
static uint32_t g_lastPgrmzMs = 0;  // Временная отметка, интервал или значение тайм-аута.
...
{
    uint8_t cs = 0;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
static bool sendNmeaToConfiguredOutput(const char* line, size_t len)
{
    ...
    bool sent = false;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
static void sanitizeCallsign(const char* src, char* dst, size_t dstSize)
{
    ...
    size_t outIdx = 0;  // Счетчик, индекс, позиция или номер элемента.
...
static void sendOwnshipRMC()
{
    ...
    char utcTime[16] = {};  // Временная отметка, интервал или значение тайм-аута.
    char utcDate[16] = {};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    ...
    char sentence[180];  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
static void sendOwnshipPGRMZ()
{
    ...
    char body[64];  // Параметр геометрии, координаты, размера или угла.
    char sentence[80];  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
static void sendOwnshipFLYRF()
{
    ...
    float lat = ThisAircraft.latitude;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    float lon = ThisAircraft.longitude;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    ...
    char callsign[16] = {};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    ...
    char body[256];  // Параметр геометрии, координаты, размера или угла.
    char sentence[280];  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    snprintf(body, sizeof(body),
...
static void sendAircraftPFLAA(const Aircraft& ac, int& totalObjects, float& hpDistance, int& hpRelNorth, int& hpRelEast, int& hpAltDiff, uint32_t& hpAddr)
{
    ...
    char climbRate[12] = {};  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    snprintf(climbRate, sizeof(climbRate), "%.1f", ac.vert_rate / 196.8504f);

    char body[220];  // Параметр геометрии, координаты, размера или угла.
    char sentence[244];  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    snprintf(body, sizeof(body),
...
static void sendPFLAU(int totalObjects, float hpDistance, int hpRelNorth, int hpRelEast, int hpAltDiff, uint32_t hpAddr)
{
    ...
    char body[160];  // Параметр геометрии, координаты, размера или угла.
    char sentence[180];  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    int relBearing = 0;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
...
{
    char line[180];  // Счетчик, индекс, позиция или номер элемента.
...
void NMEA_loop()
{
    static uint32_t lastSendMs = 0;  // Временная отметка, интервал или значение тайм-аута.
    ...
    int totalObjects = 0;  // Служебная переменная, используемая для промежуточных вычислений и логики модуля.
    float hpDistance = 1.0e9f;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    int hpRelNorth = 0;  // Параметр геометрии, координаты, размера или угла.
    int hpRelEast = 0;  // Параметр геометрии, координаты, размера или угла.
    int hpAltDiff = 0;  // Числовой параметр навигации, радиообмена, геометрии или измерения.
    uint32_t hpAddr = 0;  // Параметр конфигурации интерфейса, адресации или выбранного режима.
