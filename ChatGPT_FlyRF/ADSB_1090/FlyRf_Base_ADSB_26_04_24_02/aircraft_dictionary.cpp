...
{
     memset(callsign, '\0', kCallSignMaxNumChars + 1);  //   ,    EOS
}
...
bool Aircraft1090::DecodePosition(bool filter_cpr_position) 
{
    ...
    even_message.odd = false;
    even_message.lat_cpr = last_even_packet_.n_lat;
    even_message.lon_cpr = last_even_packet_.n_lon;
    even_message.received_timestamp_ms = last_even_packet_.received_timestamp_ms;

    NASACPRDecoder::CPRMessage odd_message;
    odd_message.odd = true;
    odd_message.lat_cpr = last_odd_packet_.n_lat;
    odd_message.lon_cpr = last_odd_packet_.n_lon;
    odd_message.received_timestamp_ms = last_odd_packet_.received_timestamp_ms;

    bool result_valid = NASACPRDecoder::DecodeAirborneGlobalCPR(even_message, odd_message, result);
...
void AircraftDictionary::Update(uint32_t timestamp_ms) 
{
    ...
    {
        if (timestamp_ms - it->second.last_message_timestamp_ms > config_.aircraft_prune_interval_ms) // 60  
        {
...
static const size_t kMaxSmSlots = 12; //     
...
bool AircraftDictionary::IngestADSBPacket(ADSBPacket packet) 
{
    ...
    if (!packet.IsValid() || !(df == ADSBPacket::kDownlinkFormatExtendedSquitter ||
                               df == ADSBPacket::kDownlinkFormatExtendedSquitterNonTransponder ||
                               df == ADSBPacket::kDownlinkFormatMilitaryExtendedSquitter)) 
    {
        return false;  // Разрешаем валидные DF17/DF18/DF19 Extended Squitter.
    }
    ...
    if (ret) 
    {
        aircraft_ptr->IncrementNumFramesReceived(true);  //     S.
    }
...
bool AircraftDictionary::InsertAircraft(const Aircraft1090& aircraft) 
{
    ...
    {
        //!!comms_manager.console_printf("CONSOLE_INFO AIrcraftDictionary::InsertAircraft      ,    %d.\r\n", kMaxNumAircraft);
