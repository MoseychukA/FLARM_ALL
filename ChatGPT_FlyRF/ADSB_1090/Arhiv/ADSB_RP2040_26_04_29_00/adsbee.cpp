...
#define RSSI_V_MIN 0.3      // 300 мВ
#define RSSI_V_MAX 1.0      // 1000 мВ
...
static bool HighPowerPreambleEnabled()
{
    return bsp.r1090_high_power_demod_state_machine_index < bsp.r1090_num_demod_state_machines;
}

static bool IsHighPowerPreambleSM(uint16_t sm_index)
{
    return HighPowerPreambleEnabled() && sm_index == bsp.r1090_high_power_demod_state_machine_index;
}
...
bool ADSBee::Init() 
{
    ...
    irq_wrapper_program_init(config_.preamble_detector_pio, irq_wrapper_sm_, irq_wrapper_offset_, preamble_detector_div);
    for (uint16_t sm_index = 0; sm_index < bsp.r1090_num_demod_state_machines; sm_index++) 
    {
        ...
        bool make_sm_wait = HighPowerPreambleEnabled() && sm_index > 0 && sm_index < bsp.r1090_high_power_demod_state_machine_index;
        ...
        // preambles.
        if (IsHighPowerPreambleSM(sm_index)) 
        {
            // High power preamble.
    ...
    const uint16_t well_formed_preamble_count =
        HighPowerPreambleEnabled() ? bsp.r1090_high_power_demod_state_machine_index : bsp.r1090_num_demod_state_machines;
    for (uint16_t sm_index = 0; sm_index < well_formed_preamble_count; sm_index++) 
    {
        pio_sm_set_enabled(config_.preamble_detector_pio, preamble_detector_sm_[sm_index], true);
    }
    // Enable high power preamble detector.
    if (HighPowerPreambleEnabled())
    {
...
{
    // Считываем 16 измерений и усредняем (сглаживание)
    //long sum = 0;
    //const int N = 16;
    //for (int i = 0; i < N; ++i) sum += analogRead(RSSI_PIN);
    //float adc = sum / (float)N;

    // В мВ
    float voltage = ReadSignalStrengthMilliVolts();

    // Ограничиваем диапазон
    if (voltage < RSSI_V_MIN) voltage = RSSI_V_MIN;
    if (voltage > RSSI_V_MAX) voltage = RSSI_V_MAX;

    // Вычисляем RSSI в dB относительно 300 мВ
    // dB = 20 * log10(V / Vmin)
    float rssi_db = 20.0 * log10(voltage / RSSI_V_MIN);
...
void ADSBee::OnDemodComplete() 
{
    ...
    for (uint16_t sm_index = 0; sm_index < bsp.r1090_num_demod_state_machines; sm_index++) 
    {
        ...
        rx_packet_[sm_index].sigs_dbm = measureRSSI();// ReadSignalStrengthdBm();
        ...
        uint demodulator_program_start =
            IsHighPowerPreambleSM(sm_index)
