...
struct RxPacketInfo { int16_t sigs_dbm; int16_t sigq_db; uint16_t source; };

static const size_t kMaxSmSlots = 12; // или другое ваше значение 
static RxPacketInfo rx_packet_[kMaxSmSlots];
...
bool AircraftDictionary::IngestDecoded1090Packet(Decoded1090Packet& packet) 
{
 ...
 //==========================================================================================
    ...
    uint16_t sm_index = static_cast<uint16_t>(source); 
    if (sm_index < kMaxSmSlots)
    { 
        // Предположим,что в Raw есть поля sigs_dbm и sigq_db 
        rx_packet_[sm_index].sigs_dbm = packet.GetRaw().sigs_dbm; rx_packet_[sm_index].sigq_db = packet.GetRaw().sigq_db; rx_packet_[sm_index].source = sm_index;

    // Отправка в Serial 
        char buf[128]; 
        int n = snprintf(buf,sizeof(buf), "RX[%u]:sigs_dbm=%d,sigq_db=%d,source=%u\r\n",
            static_cast<unsigned int>(sm_index),
            rx_packet_[sm_index].sigs_dbm,
            rx_packet_[sm_index].sigq_db,
            rx_packet_[sm_index].source);
        if (n > 0) 
        { // используем существующий интерфейс вывода 
          //  Serial.println(buf);
           // comms_manager.console_printf("%s",buf); 

            uint32_t icao_addr = packet.GetICAOAddress();
            Aircraft1090* aircraft_ptr = nullptr;

            // Ищем только уже существующие (НЕ СОЗДАЕМ!)
            auto it = dict.find(icao_addr);
            if (it != dict.end())
            {
                aircraft_ptr = &(it->second);
                // Всегда обновляем RSSI (даже если других данных нет)
                aircraft_ptr->last_message_signal_strength_dbm = rx_packet_[sm_index].sigs_dbm;  // packet.GetRaw().sigs_dbm;
                aircraft_ptr->last_message_signal_quality_db = rx_packet_[sm_index].sigq_db;  // packet.GetRaw().sigq_db;
            }
        } 
 ...
 //==========================================================================================
    ...
    const uint16_t downlink_format = packet.GetDownlinkFormat();
    switch (packet.GetBufferLenBits()) 
    {
        case Raw1090Packet::kSquitterPacketLenBits: 
            ...
            if (downlink_format != Decoded1090Packet::kDownlinkFormatAllCallReply &&  ContainsAircraft(packet.GetICAOAddress())) 
            {
        ...
        case Raw1090Packet::kExtendedSquitterPacketLenBits:
            if (packet.IsValid() ||
                ((downlink_format == Decoded1090Packet::kDownlinkFormatCommBAltitudeReply ||
                  downlink_format == Decoded1090Packet::kDownlinkFormatCommBIdentityReply) &&
...
{
    const uint16_t df = packet.GetDownlinkFormat();
    if (!packet.IsValid() ||
        (df != IdentityReplyPacket::kDownlinkFormatIdentityReply &&
         df != IdentityReplyPacket::kDownlinkFormatCommBIdentityReply)) 
    {
       return false;
    }
...
bool AircraftDictionary::IngestAltitudeReplyPacket(AltitudeReplyPacket packet) 
{
    const uint16_t df = packet.GetDownlinkFormat();
    if (!packet.IsValid() ||
        (df != AltitudeReplyPacket::kDownlinkFormatAltitudeReply &&
         df != AltitudeReplyPacket::kDownlinkFormatCommBAltitudeReply)) 
    {
        return false;
    }
