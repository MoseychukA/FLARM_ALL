...
namespace
{
    ...
    static void adsbDmaTask(void* param)
    {
        ...
        for (;;)
        {
            ...
            const size_t acceptedMsb = processDmaWords(words, wordCount, false);
            if (acceptedMsb == 0)
            {
                processDmaWords(words, wordCount, true);
            }
    ...
    static bool startDmaCaptureTask()
    {
        ...
        g_dmaCaptureActive = true;
        const BaseType_t rc = xTaskCreatePinnedToCore(adsbDmaTask, "ADSB1090DMA", 8192, nullptr, 4, &g_dmaTask, 0);
        if (rc != pdPASS)
        {
            g_dmaCaptureActive = false;
            g_dmaTask = nullptr;
