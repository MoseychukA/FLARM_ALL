...
static const char* rfProtocolName(uint8_t protocol)
{
    switch (protocol)
    {
        case RF_PROTOCOL_MAVLINK: return "MAVLink";
        case RF_PROTOCOL_OGNTP:
        default: return "OGNTP";
    }
}
...
static uint8_t parseRfProtocolArg(const String& value)
{
    ...
    return (parsed == (int)RF_PROTOCOL_MAVLINK) ? RF_PROTOCOL_MAVLINK : RF_PROTOCOL_OGNTP;
}
...
static const char* serialModeName(uint8_t mode)
{
    switch (mode)
    {
        ...
        case OUTPUT_MODE_RP2040: return "ADSB INTERNAL";
...
// Локальные переменные: runtimeProfile — параметр радиоканала или протокола: описывает частоту, мощность, профиль, режим передачи или текущее состояние rf; 
// syncBuf — текстовая строка или буфер: хранит входное сообщение, сформированный ответ либо промежуточный текст; 
// html — текстовая строка или буфер: хранит входное сообщение, сформированный ответ либо промежуточный текст.
...
// Локальные переменные: a — рабочая переменная модуля: хранит промежуточное значение, используемое в текущей логике файла; 
// b — рабочая переменная модуля: хранит промежуточное значение, используемое в текущей логике файла; 
// c — рабочая переменная модуля: хранит промежуточное значение, используемое в текущей логике файла; 
// d — рабочая переменная модуля: хранит промежуточное значение, используемое в текущей логике файла.
...
// Локальные переменные: DEG_TO_RAD_F — рабочая переменная модуля: хранит промежуточное значение, используемое в текущей логике файла; 
// R — рабочая переменная модуля: хранит промежуточное значение, используемое в текущей логике файла; 
// p1 — рабочая переменная модуля: хранит промежуточное значение, используемое в текущей логике файла; 
// p2 — рабочая переменная модуля: хранит промежуточное значение, используемое в текущей логике файла; 
// dp — рабочая переменная модуля: хранит промежуточное значение, используемое в текущей логике файла; 
// dl — рабочая переменная модуля: хранит промежуточное значение, используемое в текущей логике файла.
//------------------------------------------------------------------------------
...
// initial — рабочая переменная модуля: хранит промежуточное значение, используемое в текущей логике файла.
...
// Локальные переменные: normalized — рабочая переменная модуля: хранит промежуточное значение, используемое в текущей логике файла; 
// nullptr — рабочая переменная модуля: хранит промежуточное значение, используемое в текущей логике файла.
...
// Локальные переменные: addr1 — параметр конфигурации: хранит выбранный режим, адрес, настройки модуля или пользовательское значение; 
// addr2 — параметр конфигурации: хранит выбранный режим, адрес, настройки модуля или пользовательское значение; 
// addr3 — параметр конфигурации: хранит выбранный режим, адрес, настройки модуля или пользовательское значение; 
// s — рабочая переменная модуля: хранит промежуточное значение, используемое в текущей логике файла.
...
// Локальные переменные: html — текстовая строка или буфер: хранит входное сообщение, сформированный ответ либо промежуточный текст; 
// true — рабочая переменная модуля: хранит промежуточное значение, используемое в текущей логике файла.
...
// Локальные переменные: html — текстовая строка или буфер: хранит входное сообщение, сформированный ответ либо промежуточный текст; 
// blockAddr1 — логический флаг состояния: показывает, разрешена ли операция, активен ли режим или есть ли данные; 
// blockAddr2 — логический флаг состояния: показывает, разрешена ли операция, активен ли режим или есть ли данные; 
// blockAddr3 — логический флаг состояния: показывает, разрешена ли операция, активен ли режим или есть ли данные.
...
static String hardwarePage()
{
    ...
    html += String("<option ") + sel((settings ? settings->rf_protocol : RF_PROTOCOL_OGNTP) == RF_PROTOCOL_OGNTP) + " value='1'>OGNTP</option>";
    html += String("<option ") + sel((settings ? settings->rf_protocol : RF_PROTOCOL_OGNTP) == RF_PROTOCOL_MAVLINK) + " value='100'>MAVLink</option>";
    html += F("</select></td></tr>");
    html += F("<tr><th>LoRa &#1095;&#1072;&#1089;&#1090;&#1086;&#1090;&#1072;</th><td>868.8 MHz</td></tr>");
...
static String hardwareSavedPage(bool defaultsLoaded)
{
    ...
    html += F("<tr><th>RF protocol</th><td>"); html += rfProtocolName(settings ? settings->rf_protocol : RF_PROTOCOL_OGNTP); html += F("</td></tr>");
    html += F("<tr><th>LoRa profile</th><td>"); html += RF_GetProfileName((settings && settings->rf_protocol == RF_PROTOCOL_MAVLINK) ? RF_GetAutoLoRaProfile(RF_PROTOCOL_MAVLINK) : (settings ? settings->lora_profile : 0)); html += F("</td></tr>");
...
static String outputsPage()
{
    ...
    html += String("<option ") + sel((settings ? settings->serial_out : OUTPUT_MODE_OFF) == OUTPUT_MODE_RP2040) + " value='3'>ADSB INTERNAL</option>";
...
// Локальные переменные: html — текстовая строка или буфер: хранит входное сообщение, сформированный ответ либо промежуточный текст; 
// application — рабочая переменная модуля: хранит промежуточное значение, используемое в текущей логике файла.
...
static void handleHardwareLive()
{
    ...
    else
    {
        ...
        if (server.hasArg("rf_protocol")) settings->rf_protocol = parseRfProtocolArg(server.arg("rf_protocol"));
        settings->band = RF_BAND_RU;
        settings->lora_fixed_channel = 1;
        settings->lora_fixed_freq = 8;
