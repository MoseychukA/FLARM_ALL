...
namespace
{
    ...
    constexpr uint32_t kDiagPeriodMs = 5000UL;
    ...
    static void IRAM_ATTR sampleSlotsFromRise(uint32_t riseCycle, uint8_t* slots, size_t slotCount)
    {
        uint32_t sampleCycle = riseCycle + g_cyclesPerQuarterUs;
        const uint32_t stepCycles = g_cyclesPerHalfUs;
        for (size_t i = 0; i < slotCount; ++i)
        {
            waitUntilCycle(sampleCycle);
            slots[i] = readRxPinFast();
            sampleCycle += stepCycles;
        }
    }

    static int findPreamble(const uint8_t* slots, size_t slotCount)
    {
        ...
        if (slotCount < sizeof(kPreamble))
        {
            return -1;
        }

        for (size_t offset = 0; offset + sizeof(kPreamble) <= slotCount; ++offset)
        {
    ...
    static bool decodeModeSFrame(const uint8_t* slots, size_t slotCount, DecodedModeSFrame& frame)
    {
        const int preambleOffset = findPreamble(slots, slotCount);
        if (preambleOffset < 0)
        {
            return false;
        }
    ...
    static void processSampledSlots(const uint8_t* slots, size_t slotCount)
    {
        DecodedModeSFrame frame = {};
        if (!decodeModeSFrame(slots, slotCount, frame))
        {
        ...
        {
            ingestExtendedSquitter(frame);
        }
        else if (frame.df == 5 || frame.df == 21)
        {
            ingestIdentityReply(frame);
        }
        else if (frame.df == 4 || frame.df == 20)
        {
            ingestAltitudeReply(frame);
        }
    }

    static bool captureCandidateSlots(uint8_t* slots, size_t slotCount)
    {
        ...
        while (ADSBReceiverESP32_internalEnabled())
        {
            ...
            if (current != 0 && previous == 0)
            {
                ...
                sampleSlotsFromRise(riseCycle, slots, slotCount);
                return true;
            }
            previous = current;
    ...
    static bool preambleLooksValid(const uint8_t* slots)
    {
    ...
    static void adsbRxTask(void* param)
    {
        ...
        for (;;)
        {
            ...
            if (!captureCandidateSlots(slots, kFullSampleSlots))
            {
                vTaskDelay(1);
                continue;
            }
            ...
            processSampledSlots(slots, kFullSampleSlots);
            vTaskDelay(1);
        }
    }
}
...
void ADSBReceiverESP32_setup()
{
    ...
    Serial.printf("[ADSB-ESP32] GPIO sampler GPIO%d, OK GPIO%d, CPU %lu MHz, half-slot %lu cycles, core 0\r\n",
                  ADSB_ESP32_RX_PIN,
...
void ADSBReceiverESP32_loop()
{
    ...
    if ((nowMs - lastDiagMs) < kDiagPeriodMs)
    {
        return;
    }
    lastDiagMs = nowMs;
    ...
    const uint32_t ageMs = (diag.lastPacketMs == 0U) ? 0U : (nowMs - diag.lastPacketMs);
    Serial.printf("[ADSB-ESP32] sampled=%lu ok=%lu reject=%lu pre=%lu crc=%lu ap=%lu df17=%lu sqCnt=%lu sq=%04u pwm=%u df=%u tc=%u addr=%06lX age=%lums\r\n",
                  (unsigned long)diag.sampledFrames,
                  (unsigned long)diag.packetsAccepted,
                  (unsigned long)diag.packetsRejected,
                  (unsigned long)diag.preambleRejected,
                  (unsigned long)diag.crcRejected,
                  (unsigned long)diag.apAccepted,
                  (unsigned long)diag.df17Accepted,
                  (unsigned long)diag.squawkAccepted,
                  (unsigned)diag.lastSquawk,
                  (unsigned)diag.thresholdPwmDuty,
                  (unsigned)diag.lastDf,
                  (unsigned)diag.lastTypeCode,
                  (unsigned long)(diag.lastAddress & 0xFFFFFFUL),
                  (unsigned long)ageMs);
}
