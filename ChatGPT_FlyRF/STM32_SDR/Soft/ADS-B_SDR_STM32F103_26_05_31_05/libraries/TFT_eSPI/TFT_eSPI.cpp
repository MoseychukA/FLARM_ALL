...
#if defined(ARDUINO_ARCH_STM32) && !defined(utoa)
static char *stm32_utoa_compat(unsigned int value, char *buffer, int radix)
{
  if (radix < 2 || radix > 36) {
    buffer[0] = '\0';
    return buffer;
  ...
  do {
    ...
    temp[pos++] = digit < 10 ? char('0' + digit) : char('a' + digit - 10);
    value /= radix;
  } while (value && pos < sizeof(temp));

  uint8_t out = 0;
  while (pos) buffer[out++] = temp[--pos];
  buffer[out] = '\0';
  return buffer;
}
#define utoa(value, buffer, radix) stm32_utoa_compat((unsigned int)(value), (buffer), (radix))
#endif
