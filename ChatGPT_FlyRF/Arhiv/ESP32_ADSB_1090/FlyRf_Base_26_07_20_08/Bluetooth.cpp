...
size_t txWrite(const uint8_t* data, size_t size)
{
    ...
    if (g_txMutex != nullptr) xSemaphoreTake(g_txMutex, portMAX_DELAY);
    size_t written = 0;
    ...
    if (g_txMutex != nullptr) xSemaphoreGive(g_txMutex);
    return written;
}
...
size_t txRead(uint8_t* out, size_t size)
{
    ...
    if (g_txMutex != nullptr && xSemaphoreTake(g_txMutex, 0) != pdTRUE) return 0;
    size_t read = 0;
...
void ensureInitialized()
{
    ...
    if (g_txMutex == nullptr) g_txMutex = xSemaphoreCreateMutex();
