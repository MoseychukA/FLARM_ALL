...
void WiFi_setup()
{
    ...
    {
        WiFi_fini();
        Serial.println(F("[WIFI] Disabled by settings"));
        return;
    }
...
bool WiFi_setControlMode(uint8_t mode, bool storeNow)
{
    if (mode == WIFI_CONTROL_AP)
    {
        mode = WIFI_CONTROL_ON;
    }
    if (mode > WIFI_CONTROL_ON)
    {
        mode = WIFI_CONTROL_ON;
    }
...
uint8_t WiFi_controlMode(void)
{
    if (settings == nullptr)
    {
        return WIFI_CONTROL_ON;
    }
    return settings->wifi_mode;
}
...
const char* WiFi_controlModeName(uint8_t mode)
{
    switch (mode)
    {
        case WIFI_CONTROL_OFF: return "OFF";
        case WIFI_CONTROL_AP:  return "ON";
        case WIFI_CONTROL_ON:  return "ON";
        default:               return "ON";
    }
}
...
bool WiFi_transmitUDP(uint16_t port, const uint8_t* data, size_t size)
{
    ...
    if (mode == WIFI_AP || mode == WIFI_AP_STA)
    {
        ...
        wifi_sta_mac_ip_list_t stationInfo = {};
        if (esp_wifi_ap_get_sta_list(&stations) == ESP_OK &&
            esp_wifi_ap_get_sta_list_with_ip(&stations, &stationInfo) == ESP_OK)
        {
            for (int i = 0; i < stationInfo.num; ++i)
            {
                const IPAddress clientIP(stationInfo.sta[i].ip.addr);
                if (clientIP == IPAddress((uint32_t)0))
                {
                    continue;
                }
                if (g_udp.beginPacket(clientIP, targetPort))
                {
                    const size_t written = g_udp.write(data, size);
                    const bool ok = g_udp.endPacket() == 1;
                    sent = sent || (ok && written == size);
                }
            }
        }
    }

    IPAddress dst = g_udpBroadcastIP;
    if (mode == WIFI_STA || mode == WIFI_AP_STA)
    {
        const IPAddress ip = WiFi.localIP();
        const IPAddress mask = WiFi.subnetMask();
        if (ip != IPAddress((uint32_t)0) && mask != IPAddress((uint32_t)0))
        {
            dst = IPAddress((uint32_t)ip | ~(uint32_t)mask);
        }
    }

    if (!g_udp.beginPacket(dst, targetPort))
    {
        return sent;
    }
    const size_t written = g_udp.write(data, size);
    const bool ok = g_udp.endPacket() == 1;
    return sent || (ok && written == size);
}
