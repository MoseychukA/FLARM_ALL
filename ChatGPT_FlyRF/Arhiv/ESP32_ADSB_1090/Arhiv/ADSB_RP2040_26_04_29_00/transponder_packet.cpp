...
uint16_t Decoded1090Packet::DumpPacketBuffer(uint8_t to_buffer[kMaxPacketLenWords32 * kBytesPerWord]) const {
    ...
    for (uint16_t i = 0; i < kMaxPacketLenWords32; i++) 
    {
        // Первый полученный бит — старший бит.
...
uint32_t Decoded1090Packet::CalculateCRC24(uint16_t packet_len_bits) const 
{
    ...
    // Перерабатываем 32-битный буфер пакета слов в байтовый буфер.
...
{
    if (raw_.buffer_len_bits != Raw1090Packet::kExtendedSquitterPacketLenBits &&   raw_.buffer_len_bits != Raw1090Packet::kSquitterPacketLenBits) // Не равен 112 или 56
    {
    ...
    downlink_format_ = raw_.buffer[0] >> 27;  // Выделяем DF из пакета
    ...
    switch (static_cast<DownlinkFormat>(downlink_format_)) 
    {
        ...
        case kDownlinkFormatIdentityReply:                   // DF = 5
        {
            ...
            is_valid_ = false;  // Вычисленная контрольная сумма складывается с адресом ICAO с помощью операции XOR. См. Руководство по декодированию ADS-B, стр. 22.
            // Адрес ИКАО — это наиболее вероятный вариант, его необходимо подтвердить с помощью авиационного словаря.
            icao_address_ = parity_value ^ calculated_checksum;  // Результат равен единице в том случае, если значения отличаются
            break;
        }
        case kDownlinkFormatAllCallReply:  // DF = 11
        {
            ...
            {
                // Ответ на спонтанный запрос на получение данных.
                is_valid_ = true;
            }
            else 
            {
                // Не знаю идентификатора следователя, поэтому не могу сказать, действителен ли он.
                is_valid_ = false;
            }
            break;
        }
        ...
        {
            // DF20/DF21 use address/parity, so the syndrome is the most likely ICAO.
            is_valid_ = false;
            icao_address_ = parity_value ^ calculated_checksum;
            break;
        }

        default:  // Все остальные DF. Примечание: DF=17-19 для ADS-B.
        {
            ...
            if (calculated_checksum == parity_value)
            {
                ...
                is_valid_ = true;  // пометить пакет как действительный, если CRC соответствует битам четности
            }
            else 
            {
...
IdentityReplyPacket::IdentityReplyPacket(const Decoded1090Packet &decoded_packet) : Decoded1090Packet(decoded_packet) //Пакет ответа на идентификацию
{
    uint8_t flight_status = GetNBitWordFromBuffer(3, 5, raw_.buffer);  // FS = Bits 5-7.
   //!! comms_manager.console_printf("\r\n  ***flight_status %d\r\n", flight_status);
    switch (flight_status) {
        case 0b000:  // Нет тревоги, нет SPI, самолет в воздухе.
            has_alert_ = false;
            has_ident_ = false;
            is_airborne_ = true;
            break;
        case 0b001:  // Нет тревоги, нет SPI, самолет на земле.
            has_alert_ = false;
            has_ident_ = false;
            is_airborne_ = false;
            break;
        case 0b010:  // Тревога, SPI отсутствует, самолет в воздухе.
            has_alert_ = true;
            has_ident_ = false;
            is_airborne_ = true;
            break;
        case 0b011:  // Тревога, сигнала SPI нет, самолет на земле.
            has_alert_ = true;
            has_ident_ = false;
            is_airborne_ = false;
            break;
        case 0b100:  // Тревога, сигнала SPI нет, самолет на земле.
            has_alert_ = true;
            has_ident_ = true;
            // Значение is_airborne_ по умолчанию равно false, если неизвестно.
            break;
        case 0b101:
            // Нет тревоги, SPI, воздушное судно в воздухе или на земле.
            has_ident_ = true;
            has_alert_ = false;
            // Значение is_airborne_ по умолчанию равно false, если неизвестно.
            break;
...
uint32_t Decoded1090Packet::adsb_crc_88bits(uint32_t* bin32_divid) // for 88bits message
{
    ...
    int j;
    uint32_t crc32_GP = 0xFFFA0480;//0x04C11DB7; // Полином генератора CRC уже включает в себя самый старший первый бит, тогда как стандартный CRC обычно опускается.

    //---копируем данные сообщения во временный массив, чтобы последующие действия не испортили исходные данные
...
uint32_t adsb_crc(uint32_t* bin32_divid, int nbits)
{
    ...
    // printf("***  CRC24: %06x \n",(*tbin32_divid)>>8);
    return (tbin32_divid[0]) >> 8; //---ИЗВЛЕЧЕНИЕ 24-битного CRC, знаковое целое число повлияет на результат сдвига вправо.
}
