...
namespace
{
    ...
    constexpr uint32_t kOkPulseMs = 50UL;
    ...
    static volatile bool g_okPulseActive = false;
    static volatile uint32_t g_okPulseUntilMs = 0;
    ...
    {
        g_okPulseUntilMs = millis() + kOkPulseMs;
        digitalWrite(ADSB_ESP32_OK_PULSE_PIN, LOW);
        g_okPulseActive = true;
    }

    static void serviceOkPin()
    {
        if (!g_okPulseActive)
        {
            return;
        ...
        const uint32_t untilMs = g_okPulseUntilMs;
        if ((int32_t)(millis() - untilMs) < 0)
        {
            return;
        }

        g_okPulseActive = false;
        digitalWrite(ADSB_ESP32_OK_PULSE_PIN, HIGH);
    }

    static void printAcceptedFrameToSerial(const DecodedModeSFrame& frame)
    {
        if (settings == nullptr || settings->serial_out != OUTPUT_MODE_ADSB_ESP32)
        {
            return;
        }

        Serial.printf("[ADSB ESP32S3 RX] DF=%u ICAO=%06lX bits=%u phase=%u bytes=",
                      frame.df,
                      (unsigned long)(frame.address & 0xFFFFFFUL),
                      frame.bitLen,
                      frame.phase);
        for (uint8_t i = 0; i < frame.byteLen; ++i)
        {
    ...
    static bool processSampledSlots(uint8_t phaseSlots[kPhaseCount][kMaxSlots], size_t slotCount)
    {
        ...
        printAcceptedFrameToSerial(frame);
        pulseOkPin();
        return true;
    }
