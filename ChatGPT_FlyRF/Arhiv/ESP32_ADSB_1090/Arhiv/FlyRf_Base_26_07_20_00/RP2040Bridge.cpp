...
// Runtime counters shown in diagnostics for the ADS-B bridge.
static RP2040BridgeDiag g_diag = {};

// Sliding receive window used to recover packet alignment after UART byte loss.
static uint8_t g_rxBuffer[RP2040BRIDGE_PACKET_SIZE] = {0};
static size_t g_rxIndex = 0;
static bool g_streamStarted = false;

// Gain table sent to the RP2040 receiver after startup or user changes.
...
static bool isRP2040ReceiverEnabled()
{
    return (settings == nullptr || settings->adsb_receiver == ADSB_RECEIVER_EXTERNAL);
}
...
static void configureGainFromSettings()
{
...
static bool ensureRP2040Started(bool logDisabled)
{
    if (!isRP2040ReceiverEnabled())
    {
        g_diag.ready = false;
        if (logDisabled)
        {
            Serial.println(F("[RP2040] disabled: ESP32S3 ADS-B receiver selected"));
        }
        return false;
    }

    configureGainFromSettings();
