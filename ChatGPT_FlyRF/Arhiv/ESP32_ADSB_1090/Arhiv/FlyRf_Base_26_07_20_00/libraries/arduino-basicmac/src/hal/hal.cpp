...
#include <Arduino.h>
#include <SPI.h>
extern "C" bool FlyRfSpiLock(unsigned long timeoutMs);
...
void hal_spi_select (int on) {
    static bool hal_spi_locked = false;

    if (on) {
        hal_spi_locked = FlyRfSpiLock(100);
    }
    ...
    if (!on && hal_spi_locked) {
        FlyRfSpiUnlock();
        hal_spi_locked = false;
    }
}
