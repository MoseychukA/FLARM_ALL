...
static void makeFallbackCallsign(const Aircraft& ac, char* dst, size_t dstSize)
{
    ...
    switch (ac.protocol)
    {
        case RF_PROTOCOL_LEGACY:    prefix = "FLR"; break;
        case RF_PROTOCOL_OGNTP:     prefix = "OGN"; break;
        case RF_PROTOCOL_P3I:       prefix = "PAW"; break;
        case RF_PROTOCOL_ADSB_1090: prefix = "ADS"; break;
        case RF_PROTOCOL_ADSB_UAT:  prefix = "UAT"; break;
        case RF_PROTOCOL_FANET:     prefix = "FAN"; break;
        case RF_PROTOCOL_APRS:      prefix = "HAM"; break;
        default:                    break;
    }
    snprintf(dst, dstSize, "%s_%06lX", prefix, (unsigned long)(ac.addr & 0xFFFFFFUL));
}

static bool getOwnshipPosition(float& lat, float& lon)
{
    ...
    if (fabsf(lat) <= 0.00001f && fabsf(lon) <= 0.00001f && GNSS_coordinatesValid())
    {
        lat = GNSS_latitude();
        lon = GNSS_longitude();
    }
...
static bool outputTimeValid()
{
    return GNSS_timeValid() ||
           (settings != nullptr && FlyRfMode_usesLocalCoordinates(settings->mode));
}

static uint8_t outputCentisecond()
{
    return GNSS_timeValid() ? GNSS_timeCentisecond() : 0U;
}

static bool buildDateTimeFields(char* utcTime, size_t utcTimeSize, char* utcDate, size_t utcDateSize)
{
    if (!outputTimeValid() || utcTime == nullptr || utcDate == nullptr || utcTimeSize < 7 || utcDateSize < 7)
    {
        return false;
    }
...
static void sendOwnshipRMC()
{
    ...
    float lat = 0.0f;
    float lon = 0.0f;
    if (!getOwnshipPosition(lat, lon) || !outputTimeValid()) return;
    ...
    snprintf(body, sizeof(body),
             "GPRMC,%s.%02u,A,%02d%07.4f,%c,%03d%07.4f,%c,%.1f,%.1f,%s,,,P",
             utcTime,
             (unsigned)outputCentisecond(),
...
static void sendOwnshipGGA()
{
    ...
    char body[160];
    char sentence[180];
    const float hdop = (ThisAircraft.hdop > 0U) ? ((float)ThisAircraft.hdop / 100.0f) : 2.3f;
    snprintf(body, sizeof(body),
             "GPGGA,%s.%02u,%02d%07.4f,%c,%03d%07.4f,%c,3,,%.1f,%.1f,M,%.1f,M,,",
             utcTime,
             (unsigned)outputCentisecond(),
             latDeg, latMin, latHem,
             lonDeg, lonMin, lonHem,
             (double)hdop,
             (double)ThisAircraft.altitude,
             (double)ThisAircraft.geoid_separation);
    const size_t len = wrapNMEALine(body, sentence, sizeof(sentence));
    sendNmeaToConfiguredOutput(sentence, len);
}

static void sendOwnshipGSA()
{
    float lat = 0.0f;
    float lon = 0.0f;
    if (settings == nullptr || !getOwnshipPosition(lat, lon)) return;

    const float hdop = (ThisAircraft.hdop > 0U) ? ((float)ThisAircraft.hdop / 100.0f) : 2.3f;
    const float vdop = 1.2f;
    const float pdop = sqrtf(hdop * hdop + vdop * vdop);
    char body[96];
    char sentence[112];
    snprintf(body, sizeof(body), "GPGSA,A,3,,,,,,,,,,,,,%.1f,%.1f,%.1f",
...
static void sendOwnshipPGRMZ()
{
    ...
    char body[64];
    char sentence[80];
    snprintf(body, sizeof(body), "PGRMZ,%d,f,%c", altitudeFt,
             GNSS_coordinatesValid() ? '3' : '1');
    const size_t len = wrapNMEALine(body, sentence, sizeof(sentence));
    if (len > 0)
    {
        sendNmeaToConfiguredOutput(sentence, len);
...
static void sendOwnshipLK8EX1()
{
    const int altitudeMeters =
        constrain((int)lroundf(ThisAircraft.pressure_altitude), -1000, 99998);
    const int verticalSpeedCms =
        (int)lroundf((float)ThisAircraft.vert_rate * 0.508f);
    const int temperatureC =
        constrain((int)lroundf(Baro_available() ? Baro_temperatureC() : 0.0f), -99, 98);

    char body[96];
    char sentence[112];
    snprintf(body, sizeof(body), "LK8EX1,999999,%d,%d,%d,0.0",
             altitudeMeters, verticalSpeedCms, temperatureC);
    const size_t len = wrapNMEALine(body, sentence, sizeof(sentence));
    sendNmeaToConfiguredOutput(sentence, len);
}

static void sendOwnshipHeartbeat()
{
    if (settings == nullptr || ThisAircraft.addr == 0U) return;
    ...
    char body[96];
    char sentence[112];
    snprintf(body, sizeof(body), "PSRFH,%06lX,%u,%lu,%lu,0",
             (unsigned long)(ThisAircraft.addr & 0xFFFFFFUL),
             (unsigned)settings->rf_protocol,
...
{
    /*
     * Совместимость с форматом FlyRf24_08_30_01:
     *   0 - LoRa/FLARM
     *   1 - ADS-B/DUMP1090
     *   2 - текстовая строка GSM/Iridium
     *
     * Внутри текущего проекта TrafficSource использует другие значения
     * (1 = FLARM/LoRa, 2 = ADS-B). Для $FLYRF наружу возвращаем старую
     * кодировку, иначе сторонние UDP-приемники смещают/отбрасывают цель.
     */
    switch (source)
    {
...
static bool aircraftAltitudeLooksMissing(const Aircraft& ac)
{
    return ac.altitude <= 0.5f && ThisAircraft.altitude > 100.0f;
}

static float exportAircraftAltitude(const Aircraft& ac)
{
    if (aircraftAltitudeLooksMissing(ac))
    {
        return ThisAircraft.altitude;
    }
    return ac.altitude;
}

static float exportAircraftPressureAltitude(const Aircraft& ac, float exportAltitude)
{
    if (ac.pressure_altitude < -100.0f || ac.pressure_altitude <= 0.5f)
    {
        return exportAltitude;
    }
    return ac.pressure_altitude;
}

// - body: Параметр геометрии, координаты, размера или угла.
static void sendAircraftFLYRF(const Aircraft& ac)
{
    if (settings == nullptr) return;
    if (!ac.valid || ac.addr == 0U || !aircraftHasCoordinates(ac)) return;

    char callsign[9] = {};
    memcpy(callsign, ac.callsign, 8);
    callsign[8] = '\0';

    const float exportAltitude = exportAircraftAltitude(ac);
    const float exportPressureAltitude = exportAircraftPressureAltitude(ac, exportAltitude);

    char body[256];
    char sentence[280];
    snprintf(body, sizeof(body),
             "FLYRF,%06lX,%d,%8.8s,%d,%d,%d,%d,%d,%.6f,%.6f,%d,%d,%d,%d,%u",
             (unsigned long)(ac.addr & 0xFFFFFFUL),
             ac.squawk,
             callsign,
             (int)lroundf(exportAltitude),
             (int)lroundf(exportPressureAltitude),
             (int)lroundf(ac.speed),
             (int)lroundf(ac.course),
             ac.vert_rate,
             (double)ac.latitude,
             (double)ac.longitude,
             (int)ac.aircraft_type,
             exportSignalSource(ac.signal_source),
             (int)ac.hour_msg,
             (int)ac.min_msg,
             (unsigned)ac.delay_time_msg);
...
static void sendAircraftPFLAA(const Aircraft& ac, int& totalObjects, float& hpDistance, int& hpRelNorth, int& hpRelEast, int& hpAltDiff, uint32_t& hpAddr)
{
    ...
    const int relVertical = (int)lroundf(exportAltitude - ThisAircraft.altitude);
    ...
    char sentence[244];
    snprintf(body, sizeof(body),
             "PFLAA,%d,%d,%d,%d,%u,%06lX!%s,%d,,%d,%s,%X",
             alarmLevel,
             relNorth,
             relEast,
             relVertical,
             (unsigned)min((uint8_t)3U, ac.addr_type),
             (unsigned long)(ac.addr & 0xFFFFFFUL),
             callsign,
             (int)lroundf(ac.course),
             (int)lroundf(ac.speed),
             climbRate,
             (unsigned)ac.aircraft_type);
...
static void sendPFLAU(int totalObjects, float hpDistance, int hpRelNorth, int hpRelEast, int hpAltDiff, uint32_t hpAddr)
{
    ...
    float ownLat = 0.0f;
    float ownLon = 0.0f;
    const int gnssStatus = getOwnshipPosition(ownLat, ownLon) ? 2 : 0;
    int relBearing = 0;
