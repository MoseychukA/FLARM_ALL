...
static uint32_t otaMaxSketchSpace()
{
    ...
    if (freeSpace <= 0x1000U) return 0U;
    return (freeSpace - 0x1000U) & 0xFFFFF000U;
}
...
static const char* serialModeName(uint8_t mode)
{
    switch (mode)
    {
        ...
        case OUTPUT_MODE_ADSB_ESP32: return "ADSB ESP32S3 RX";
...
static String rootPage()
{
    ...
    html += F("</td></tr>");
    html += F("<tr><th>Подтверждение запуска OTA</th><td>");
    if (SystemOtaPendingVerification()) html += F("Проверка стабильности (15 секунд)");
    else if (SystemOtaBootConfirmed()) html += F("Прошивка подтверждена");
    else html += F("Не подтверждена");
    html += F("</td></tr>");
    ...
    html += F("</td></tr>");
    TrackerDiagnostics trackerDiag = {};
    Tracker_getDiagnostics(trackerDiag);
    html += F("<tr><th>&#1057;&#1074;&#1103;&#1079;&#1100; &#1089; &#1090;&#1088;&#1077;&#1082;&#1077;&#1088;&#1086;&#1084;</th><td id='trackerState'>");
    html += trackerDiag.linkActive ? F("&#1040;&#1082;&#1090;&#1080;&#1074;&#1085;&#1072;") : F("&#1053;&#1077;&#1090; &#1076;&#1072;&#1085;&#1085;&#1099;&#1093;");
    html += F("</td></tr>");
    html += F("<tr><th>&#1054;&#1073;&#1084;&#1077;&#1085; &#1089; &#1090;&#1088;&#1077;&#1082;&#1077;&#1088;&#1086;&#1084;</th><td id='trackerCounters'>Rx&nbsp; ");
    html += String(trackerDiag.rxFrames);
    html += F(" / ");
    html += String(trackerDiag.rxBytes);
    html += F(" B&nbsp;&nbsp;&nbsp; Tx&nbsp; ");
    html += String(trackerDiag.txFrames);
    html += F(" / ");
    html += String(trackerDiag.txBytes);
    html += F(" B</td></tr>");
    html += F("<tr><th>&#1055;&#1086;&#1089;&#1083;&#1077;&#1076;&#1085;&#1080;&#1081; &#1086;&#1073;&#1084;&#1077;&#1085;</th><td id='trackerAges'>Rx&nbsp; --&nbsp;&nbsp;&nbsp; Tx&nbsp; --</td></tr>");
    html += F("<tr><th>&#1055;&#1088;&#1080;&#1085;&#1103;&#1090;&#1086; &#1086;&#1090; &#1090;&#1088;&#1077;&#1082;&#1077;&#1088;&#1072;</th><td id='trackerRxText' class='mono left' style='overflow-wrap:anywhere'>--</td></tr>");
    html += F("<tr><th>&#1055;&#1077;&#1088;&#1077;&#1076;&#1072;&#1085;&#1086; &#1074; &#1090;&#1088;&#1077;&#1082;&#1077;&#1088;</th><td id='trackerTxText' class='mono left' style='overflow-wrap:anywhere'>--</td></tr>");
    ...
    html += F("<tr><th>Скорость GPS / GNSS UART</th><td>");
    html += String(settings ? EEPROM_normalizeGnssBaud(settings->gnss_baud) : 9600UL);
    html += F(" бод</td></tr>");
...
(function(){
  ...
  var trackerState=document.getElementById('trackerState');
  var trackerCounters=document.getElementById('trackerCounters');
  var trackerAges=document.getElementById('trackerAges');
  var trackerRxText=document.getElementById('trackerRxText');
  var trackerTxText=document.getElementById('trackerTxText');
  var busy=false;
  ...
  function updateCounters(){
    ...
    fetch('/packet_counters',{cache:'no-store'})
      ...
      .then(function(data){
        ...
        if(trackerState) trackerState.innerHTML=data.tracker_active?'&#1040;&#1082;&#1090;&#1080;&#1074;&#1085;&#1072;':'&#1053;&#1077;&#1090; &#1076;&#1072;&#1085;&#1085;&#1099;&#1093;';
        if(trackerCounters) trackerCounters.innerHTML='Rx&nbsp; '+(data.tracker_rx_frames||0)+' / '+(data.tracker_rx_bytes||0)+' B&nbsp;&nbsp;&nbsp; Tx&nbsp; '+(data.tracker_tx_frames||0)+' / '+(data.tracker_tx_bytes||0)+' B';
        if(trackerAges){var rx=data.tracker_rx_age_ms<0?'--':(data.tracker_rx_age_ms/1000).toFixed(1)+' s';var tx=data.tracker_tx_age_ms<0?'--':(data.tracker_tx_age_ms/1000).toFixed(1)+' s';trackerAges.innerHTML='Rx&nbsp; '+rx+'&nbsp;&nbsp;&nbsp; Tx&nbsp; '+tx;}
        if(trackerRxText) trackerRxText.textContent=data.tracker_rx_text||'--';
        if(trackerTxText) trackerTxText.textContent=data.tracker_tx_text||'--';
      })
      .catch(function(){} )
...
static String hardwarePage()
{
    ...
    html += F("</select></td></tr>");
    const uint32_t currentGnssBaud = settings ? EEPROM_normalizeGnssBaud(settings->gnss_baud) : 9600UL;
    html += F("<tr><th>Скорость GPS / GNSS UART (бод)</th><td><select class='wMed hardwareCtl50' name='gnss_baud'>");
    html += String("<option ") + sel(currentGnssBaud == 4800UL) + " value='4800'>4800</option>";
    html += String("<option ") + sel(currentGnssBaud == 9600UL) + " value='9600'>9600</option>";
    html += String("<option ") + sel(currentGnssBaud == 19200UL) + " value='19200'>19200</option>";
    html += String("<option ") + sel(currentGnssBaud == 38400UL) + " value='38400'>38400</option>";
    html += String("<option ") + sel(currentGnssBaud == 57600UL) + " value='57600'>57600</option>";
    html += String("<option ") + sel(currentGnssBaud == 115200UL) + " value='115200'>115200</option>";
    html += String("<option ") + sel(currentGnssBaud == 230400UL) + " value='230400'>230400</option>";
    html += String("<option ") + sel(currentGnssBaud == 460800UL) + " value='460800'>460800</option>";
    html += String("<option ") + sel(currentGnssBaud == 921600UL) + " value='921600'>921600</option>";
    html += F("</select></td></tr>");
...
static String outputsPage()
{
    ...
    html += String("<option ") + sel((settings ? settings->serial_out : OUTPUT_MODE_OFF) == OUTPUT_MODE_ADSB_ESP32) + " value='7'>ADSB ESP32S3 RX</option>";
    html += F("</select></td></tr>");
...
static void handleHardwareLive()
{
    ...
    else
    {
        ...
        if (server.hasArg("serial_out")) settings->serial_out = (uint8_t)clampInt(server.arg("serial_out").toInt(), OUTPUT_MODE_OFF, OUTPUT_MODE_ADSB_ESP32);
        ...
        if (server.hasArg("gnss_baud"))
        {
            settings->gnss_baud = EEPROM_normalizeGnssBaud((uint32_t)server.arg("gnss_baud").toInt());
        }
...
static void handleOutputsSave()
{
    if (settings)
    {
        const uint8_t previousRs485Mode = settings->rs485_out;
        if (server.hasArg("serial_out")) settings->serial_out = (uint8_t)clampInt(server.arg("serial_out").toInt(), OUTPUT_MODE_OFF, OUTPUT_MODE_ADSB_ESP32);
        if (server.hasArg("rs485_out")) settings->rs485_out = (uint8_t)clampInt(server.arg("rs485_out").toInt(), OUTPUT_MODE_OFF, OUTPUT_MODE_MAVLINK);
        if (server.hasArg("nmea_out")) settings->nmea_out = (uint8_t)clampInt(server.arg("nmea_out").toInt(), NMEA_OUTPUT_OFF, NMEA_OUTPUT_BLUETOOTH);
        settings->bluetooth = (settings->nmea_out == NMEA_OUTPUT_BLUETOOTH) ? BLUETOOTH_LE : BLUETOOTH_OFF;
    ...
    if (settings->udp_port == 0) settings->udp_port = (uint16_t)NMEA_UDP_PORT;
        ...
        if (settings->rs485_out != previousRs485Mode)
        {
            NMEA_fini();
            RS485Display_fini();
            RS485Display_setup();
...
static void handlePacketCounters()
{
    ...
    json += F("\",\"tracker_active\":");
    json += trackerDiag.linkActive ? F("true") : F("false");
    json += F(",\"tracker_rx_frames\":");
    json += String(trackerDiag.rxFrames);
    json += F(",\"tracker_tx_frames\":");
    json += String(trackerDiag.txFrames);
    json += F(",\"tracker_rx_bytes\":");
    json += String(trackerDiag.rxBytes);
    json += F(",\"tracker_tx_bytes\":");
    json += String(trackerDiag.txBytes);
    json += F(",\"tracker_rx_age_ms\":");
    json += (trackerDiag.lastRxAgeMs == UINT32_MAX) ? String(-1) : String(trackerDiag.lastRxAgeMs);
    json += F(",\"tracker_tx_age_ms\":");
    json += (trackerDiag.lastTxAgeMs == UINT32_MAX) ? String(-1) : String(trackerDiag.lastTxAgeMs);
    json += F(",\"tracker_rx_text\":\"");
    json += jsonEscape(String(trackerDiag.lastRxText));
    json += F("\",\"tracker_tx_text\":\"");
    json += jsonEscape(String(trackerDiag.lastTxText));
    json += F("\"");
    json += F("}");
    sendNoCache();
    server.send(200, "application/json; charset=utf-8", json);
...
static void handleUpdatePrepare()
{
    ...
    const uint32_t maxSketchSpace = otaMaxSketchSpace();
    if (maxSketchSpace == 0U)
    {
        g_updateMessage = F("FAIL: OTA partition unavailable; flash once by USB");
...
static void handleUpdateUpload()
{
    ...
    if (upload.status == UPLOAD_FILE_START)
    {
        ...
        uint32_t maxSketchSpace = otaMaxSketchSpace();
        if (maxSketchSpace == 0U)
        {
            g_updateMessage = F("FAIL: OTA partition unavailable; flash once by USB");
            g_updateOk = false;
