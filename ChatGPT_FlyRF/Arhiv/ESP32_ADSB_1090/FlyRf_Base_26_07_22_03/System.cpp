...
// Arduino-ESP32 normally confirms a new OTA image before setup() starts.
// Defer confirmation until FlyRF completes setup and runs stably.
...
namespace
{
    static bool g_otaPendingVerification = false;
    static bool g_otaBootConfirmed = true;
    static uint32_t g_otaVerificationStartMs = 0;
    static uint32_t g_otaLastConfirmAttemptMs = 0;
    static uint32_t g_healthyLoopCount = 0;
    constexpr uint32_t kOtaConfirmationDelayMs = 15000UL;
    constexpr uint32_t kOtaRequiredLoopCount = 100UL;

    static void beginOtaStartupVerification()
    {
        const esp_partition_t* running = esp_ota_get_running_partition();
        esp_ota_img_states_t state = ESP_OTA_IMG_UNDEFINED;
        g_otaPendingVerification = running &&
            esp_ota_get_state_partition(running, &state) == ESP_OK &&
            state == ESP_OTA_IMG_PENDING_VERIFY;
        g_otaBootConfirmed = !g_otaPendingVerification;
        g_otaVerificationStartMs = millis();
        g_healthyLoopCount = 0;

        if (g_otaPendingVerification)
            Serial.println(F("[OTA] New firmware pending verification; stability test started"));
        else
            Serial.println(F("[OTA] Running firmware is already confirmed"));
    }

    static void serviceOtaStartupVerification()
    {
        if (!g_otaPendingVerification) return;
        ++g_healthyLoopCount;
        const uint32_t now = millis();
        if ((uint32_t)(now - g_otaVerificationStartMs) < kOtaConfirmationDelayMs ||
            g_healthyLoopCount < kOtaRequiredLoopCount ||
            (uint32_t)(now - g_otaLastConfirmAttemptMs) < 1000UL)
            return;

        g_otaLastConfirmAttemptMs = now;
        const esp_err_t result = esp_ota_mark_app_valid_cancel_rollback();
        if (result == ESP_OK)
        {
            g_otaPendingVerification = false;
            g_otaBootConfirmed = true;
            Serial.println(F("[OTA] Startup verified; new firmware confirmed"));
        }
        else
        {
    ...
    {
        const bool simulatedTrafficMode = (mode == FLYRF_MODE_TXRX_TEST3 ||
                                           mode == FLYRF_MODE_TXRX_TEST4);
...
void SystemSetup()
{
    ...
    beginOtaStartupVerification();
    Serial.println(F("Setup End"));
}

bool SystemOtaPendingVerification()
{
    return g_otaPendingVerification;
}

bool SystemOtaBootConfirmed()
{
    return g_otaBootConfirmed;
}
