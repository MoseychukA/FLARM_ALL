...
static int16_t clampThresholdLevel(int value)
{
...
bool EEPROM_isStandardGnssBaud(uint32_t baud)
{
    switch (baud)
    {
        case 4800UL:
...
uint32_t EEPROM_normalizeGnssBaud(uint32_t baud)
{
    return EEPROM_isStandardGnssBaud(baud) ? baud : 9600UL;
}
...
static void setDefaultsBody()
{
    ...
    eeprom_block.field.settings.adsb_receiver = ADSB_RECEIVER_EXTERNAL;
    eeprom_block.field.settings.adsb_esp32_serial_out = 0;
    eeprom_block.field.settings.gnss_baud = 9600UL;
...
void EEPROM_setup(void)
{
    ...
    else
    {
        ...
        if (eeprom_block.field.version != FLYRF_EEPROM_VERSION)
        {
            ...
            {
                settings->threshold_level = clampThresholdLevel(settings->threshold_level);
                settings->adsb_esp32_serial_out = 0;
                settings->gnss_baud = 9600UL;
                eeprom_block.field.version = FLYRF_EEPROM_VERSION;
                EEPROM_store();
            }
            else
    ...
    if (settings != nullptr)
    {
        if (!EEPROM_isStandardGnssBaud(settings->gnss_baud))
        {
            settings->gnss_baud = 9600UL;
            changed = true;
        }
        ...
        const int16_t threshold = clampThresholdLevel(settings->threshold_level);
        if (settings->threshold_level != threshold)
        {
            settings->threshold_level = threshold;
            changed = true;
        }
        ...
        if (settings->adsb_receiver > ADSB_RECEIVER_INTERNAL)
        {
            settings->adsb_receiver = ADSB_RECEIVER_EXTERNAL;
            changed = true;
        }
        if (settings->adsb_esp32_serial_out > 1)
        {
            settings->adsb_esp32_serial_out = 0;
            changed = true;
        }
...
void EEPROM_setThresholdLevel(int16_t level, bool storeNow)
{
    ...
    settings->threshold_level = clampThresholdLevel(level);
    if (storeNow)
    {
        EEPROM_store();
    }
