...
static void Draw_circular_scale()
{
    ...
    for (int i = 0; i < 36; i++)
    {
        ...
        if (i % 3 == 0)
        {
            ...
            if (i == 0)
            {
                backsprite.drawString("N", lx[i * koeff_line] + 1, ly[i * koeff_line], 2);
            }
            if (i == 9)
            {
                backsprite.drawString("E", lx[i * koeff_line], ly[i * koeff_line], 2);
            }
            if (i == 18)
            {
                backsprite.drawString("S", lx[i * koeff_line], ly[i * koeff_line], 2);
            }
            if (i == 27)
            {
                backsprite.drawString("W", lx[i * koeff_line], ly[i * koeff_line], 2);
            }
        }
        else
        {
...
static void drawRadarBaseAndStatus()
{
    ...
    if (divider <= 32767)
    {
        ...
        const bool manualRange = set_view_range != 0 ||
            (settings != nullptr && settings->radar_range_mode != 0);
        const uint16_t rangeFrameColor = manualRange ? TFT_RED : TFT_DARKGREY;
        back.drawRect(data_KM_x - 30, data_KM_y - 1, 56, 17, rangeFrameColor);
    }
                        ...
                        */
                        ...
                        if (Container[i].latitude != 0.0 && Container[i].longitude != 0.0)
                        {
                            ...
                            */
                            ...
                            yield();                                        /*===============  Прямая отрисовка цели и формуляра в общий буфер ===============*/
