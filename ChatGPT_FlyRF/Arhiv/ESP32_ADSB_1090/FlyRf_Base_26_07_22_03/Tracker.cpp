...
namespace
{
    ...
    static uint32_t g_lastRxDataMs = 0U;
    static uint32_t g_lastTxDataMs = 0U;
    static uint32_t g_rxFrames = 0U;
    static uint32_t g_txFrames = 0U;
    static uint32_t g_rxBytes = 0U;
    static uint32_t g_txBytes = 0U;
    static char g_lastRxText[TRACKER_INPUT_BUFFER_SIZE + 1U] = {};
    static char g_lastTxText[TRACKER_INPUT_BUFFER_SIZE + 1U] = {};
    constexpr uint32_t TRACKER_LINK_TIMEOUT_MS = 10000UL;
    ...
    static void rememberTrackerText(char* destination, const char* text, size_t length)
    {
        if (destination == nullptr) return;
        const size_t copyLength = (text == nullptr) ? 0U : min(length, (size_t)TRACKER_INPUT_BUFFER_SIZE);
        if (copyLength > 0U) memcpy(destination, text, copyLength);
        destination[copyLength] = '\0';
        while (copyLength > 0U)
        {
            const size_t tail = strlen(destination);
            if (tail == 0U || (destination[tail - 1U] != '\r' && destination[tail - 1U] != '\n')) break;
            destination[tail - 1U] = '\0';
        }
    }
    ...
    static void trackerWriteLine(const char* text)
    {
        ...
        rememberTrackerText(g_lastTxText, text, strlen(text));
        g_txBytes += (uint32_t)strlen(text) + 2U;
        ++g_txFrames;
        g_lastTxDataMs = millis();
    }
    ...
    static void trackerWriteRawLine(const char* text)
    {
        ...
        rememberTrackerText(g_lastTxText, text, len);
        g_txBytes += (uint32_t)len + 2U;
        ++g_txFrames;
        g_lastTxDataMs = millis();
    }
...
// - line: Счетчик, индекс, позиция или номер элемента.
    static void trackerWriteNmeaBody(const char* body)
    {
        ...
        rememberTrackerText(g_lastTxText, line, strlen(line));
        g_txBytes += (uint32_t)strlen(line);
        ++g_txFrames;
        g_lastTxDataMs = millis();
    }
...
// - versionBuf: Буфер, текстовая строка или рабочее сообщение.
    ...
    static void finalizeIncomingFrame()
    {
        ...
        rememberTrackerText(g_lastRxText, g_rxBuffer, g_rxLen);
        ++g_rxFrames;
...
{
    const uint32_t nowMs = millis();
    diagnostics.rxFrames = g_rxFrames;
    diagnostics.txFrames = g_txFrames;
    diagnostics.rxBytes = g_rxBytes;
    diagnostics.txBytes = g_txBytes;
    diagnostics.lastRxAgeMs = (g_rxFrames > 0U) ? (uint32_t)(nowMs - g_lastRxDataMs) : UINT32_MAX;
    diagnostics.lastTxAgeMs = (g_txFrames > 0U) ? (uint32_t)(nowMs - g_lastTxDataMs) : UINT32_MAX;
    diagnostics.linkActive = g_rxFrames > 0U && diagnostics.lastRxAgeMs <= TRACKER_LINK_TIMEOUT_MS;
    strncpy(diagnostics.lastRxText, g_lastRxText, sizeof(diagnostics.lastRxText) - 1U);
    diagnostics.lastRxText[sizeof(diagnostics.lastRxText) - 1U] = '\0';
    strncpy(diagnostics.lastTxText, g_lastTxText, sizeof(diagnostics.lastTxText) - 1U);
    diagnostics.lastTxText[sizeof(diagnostics.lastTxText) - 1U] = '\0';
}
