...
namespace
{
    ...
    static uint8_t g_lastTimeCentisecond = 0;
    ...
    static bool parseNmeaTime(const char* value, int& hh, int& mm, int& ss, int& hsec)
    {
        ...
        hsec = 0;
        if (value[6] == '.' && value[7] >= '0' && value[7] <= '9')
        {
            hsec = (value[7] - '0') * 10;
...
// - degBuf: Буфер, текстовая строка или рабочее сообщение.
    ...
    static void applyUtcDateTime(int year, int month, int day, int hourUtc, int minuteUtc,
                                 int secondUtc, int centisecondUtc)
    {
        setTime(hourUtc, minuteUtc, secondUtc, day, month, year);
        g_lastTimeCentisecond = (uint8_t)centisecondUtc;
        g_lastDateYear = year;
...
// - year: Параметр геометрии, координаты, размера или угла.
    static void parseRmc(char* fields[], size_t count)
    {
        ...
        int hh = 0, mm = 0, ss = 0, hsec = 0;
        int day = 0, month = 0, year = 0;
        if (parseNmeaTime(fields[1], hh, mm, ss, hsec) && parseNmeaDate(fields[9], day, month, year))
        {
            applyUtcDateTime(year, month, day, hh, mm, ss, hsec);
        }
    }
    ...
    static void parseGga(char* fields[], size_t count)
    {
        ...
        {
            int hh = 0, mm = 0, ss = 0, hsec = 0;
            if (parseNmeaTime(fields[1], hh, mm, ss, hsec))
            {
                applyUtcDateTime(g_lastDateYear, g_lastDateMonth, g_lastDateDay, hh, mm, ss, hsec);
            }
        }
    }
...
void GNSS_setup()
{
    ...
    const uint32_t configuredBaud = settings
        ? EEPROM_normalizeGnssBaud(settings->gnss_baud)
        : GNSS_SERIAL_BAUD;
    Serial_GNSS.begin(configuredBaud, GNSS_SERIAL_CONFIG, PIN_GNSS_RX, PIN_GNSS_TX, false, 256);
    Serial_GNSS.enableIntTx(false);

    Serial.printf("[SETUP] GNSS RX=%d TX=%d baud=%lu\r\n", PIN_GNSS_RX, PIN_GNSS_TX, (unsigned long)configuredBaud);
    GNSS_applyCurrentStateToThisAircraft();
    refreshRs485Aux();
}
