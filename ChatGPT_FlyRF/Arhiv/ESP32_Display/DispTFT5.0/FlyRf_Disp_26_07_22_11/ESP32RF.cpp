/*
  ...
  This module keeps the original FlyRF data processing, alarms, scale control
  and button logic, then sends one hardware-independent state to the RGB-panel
  renderer.
*/
...
{
    // Пороговые значения полностью соответствуют FlyRf_Base_26_07_22_02:
    // ближайшая цель должна находиться внутри основного кольца.
    if (nearestM > 16000U) return 0;
    if (nearestM >  8000U) return 1;
    if (nearestM >  4000U) return 2;
    if (nearestM >  2000U) return 3;
    if (nearestM >  1000U) return 4;
    if (nearestM >   500U) return 5;
    if (nearestM >   250U) return 6;
    return 7;
}
...
{
    // Как в проекте базы: первое ручное переключение из автоматики
    // включает максимальный диапазон 32 км, затем диапазон уменьшается.
...
void processTraffic()
{
    ...
    g_state.radarRangeManual = set_view_range != 0 ||
        (settings != nullptr && settings->radar_range_mode != 0);
}
...
void buildState()
{
    ...
    g_state.baseConnected = RS485Display_lastRxMs() != 0 &&
        (uint32_t)(millis() - RS485Display_lastRxMs()) < BASE_LINK_TIMEOUT_MS;
