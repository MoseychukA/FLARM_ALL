...
{
  // Заполненные сечения исключают промежутки между параллельными линиями
  // на диагоналях и обеспечивают одинаковое смешение RGB-субпикселей.
  int dx = abs(x1 - x0);
  int sx = x0 < x1 ? 1 : -1;
  int dy = -abs(y1 - y0);
  int sy = y0 < y1 ? 1 : -1;
...
static void drawTrendArrow(int centerX, int topY, int bottomY, bool upward,
                           uint16_t color)
{
  const int headHeight = 7;
  const int headHalfWidth = 6;
  if (upward) {
    for (int row = 0; row < headHeight; ++row) {
      const int halfWidth = (row * headHalfWidth) / (headHeight - 1);
      fillRect(centerX - halfWidth, topY + row, halfWidth * 2 + 1, 1, color);
    }
    fillRect(centerX - 1, topY + headHeight, 3,
             max(1, bottomY - topY - headHeight + 1), color);
  } else {
    fillRect(centerX - 1, topY, 3,
             max(1, bottomY - topY - headHeight + 1), color);
    const int headTop = bottomY - headHeight + 1;
    for (int row = 0; row < headHeight; ++row) {
      const int halfWidth = ((headHeight - 1 - row) * headHalfWidth) /
                            (headHeight - 1);
      fillRect(centerX - halfWidth, headTop + row,
...
static float radarDistanceToPixels(uint16_t ringMeters, uint32_t distanceMeters,
                                   float ringRadius, float outerRadius)
{
  const uint32_t maxVisibleMeters = (uint32_t)ringMeters * 2U;
  const float clampedDistance = min((float)distanceMeters,
                                    (float)maxVisibleMeters);
  if (clampedDistance <= (float)ringMeters) {
    return ringMeters > 0U ?
      clampedDistance * ringRadius / (float)ringMeters : 0.0f;
  }
  return ringRadius +
    (clampedDistance - (float)ringMeters) * (outerRadius - ringRadius) /
    (float)(maxVisibleMeters - ringMeters);
}

enum RadarLabelSide : uint8_t {
  RADAR_LABEL_BELOW = 0,
  RADAR_LABEL_LEFT  = 1,
  RADAR_LABEL_ABOVE = 2,
  RADAR_LABEL_RIGHT = 3
...
static RadarLabelSide radarLabelSideFromCourse(int courseDeg)
{
  int normalized = courseDeg % 360;
  if (normalized < 0) normalized += 360;
  if (normalized < 45 || normalized >= 315) return RADAR_LABEL_BELOW;
  if (normalized < 135) return RADAR_LABEL_LEFT;
  if (normalized < 225) return RADAR_LABEL_ABOVE;
  return RADAR_LABEL_RIGHT;
}

static bool radarRectsOverlap(const RadarLabelRect &a, const RadarLabelRect &b,
                              int padding)
{
...
static RadarLabelRect radarLabelCandidate(RadarLabelSide side, int targetX,
                                          int targetY, int width, int height,
                                          int gap)
{
  RadarLabelRect result = {targetX - width / 2, targetY - height / 2,
                           width, height};
  switch (side) {
    case RADAR_LABEL_LEFT:
      result.x = targetX - 8 - gap - width;
      break;
    case RADAR_LABEL_RIGHT:
      result.x = targetX + 8 + gap;
      break;
    case RADAR_LABEL_ABOVE:
      result.y = targetY - 8 - gap - height;
      break;
    case RADAR_LABEL_BELOW:
    default:
      result.y = targetY + 8 + gap;
      break;
  }
  result.x = constrain(result.x, 0, LCD_WIDTH - width);
  result.y = constrain(result.y, 130, LCD_HEIGHT - height);
  return result;
}
...
static void drawRadar()
{
  ...
  char rangeText[20];
  if (rangeMeters >= 1000U) {
    snprintf(rangeText, sizeof(rangeText), "%u km", (unsigned)(rangeMeters / 1000U));
  } else {
    snprintf(rangeText, sizeof(rangeText), "%u m", (unsigned)rangeMeters);
  }
  // Исходная рамка (216,302,56,17), масштаб 1,5 и горизонтальное поле 40 px.
  const uint16_t rangeFrameColor = displayState.radarRangeManual ?
                                   COLOR_RED : COLOR_GRID;
  drawRectOutline(364, 453, 84, 26, rangeFrameColor);
  ...
  RadarLabelRect placedLabels[GL050001C0_40_MAX_TARGETS] = {};
  int placedTargetX[GL050001C0_40_MAX_TARGETS] = {};
  int placedTargetY[GL050001C0_40_MAX_TARGETS] = {};
  ...
  for (uint8_t index = 0; index < displayState.targetCount; ++index) {
    ...
    const float radius = radarDistanceToPixels(rangeMeters, target.distanceM,
                                               180.0f, (float)targetRadius);
    ...
    // Длина 30 px увеличена на 25% и округлена до 38 px.
    const int courseX = x + (int)lroundf(sinf(courseAngle) * 38.0f);
    const int courseY = y - (int)lroundf(cosf(courseAngle) * 38.0f);
    drawLine3px(x, y, courseX, courseY, color);
    char label[20];
    const int relativeAltitudeTens =
      (int)lroundf((float)target.relativeAltitudeM / 10.0f);
    if (relativeAltitudeTens > 0)
      snprintf(label, sizeof(label), "+%d", relativeAltitudeTens);
    else
      snprintf(label, sizeof(label), "%d", relativeAltitudeTens);
    const bool hasArrow = target.verticalRate != 0;
    const int textWidth = smoothTextWidth(label, FONT_SMALL);
    const int groupWidth = textWidth + (hasArrow ? 17 : 0);
    const int labelBlockWidth = max(58, groupWidth + 4);
    const int labelBlockHeight = 24;
    const int relativeCourse = ((int)target.courseDeg - (int)effectiveCourse + 360) % 360;
    const RadarLabelSide preferredSide = radarLabelSideFromCourse(relativeCourse);
    const RadarLabelSide sideOrder[4] = {
      preferredSide,
      (RadarLabelSide)((preferredSide + 1U) & 3U),
      (RadarLabelSide)((preferredSide + 3U) & 3U),
      (RadarLabelSide)((preferredSide + 2U) & 3U)
    };
    static const int gapVariants[] = {4, 10, 16, 22, 28};
    RadarLabelRect labelRect = radarLabelCandidate(preferredSide, x, y,
                                                   labelBlockWidth,
                                                   labelBlockHeight, 4);
    RadarLabelSide selectedSide = preferredSide;
    bool positionFound = false;
    for (uint8_t sideIndex = 0; sideIndex < 4 && !positionFound; ++sideIndex) {
      for (uint8_t gapIndex = 0; gapIndex < sizeof(gapVariants) / sizeof(gapVariants[0]); ++gapIndex) {
        const RadarLabelRect candidate = radarLabelCandidate(
          sideOrder[sideIndex], x, y, labelBlockWidth, labelBlockHeight,
          gapVariants[gapIndex]);
        bool overlaps = false;
        for (uint8_t placed = 0; placed < placedCount && !overlaps; ++placed) {
          overlaps = radarRectsOverlap(candidate, placedLabels[placed], 3);
          const RadarLabelRect symbolArea = {placedTargetX[placed] - 18,
                                             placedTargetY[placed] - 18,
                                             36, 36};
          overlaps = overlaps || radarRectsOverlap(candidate, symbolArea, 0);
        }
        if (!overlaps) {
          labelRect = candidate;
          selectedSide = sideOrder[sideIndex];
          positionFound = true;
          break;
        }
      }
    ...
    // Слева от цели вся группа выравнивается по правому краю формуляра.
    // На остальных сторонах она начинается от левого края, как в исходнике.
    const int labelX = selectedSide == RADAR_LABEL_LEFT ?
      labelRect.x + labelRect.width - groupWidth - 1 : labelRect.x + 2;
    const int labelBaselineY = labelRect.y + 20;
    drawSmoothString(labelX, labelBaselineY, label, color, FONT_SMALL);

    // Как в исходном проекте: стрелка начинается после последней цифры
    // формуляра, а не в фиксированной позиции относительно символа цели.
    const int arrowX = labelX + smoothTextWidth(label, FONT_SMALL) + 5;
    const int arrowTopY = labelBaselineY - 17;
    const int arrowBottomY = labelBaselineY + 1;
    if (target.verticalRate > 0) {
      drawTrendArrow(arrowX, arrowTopY, arrowBottomY, true, color);
    } else if (target.verticalRate < 0) {
      drawTrendArrow(arrowX, arrowTopY, arrowBottomY, false, color);
    }

    if (placedCount < GL050001C0_40_MAX_TARGETS) {
      placedLabels[placedCount] = labelRect;
      placedTargetX[placedCount] = x;
      placedTargetY[placedCount] = y;
      ++placedCount;
    }
  }
}
                         ...
                         const GL050001C0_40_State &b)
{
  if (a.baseConnected != b.baseConnected || a.sosActive != b.sosActive ||
      ...
      a.radarRangeManual != b.radarRangeManual ||
