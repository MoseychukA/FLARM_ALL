...
void SystemLoop()
{
    ...
    if ((uint32_t)(now - g_lastLoopMs) > 500)
    {
        g_lastLoopMs = now;
#if LED_PIN >= 0
        digitalWrite(LED_PIN, (RS485Display_lastRxMs() &&
            (uint32_t)(now - RS485Display_lastRxMs()) < BASE_LINK_TIMEOUT_MS) ? HIGH : LOW);
#endif
    }
}
...
{
    return OtaRollback_pendingVerification();
}

bool SystemOtaBootConfirmed()
{
    return OtaRollback_bootConfirmed();
}
