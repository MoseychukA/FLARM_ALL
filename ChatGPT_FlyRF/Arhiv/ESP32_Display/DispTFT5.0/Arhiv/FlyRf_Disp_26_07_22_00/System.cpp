...
void SystemSetup()
{
    ...
    EEPROM_setup();
    // RGB panel and active WiFi can interfere with each other. Every boot
    // therefore starts with WiFi off; Bluetooth may enable it afterwards.
    WiFi_setControlMode(WIFI_CONTROL_OFF, false);
    TrafficDB.init();
    RS485Display_setup();
    WiFi_setup();
    Web_setup();
