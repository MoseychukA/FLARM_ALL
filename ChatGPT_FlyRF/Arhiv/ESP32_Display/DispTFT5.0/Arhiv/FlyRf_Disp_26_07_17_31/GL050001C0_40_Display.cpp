...
static bool frameUpdateRequested = false;
...
bool GL050001C0_40_setup()
{
  ...
  config.num_fbs = 1;
  // DMA читает данные порциями по 20 строк из внутренних bounce-буферов.
  ...
  drawShell();
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
  Serial.println(F("[GL050001C0-40] RGB panel initialized: PCLK 24 MHz, bounce buffer 20 lines"));
#else
...
void GL050001C0_40_requestFrameUpdate()
{
...
{
  return ready && (!operationalFrameShown || frameUpdateRequested);
}
...
void GL050001C0_40_loop()
{
  ...
  if (!ready || !stateAvailable ||
      (operationalFrameShown && !frameUpdateRequested) ||
