...
bool GL050001C0_40_setup()
{
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
  config.num_fbs = 1;
  // Два внутренних DMA-буфера по 40 строк сглаживают паузы PSRAM при работе Wi-Fi.
  config.bounce_buffer_size_px = LCD_WIDTH * 40;
  ...
  drawShell();
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
  Serial.println(F("[GL050001C0-40] RGB panel initialized: PCLK 18 MHz, bounce buffer 40 lines, DMA burst 64 bytes"));
#else
