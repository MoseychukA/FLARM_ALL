...
static String exitPage()
{
    ...
    h += F("<h1>Выход из WEB-интерфейса</h1><p id='closeState'>Нажмите кнопку закрытия.</p>");
    h += F("<button class='btn' onclick='closeInterface()'>ЗАКРЫТЬ СТРАНИЦУ</button><a class='btn' href='/'>Вернуться</a></div>");
    h += F("<script>function closeInterface(){var s=document.getElementById('closeState');if(s)s.textContent='WEB-интерфейс закрыт. Если браузер запрещает закрытие вкладки, будет открыта пустая страница.';try{window.open('','_self');window.close();}catch(e){}setTimeout(function(){try{location.replace('about:blank');}catch(e){document.body.innerHTML='';}},350);}</script></body></html>");
    return h;
}
