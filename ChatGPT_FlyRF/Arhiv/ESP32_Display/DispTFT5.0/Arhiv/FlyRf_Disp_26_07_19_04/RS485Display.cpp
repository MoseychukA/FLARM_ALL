...
static bool bufferedFrameValid(size_t payloadSize)
{
    ...
    if (g_rxBuffer[footerOffset] != 0xAA ||
        g_rxBuffer[footerOffset + 1] != 0xBB ||
        g_rxBuffer[footerOffset + 2] != 0xCC ||
        g_rxBuffer[footerOffset + 3] != 0xDD) return false;
    uint16_t receivedCrc = 0;
    memcpy(&receivedCrc, &g_rxBuffer[crcOffset], sizeof(receivedCrc));
    return receivedCrc == crc16_ccitt(&g_rxBuffer[4], payloadSize);
}
...
bool receivePacket_RS485(full_packet_net_t* pkt, uint8_t* btn1, uint8_t* btn2)
{
    ...
    while (RS485_SERIAL.available() && g_rxBuffered < sizeof(g_rxBuffer)) {
        g_rxBuffer[g_rxBuffered++] = (uint8_t)RS485_SERIAL.read();
        ++g_rxBytes;
    }
    if (g_rxBuffered < 4) return false;
    size_t start = 0;
    while (g_rxBuffered - start >= 4) {
        if (g_rxBuffer[start] == 0xDD && g_rxBuffer[start + 1] == 0xCC &&
            g_rxBuffer[start + 2] == 0xBB && g_rxBuffer[start + 3] == 0xAA) break;
        ++start;
    }
    if (start > 0) {
