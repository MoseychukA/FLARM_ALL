...
void updatePower()
{
    ...
    if (g_lastInaReadMs && (uint32_t)(now - g_lastInaReadMs) < 1000UL) return;
    g_lastInaReadMs = now;
    ...
    g_powerValid = isfinite(voltage) && isfinite(current) &&
                   voltage > 0.0f && voltage < 40.0f && current < 10000.0f;
    if (g_powerValid)
    {
        g_voltageV = voltage;
        g_currentMa = current;
        g_batteryPercent = batteryPercent(voltage);
    }
}
...
void Display_setup()
{
    ...
    if (g_displayReady)
    {
        GL050001C0_40_showStartup("FlyRf_Disp_26_07_19_02");
        buildState();
