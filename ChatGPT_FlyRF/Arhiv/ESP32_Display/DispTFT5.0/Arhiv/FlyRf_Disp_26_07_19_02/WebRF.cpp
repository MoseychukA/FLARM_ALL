...
static String exitPage()
{
    ...
    h += F("<h1>WEB-интерфейс закрыт</h1><p>Точку доступа можно оставить подключённой. Эту вкладку разрешается закрыть.</p>");
    h += F("<button class='btn' onclick='window.close()'>Закрыть страницу</button><a class='btn' href='/'>Вернуться</a></div>");
    h += F("<script>setTimeout(function(){window.open('','_self');window.close();},250);</script></body></html>");
    return h;
}
