...
static void flushFrame()
{
  if (ready && panel != NULL && frameBuffer != NULL) {
    ...
    // Кадр уже находится во внутреннем framebuffer драйвера. Перезапуск по VSYNC
    // начинает его передачу с первого пикселя и исключает постоянное смещение DMA.
...
bool GL050001C0_40_setup()
{
  ...
  // Заполняем framebuffer до запуска RGB DMA, чтобы панель не читала пустую память.
  drawShell();
  ...
  ready = true;
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
  Serial.println(F("[GL050001C0-40] RGB panel initialized: direct driver framebuffer, PCLK 18 MHz, bounce buffer 10 lines, DMA burst 64 bytes"));
#else
