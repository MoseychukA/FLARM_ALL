...
static void drawRadar()
{
  ...
  for (uint8_t index = 0; index < displayState.targetCount; ++index) {
    ...
    const int labelX = x + 16;
    const int labelBaselineY = y - 7;
    ...
    if (target.verticalRate > 0) {
      drawLine3px(arrowX, arrowBottomY, arrowX, arrowTopY, color);
      drawLine3px(arrowX, arrowTopY, arrowX - 5, arrowTopY + 6, color);
      drawLine3px(arrowX, arrowTopY, arrowX + 5, arrowTopY + 6, color);
    } else if (target.verticalRate < 0) {
      drawLine3px(arrowX, arrowTopY, arrowX, arrowBottomY, color);
      drawLine3px(arrowX, arrowBottomY, arrowX - 5, arrowBottomY - 6, color);
      drawLine3px(arrowX, arrowBottomY, arrowX + 5, arrowBottomY - 6, color);
    }
  }
}
