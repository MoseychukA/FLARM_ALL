...
static bool g_ina219ShuntOverrangeReported = false;
...
static void updatePowerTelemetry()
{
    ...
    const float voltageDropV = INA219_FIXED_INPUT_VOLTAGE_V - busVoltage;
    float currentMa = (voltageDropV * 1000.0f) / INA219_SHUNT_RESISTANCE_OHM;
    digitalWrite(POWER_ON_PIN, HIGH);

    if (currentMa < 0.0f) currentMa = 0.0f;

    if (!g_ina219ShuntOverrangeReported && fabsf(shuntVoltage) >= 319.0f)
    {
        Serial.println(F("[INA219] Shunt input is overrange; current uses (12.5 V - VIN-) / 10 ohm"));
        g_ina219ShuntOverrangeReported = true;
    }

    if (!(busVoltage >= 0.0f && busVoltage < 40.0f &&
          shuntVoltage > -1000.0f && shuntVoltage < 1000.0f &&
          currentMa >= 0.0f && currentMa < 10000.0f &&
          INA219_SHUNT_RESISTANCE_OHM > 0.0f))
    {
        g_powerTelemetryValid = false;
        return;
    }

    g_powerVoltageV = busVoltage;
    g_powerCurrentMa = currentMa;
    g_powerBatteryPercent = powerBatteryPercentFromVoltage(busVoltage);
...
{
    return RS485Display_baseConnected();
}
...
{
#if PROJECT_USE_GL050001C0_40
    static uint32_t lastDisplayedRs485RxPackets = 0U;
    static uint32_t lastRs485FrameRequestMs = 0U;
    static bool previousBaseLinkActive = false;
    static bool baseLinkStateInitialized = false;
    ...
    const uint32_t nowMs = millis();
    const uint32_t rs485RxPackets = RS485Display_rxPackets();
    const bool baseLinkActive = isBaseLinkActive();
    bool requestFrameUpdate = false;

    if (rs485RxPackets != lastDisplayedRs485RxPackets &&
        (lastRs485FrameRequestMs == 0U ||
         (uint32_t)(nowMs - lastRs485FrameRequestMs) >= PROJECT_RS485_DISPLAY_REFRESH_MS))
    {
        lastDisplayedRs485RxPackets = rs485RxPackets;
        lastRs485FrameRequestMs = nowMs;
        requestFrameUpdate = true;
    }

    if (!baseLinkStateInitialized)
    {
        previousBaseLinkActive = baseLinkActive;
        baseLinkStateInitialized = true;
    }
    else if (baseLinkActive != previousBaseLinkActive)
    {
        previousBaseLinkActive = baseLinkActive;
        requestFrameUpdate = true;
    }
    ...
    {
        GL050001C0_40_requestFrameUpdate();
    }

    if (GL050001C0_40_needsFrameUpdate())
    {
