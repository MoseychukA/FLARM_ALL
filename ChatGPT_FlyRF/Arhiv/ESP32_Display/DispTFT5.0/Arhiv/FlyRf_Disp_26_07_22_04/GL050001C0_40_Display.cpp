...
static void drawArrowLine(int x0, int y0, int x1, int y1, uint16_t color)
{
  const int dx = x1 - x0;
  const int dy = y1 - y0;
  const float length = sqrtf((float)(dx * dx + dy * dy));
  ...
  const int offsetX = (int)lroundf(-(float)dy / length);
  const int offsetY = (int)lroundf( (float)dx / length);
  drawLine3px(x0, y0, x1, y1, color);
  drawLine(x0 + 2 * offsetX, y0 + 2 * offsetY,
           x1 + 2 * offsetX, y1 + 2 * offsetY, color);
  drawLine(x0 - 2 * offsetX, y0 - 2 * offsetY,
           x1 - 2 * offsetX, y1 - 2 * offsetY, color);
}
...
static void drawRadar()
{
  ...
  for (uint8_t index = 0; index < displayState.targetCount; ++index) {
    ...
    if (target.verticalRate > 0) {
      drawArrowLine(arrowX, arrowBottomY, arrowX, arrowTopY, color);
      drawArrowLine(arrowX, arrowTopY, arrowX - 5, arrowTopY + 6, color);
      drawArrowLine(arrowX, arrowTopY, arrowX + 5, arrowTopY + 6, color);
    } else if (target.verticalRate < 0) {
      drawArrowLine(arrowX, arrowTopY, arrowX, arrowBottomY, color);
      drawArrowLine(arrowX, arrowBottomY, arrowX - 5, arrowBottomY - 6, color);
      drawArrowLine(arrowX, arrowBottomY, arrowX + 5, arrowBottomY - 6, color);
    }
