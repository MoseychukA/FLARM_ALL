...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
    const size_t frameSize = LCD_WIDTH * LCD_HEIGHT * sizeof(uint16_t);
    esp_err_t err = esp_cache_msync(frameBuffer, frameSize,
                                    ESP_CACHE_MSYNC_FLAG_DIR_C2M |
                                    ESP_CACHE_MSYNC_FLAG_TYPE_DATA);
    if (err != ESP_OK) {
      Serial.print(F("[GL050001C0-40] framebuffer cache sync failed: "));
      Serial.println(err);
      return;
    }
    err = esp_lcd_panel_draw_bitmap(panel, 0, 0, LCD_WIDTH, LCD_HEIGHT, frameBuffer);
    if (err != ESP_OK) {
      Serial.print(F("[GL050001C0-40] framebuffer submit failed: "));
      Serial.println(err);
      return;
    }
    // Следующий VSYNC начинает передачу подготовленного кадра с первого пикселя.
...
bool GL050001C0_40_setup()
{
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
  config.num_fbs = 1;
  // Два внутренних DMA-буфера по 10 строк сглаживают паузы PSRAM при работе Wi-Fi.
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
  void *driverFrameBuffer = NULL;
  err = esp_lcd_rgb_panel_get_frame_buffer(panel, 1, &driverFrameBuffer);
  if (err != ESP_OK || driverFrameBuffer == NULL) {
    Serial.print(F("[GL050001C0-40] driver framebuffer request failed: "));
    Serial.println(err);
    panel = NULL;
    return false;
  }
  frameBuffer = static_cast<uint16_t *>(driverFrameBuffer);
#else
  ...
  // В IDF 5.5 framebuffer окончательно готов после panel_init().
  drawShell();
  ready = true;
  flushFrame();
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
  Serial.println(F("[GL050001C0-40] RGB panel initialized: direct framebuffer with cache sync, PCLK 18 MHz, bounce buffer 10 lines, DMA burst 64 bytes"));
#else
