...
{
    static uint8_t buffer[sizeof(full_packet_net_t) + 16];
    static size_t idx = 0;
    while (RS485_SERIAL.available() && idx < sizeof(buffer)) buffer[idx++] = (uint8_t)RS485_SERIAL.read();
    ...
    const size_t crcOff = 4 + sizeof(full_packet_net_t);
    const size_t footerOff = crcOff + 2;
    bool footerOk = buffer[footerOff] == 0xAA && buffer[footerOff + 1] == 0xBB && buffer[footerOff + 2] == 0xCC && buffer[footerOff + 3] == 0xDD;
    if (!footerOk) { idx = 0; return false; }
    uint16_t crcRx; memcpy(&crcRx, &buffer[crcOff], sizeof(crcRx));
    uint16_t crcCalc = crc16_ccitt(&buffer[4], sizeof(full_packet_net_t));
    if (crcRx != crcCalc) { idx = 0; return false; }
    memcpy(pkt, &buffer[4], sizeof(full_packet_net_t));
    if (btn1) *btn1 = pkt->BUTTON1;
    if (btn2) *btn2 = pkt->BUTTON2;
    idx -= frameLen;
    if (idx > 0) memmove(buffer, buffer + frameLen, idx);
    return true;
}
...
{
    const bool firstValidPacket = (g_rxPackets == 0U);
    g_lastPacket = p;
    g_inAux = p.AuxData;
    ...
    ++g_rxPackets;
    if (firstValidPacket) {
        Serial.print(F("[RS485] Link active. Payload bytes: "));
        Serial.print(sizeof(full_packet_net_t));
        Serial.print(F(", frame bytes: "));
        Serial.println(sizeof(full_packet_net_t) + 10U);
    }
...
{
    full_packet_net_t pkt; uint8_t b1 = 0, b2 = 0;
