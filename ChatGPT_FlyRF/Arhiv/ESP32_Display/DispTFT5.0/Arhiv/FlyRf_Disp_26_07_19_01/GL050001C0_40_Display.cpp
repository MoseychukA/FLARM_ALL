...
static const uint32_t DISPLAY_UPDATE_INTERVAL_MS = 500UL;
...
static void drawRadar()
{
  ...
  for (uint8_t index = 0; index < displayState.targetCount; ++index) {
    ...
    if (target.verticalRate > 0) {
      drawLine(x + 7, y + 11, x + 7, y + 1, color);
      drawLine(x + 7, y + 1, x + 3, y + 5, color);
      drawLine(x + 7, y + 1, x + 11, y + 5, color);
    } else if (target.verticalRate < 0) {
      drawLine(x + 7, y + 1, x + 7, y + 11, color);
      drawLine(x + 7, y + 11, x + 3, y + 7, color);
      drawLine(x + 7, y + 11, x + 11, y + 7, color);
    }
  }
}
  ...
  if (displayState.showPowerVoltage || displayState.showPowerCurrent) {
    if (displayState.powerValid) {
      if (displayState.showPowerVoltage && displayState.showPowerCurrent) {
        ...
        snprintf(buffer, sizeof(buffer), "%.2f V", displayState.voltageV);
      } else {
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
  config.num_fbs = 1;
  // Параметры стабильного теста №7: 60 строк во внутренних bounce-буферах.
  config.bounce_buffer_size_px = LCD_WIDTH * 60;
#endif
...
void GL050001C0_40_loop()
{
  ...
  // Сначала готовим только изменившиеся части внешнего кадра в PSRAM.
  // В RGB-панель затем передаются отдельные горизонтальные полосы.
  if (updateTop) {
    fillRect(0, 0, LCD_WIDTH, 38, COLOR_SCREEN_BG);
    drawTopBar();
  }
  if (updateTable) {
  ...
  if (updateRadar) {
    fillRect(0, 126, LCD_WIDTH, LCD_HEIGHT - 126, COLOR_SCREEN_BG);
    drawRadar();
    drawStatusBlocks();
    drawOperationalOverlays();
