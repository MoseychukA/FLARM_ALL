...
#if PROJECT_USE_GL050001C0_40
// ESP32RF остаётся владельцем всей логики экрана. Этот объект является
// подготовленной моделью кадра; RGB-драйвер получает только готовые данные.
static GL050001C0_40_State g_glPreparedState = {};
static void populateGlPlaneTable();
#endif
...
#if PROJECT_USE_GL050001C0_40
    // Для RGB-дисплея функция сохраняет исходное назначение: выбирает строки
    // таблицы в соответствии с display_set. Рисование выполняется позже
    // драйвером GL050001C0-40 по подготовленной модели.
    populateGlPlaneTable();
    return;
#endif
...
static void copyContainerToGlTarget(GL050001C0_40_Target& target, int index)
{
    ...
    target.relativeAltitudeM = clampToInt16(height_difference[index]);
    target.speedKmh = clampToUint16(alien_speed_tmr[index] > 0 ?
                                    alien_speed_tmr[index] :
                                    (int)lroundf(Container[index].speed));
    target.courseDeg = (uint16_t)normalizeDisplayHeading360(
        (int)lroundf(Container[index].course));
    ...
    target.bearingDeg = (uint16_t)normalizeDisplayHeading360(
        (int)lroundf(Container[index].bearing));
    target.verticalRate = arrow_up_down[index] == 1 ? 1 :
                          (arrow_up_down[index] == 2 ? -1 : 0);
    target.alarmLevel = Container[index].alarm_level;
}

static void populateGlPlaneTable()
{
    g_glPreparedState.tableTargetCount = 0;
    g_glPreparedState.tableVisible = settings == nullptr ||
        settings->display_set == INFO_DISPLAY_COORDINATE ||
        settings->display_set == INFO_DISPLAY_MAXI;

    for (int index = 0; index < MAX_TRACKING_OBJECTS &&
                        g_glPreparedState.tableTargetCount < GL050001C0_40_MAX_TARGETS; ++index)
    {
        if (Container[index].addr == 0) continue;

        bool needDraw = settings == nullptr || settings->display_set == INFO_DISPLAY_MAXI;
        if (settings != nullptr && settings->display_set == INFO_DISPLAY_COORDINATE)
        {
            needDraw = Container[index].latitude != 0.0f &&
                       Container[index].longitude != 0.0f;
        }
        if (!needDraw) continue;

        GL050001C0_40_Target& row =
            g_glPreparedState.tableTargets[g_glPreparedState.tableTargetCount++];
        copyContainerToGlTarget(row, index);
    }
    line_Sprite = g_glPreparedState.tableTargetCount;
    prev_lines_count = line_Sprite;
}

static uint32_t processGlTrafficModel()
{
    thisAircraft_speed_tmr = (int)lroundf(ThisAircraft.speed);
    thisAircraft_altitude_tmr = (int)lroundf(ThisAircraft.altitude);
    thisAircraft_course_tmr = normalizeDisplayHeading360(
        (int)lroundf(SystemDisplayCourseDeg()));
    ...
    g_glPreparedState.targetCount = 0;

    for (int index = 0; index < MAX_TRACKING_OBJECTS; ++index)
    {
        if (Container[index].addr == 0)
        {
            isTeam_all[index] = false;
            continue;
        }
        isTeam_all[index] = true;
        updateTrafficPolarFromCoords(Container[index]);

        const uint32_t distanceMeters =
            (uint32_t)max(0, (int)lroundf(Container[index].distance));
        if (distanceMeters > 0 && distanceMeters < nearestMeters)
        {
            nearestMeters = distanceMeters;
            index_nearest_aircraft = index;
            alient_course0 = Container[index].course;
            alient_speed0 = Container[index].speed;
        }

        // Исходные накопительные фильтры скорости и высоты остаются в ESP32RF.
        alien_speed_filtre[index][alien_speed_array_count[index]] =
            (int)lroundf(Container[index].speed);
        alien_speed_array_count[index]++;
        if (alien_speed_array_count[index] >= speed_array_size)
        {
            alien_speed_array_count[index] = 0;
            alien_speed_array_countMax[index] = true;
        }
        if (alien_speed_array_countMax[index])
        {
            int sum = 0;
            for (int item = 0; item < speed_array_size; ++item)
                sum += alien_speed_filtre[index][item];
            alien_speed_tmr[index] = sum / speed_array_size;
        }
        else
        {
            alien_speed_tmr[index] = (int)lroundf(Container[index].speed);
        }

        if ((int)lroundf(Container[index].altitude) != 0)
        {
            alien_altitude_filtre[index][alien_altitude_array_count[index]] =
                (int)lroundf(Container[index].altitude);
        }
        alien_altitude_array_count[index]++;
        if (alien_altitude_array_count[index] >= altitude_array_size)
        {
            alien_altitude_array_count[index] = 0;
            alien_altitude_array_countMax[index] = true;
        }

        int filteredAltitude = (int)lroundf(Container[index].altitude);
        if (alien_altitude_array_countMax[index])
        {
            int sum = 0;
            for (int item = 0; item < altitude_array_size; ++item)
                sum += alien_altitude_filtre[index][item];
            filteredAltitude = sum / altitude_array_size;
        }
        if (filteredAltitude != 0)
            alien_altitude_actual[index] = filteredAltitude;

        const int hysteresisM = 10;
        if (abs(alien_altitude_actual[index] - alien_altitude_old[index]) > hysteresisM)
        {
            alien_altitude_old[index] = alien_altitude_actual[index];
            alien_altitude_hysteresis[index] = alien_altitude_actual[index];
        }
        if (alien_altitude_hysteresis[index] == 0)
            alien_altitude_hysteresis[index] = filteredAltitude;

        height_difference[index] = alien_altitude_hysteresis[index] -
                                   thisAircraft_altitude_tmr;
        if (alien_altitude_array_countMax[index] &&
            alien_altitude_hysteresis[index] != 0 &&
            (uint32_t)(millis() - tmr_array[index]) >
                100UL + (DATA_MEASURE_THRESHOLD * 4UL))
        {
            tmr_array[index] = millis();
            arrow_up_down[index] = alien_altitude_hysteresis[index] > old_alien_altitude_arrow[index] &&
                                   old_alien_altitude_arrow[index] != 0 ? 1 :
                                   (alien_altitude_hysteresis[index] < old_alien_altitude_arrow[index] &&
                                    old_alien_altitude_arrow[index] != 0 ? 2 : 0);
            old_alien_altitude_arrow[index] = alien_altitude_hysteresis[index];
        }

        const bool verticalValid = alien_altitude_hysteresis[index] != 0 &&
                                   thisAircraft_altitude_tmr != 0;
        const int verticalDistance = abs(height_difference[index]);
        little_air_color[index] = radarAlertColorForTarget(
            distanceMeters, verticalDistance, verticalValid);
        Container[index].alarm_level = radarAlertLevelForColor(little_air_color[index]);

        if (g_glPreparedState.targetCount < GL050001C0_40_MAX_TARGETS)
        {
            GL050001C0_40_Target& target =
                g_glPreparedState.targets[g_glPreparedState.targetCount++];
            copyContainerToGlTarget(target, index);
        }
        esp_task_wdt_reset();
    }

    g_lastAutoRadarScaleIndex = autoRadarScaleIndex(nearestMeters);
    const uint8_t scaleIndex = effectiveRadarScaleIndex(nearestMeters);
    g_lastEffectiveRadarScaleIndex = scaleIndex;
    divider = kRadarScaleDefs[scaleIndex].ringMeters;
    divider_num = scaleIndex + 1;
    return nearestMeters;
}

static void processGlDisplayControls()
{
    ...
    const uint8_t event = takeDisplayButtonEvent();
    switch (event)
    {
        case 1: (void)Tracker_confirmActiveTextMessage(); break;
        case 2: switchManualRadarToSmallerRange(); break;
        case 3: clearManualRadarRangeOverride(); break;
        default: break;
    }
}
...
static GL050001C0_40_State buildGlDisplayState()
{
    ...
    g_glPreparedState = {};
    GL050001C0_40_State& state = g_glPreparedState;
    ...
    state.sosActive = sosOverlayEnabledAndActive();
    ...
    state.showLanStatus = settings != nullptr && settings->lan_state_view != 0 &&
                          Remote_lanStatusReceived();
    state.lanReady = Remote_lanReady();
    state.lanLinkUp = Remote_lanLinkUp();
    state.lanUdpWorking = Remote_lanUdpWorking();
    Remote_lanIp(state.lanIp);
    state.lanUdpPort = Remote_lanUdpPort();
    state.lanTxPackets = Remote_lanTxPackets();
    state.lanRxPackets = Remote_lanRxPackets();
    ...
    processGlTrafficModel();
    const uint8_t scaleIndex = g_lastEffectiveRadarScaleIndex;
    state.radarRangeM = (uint16_t)kRadarScaleDefs[scaleIndex].ringMeters;

    const char* trackerMessage = Tracker_getActiveTextMessage();
    state.trackerMessageVisible = Tracker_hasActiveTextMessage() &&
                                  trackerMessage != nullptr &&
                                  trackerMessageBlinkVisible(trackerMessage);
    if (trackerMessage != nullptr)
    {
        strncpy(state.trackerMessage, trackerMessage, sizeof(state.trackerMessage) - 1U);
        state.trackerMessage[sizeof(state.trackerMessage) - 1U] = '\0';
    }
    aux_t aux = {};
    RS485Display_getIncomingAux(&aux);
    state.gnssStatusVisible = !Remote_baseTestMode() &&
                              (aux.gps_waiting_M || aux.gps_no_data_M);

    // Сохраняем исходную точку входа формирования таблицы.
    displayAllPlanes();
    return state;
}
#endif
...
void Display_loop()
{
    ...
    processGlDisplayControls();
    static uint32_t glModelUpdateMs = 0;
    const uint32_t nowMs = millis();
    if (glModelUpdateMs == 0 ||
        (uint32_t)(nowMs - glModelUpdateMs) >= DATA_MEASURE_THRESHOLD)
    {
        glModelUpdateMs = nowMs;
