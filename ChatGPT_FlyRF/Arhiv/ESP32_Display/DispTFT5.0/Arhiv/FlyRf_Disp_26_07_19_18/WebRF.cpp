...
static uint32_t g_displayUpdateNotBeforeMs = 0;

static const char* sel(bool v) { return v ? "selected" : ""; }
static int clampInt(int v, int lo, int hi) { return v < lo ? lo : (v > hi ? hi : v); }
static void deferDisplayUpdate(uint32_t pauseMs=350){ g_displayUpdateNotBeforeMs = millis() + pauseMs; }
static void sendNoCache(){ deferDisplayUpdate(); server.sendHeader("Cache-Control","no-cache, no-store, must-revalidate"); server.sendHeader("Pragma","no-cache"); server.sendHeader("Expires","-1"); }
...
static String htmlBegin(const String& title)
{
    ...
    s += F(".viewToggleWrap{display:flex;justify-content:flex-end;gap:8px;margin:0 0 8px 0;}.viewToggle{display:inline-block;font-family:Arial,'Times New Roman',serif;font-size:15px;line-height:1.1;padding:7px 12px;border:1px solid #8b8b8b;border-radius:3px;background:#ececec;color:#000;text-decoration:none;cursor:pointer;}.exitBtn{background:#f5dddd;border-color:#b55;}");
...
static String rs485State()
{
    ...
    if (last && millis() - last < 3000) return F("<span class='ok'>связь с базой есть</span>");
...
static String settingsPage()
{
    ...
    h += F("</table></div><div class='btnRowEqual'><button class='btn' type='submit'><span class='btnIcon'>&#128190;</span>Применить и остаться</button><a class='btn' href='/'><span class='btnIcon'>&#8634;</span>Возврат без сохранения</a></div></form>");
    h += htmlEnd(); return h;
}
...
static String firmwarePage()
{
    ...
    h += F("<div class='settingsBlock'><div class='blockTitle blockTitleCenter'>Загрузка прошивки внешнего дисплея</div>");
    h += F("<form id='upload_form' method='POST' action='/update' enctype='multipart/form-data'>");
    h += F("<input id='fwFile' name='update' type='file' accept='.bin,application/octet-stream'>");
    h += F("<div class='btnRowEqual'><label class='btn' for='fwFile'>Выбрать файл .bin</label><button id='uploadBtn' class='btn danger' type='submit' disabled>Загрузить прошивку</button></div>");
    h += F("<p id='fileName' class='center muted'>Файл не выбран</p>");
    h += F("<div class='progress'><div id='bar' class='bar'></div></div>");
    h += F("<p id='st' class='center muted'>Выберите файл прошивки с расширением .bin</p>");
    h += F("</form><div class='btnRowEqual'><a class='btn' href='/'>Назад</a></div></div>");
    h += F("<script>");
    h += F("let form=document.getElementById('upload_form'),file=document.getElementById('fwFile'),btn=document.getElementById('uploadBtn'),bar=document.getElementById('bar'),st=document.getElementById('st'),fn=document.getElementById('fileName');");
    h += F("file.onchange=function(){let ok=file.files&&file.files.length>0;btn.disabled=!ok;if(ok){fn.textContent=file.files[0].name;st.textContent='Файл выбран. Нажмите Загрузить прошивку';bar.style.width='0%';}else{fn.textContent='Файл не выбран';st.textContent='Выберите файл прошивки с расширением .bin';}};");
    h += F("form.onsubmit=function(e){e.preventDefault();if(!file.files||file.files.length==0){st.textContent='Сначала выберите файл .bin';return;}btn.disabled=true;let fd=new FormData(form),x=new XMLHttpRequest();x.upload.onprogress=function(ev){if(ev.lengthComputable){let p=Math.round(ev.loaded*100/ev.total);bar.style.width=p+'%';st.textContent='Загрузка '+p+'%';}};x.onload=function(){if(x.responseText=='OK'){bar.style.width='100%';st.textContent='Обновление завершено. Перезагрузка...';}else{btn.disabled=false;st.textContent='Ошибка обновления';}};x.onerror=function(){btn.disabled=false;st.textContent='Ошибка связи при загрузке';};x.open('POST','/update');x.send(fd);};");
    h += F("</script>");
    h += htmlEnd();
    return h;
}

static String exitPage()
{
    String h;
    h.reserve(1600);
    h += F("<!DOCTYPE html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Выход</title>");
    h += F("<style>body{font-family:Arial,sans-serif;background:#f3f3f3;text-align:center;padding:40px 12px}.box{max-width:560px;margin:auto;background:#fff;border:1px solid #ccc;border-radius:8px;padding:24px}.btn{display:inline-block;font-size:20px;padding:12px 18px;margin:8px;border:1px solid #888;border-radius:4px;background:#eee;color:#000;text-decoration:none;cursor:pointer}</style></head><body><div class='box'>");
    h += F("<h1>Выход из WEB-интерфейса</h1><p>После подтверждения ESP32-S3 будет перезагружен. Точка доступа временно отключится.</p>");
    h += F("<form method='POST' action='/reboot'><button class='btn' type='submit' style='background:#b53333;border-color:#d94a4a;color:#fff'>ВЫХОД И ПЕРЕЗАГРУЗКА</button></form><a class='btn' href='/'>Вернуться</a></div></body></html>");
    return h;
}

static String rebootPage()
{
    return F("<!DOCTYPE html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Перезагрузка</title><style>body{font-family:Arial,sans-serif;background:#f3f3f3;text-align:center;padding:48px 12px}.box{max-width:560px;margin:auto;background:#fff;border:1px solid #ccc;border-radius:8px;padding:24px}</style></head><body><div class='box'><h1>Устройство перезагружается</h1><p>Команда принята. Через несколько секунд точка доступа появится снова.</p></div><script>setTimeout(function(){try{location.replace('about:blank');}catch(e){}},1200);</script></body></html>");
}
...
static void handleExit(){ sendNoCache(); server.send(200,"text/html; charset=utf-8",exitPage()); }
static void handleReboot(){ sendNoCache(); server.send(200,"text/html; charset=utf-8",rebootPage()); scheduleReboot(1800); }
static void handleCaptivePortal(){ deferDisplayUpdate(); server.sendHeader("Location","http://192.168.1.1/",true); server.send(302,"text/plain",""); }

static void handleUpdateFinished(){ sendNoCache(); server.send(200,"text/plain; charset=utf-8",(Update.hasError()||!g_updateOk)?"FAIL":"OK"); if(!Update.hasError()&&g_updateOk) scheduleReboot(4000); }
static void handleUpdateUpload()
{
    HTTPUpload& upload=server.upload();
    if(upload.status==UPLOAD_FILE_START){ g_updateOk=false; g_otaInProgress=true; g_otaWritten=0; g_otaTotal=upload.totalSize; g_updateMessage="Подготовка обновления"; uint32_t maxSketchSpace=((ESP.getFreeSketchSpace()-0x1000)&0xFFFFF000); if(!Update.begin(maxSketchSpace)){g_updateMessage="Не удалось начать обновление"; Update.printError(Serial);} }
    else if(upload.status==UPLOAD_FILE_WRITE){ if(Update.write(upload.buf,upload.currentSize)!=upload.currentSize){ g_updateOk=false; g_otaInProgress=false; g_updateMessage="Ошибка записи прошивки"; Update.printError(Serial);} else {g_otaWritten += upload.currentSize; g_updateMessage="Передача файла";} }
    else if(upload.status==UPLOAD_FILE_END){ g_otaInProgress=false; if(Update.end(true)){g_updateOk=true; g_otaWritten=g_otaTotal; g_updateMessage="Обновление завершено";} else {g_updateOk=false; g_updateMessage="Ошибка завершения обновления"; Update.printError(Serial);} }
    else if(upload.status==UPLOAD_FILE_ABORTED){ g_otaInProgress=false; g_updateOk=false; g_updateMessage="Загрузка отменена"; Update.abort(); }
    yield();
}
...
static void handleNotFound(){ handleCaptivePortal(); }
...
void Web_setup()
{
    ...
    dnsServer.start(53, "*", WiFi_apIP());
    server.begin();
    Serial.println(F("[WEB] Сервер внешнего дисплея запущен"));
}
void Web_loop(){ dnsServer.processNextRequest(); server.handleClient(); if(g_rebootPending && (int32_t)(millis()-g_rebootAtMs)>=0){ g_rebootPending=false; ESP.restart(); } }
void Web_fini(){ dnsServer.stop(); server.stop(); }
bool Web_displayUpdateAllowed(){ return !g_otaInProgress && (int32_t)(millis()-g_displayUpdateNotBeforeMs)>=0; }
