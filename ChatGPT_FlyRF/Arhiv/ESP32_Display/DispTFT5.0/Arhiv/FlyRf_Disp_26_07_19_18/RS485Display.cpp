...
static volatile uint32_t g_rxPackets = 0;
static volatile uint32_t g_txPackets = 0;
static TaskHandle_t g_rxTask = nullptr;
...
void RS485Display_setup()
{
    ...
    if (g_rxTask == nullptr)
    {
        const BaseType_t created = xTaskCreatePinnedToCore(
            rxTask, "RS485_RX", 6144, nullptr, 2, &g_rxTask, 0);
        Serial.println(created != 0 ?
                       F("[RS485] continuous RX task started on core 0") :
                       F("[RS485] ERROR: RX task was not created"));
    }
}
