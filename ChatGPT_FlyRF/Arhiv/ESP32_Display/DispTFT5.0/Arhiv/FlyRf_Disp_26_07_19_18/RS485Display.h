...
// Структура приёма из исходной базы. Она полностью повторяет ufo_net_t;
// единственное отличие — передаваемый базой timestamp занимает 32 бита.
typedef struct __attribute__((packed)) {
    uint32_t addr;
    int      squawk;
    uint8_t  callsign[8];
...
typedef struct __attribute__((packed)) {
    ufo_net_rx_t ThisAircraft;
    ufo_net_rx_t Container[MAX_TRACKING_OBJECTS];
    aux_t        AuxData;
    uint8_t      BUTTON1;
    uint8_t      BUTTON2;
} full_packet_rx_t;
...
static_assert(sizeof(ufo_net_rx_t) == 59U, "ufo_net_rx_t must be 59 bytes");
static_assert(sizeof(full_packet_rx_t) == 1223U, "full_packet_rx_t must be 1223 bytes");
