...
static uint8_t g_rxBuffer[RS485_FRAME_SIZE + 64U] = {};
...
bool receivePacket_RS485(full_packet_net_t* packet, uint8_t* button1, uint8_t* button2)
{
    ...
    if (g_rxBuffered < RS485_FRAME_SIZE) return false;

    // Формат исходной базы один и фиксированный:
    // header(4) + payload(1223) + CRC(2) + footer(4).
    constexpr size_t crcOffset = 4U + RS485_PAYLOAD_SIZE;
    constexpr size_t footerOffset = crcOffset + 2U;
    const bool footerOk =
        g_rxBuffer[footerOffset] == 0xAA &&
        g_rxBuffer[footerOffset + 1U] == 0xBB &&
        g_rxBuffer[footerOffset + 2U] == 0xCC &&
        g_rxBuffer[footerOffset + 3U] == 0xDD;
    if (!footerOk)
    {
    ...
    // Сначала принимаем точную отдельную структуру исходной базы. Только
    // после CRC переводим каждое поле в нативные структуры приложения.
    base_packet_wire_t wirePacket = {};
    memcpy(&wirePacket, &g_rxBuffer[4], sizeof(wirePacket));
    wirePacketToNative(wirePacket, *packet);
    if (button1) *button1 = packet->BUTTON1;
    if (button2) *button2 = packet->BUTTON2;
    g_rxBuffered -= RS485_FRAME_SIZE;
    if (g_rxBuffered > 0U)
        memmove(g_rxBuffer, g_rxBuffer + RS485_FRAME_SIZE, g_rxBuffered);
    return true;
}
