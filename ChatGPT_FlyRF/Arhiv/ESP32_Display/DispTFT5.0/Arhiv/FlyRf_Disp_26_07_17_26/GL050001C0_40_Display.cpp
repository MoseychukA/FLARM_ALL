...
bool GL050001C0_40_setup()
{
  ...
  config.num_fbs = 1;
  // В пакете 3.3.10 отключаем новый bounce-режим для точного повтора версии 19.
  ...
  drawShell();
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
  Serial.println(F("[GL050001C0-40] RGB panel initialized: version 19 rollback, PCLK 24 MHz, bounce buffer disabled"));
#else
