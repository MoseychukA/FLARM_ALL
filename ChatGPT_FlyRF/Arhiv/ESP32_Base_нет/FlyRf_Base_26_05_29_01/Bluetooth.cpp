...
void Bluetooth_loop()
{
    ...
    const uint8_t mode = dataOutputEnabled() ? BLUETOOTH_LE : BLUETOOTH_OFF;  // Рабочая переменная модуля: хранит промежуточное значение, используемое в текущей логике файла.
    if (mode != g_lastMode)
    {
        g_lastMode = mode;
        ensureInitialized();
