...
#include <stdlib.h> // для free, если используете
...
public:
    ...
    /**
      * Инициализирует CommsManager. Настраивает UART и другие необходимые периферийные устройства.
      * @retval True, если инициализация прошла успешно, в противном случае false.
      */
    bool Init();

    /**
     * Обновить CommsManager. Запустить все подпрограммы обновления, необходимые для нормальной работы.
    ...
    SettingsManager::LogLevel log_level = SettingsManager::LogLevel::kInfo;  // Начните с максимальной детализации по умолчанию.

    // Очередь для хранения пакетов транспондера до их отправки.
   ...
   private:
    ...
    /**
    * Отправляет данные транспондера в формате Mode S Beast по выбранному последовательному интерфейсу. Сообщает обо всех пакетах транспондера
    * в предоставленном массиве packets_to_report, который используется для печати произвольных блоков пакетов транспондера,
    * полученных через встроенную в CommsManager очередь transponder_packet_reporting_queue_.
    * @param[in] iface SerialInterface для трансляции сообщений Mode S Beast.
    * @param[in] packets_to_report Массив пакетов транспондера для отчёта.
    * @param[in] num_packets_to_report Количество пакетов для отчёта из массива packets_to_report.
    * @retval True в случае успешного выполнения, false в случае возникновения ошибки.
    */
    bool ReportBeast(SettingsManager::SerialInterface iface, const Decoded1090Packet packets_to_report_1090[],
                     uint16_t num_packets_to_report);

    /**
    * Отправляет информацию о каждом самолёте из словаря, разделённую запятыми.
    * @param[in] iface SerialInterface для передачи информации о самолёте.
