...
namespace
{
    static bool IsADSBExtendedSquitter(uint16_t df)
    {
        return df == Decoded1090Packet::kDownlinkFormatExtendedSquitter ||
               df == Decoded1090Packet::kDownlinkFormatExtendedSquitterNonTransponder ||
               df == Decoded1090Packet::kDownlinkFormatMilitaryExtendedSquitter;
    }

    static bool CorrectedADSBPacketLooksPlausible(const Decoded1090Packet& packet)
    {
        if (!packet.IsValid() || !IsADSBExtendedSquitter(packet.GetDownlinkFormat()))
        {
            return false;
        }

        const uint32_t icao = packet.GetICAOAddress();
        if (icao == 0 || icao == 0xFFFFFF)
        {
            return false;
        }

        const ADSBPacket adsb_packet(packet);
        const uint16_t type_code = adsb_packet.GetTypeCode();
        return type_code >= 1 && type_code <= 31;
    }
}
...
bool PacketDecoder::UpdateDecoderLoop() 
{
    ...
    for (uint16_t i = 0; i < num_packets_to_process; i++)
    {
        ...
        if (decoded_packet.IsValid() ||
            decoded_packet.GetDownlinkFormat() == Decoded1090Packet::kDownlinkFormatAltitudeReply ||
            decoded_packet.GetDownlinkFormat() == Decoded1090Packet::kDownlinkFormatIdentityReply ||
            decoded_packet.GetDownlinkFormat() == Decoded1090Packet::kDownlinkFormatAllCallReply ||
            decoded_packet.GetDownlinkFormat() == Decoded1090Packet::kDownlinkFormatCommBAltitudeReply ||
            decoded_packet.GetDownlinkFormat() == Decoded1090Packet::kDownlinkFormatCommBIdentityReply)
        {
            ...
            strncpy(decode_debug_message.message, "[VALID     ] ", DebugMessage::kMessageMaxLen);
                 ...
                 IsADSBExtendedSquitter(decoded_packet.GetDownlinkFormat()))
        {
            ...
            if (bit_flip_index >= Decoded1090Packet::kDFNUmBits)
            {
                ...
                Decoded1090Packet corrected_packet(*raw_packet_ptr);
                if (CorrectedADSBPacketLooksPlausible(corrected_packet))
                {
                    decoded_1090_packet_bit_flip_locations_out_queue.Push(bit_flip_index);
                    decoded_1090_packet_out_queue.Push(corrected_packet);
