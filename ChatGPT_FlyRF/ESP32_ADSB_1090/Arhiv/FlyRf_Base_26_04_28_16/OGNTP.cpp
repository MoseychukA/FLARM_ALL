/*
  Модуль OGNTP.cpp
  Назначение:
  - Поддержка протокола OGNTP для LoRa-обмена.

  Основные задачи модуля:
  - Описывать параметры радиопротокола OGNTP.
  - Кодировать данные нашего самолета в формат OGNTP.
  - Декодировать принятые пакеты и извлекать данные сторонних целей.
*/
...
#endif
  .tx_interval_min = OGNTP_TX_INTERVAL_MIN,  // 600
  .tx_interval_max = OGNTP_TX_INTERVAL_MAX,  // 1400
  .slot0           = {400,  800},
  .slot1           = {800, 1200}
};

static GPS_Position pos;  // Счетчик или индекс: указывает позицию элемента, номер строки, слота или текущую стадию перебора.
static OGN_TxPacket ogn_tx_pkt;  // Навигационный или геометрический параметр: координаты, угол, дальность, размер или положение объекта.
static OGN_RxPacket ogn_rx_pkt;  // Навигационный или геометрический параметр: координаты, угол, дальность, размер или положение объекта.

//------------------------------------------------------------------------------
// Назначение функции: Выполняет логику функции `ogntp_init` и обрабатывает протокол OGNTP в контексте модуля OGNTP.cpp.
...
//------------------------------------------------------------------------------
// Назначение функции: Выполняет логику функции `ogntp_fini` и обрабатывает протокол OGNTP fini в контексте модуля OGNTP.cpp.
// Локальные переменные: явные локальные переменные не используются; функция работает через параметры, глобальные структуры и результаты вызовов.
//------------------------------------------------------------------------------
void ogntp_fini()
{
...
//------------------------------------------------------------------------------
// Назначение функции: Выполняет логику функции `ogntp_decode` и обрабатывает протокол OGNTP в контексте модуля OGNTP.cpp.
// Локальные переменные: явные локальные переменные не используются; функция работает через параметры, глобальные структуры и результаты вызовов.
// - key: Параметр геометрии, координаты, размера или угла.
//------------------------------------------------------------------------------
...
//  }
  ...
  uint8_t addr_type = ADDR_TYPE_ANONYMOUS;  // Навигационный или геометрический параметр: координаты, угол, дальность, размер или положение объекта.
#else
  uint8_t addr_type = (this_aircraft->addr == FLYRF_ADDRESS ?
...
//------------------------------------------------------------------------------
// Назначение функции: Выполняет логику функции `ogntp_encode` и обрабатывает протокол OGNTP в контексте модуля OGNTP.cpp.
// Локальные переменные: явные локальные переменные не используются; функция работает через параметры, глобальные структуры и результаты вызовов.
// - key: Параметр геометрии, координаты, размера или угла.
//------------------------------------------------------------------------------
...
#else
  ogn_tx_pkt.Packet.Header.AddrType = (this_aircraft->addr == FLYRF_ADDRESS ?
                                      ADDR_TYPE_ICAO : ADDR_TYPE_ANONYMOUS);
#endif
