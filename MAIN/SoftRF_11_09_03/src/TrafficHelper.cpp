 ...
 * Прогнозирование столкновений на основе линейного, CoG и GS.
 */
static int8_t Alarm_Vector(ufo_t *this_aircraft, ufo_t *fop)
{
  ...
  { /* no warnings if too high or too low. нет предупреждений, если слишком высокий или слишком низкий*/

    /* Subtract 2D velocity vector of traffic from 2D velocity vector of this aircraft */ 
    /* Вычитаем двумерный вектор скорости движения из двумерного вектора скорости этого самолета */
    ...
    /* преобразуем математический угол в курс относительно севера */
    V_rel_direction = (V_rel_direction <= 90.0 ? 90.0 - V_rel_direction :
                                                450.0 - V_rel_direction);

    /* +- 10 degrees tolerance for collision course.
    +- 10 градусов допуск на курс столкновения*/
    if (V_rel_magnitude > 0.1 && fabs(V_rel_direction - fop->bearing) < 10.0) 
    {

      /* time is seconds prior to impact.
         время указано в секундах до  столкновения*/
      float t = fop->distance / V_rel_magnitude;

      /* time limit values are compliant with FLARM data port specs. 
      значения ограничения по времени соответствуют спецификациям порта данных FLARM */
      if (t < 9.0) 
      {
        rval = ALARM_LEVEL_URGENT;    // УРОВЕНЬ ТРЕВОГИ СРОЧНО
      }
      else if (t < 13.0) 
      {
        rval = ALARM_LEVEL_IMPORTANT; // УРОВЕНЬ ТРЕВОГИ ВАЖНО
      }
      else if (t < 19.0) 
      {
        rval = ALARM_LEVEL_LOW;      // НИЗКИЙ УРОВЕНЬ ТРЕВОГИ
      }    
    }
  }
...
void Traffic_Update(ufo_t *fop)
{
 ...
 /* Serial.print("fop->distance ");
  Serial.println(fop->distance);

  Serial.print("fop->bearing ");
  Serial.println(fop->bearing);*/
