...
int Speed_alien_filtre[MAX_TRACKING_OBJECTS][speed_array_size]; // Фильтр скорости стороннего самолета
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
        ...
        for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
        {
            if (Container[i].addr)
            {
               // speed_tmr[i] = (int)Container[i].speed;
                ...
                Speed_alien_filtre[i][speed_array_count] = (int)Container[i].speed;

                speed_array_count++;
                int val_speed = 0;
                if (speed_array_count > speed_array_size)                    // проверка заполнения массива первичными данными о скорости
                {
                    speed_array_count = 0;
                    speed_array_countMax = true;                             //Разрешить выдавать данные о величине скорости
                }

                speed_sum = 0;

                if (speed_array_countMax)                                   // формируем данные о величине скорости
                {
                    for (int k = 0; k < speed_array_size; k++)
                    {
                        speed_sum += Speed_alien_filtre[i][speed_array_count];
                    }
                    val_speed = speed_sum / speed_array_size;
                }
                else
                {
                    for (int k = 0; k < speed_array_count; k++)               //формируем первичные (заполняем массив) данные об уровне напряжения аккумулятора
                    {
                        speed_sum += Speed_alien_filtre[i][speed_array_count-1];
                    }
                    val_speed = speed_sum / speed_array_count;
                }

                speed_sum = 0;
                speed_tmr[i] = val_speed;

                //Serial.print("cont_speed");
                //Serial.print((int)Container[i].speed);
  ...
  //      data_KM.fillSprite(backColor);
        ...
        data_az.drawSmoothRoundRect(0, 0, 5, 5, 50, 25, TFT_WHITE);
       ...
       // data_az.drawRect(0, 0, 48, 25, TFT_WHITE);
        ...
        if (arr_min != 32767)
        {
            dist_info.drawSmoothRoundRect(0, 0, 5, 5, 80, 25, txt_color);
        ...
        {
            dist_info.drawSmoothRoundRect(0, 0, 5, 5, 80, 25, TFT_WHITE);
           // dist_info.drawRect(0, 0, 80, 25, TFT_WHITE);
        ...
        for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
        {
            if (isTeam_all[i] == true)
            {
               ...
               if (Air_txt_left[i])
               {
                   Air_txt_Sprite[i]->drawSmoothRoundRect(0, 0, 7, 7, 65, 29, TFT_SKYBLUE);
                   Air_txt_Sprite[i]->fillTriangle(76, 15, 67, 7, 67, 23, TFT_SKYBLUE);

               }
               else
               {
                   Air_txt_Sprite[i]->drawSmoothRoundRect(11, 0, 7, 7, 64, 29, TFT_SKYBLUE);
                   Air_txt_Sprite[i]->fillTriangle(0, 15, 9, 7, 9, 23, TFT_SKYBLUE);
               }
