...
TFT_eSprite* txt_airplane[MAX_TRACKING_OBJECTS];    //
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::setup(TFTMenu* menuManager)
 {
   ...
   for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
   {
       ...
       txt_airplane[i] = new TFT_eSprite(&tft);   // Спрайт информации стороннего воздушного объекта
       txt_airplane[i]->createSprite(20, 160);
       txt_airplane[i]->setPivot(10, 0);
 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
  ...
  /*                  Serial.print(i);
               ...
               /* Определяем разность высот и устанавливаем цвет предупреждения*/
                ...
                txt_airplane[i]->fillSprite(TFT_BLACK);      // Закрасим поле 
       ...
       // angle = (360 - test_curse) % 360;                       // Определяем направление нашего самолета и вращаем в другую сторону шкалу
        ...
        for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
        {
            if (isTeam_all[i] == true)
            {
               ...
               txt_airplane[i]->drawRect(0, 0, 20, 160, TFT_WHITE);
               txt_airplane[i]->pushRotated(&back, 180 + new_angle, TFT_BLACK); 
                isTeam_all[i] = false;
                esp_task_wdt_reset();
            }
        }  
