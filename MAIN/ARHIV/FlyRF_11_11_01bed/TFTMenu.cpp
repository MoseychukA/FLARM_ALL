...
TFT_eSprite* Air_txt_Sprite[MAX_TRACKING_OBJECTS];  // Спрайт отображения стороннего самолета
TFT_eSprite* little_airplane[MAX_TRACKING_OBJECTS]; // Спрайт отображения формуляра стороннего самолета

TFT_eSprite* arrow_Sprite[MAX_TRACKING_OBJECTS];       //На этом спрайте располагаются спрайты самолета и текста. Спрайт вращается в виде радиуса(стрелы от центра)
TFT_eSprite* area_airplane[MAX_TRACKING_OBJECTS];      // Этот спрайт, площадка в котором будет располагатся сторонний самолет
TFT_eSprite* area_txt_airplane[MAX_TRACKING_OBJECTS];  // Этот спрайт, площадка в котором будет располагатся формуляр стороннего самолета
...
int distance_arrow[MAX_TRACKING_OBJECTS];             // Дистанция на стрелке вращения стороннего самолета
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::setup(TFTMenu* menuManager)
 {
   ...
   for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
   {
       ...
       Air_txt_Sprite[i]->setPivot(25, 26);
       ...
       little_airplane[i]->createSprite(22, 16);
       little_airplane[i]->setPivot(11, 8);

       arrow_Sprite[i] = new TFT_eSprite(&tft);      //На этом спрайте располагаются спрайты самолета и текста. Спрайт вращается в виде радиуса(стрелы от центра)
       arrow_Sprite[i]->createSprite(60, 160);
       arrow_Sprite[i]->setPivot(30, 0);
       ...
       area_txt_airplane[i] = new TFT_eSprite(&tft);   // Этот спрайт, площадка в котором будет располагатся формуляр стороннего самолета
       area_txt_airplane[i]->createSprite(56, 56);
       area_txt_airplane[i]->setPivot(28, 28);
 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
   ...
   static uint32_t tmr = millis();
    ...
    if (millis() - tmr > DATA_MEASURE_THRESHOLD)
    {
        ...
        if (arr_min > 7900)
        {
            TFT_zoom = 16000; //16 km
            divider = 16000; /* 16000m */
            data_KM.drawString("16 KM", 35, 14);
        }
        else if (arr_min <= 7900 && arr_min > 3900)
        {
            TFT_zoom = 8000; //8km
            divider = 8000; /*8000m*/
            data_KM.drawString("8 KM", 35, 14);
        }
        else if (arr_min <= 3900 && arr_min > 1900)
        {
            TFT_zoom = 4000; //4km
            divider = 4000; /* 4000m */
        ...
        else if (arr_min <= 900 && arr_min > 490)
        {
            TFT_zoom = 1000;   // 1km
            divider = 1000; /*1000m */
            data_KM.drawString("1 KM", 35, 14);
        }
        else if (arr_min <= 490 && arr_min > 90)
        {
            TFT_zoom = 500; //500m
            divider = 500; /* 500m */
        ...
        for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
        {
            if (Container[i].addr)
            {
                ...
                //================================================
                bearing_tmr[i] = (int)Container[i].bearing;
                bearing_tmr1[i] = (int)Container[i].bearing;
                distance_tmr[i] = (int)Container[i].distance;

                /* вычисляем угол в градусах между нашим самолетом и сторонним */
                //if (settings->pointer == DIRECTION_TRACK_UP)
                //{
                    bearing_tmr[i] = (360 + bearing_tmr[i] - (int)ThisAircraft.course) % 360;
                //}
  /*                  Serial.print(i);
                    Serial.print(" bearing_tmr[i] ");
                    Serial.println(bearing_tmr[i] );*/
                ...
                //    y = map(x, 1, 50, 50, -100);
                    ...
                    case 16000:
                        //выполняется, когда 
                        distance_arrow[i] = map(distance_tmr[i], 0, 16000, 0, arrow_max);
                        break;
                    case 8000:
                        //выполняется когда 
                        distance_arrow[i] = map(distance_tmr[i], 0, 8000, 0, arrow_max);
                        break;
                    case 4000:
                        //выполняется, когда
                        distance_arrow[i] = map(distance_tmr[i], 0, 4000, 0, arrow_max);
                        break;
                    case 2000:
                        //выполняется когда
                        distance_arrow[i] = map(distance_tmr[i], 0, 2000, 0, arrow_max);
                        break;
                    case 1000:
                        //выполняется, когда
                        distance_arrow[i] = map(distance_tmr[i], 0, 1000, 0, arrow_max);
                        break;
                    case 500:
                        //выполняется когда 
                        distance_arrow[i] = map(distance_tmr[i], 0, 500, 0, arrow_max);
                        break;
                    case 100:
                        //выполняется когда
                        distance_arrow[i] = map(distance_tmr[i], 0, 100, 0, arrow_max);
                        break;
                    case 50:
                        //выполняется когда
                        distance_arrow[i] = map(distance_tmr[i], 0, 50, 0, arrow_max);
                        break;
                  //  default:
                    ...
                    Serial.print("distance_arrow[i]");
                    Serial.println(distance_arrow[i]);


                /*вычисляем курс стороннего самолета. (Определяем в какую сторону летит сторонний самолет) */
                ...
                if (Container[i].latitude != alien_latitude_old[i])
                {
                    ...
                    /* Корректируем курс (градусы) согласно углу поворота картинки*/
       ...
       /*             if (alien_curse[i] > 360)
               ...
               /* Определяем разность высот и устанавливаем цвет предупреждения*/
                ...
                Height_difference[i] = altitude_old[i] - (int)ThisAircraft.altitude;
                ...
                Air_txt_Sprite[i]->setTextDatum(0);                              // Определим как будет выводится текс
                ...
                Air_txt_Sprite[i]->drawString(String(speed_tmr[i]), 10, 37, 1);    // Записать скорость в формуляр
                ...
                little_airplane[i]->drawLine(6, 15, 16, 15, little_air_color[i]);
                little_airplane[i]->drawLine(7, 16, 15, 16, little_air_color[i]);
                esp_task_wdt_reset();

                /* только на момент отладки*/
                arrow_Sprite[i]->fillSprite(TFT_BLACK);                  // Закрасим поле. На этом спрайте располагаются спрайты самолета и текста. Спрайт вращается в виде радиуса(стрелы от центра) 
                arrow_Sprite[i]->drawRect(0, 0, 60, 160, TFT_WHITE);

                area_airplane[i]->fillSprite(TFT_BLACK);                 // Закрасим поле. Этот спрайт, площадка в котором будет располагатся сторонний самолет
                area_airplane[i]->drawRect(0, 0, 28, 28, TFT_WHITE);
       
                area_txt_airplane[i]->fillSprite(TFT_BLACK);             // Закрасим поле. Этот спрайт, площадка в котором будет располагатся формуляр стороннего самолета 
                area_txt_airplane[i]->drawRect(0, 0, 56, 56, TFT_WHITE);
            }
        }
       ...
       // angle = (360 - test_curse) % 360;                       // Определяем направление нашего самолета и вращаем в другую сторону шкалу
        ...
        {
            dist_info.drawRect(0, 0, 120, 20, TFT_WHITE);
            dist_info.setTextColor(TFT_WHITE, TFT_BLACK);
            dist_info.drawString("Distance  -----" , 2, 4);
            dist_info.pushToSprite(&back, 3, 218, TFT_BLACK);
        }
        ...
        for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
        {
            if (isTeam_all[i] == true)
            {
              ...
              // Air_txt_Sprite[i]->pushToSprite(&back, radar_center_x + Container_alien_X[i] -10, radar_center_y - Container_alien_Y[i] - 20, TFT_BLACK);

                //if (new_angle > 0 && new_angle < 180)
                //{
                //    little_airplane[i]->pushRotated(area_airplane[i], alien_curse[i] + 105, TFT_BLACK);    // Записать спрайт изображения стороннего самолета на площадку 
                //}
                //else
                //{
                //    little_airplane[i]->pushRotated(area_airplane[i], alien_curse[i] - 75, TFT_BLACK);    // Записать спрайт изображения стороннего самолета на площадку 
                //}

                little_airplane[i]->pushRotated(area_airplane[i], 180 + alien_curse[i], TFT_BLACK);    // Записать спрайт изображения стороннего самолета на площадку 
                Air_txt_Sprite[i]->pushRotated(area_txt_airplane[i], new_angle, TFT_BLACK);
                area_airplane[i]->pushToSprite(arrow_Sprite[i], 15, distance_arrow[i], TFT_BLACK);       //Этот спрайт, площадка в котором будет располагатся сторонний самолет
                area_txt_airplane[i]->pushToSprite(arrow_Sprite[i], 2, distance_arrow[i] +22, TFT_BLACK);  //Этот спрайт, площадка в котором будет располагатся формуляр стороннего самолета 
                arrow_Sprite[i]->pushRotated(&back, 180 + new_angle, TFT_BLACK);          // На этом спрайте располагаются спрайты самолета и текста. Спрайт вращается в виде радиуса(стрелы от центра)
                isTeam_all[i] = false;
                esp_task_wdt_reset();
            }
        }  
