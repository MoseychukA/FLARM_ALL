...
static void sendLineToSelectedOutputs(const char* line, size_t len, bool nmeaLine)
{
    ...
    const uint8_t lanMode = settings->nmea_out;

    g_diag.serialMode = serialMode;
    g_diag.rs485Mode = rs485Mode;
    g_diag.lanMode = lanMode;
    ...
    if (nmeaLine && (lanMode & NMEA_OUT_UDP))
    {
...
void NMEA_loop()
{
    ...
    for (int i = 0; i < MAX_AIRCRAFT; ++i)
    {
        ...
        if (aircraftHasCoordinates(list[i]))
        {
            ...
            sentAnyNmeaToLan = true;
        }
        sendAircraftContainer(list[i]);
    }

    if (!sentAnyNmeaToLan && settings != nullptr && (settings->nmea_out & NMEA_OUT_UDP))
    {
        sendOwnshipHeartbeatNMEA();
    }
