...
void Traffic_Update(ufo_t *fop)
{
    ...
    {
        if (fop->old_latitude == 0.0f && fop->old_longitude == 0.0f)
        {
            fop->old_latitude = fop->latitude;
            fop->old_longitude = fop->longitude;
        }
    }
...
bool Traffic_Add(ufo_t *fop)
{
    ...
    if (settings != nullptr && fop->addr == settings->block_addr)
    {
        return false;
    }
    ...
    for (int i = 0; i < MAX_TRACKING_OBJECTS; ++i)
    {
        if (Container[i].addr == fop->addr)
        {
            if (fop->latitude != 0.0f || fop->longitude != 0.0f)
            {
                Container[i].old_latitude = Container[i].latitude;
                Container[i].old_longitude = Container[i].longitude;
            }
            Container[i] = *fop;
