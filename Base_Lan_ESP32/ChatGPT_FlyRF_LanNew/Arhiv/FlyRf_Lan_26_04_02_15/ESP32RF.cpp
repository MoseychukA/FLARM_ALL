...
void Display_setup()
{
   ...
   // byte rval = DISPLAY_NONE;
    ...
    for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
    {
        ...
        spriteInitOk &= createSpriteChecked(Air_txt_Sprite[i], 55, 15, 8, "Air_txt_Sprite");
        if (Air_txt_Sprite[i] != nullptr && Air_txt_Sprite[i]->created())
        {
            Air_txt_Sprite[i]->setPivot(27, 7);
        }

        airplane[i] = new TFT_eSprite(&tft);           // Спрайт информации стороннего воздушного объекта
        spriteInitOk &= createSpriteChecked(airplane[i], 100, 100, 8, "airplane");
        if (airplane[i] != nullptr && airplane[i]->created())
        {
        ...
        spriteInitOk &= createSpriteChecked(area_airplane[i], 100, 100, 8, "area_airplane");
        if (area_airplane[i] != nullptr && area_airplane[i]->created())
        {
        ...
        spriteInitOk &= createSpriteChecked(airplane_alient_info[i], 480, INFO_HEIGHT, 8, "airplane_alient_info");
        if (airplane_alient_info[i] != nullptr && airplane_alient_info[i]->created())
        {
    ...
    spriteInitOk &= createSpriteChecked(power1, 74, 20, 8, "power1");
    spriteInitOk &= createSpriteChecked(time_info, 60, 25, 8, "time_info");
...
void displayAllPlanes() // Вывести текстовую информацию по самолетам на дисплей
{
    ...
    if (has_data_to_display)
    {
        if (!info_header.created())
        {
            if (!createSpriteChecked(info_header, 480, 20, 8, "info_header"))
            {
                return;
            }
