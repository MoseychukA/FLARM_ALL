...
TFT_eSprite* airplane_alient_info[MAX_TRACKING_OBJECTS] = { nullptr };
...
{
    const bool showLanBlock = !(settings && settings->lan_state_view == 0);
    const bool showMemoryLine = (settings && settings->tft_memory_view);
    const bool showRfLine = (settings && settings->rssi_view == VIEW_RSSI_ON);

    if (!showLanBlock && !showMemoryLine && !showRfLine)
    {
        return;
    }
    ...
    if (showMemoryLine)
    {
        const uint32_t freeHeap = ESP.getFreeHeap() / 1024UL;
        const uint32_t minHeap = ESP.getMinFreeHeap() / 1024UL;
        const uint32_t largestBlock = heap_caps_get_largest_free_block(MALLOC_CAP_8BIT) / 1024UL;
        snprintf(lineBuf, sizeof(lineBuf), "MEM %luk M%luk L%luk",
                 (unsigned long)freeHeap,
                 (unsigned long)minHeap,
                 (unsigned long)largestBlock);
        back.drawString(lineBuf, xLeftInfo, y0 - dy, 2);
    }
    else
    {
        back.fillRect(0, y0 - dy, 170, 12, TFT_BLACK);
    }
    ...
    if (showRfLine)
    {
        ...
        if (rfFreqHz > 0UL)
        {
            ...
            snprintf(lineBuf, sizeof(lineBuf), "RF %lu.%03lu", (unsigned long)mhz, (unsigned long)khz);
            back.drawString(lineBuf, xLeftInfo, y0, 2);
        }
        else
        {
            back.drawString("RF ---", xLeftInfo, y0, 2);
        }
    }
    ...
    if (showLanBlock)
    {
        ...
        back.drawString(addrModeText, xRight, y0, 2);
        ...
        back.drawString(lineBuf, xRight, y0 + dy, 2);
        ...
        if (millis() - tmr > DATA_MEASURE_THRESHOLD)
        {
           ...
           //=====================================================================================
                ...
                for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
                {
                    ...
                    if (settings->rssi_view == VIEW_RSSI_ON) // Если вывод уровня сигнала разрешен в WEB интерфейсе
                    {
                        ...
                        if (Container[i].rssi_LoRa < 0 && Container[i].signal_source == 1)
                        {
                            ...
                            snprintf(rssiBuf, sizeof(rssiBuf), "RSSI %d db", Container[i].rssi_LoRa);
                            back.drawString(rssiBuf, 2, rssi_y);
                            rssi_off = true;
                        }
            ...
            if (settings->view_test_coord == VIEW_COORD_ON) // Стационарный вариант
            {
                ...
                back.setTextColor(TFT_DARKGREY, TFT_BLACK);
                char coordBuf[32];
                snprintf(coordBuf, sizeof(coordBuf), "Lat: %.5f", (double)ThisAircraft.local_latitude);
                back.drawString(coordBuf, 1, 287);
                snprintf(coordBuf, sizeof(coordBuf), "Lon: %.5f", (double)ThisAircraft.local_longitude);
                back.drawString(coordBuf, 1, 303);
            }
