...
{
    if (settings && settings->udp_port != 0) return settings->udp_port;
...
static String htmlBegin(const String& title)
{
    ...
    s += F(".settingsBlock{background:#fff;border:1px solid #cfcfcf;border-radius:8px;padding:10px 12px 12px 12px;margin:0 0 12px 0;box-shadow:0 1px 2px rgba(0,0,0,.05);}");
    ...
    s += F(".btn{display:inline-flex;justify-content:center;align-items:center;font-family:Arial,'Times New Roman',serif;font-size:22px;line-height:1.12;padding:13px 16px;margin:0;border:1px solid #8b8b8b;border-radius:3px;background:#ececec;color:#000;text-decoration:none;cursor:pointer;width:auto;min-width:168px;max-width:100%;box-sizing:border-box;text-align:center;white-space:nowrap;}.btnIcon{display:inline-block;font-size:1.48em;line-height:1;vertical-align:-0.12em;margin-right:0.30em;}.settingsTopCtl,.hardwareCtl,.settingsTopCtl50,.settingsTopCtl30,.hardwareCtl50,.hardwareCtl30,.hardwareCtl60,.wShort,.wMed,.wLong{width:auto;min-width:168px;max-width:100%;}");
    s += F("input[type=text],input[type=number],select{font-family:Arial,'Times New Roman',serif;font-size:20px;line-height:1.1;box-sizing:border-box;border:1px solid #8b8b8b;border-radius:3px;background:#fff;height:42px;padding:6px 9px;width:auto;min-width:168px;max-width:100%;}");
    ...
    s += F("body.mobileUi .wShort,body.mobileUi .wMed,body.mobileUi .wLong,body.mobileUi .settingsTopCtl,body.mobileUi .settingsTopCtl50,body.mobileUi .settingsTopCtl30,body.mobileUi .hardwareCtl,body.mobileUi .hardwareCtl50,body.mobileUi .hardwareCtl30,body.mobileUi .hardwareCtl60{width:100%;min-width:0;}body.mobileUi .btn{display:block;width:100%;min-width:0;font-size:18px;line-height:1.15;padding:12px 12px;margin:6px 0;}");
    ...
    s += F("@media (max-width:760px){h1{font-size:30px;}th,td{font-size:16px;}input[type=text],input[type=number],input[type=file],select{font-size:16px;width:170px;height:34px;}.btn{font-size:15px;padding:7px 12px;}.monTable th,.monTable td{font-size:13px;padding:4px 3px;}}");    s += F("</style><script>(function(){var mode='desktop';try{mode=localStorage.getItem('flyrf_ui_mode')||'desktop';}catch(e){}if(mode==='mobile'){document.documentElement.className+=' mobileUi';}})();</script></head><body><div class='page'><div class='viewToggleWrap'><button type='button' id='uiModeToggle' class='viewToggle'><span class='btnIcon'>&#128187;</span>&#1056;&#1077;&#1078;&#1080;&#1084;: &#1055;&#1050;</button></div>");
...
{
    String html = htmlBegin("FlyRf LAN");
    html += F("<h1>FlyRf Lan</h1><table>");
    ...
    html += F("</td></tr>");
    html += F("<tr><th>Local IP(UDP)</th><td>");
    html += ipToString(settings ? settings->g_localIP : nullptr);
    html += F("</td></tr>");
    html += F("<tr><th>Gateway(UDP)</th><td>");
    html += ipToString(settings ? settings->g_gatewayIP : nullptr);
    html += F("</td></tr>");
    html += F("<tr><th>Subnet mask(UDP)</th><td>");
    html += ipToString(settings ? settings->g_subnetMask : nullptr);
    html += F("</td></tr>");
    html += F("<tr><th>DNS Server(UDP)</th><td>");
    html += ipToString(settings ? settings->g_dns_server : nullptr);
    html += F("</td></tr>");
    html += F("<tr><th>UDP port</th><td>");
    html += String(udpPortValue());
    html += F("</td></tr>");
    html += F("<tr><th>MAC &#1072;&#1076;&#1088;&#1077;&#1089; LAN</th><td id='macAddr'>");
    html += LAN_macAddressStr();
    html += F("</td></tr>");
    html += F("<tr><th>LAN/UDP &#1072;&#1076;&#1088;&#1077;&#1089;</th><td id='tcpAddr'>");
    html += LAN_tcpAddressStr();
    html += F("</td></tr>");
    html += F("<tr><th>&#1057;&#1086;&#1089;&#1090;&#1086;&#1103;&#1085;&#1080;&#1077; LAN</th><td id='lanLinkState'>");
    html += LAN_linkStatusStr();
    html += LAN_dhcpLeaseActive() ? F(" / DHCP") : F(" / Static");
    html += F("</td></tr>");
    html += F("<tr><th>&#1055;&#1072;&#1082;&#1077;&#1090;&#1099; LAN</th><td id='lanPktCounters'>Tx&nbsp; ");
...
(function(){
  ...
  var macEl=document.getElementById('macAddr');
  var tcpEl=document.getElementById('tcpAddr');
  var lanEl=document.getElementById('lanPktCounters');
  var linkEl=document.getElementById('lanLinkState');
  if(!el) return;
  var busy=false;
  ...
  function updateCounters(){
    ...
    fetch('/packet_counters',{cache:'no-store'})
      ...
      .then(function(data){
        ...
        if(macEl && data.mac) macEl.textContent=data.mac;
        if(tcpEl && data.tcp_addr) tcpEl.textContent=data.tcp_addr;
        if(linkEl) linkEl.textContent=(data.lan_link_text||'LAN Off')+' / '+((data.lan_dhcp||false)?'DHCP':'Static');
        if(lanEl) lanEl.innerHTML='Tx&nbsp; '+(data.lan_tx||0)+'&nbsp;&nbsp;&nbsp; Rx&nbsp; '+(data.lan_rx||0);
      })
      .catch(function(){} )
...
static String settingsPage()
{
    ...
    html += F("<div class='settingsBlock'><div class='blockTitle'>&#1051;&#1086;&#1082;&#1072;&#1083;&#1100;&#1085;&#1099;&#1077; &#1082;&#1086;&#1086;&#1088;&#1076;&#1080;&#1085;&#1072;&#1090;&#1099;</div><table>");
    html += F("<tr><th>&#1055;&#1086;&#1083;&#1091;&#1096;&#1072;&#1088;&#1080;&#1077; (&#1089;&#1077;&#1074;&#1077;&#1088;/&#1102;&#1075;)</th><td><select class='wShort settingsTopCtl50' name='input_N_S'>");
    ...
    html += F("</select></td></tr>");
    html += F("<tr><th>latitude (&#1096;&#1080;&#1088;&#1086;&#1090;&#1072; 0&#176; - 90&#176;)</th><td><input class='settingsTopCtl50' type='text' name='local_latitude' value='");
    html += String(settings ? settings->local_latitude : 55.95501f, 5);
    html += F("'></td></tr>");
    html += F("<tr><th>&#1044;&#1086;&#1083;&#1075;&#1086;&#1090;&#1072; (&#1074;&#1086;&#1089;&#1090;&#1086;&#1082;/&#1079;&#1072;&#1087;&#1072;&#1076;)</th><td><select class='wShort settingsTopCtl50' name='input_E_W'>");
    ...
    html += F("</select></td></tr>");
    html += F("<tr><th>longitude (0&#176; - 180&#176;)</th><td><input class='settingsTopCtl30' type='text' name='local_longitude' value='");
    html += String(settings ? settings->local_longitude : 37.23166f, 5);
    html += F("'></td></tr>");
    html += F("</table></div>");
    html += F("<div class='settingsBlock'><div class='blockTitle'>UDP / LAN</div><table>");
    html += F("<tr><th>Local IP(UDP) address</th><td><input class='settingsTopCtl30' type='text' name='local_ip' value='");
    html += ipToString(settings ? settings->g_localIP : nullptr);
    html += F("'></td></tr>");
    html += F("<tr><th>Gateway IP(UDP) address</th><td><input class='settingsTopCtl30' type='text' name='gateway_ip' value='");
    html += ipToString(settings ? settings->g_gatewayIP : nullptr);
    html += F("'></td></tr>");
    html += F("<tr><th>Subnet mask(UDP)</th><td><input class='settingsTopCtl30' type='text' name='subnet_mask' value='");
    html += ipToString(settings ? settings->g_subnetMask : nullptr);
    html += F("'></td></tr>");
    html += F("<tr><th>DNS Server(UDP)</th><td><input class='settingsTopCtl30' type='text' name='dns_server' value='");
    html += ipToString(settings ? settings->g_dns_server : nullptr);
    html += F("'></td></tr>");
    html += F("<tr><th>UDP port</th><td><input class='settingsTopCtl50' type='number' min='1' max='65535' name='udp_port' value='");
    html += String(udpPortValue());
    html += F("'></td></tr>");
...
static String settingsSavedPage()
{
    ...
    html += F("<tr><th>NMEA Out</th><td>"); html += nmeaName(settings ? settings->nmea_out : 0); html += F("</td></tr>");
    ...
    html += F("<tr><th>&#1044;&#1086;&#1083;&#1075;&#1086;&#1090;&#1072;</th><td>"); html += (settings && settings->input_E_W ? "W" : "E"); html += F(" "); html += String(settings ? settings->local_longitude : 37.23166f, 5); html += F("</td></tr>");
    html += F("<tr><th>Local IP(UDP)</th><td>"); html += ipToString(settings ? settings->g_localIP : nullptr); html += F("</td></tr>");
    html += F("<tr><th>Gateway IP(UDP)</th><td>"); html += ipToString(settings ? settings->g_gatewayIP : nullptr); html += F("</td></tr>");
    html += F("<tr><th>Subnet Mask(UDP)</th><td>"); html += ipToString(settings ? settings->g_subnetMask : nullptr); html += F("</td></tr>");
    html += F("<tr><th>DNS Server(UDP)</th><td>"); html += ipToString(settings ? settings->g_dns_server : nullptr); html += F("</td></tr>");
...
static String hardwarePage()
{
    ...
    html += F("<div class='settingsBlock'><div class='blockTitle'>&#1044;&#1080;&#1089;&#1087;&#1083;&#1077;&#1081; &#1080; LAN</div><table>");
    html += F("<tr><th>&#1054;&#1090;&#1086;&#1073;&#1088;&#1072;&#1079;&#1080;&#1090;&#1100; &#1091;&#1088;&#1086;&#1074;&#1077;&#1085;&#1100; &#1089;&#1080;&#1075;&#1085;&#1072;&#1083;&#1072; LoRa</th><td><select class='wMed hardwareCtl50' name='rssi_view'>");
    ...
    html += F("</select></td></tr>");
    html += F("<tr><th>&#1055;&#1086;&#1082;&#1072;&#1079;&#1099;&#1074;&#1072;&#1090;&#1100; &#1087;&#1072;&#1084;&#1103;&#1090;&#1100; &#1085;&#1072; TFT &#1076;&#1080;&#1089;&#1087;&#1083;&#1077;&#1077;</th><td><select class='wMed hardwareCtl50' name='tft_memory_view'>");
    html += String("<option ") + sel(!(settings && settings->tft_memory_view)) + " value='0'>&#1042;&#1099;&#1082;&#1083;&#1102;&#1095;&#1080;&#1090;&#1100;</option>";
    html += String("<option ") + sel(settings && settings->tft_memory_view) + " value='1'>&#1042;&#1082;&#1083;&#1102;&#1095;&#1080;&#1090;&#1100;</option>";
    html += F("</select></td></tr>");
    html += F("<tr><th>&#1054;&#1090;&#1086;&#1073;&#1088;&#1072;&#1078;&#1072;&#1090;&#1100; &#1089;&#1086;&#1089;&#1090;&#1086;&#1103;&#1085;&#1080;&#1077; LAN</th><td><select class='wMed hardwareCtl50' name='lan_state_view'>");
    ...
    html += F("</select></td></tr>");
    html += F("<tr><th>&#1044;&#1080;&#1072;&#1087;&#1072;&#1079;&#1086;&#1085; &#1087;&#1088;&#1086;&#1089;&#1084;&#1086;&#1090;&#1088;&#1072; TFT</th><td><select class='wLong hardwareCtl50' name='radar_range_mode'>");
    html += String("<option ") + sel(!(settings && settings->radar_range_mode)) + " value='0'>&#1040;&#1074;&#1090;&#1086;&#1084;&#1072;&#1090;&#1080;&#1095;&#1077;&#1089;&#1082;&#1080;&#1081;</option>";
    html += String("<option ") + sel(settings && settings->radar_range_mode) + " value='1'>&#1060;&#1080;&#1082;&#1089;&#1080;&#1088;&#1086;&#1074;&#1072;&#1085;&#1085;&#1099;&#1081; (32 &#1082;&#1084;)</option>";
    html += F("</select></td></tr>");
    html += F("</table></div>");
    html += F("<div class='settingsBlock'><div class='blockTitle'>LoRa</div><table>");
    ...
    html += F("</select></td></tr>");
    html += F("<tr><th>LoRa &#1087;&#1088;&#1086;&#1092;&#1080;&#1083;&#1100;</th><td><select class='wLong hardwareCtl50' name='lora_profile'>");
    ...
    html += F("</select></td></tr>");
    html += F("<tr><th>&#1056;&#1077;&#1082;&#1086;&#1084;&#1077;&#1085;&#1076;&#1072;&#1094;&#1080;&#1103;</th><td><div class='note'>OGN compatible &#8212; &#1076;&#1083;&#1103; &#1089;&#1086;&#1074;&#1084;&#1077;&#1089;&#1090;&#1080;&#1084;&#1086;&#1089;&#1090;&#1080; &#1089; &#1076;&#1088;&#1091;&#1075;&#1080;&#1084;&#1080; &#1084;&#1086;&#1076;&#1091;&#1083;&#1103;&#1084;&#1080;. Long range / Max range &#8212; &#1090;&#1086;&#1083;&#1100;&#1082;&#1086; &#1077;&#1089;&#1083;&#1080; &#1085;&#1072; &#1076;&#1088;&#1091;&#1075;&#1086;&#1081; &#1089;&#1090;&#1086;&#1088;&#1086;&#1085;&#1077; &#1091;&#1089;&#1090;&#1072;&#1085;&#1086;&#1074;&#1083;&#1077;&#1085;&#1099; &#1090;&#1072;&#1082;&#1080;&#1077; &#1078;&#1077; &#1087;&#1072;&#1088;&#1072;&#1084;&#1077;&#1090;&#1088;&#1099;.</div></td></tr>");
    html += F("</table></div>");
    html += F("<div class='settingsBlock'><div class='blockTitle'>&#1042;&#1099;&#1074;&#1086;&#1076; &#1076;&#1072;&#1085;&#1085;&#1099;&#1093;</div><table>");
    html += F("<tr><th>&#1042;&#1099;&#1074;&#1086;&#1076; &#1076;&#1072;&#1085;&#1085;&#1099;&#1093; &#1074; Serial</th><td><select class='wLong hardwareCtl50' name='serial_out'>");
    html += String("<option ") + sel((settings ? settings->serial_out : OUTPUT_MODE_OFF) == OUTPUT_MODE_OFF) + " value='0'>&#1053;&#1077; &#1074;&#1099;&#1074;&#1086;&#1076;&#1080;&#1090;&#1100;</option>";
    html += String("<option ") + sel((settings ? settings->serial_out : OUTPUT_MODE_CONTAINER) == OUTPUT_MODE_CONTAINER) + " value='1'>Container</option>";
    html += String("<option ") + sel((settings ? settings->serial_out : OUTPUT_MODE_NMEA) == OUTPUT_MODE_NMEA) + " value='2'>NMEA</option>";
    html += String("<option ") + sel((settings ? settings->serial_out : OUTPUT_MODE_RP2040) == OUTPUT_MODE_RP2040) + " value='3'>RP2040 RX</option>";
    html += String("<option ") + sel((settings ? settings->serial_out : OUTPUT_MODE_FLARM) == OUTPUT_MODE_FLARM) + " value='4'>FLARM RX</option>";
    html += F("</select></td></tr>");
    html += F("<tr><th>&#1042;&#1099;&#1074;&#1086;&#1076; &#1076;&#1072;&#1085;&#1085;&#1099;&#1093; &#1074; RS485</th><td><select class='wLong hardwareCtl50' name='rs485_out'>");
    html += String("<option ") + sel((settings ? settings->rs485_out : OUTPUT_MODE_OFF) == OUTPUT_MODE_OFF) + " value='0'>&#1053;&#1077; &#1074;&#1099;&#1074;&#1086;&#1076;&#1080;&#1090;&#1100;</option>";
    html += String("<option ") + sel((settings ? settings->rs485_out : OUTPUT_MODE_CONTAINER) == OUTPUT_MODE_CONTAINER) + " value='1'>Container</option>";
    html += String("<option ") + sel((settings ? settings->rs485_out : OUTPUT_MODE_NMEA) == OUTPUT_MODE_NMEA) + " value='2'>NMEA</option>";
    html += String("<option ") + sel((settings ? settings->rs485_out : OUTPUT_MODE_RS485_DISPLAY) == OUTPUT_MODE_RS485_DISPLAY) + " value='3'>&#1042;&#1085;&#1077;&#1096;&#1085;&#1080;&#1081; &#1076;&#1080;&#1089;&#1087;&#1083;&#1077;&#1081;</option>";
    html += String("<option ") + sel((settings ? settings->rs485_out : OUTPUT_MODE_FLARM) == OUTPUT_MODE_FLARM) + " value='4'>FLARM RX</option>";
    html += F("</select></td></tr>");
    html += F("</table></div>");
...
static String hardwareSavedPage(bool defaultsLoaded)
{
    ...
    html += F("<tr><th>&#1054;&#1090;&#1086;&#1073;&#1088;&#1072;&#1079;&#1080;&#1090;&#1100; &#1091;&#1088;&#1086;&#1074;&#1077;&#1085;&#1100; &#1089;&#1080;&#1075;&#1085;&#1072;&#1083;&#1072; LoRa</th><td>"); html += yesNo(settings ? settings->rssi_view : 0, "&#1042;&#1099;&#1082;&#1083;&#1102;&#1095;&#1077;&#1085;", "&#1042;&#1082;&#1083;&#1102;&#1095;&#1077;&#1085;"); html += F("</td></tr>");
    html += F("<tr><th>&#1056;&#1077;&#1078;&#1080;&#1084; &#1088;&#1072;&#1073;&#1086;&#1090;&#1099; &#1076;&#1080;&#1089;&#1087;&#1083;&#1077;&#1103;</th><td>"); html += displayModeName(settings ? settings->display_set : INFO_DISTLAY_OFF); html += F("</td></tr>");
    html += F("<tr><th>&#1055;&#1072;&#1084;&#1103;&#1090;&#1100; &#1085;&#1072; TFT</th><td>"); html += tftMemoryViewName(settings ? settings->tft_memory_view : 0); html += F("</td></tr>");
    html += F("<tr><th>&#1054;&#1090;&#1086;&#1073;&#1088;&#1072;&#1078;&#1072;&#1090;&#1100; &#1089;&#1086;&#1089;&#1090;&#1086;&#1103;&#1085;&#1080;&#1077; LAN</th><td>"); html += yesNo(settings ? settings->lan_state_view : 0, "&#1042;&#1099;&#1082;&#1083;&#1102;&#1095;&#1080;&#1090;&#1100;", "&#1042;&#1082;&#1083;&#1102;&#1095;&#1080;&#1090;&#1100;"); html += F("</td></tr>");
    ...
    html += F("<tr><th>LoRa TX power</th><td>20 dBm</td></tr>");
...
static String outputsPage()
{
    ...
    html += F("<tr><th>LAN NMEA UDP</th><td><select class='wShort' name='udp_on'>");
    html += String("<option ") + sel(!(settings && (settings->nmea_out & NMEA_OUT_UDP))) + " value='0'>Off</option>";
    html += String("<option ") + sel(settings && (settings->nmea_out & NMEA_OUT_UDP)) + " value='1'>On</option>";
    html += F("</select></td></tr>");
    html += F("<tr><th>UDP port</th><td><input class='wShort' type='number' min='1' max='65535' name='udp_port' value='");
    html += String(udpPortValue());
    html += F("'></td></tr>");
    html += F("<tr><th>&#1042;&#1099;&#1074;&#1086;&#1076; &#1076;&#1072;&#1085;&#1085;&#1099;&#1093; &#1074; Serial</th><td><select class='wMed hardwareCtl50' name='serial_out'>");
    html += String("<option ") + sel((settings ? settings->serial_out : OUTPUT_MODE_OFF) == OUTPUT_MODE_OFF) + " value='0'>&#1053;&#1077; &#1074;&#1099;&#1074;&#1086;&#1076;&#1080;&#1090;&#1100;</option>";
    html += String("<option ") + sel((settings ? settings->serial_out : OUTPUT_MODE_OFF) == OUTPUT_MODE_CONTAINER) + " value='1'>Container</option>";
    html += String("<option ") + sel((settings ? settings->serial_out : OUTPUT_MODE_OFF) == OUTPUT_MODE_NMEA) + " value='2'>NMEA</option>";
    ...
    html += F("</select></td></tr>");
    html += F("<tr><th>&#1042;&#1099;&#1074;&#1086;&#1076; &#1076;&#1072;&#1085;&#1085;&#1099;&#1093; &#1074; RS485</th><td><select class='wMed hardwareCtl30' name='rs485_out'>");
    html += String("<option ") + sel((settings ? settings->rs485_out : OUTPUT_MODE_OFF) == OUTPUT_MODE_OFF) + " value='0'>&#1053;&#1077; &#1074;&#1099;&#1074;&#1086;&#1076;&#1080;&#1090;&#1100;</option>";
...
static String outputsSavedPage()
{
    ...
    html += F("<tr><th>LAN NMEA UDP</th><td>"); html += ((settings && (settings->nmea_out & NMEA_OUT_UDP)) ? "On" : "Off"); html += F("</td></tr>");
    html += F("<tr><th>UDP port</th><td>"); html += String(udpPortValue()); html += F("</td></tr>");
    html += F("<tr><th>&#1042;&#1099;&#1074;&#1086;&#1076; &#1076;&#1072;&#1085;&#1085;&#1099;&#1093; &#1074; Serial</th><td>"); html += serialModeName(settings ? settings->serial_out : OUTPUT_MODE_OFF); html += F("</td></tr>");
    html += F("<tr><th>&#1042;&#1099;&#1074;&#1086;&#1076; &#1076;&#1072;&#1085;&#1085;&#1099;&#1093; &#1074; RS485</th><td>"); html += rs485ModeName(settings ? settings->rs485_out : OUTPUT_MODE_OFF); html += F("</td></tr>");
    html += F("</table><div class='hr'></div><div class='confirmTitle'>&#1053;&#1072;&#1089;&#1090;&#1088;&#1086;&#1081;&#1082;&#1080; &#1089;&#1086;&#1093;&#1088;&#1072;&#1085;&#1077;&#1085;&#1099; &#1073;&#1077;&#1079; &#1087;&#1077;&#1088;&#1077;&#1079;&#1072;&#1075;&#1088;&#1091;&#1079;&#1082;&#1080;.</div><div class='btnRow'><a class='btn' href='/'><span class='btnIcon'>&#127968;</span>&#1053;&#1072; &#1075;&#1083;&#1072;&#1074;&#1085;&#1091;&#1102;</a></div></div></body></html>");
...
static void handleAircraftJson()
{
    ...
    bool first = true;
    const float rxLat = settings ? settings->local_latitude : 0.0f;
    const float rxLon = settings ? settings->local_longitude : 0.0f;
...
static void handleInput()
{
    if (settings)
    {
        ...
        if (server.hasArg("nmea_out")) settings->nmea_out = (uint8_t)(clampInt(server.arg("nmea_out").toInt(), 0, 1) ? NMEA_OUT_UDP : 0);
...
static void handleHardwareLive()
{
    ...
    else
    {
        ...
        if (server.hasArg("tft_memory_view")) settings->tft_memory_view = (uint8_t)clampInt(server.arg("tft_memory_view").toInt(), 0, 1);
...
static void handleOutputsLive()
{
    ...
    if (server.hasArg("rs485_out")) settings->rs485_out = (uint8_t)clampInt(server.arg("rs485_out").toInt(), OUTPUT_MODE_OFF, OUTPUT_MODE_FLARM);
    uint8_t lanMode = 0;
    if (server.hasArg("udp_on") && server.arg("udp_on").toInt() == 1) lanMode |= NMEA_OUT_UDP;
    settings->nmea_out = lanMode;
    if (server.hasArg("udp_port")) settings->udp_port = (uint16_t)clampInt(server.arg("udp_port").toInt(), 1, 65535);
    EEPROM_store();
