...
SemaphoreHandle_t containerMutex = nullptr;
...
void ServiceClass::set_deletion_time(bool On_Off)
{
	flags_MailOn = On_Off;
}

bool ServiceClass::get_deletion_time()
{
	return flags_MailOn;
}
...
void ServiceClass::set_num_button(uint8_t button)
{
	num_button = button;
}

uint8_t ServiceClass::get_num_button()
{
	return num_button;
}
...
void ServiceClass::initContainerMutex()
{
    containerMutex = xSemaphoreCreateMutex();
    if (containerMutex == nullptr) {
        Serial.println("Не удалось создать mutex");
    }
}

bool ServiceClass::lockContainer(TickType_t timeout)
{
    if (xSemaphoreTake(containerMutex, timeout) == pdTRUE) 
    {
        return true;
    }
    return false;
}
void ServiceClass::unlockContainer() 
{
    xSemaphoreGive(containerMutex);
}

void ServiceClass::set_connection_base(bool connection)
{
    connection_ = connection;

}
bool ServiceClass::get_connection_base()
{

    return connection_;
}

void ServiceClass::setViewTime(bool On_Off)
{
    flags_ViewTime = On_Off;
}

bool ServiceClass::getViewTime()
{
    return flags_ViewTime;
}
//--------------------------------------------------------------------------------------------------------------------------------
