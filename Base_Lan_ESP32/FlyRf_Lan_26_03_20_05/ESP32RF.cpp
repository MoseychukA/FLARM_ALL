...
void GPS_Enable()
{
    if (settings->mode == FLYRF_MODE_NORMAL)
    {
        ...
        service.set_GNSS_on_off(!fix_tmp);
    }
    else
    {
        service.set_GNSS_on_off(false);
    }
...
void set_time()
{
    if (settings->mode == FLYRF_MODE_NORMAL)
    {
        ...
        {
            service.set_time_hour(gnss.time.hour());
            service.set_time_minute(gnss.time.minute());
        }
        else
        {
    ...
    {
        service.set_time_hour(10);
        service.set_time_minute(10);
    }
}
...
//void block_botton()
//{
//    uint8_t new_buttton = service.get_num_button();
//
//    if (new_buttton != 0)
//    {
//        //  count_buttton_tmp = 0;
...
//        {
//        case 1:
//            setMessageRead = service.getMessageRead();  //  Получить признак наличия сообщения
//           // Serial.print("setMessageRead1"); Serial.println(setMessageRead);
//            if (setMessageRead)
//            {
//                // Serial.println("setMessageRead2");
//                service.setMessageRead(false);         // Сбросить признак наличия сообщения
//                vTaskDelay(pdMS_TO_TICKS(4));
//                /* проконтролируем в КОМ порту количество неподтвержденных сообщений*/
//
//                //----------------------------------------------------------------------------------
//                bool confirm_message = service.get_confirm_message();                             // Получен специальный код признака, означает что подтверждение не отправлено.Нужно отправить
...
//                    service.set_deletion_time(false);                                               // Отключить отсчет времени удаления сообщения через 10 минут
...
static void drawMessage()  //Уточнить нужно ли?
{
...
static void display_state_SOS_button() //Уточнить нужно ли?
{
...
//static void execution_state_SOS_button() //Уточнить нужно ли?
//{
//    if (settings->sos_view == VIEW_SOS_ON)
//    {
    ...
    { 
       ...
       // execution_state_SOS_button();
        ...
        if (millis() - tmr > DATA_MEASURE_THRESHOLD)
        {
           ...
           //=====================================================================================
                ...
                for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
                {
                    ...
                    if (settings->rssi_view == VIEW_RSSI_ON) // Если вывод уровня сигнала разрешен в WEB интерфейсе
                    {
                        ...
                        if (Container[i].rssi_LoRa < 0 && Container[i].signal_source == 0)
                        {
            ...
            //Стационарный вариант. Уточнить нужно ли это вычисление
          ...
          //  if ((settings->view_test_coord == VIEW_COORD_ON) && (settings->mode == FLYRF_MODE_TXRX_TEST1) || (settings->mode == FLYRF_MODE_TXRX_TEST2) || (settings->mode == FLYRF_MODE_TXRX_TEST3) || (settings->mode == FLYRF_MODE_TXRX_TEST4) || (settings->mode == FLYRF_MODE_TXRX_TEST5)) // Эта проверка проводится в тестовом режиме
            ...
            //if (settings->power_view == VIEW_AKK_ON)
            //{
            //    float akk = service.battery_read();
            ...
            //{
            //    service.set_deletion_time(true);                                                 // Начинаем отсчет времени удаления сообщения через 10 минут
            //    service.setNewMessageFlag(false);                                        // Сбросить флаг нового сообщения. Программа извещена и приступила к обработке нового сообщения.
            ...
            //bool flags_MailOn = service.get_deletion_time();
            ...
            //    {
            //        mail_off = millis();
            //        service.set_deletion_time(true);                                        // Время 10 минут закончилось
            //        service.setAllow_flashing(false);                                       // Запретить мигание сообщения
            //        rows_Message.fillSprite(TFT_BLACK);
            //        rows_Message.pushToSprite(&back, 0, 0, TFT_BLACK);
            //        rows_Message.deleteSprite();
