...
uint32_t HIDComposite::SetReportParser(uint32_t id, HIDReportParser *prs) {
        for(uint8_t i = 0; i < MAX_REPORT_PARSERS; i++) {
...
HIDReportParser* HIDComposite::GetReportParser(uint32_t id) {
        if(!bHasReportId)
...
uint32_t HIDComposite::Init(uint32_t parent, uint32_t port, uint32_t lowspeed) {
...
void HIDComposite::EndpointXtract(uint32_t conf, uint32_t iface, uint32_t alt, uint32_t proto, const USB_ENDPOINT_DESCRIPTOR *pep) {
