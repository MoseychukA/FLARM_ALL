...
public:
        SRWS1(USBHost *p) : HIDUniversal(p) {};
        bool connected() {
...
private:
        void ParseHIDData(HID *hid, uint32_t is_rpt_id, uint32_t len, uint8_t *buf); // Called by the HIDUniversal library
        uint32_t OnInitSuccessful() { // Called by the HIDUniversal library on success
