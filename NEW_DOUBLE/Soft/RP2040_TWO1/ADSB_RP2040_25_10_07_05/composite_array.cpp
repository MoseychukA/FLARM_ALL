...
bool CompositeArray::UnpackRawPacketsBuffer(CompositeArray::RawPackets& packets, uint8_t* buf, uint16_t buf_len_bytes) {
    ...
    //packets.uat_adsb_packets =  reinterpret_cast<RawUATADSBPacket*>(packets.mode_s_packets + packets.header->num_mode_s_packets);
    //packets.uat_uplink_packets =  reinterpret_cast<RawUATUplinkPacket*>(packets.uat_adsb_packets + packets.header->num_uat_adsb_packets);
