...
void waiting_txt() // Вывод текста ожидания определения координат
{
...
void drawMessage()
{
   static char msg_mem[Number_of_bytes_block] = "";
    ...
    { /* TFT_display_frontpage  Основная программа отображения воздушной обстановки*/ 
        ...
        if (millis() - tmr > DATA_MEASURE_THRESHOLD)
        {
                        ...
                        */
                        ...
                        if (Container[i].latitude != 0.0 && Container[i].longitude != 0.0)
                        {
                            ...
                            */
                            ...
                            //    airplane[i]->drawLine(49, 41, 49, alien_speed_view[i] - 4, little_air_color[i]); // Рисуем прямую линию"скорости" с носа самолета
                            //    airplane[i]->drawLine(50, 41, 50, alien_speed_view[i] - 4, little_air_color[i]); // Рисуем прямую линию"скорости" с носа самолета
                            //    airplane[i]->drawLine(51, 41, 51, alien_speed_view[i] - 4, little_air_color[i]); // Рисуем прямую линию"скорости" с носа самолета
                            //    area_airplane[i]->fillSprite(TFT_BLACK);      // Закрасим поле 
                            //}
           ...
           // int rssi_y = 178;
            ...
            //}
            if (settings->view_test_coord == VIEW_TEST_COORD_ON)
            {
                if ((settings->mode == FLYRF_MODE_TXRX_TEST1) || (settings->mode == FLYRF_MODE_TXRX_TEST2) || (settings->mode == FLYRF_MODE_TXRX_TEST3) || (settings->mode == FLYRF_MODE_TXRX_TEST4) || (settings->mode == FLYRF_MODE_TXRX_TEST5)) // Эта проверка проводится в тестовом режиме
                {
                    back.setTextDatum(0);
                    back.setTextColor(TFT_DARKGREY, TFT_BLACK);
