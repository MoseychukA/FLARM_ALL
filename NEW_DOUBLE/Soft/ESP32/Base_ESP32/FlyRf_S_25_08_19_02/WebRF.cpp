...
<td align=right>\
<INPUT type='number' name='akk_min' min='3.10' max='3.60' value='%d'>\
</td>\
</tr>\
<tr>\
<th align=left>Максимальное значение заряда аккумулятора</th>\
<td align=right>\
<INPUT type='number' name='akk_max' min='3.60' max='4.40' value='%d'>\
</td>\
</tr>\
<tr>\
<th align=left>Коэффициет пересчета заряда аккумулятора</th>\
<td align=right>\
<INPUT type='number' name='akk_koef' min='30' max='300' value='%d'>\
</td>\
</tr>\
</table>"),
...
void hardwareInput() 
{
    ...
    for (uint8_t i = 0; i < server.args(); i++) 
    {
        if (server.argName(i).equals("rfc")) {
            ...
            settings->akk_koef = server.arg(i).toInt();
        }
    }
...
<tr><th align=left>Минимальное значение заряда аккумулятора</th><td align=right>%d</td></tr>\
<tr><th align=left>Максимальное значение заряда аккумулятора</th><td align=right>%d</td></tr>\
<tr><th align=left>Коэффициет пересчета заряда аккумулятора</th><td align=right>%d</td></tr>\
</table>\
<hr>\
 <p align=center><h2 align=center>Выполняется перезагрузка... Пожалуйста, подождите!</h2></p>\
</body>\
