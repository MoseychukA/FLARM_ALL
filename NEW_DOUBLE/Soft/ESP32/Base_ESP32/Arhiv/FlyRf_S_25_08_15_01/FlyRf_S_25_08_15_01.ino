...
// Максимальный буфер для передачи данных
uint16_t txBuffer[MAX_MODBUS_BLOCK];
...
typedef struct {
    ...
    uint32_t transmission_stats[5]; // Статистика времени передачи
} modbus_control_t;
...
void preTransmission() 
{
    ...
    delayMicroseconds(20); // Уменьшена задержка
}
...
// Оптимизированная функция записи блока регистров
bool writeRegistersBlockOptimized(uint16_t start_address, uint16_t* data, uint16_t count) 
{
    uint16_t sent = 0;

    while (sent < count) {
        uint16_t chunk_size = min(MAX_MODBUS_BLOCK, count - sent);

        // Копируем данные в буфер передачи (если нужно)
        if (data != txBuffer) {
            memcpy(txBuffer, &data[sent], chunk_size * sizeof(uint16_t));
        }

        uint8_t result = node.writeSingleRegister(start_address + sent, txBuffer[0]);
        if (chunk_size > 1) {
            // Для блоков больше 1 регистра используем запись отдельных регистров
            for (uint16_t i = 1; i < chunk_size; i++) {
                result = node.writeSingleRegister(start_address + sent + i, data[sent + i]);
        ...
        sent += chunk_size;
        if (sent < count) {
            vTaskDelay(pdMS_TO_TICKS(1)); // Минимальная задержка
        }
    }
    return true;
}
...
uint16_t packUFOToBuffer(const ufo_t* obj, uint16_t* buffer) 
{
    ...
    return pos; // Возвращаем количество использованных регистров
}
...
bool sendContainerObjectFast(uint8_t index) 
{
    ...
    // Заголовок (3 регистра)
    txBuffer[0] = CMD_SEND_CONTAINER;
    txBuffer[1] = index;

    uint16_t data_size = packUFOToBuffer(&Container[index], &txBuffer[3]);
    txBuffer[2] = data_size;

    // Отправляем заголовок + данные одним блоком
    uint16_t total_size = 3 + data_size;

    if (total_size <= MAX_MODBUS_BLOCK) 
    {
        // Помещается в один блок
        return writeRegistersBlockOptimized(0x2000 + (index * 100), txBuffer, total_size);
    }
    else 
    {
        // Разбиваем на блоки
        bool result = writeRegistersBlockOptimized(0x1000, txBuffer, 3); // Заголовок
        if (result) {
            result = writeRegistersBlockOptimized(0x2000 + (index * 100), &txBuffer[3], data_size); // Данные
        }
        return result;
    }
}
...
bool sendThisAircraftFast() 
{
    ...
    uint16_t data_size = packUFOToBuffer(&ThisAircraft, &txBuffer[3]);
    txBuffer[2] = data_size;
    ...
    if (total_size <= MAX_MODBUS_BLOCK) 
    {
        return writeRegistersBlockOptimized(0x3000, txBuffer, total_size);
    }
    else 
    {
        bool result = writeRegistersBlockOptimized(0x1000, txBuffer, 3);
        if (result) 
        {
            result = writeRegistersBlockOptimized(0x3000, &txBuffer[3], data_size);
        }
        return result;
    }
}
...
bool sendAdditionalDataFast() 
{
    ...
    txBuffer[pos++] = CMD_SEND_ADDITIONAL;
    txBuffer[pos++] = 0;
    txBuffer[pos++] = 40; // Размер данных

    // Дополнительные данные
    txBuffer[pos++] = new_flagM ? 1 : 0;
    txBuffer[pos++] = new_buttonM;
    txBuffer[pos++] = setMessageReadM? 1 : 0;
    txBuffer[pos++] = MessageReadM ? 1 : 0;
    txBuffer[pos++] = SOS_Sprite_on_offM ? 1 : 0;
    txBuffer[pos++] = SOS_View_on_offM ? 1 : 0;
    txBuffer[pos++] = new_SOS_flagM ? 1 : 0;
    txBuffer[pos++] = confirm_messageM ? 1 : 0;
    txBuffer[pos++] = isValidGNSSM ? 1 : 0;
    txBuffer[pos++] = FLYRF_MODE_TESTM;

    // msg_resp (60 символов = 30 регистров)
    for (int i = 0; i < 30; i++) 
    {
        uint16_t combined = 0;
        if (i * 2 < 60) combined |= (msg_respM[i * 2] << 8);
        if (i * 2 + 1 < 60) combined |= msg_respM[i * 2 + 1];
        txBuffer[pos++] = combined;
    }

    return writeRegistersBlockOptimized(0x4000, txBuffer, pos);
}
...
bool sendAnalogSignalFast() 
{
    ...
    txBuffer[1] = 0;
    txBuffer[2] = 1;
    txBuffer[3] = analog_signalM;

    return writeRegistersBlockOptimized(0x5000, txBuffer, 4);
}
    ...
    while (true)
    {
        ...
        if (xSemaphoreTake(modbus_ctrl.rs485_mutex, pdMS_TO_TICKS(100)) == pdTRUE)
        {
            ...
            for (int i = 0; i < MAX_TRACKING_OBJECTS; i++) 
            {
                ...
                {
                    all_success = false;
                    Serial.printf("Failed to send Container[%d]\n", i);
                }
                vTaskDelay(pdMS_TO_TICKS(INTER_COMMAND_DELAY));
            }
            ...
            {
                all_success = false;
                Serial.printf("Failed to send ThisAircraft\n");
            }
            vTaskDelay(pdMS_TO_TICKS(INTER_COMMAND_DELAY));
            ...
            {
                all_success = false;
                Serial.printf("Failed to send AdditionalData\n");
            }
            vTaskDelay(pdMS_TO_TICKS(INTER_COMMAND_DELAY));
            ...
            {
                all_success = false;
                Serial.printf("Failed to send AnalogSignal\n");
            }
            ...
            if (all_success) 
            {
                Serial.printf("Full cycle completed in %d ms\n", transmission_time);
            }
            else 
            {
                Serial.printf("Cycle completed with errors in %d ms\n", transmission_time);
            }
        ...
        // Ожидание до следующего цикла (0.5 секунды)
...
void initModbusSource() 
{
    ...
    Serial.printf("MODBUS Source initialized on Core 0, max block size: %d registers\n", MAX_MODBUS_BLOCK);
...
{
    Serial.printf("MODBUS Stats - Active: %s, Last cycle: %d ms ago, Avg time: %d ms\n",
