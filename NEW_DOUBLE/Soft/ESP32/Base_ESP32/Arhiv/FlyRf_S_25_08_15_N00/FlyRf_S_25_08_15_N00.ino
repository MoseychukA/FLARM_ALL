...
uint16_t txBuffer[125];  // Максимальный размер блока данных MODBUS
...
// Функция записи блока регистров через отдельные записи
bool writeRegistersBlock(uint16_t start_address, uint16_t* data, uint16_t count) {
    for (uint16_t i = 0; i < count; i++) {
        if (!writeSingleRegister(start_address + i, data[i])) {
            Serial.printf("Failed to write register %d\n", start_address + i);
            return false;
        }
        delay(2); // Небольшая задержка между записями
    }
    return true;
}

// Передача одного объекта Container
bool sendContainerObject(uint8_t index) {
    if (index >= MAX_TRACKING_OBJECTS) return false;

    ufo_t* obj = &Container[index];
    uint16_t reg_count = 0;

    // Сначала отправляем заголовок
    txBuffer[0] = CMD_SEND_CONTAINER;
    txBuffer[1] = index;
    txBuffer[2] = 80; // Примерное количество регистров для UFO

    if (!writeRegistersBlock(0x1000, txBuffer, 3)) {
        return false;
    }
    ...
    // timestamp
    timeToRegisters(obj->timestamp, &txBuffer[reg_count], &txBuffer[reg_count + 1]);
    reg_count += 2;

    // addr
    txBuffer[reg_count++] = (uint16_t)(obj->addr >> 16);
    txBuffer[reg_count++] = (uint16_t)(obj->addr & 0xFFFF);
    txBuffer[reg_count++] = obj->addr_type;

    // latitude
    floatToRegisters(obj->latitude, &txBuffer[reg_count], &txBuffer[reg_count + 1]);
    reg_count += 2;

    // longitude
    floatToRegisters(obj->longitude, &txBuffer[reg_count], &txBuffer[reg_count + 1]);
    reg_count += 2;

    // old_latitude
    floatToRegisters(obj->old_latitude, &txBuffer[reg_count], &txBuffer[reg_count + 1]);
    reg_count += 2;

    // old_longitude
    floatToRegisters(obj->old_longitude, &txBuffer[reg_count], &txBuffer[reg_count + 1]);
    reg_count += 2;

    // altitude
    floatToRegisters(obj->altitude, &txBuffer[reg_count], &txBuffer[reg_count + 1]);
    reg_count += 2;

    // pressure_altitude
    floatToRegisters(obj->pressure_altitude, &txBuffer[reg_count], &txBuffer[reg_count + 1]);
    reg_count += 2;

    // course
    floatToRegisters(obj->course, &txBuffer[reg_count], &txBuffer[reg_count + 1]);
    reg_count += 2;

    // speed
    floatToRegisters(obj->speed, &txBuffer[reg_count], &txBuffer[reg_count + 1]);
    reg_count += 2;

    txBuffer[reg_count++] = obj->aircraft_type;

    // flight (16 символов = 8 регистров)
    for (int i = 0; i < 8; i++) {
        uint16_t combined = 0;
        if (i * 2 < 16) combined |= (obj->flight[i * 2] << 8);
        if (i * 2 + 1 < 16) combined |= obj->flight[i * 2 + 1];
        txBuffer[reg_count++] = combined;
    }

    // vert_rate
    txBuffer[reg_count++] = (uint16_t)(obj->vert_rate >> 16);
    txBuffer[reg_count++] = (uint16_t)(obj->vert_rate & 0xFFFF);

    // Squawk
    txBuffer[reg_count++] = (uint16_t)(obj->Squawk >> 16);
    txBuffer[reg_count++] = (uint16_t)(obj->Squawk & 0xFFFF);

    // timemsg
    timeToRegisters(obj->timemsg, &txBuffer[reg_count], &txBuffer[reg_count + 1]);
    reg_count += 2;

    // vs
    floatToRegisters(obj->vs, &txBuffer[reg_count], &txBuffer[reg_count + 1]);
    reg_count += 2;

    // geoid_separation
    floatToRegisters(obj->geoid_separation, &txBuffer[reg_count], &txBuffer[reg_count + 1]);
    reg_count += 2;

    txBuffer[reg_count++] = obj->hdop;
    txBuffer[reg_count++] = (uint16_t)obj->rssi;

    // distance
    floatToRegisters(obj->distance, &txBuffer[reg_count], &txBuffer[reg_count + 1]);
    reg_count += 2;

    // bearing
    floatToRegisters(obj->bearing, &txBuffer[reg_count], &txBuffer[reg_count + 1]);
    reg_count += 2;

    txBuffer[reg_count++] = obj->signal_source;

    // seen
    timeToRegisters(obj->seen, &txBuffer[reg_count], &txBuffer[reg_count + 1]);
    reg_count += 2;

    txBuffer[reg_count++] = obj->hour_msg;
    txBuffer[reg_count++] = obj->min_msg;
    txBuffer[reg_count++] = obj->delay_time_msg;
    ...
    while (sent < reg_count) {
        uint16_t chunk_size = min(50, reg_count - sent); // Блоки по 50 регистров

        if (!writeRegistersBlock(base_addr + sent, &txBuffer[sent], chunk_size)) {
            return false;
        }
...
bool sendThisAircraft() {
    uint16_t reg_count = 0;

    // Заголовок
    txBuffer[0] = CMD_SEND_THISAIRCRAFT;
    txBuffer[1] = 0;
    txBuffer[2] = 80;
    ...
    // Данные ThisAircraft (аналогично Container)
    reg_count = 0;

    timeToRegisters(ThisAircraft.timestamp, &txBuffer[reg_count], &txBuffer[reg_count + 1]);
    reg_count += 2;

    txBuffer[reg_count++] = (uint16_t)(ThisAircraft.addr >> 16);
    txBuffer[reg_count++] = (uint16_t)(ThisAircraft.addr & 0xFFFF);
    txBuffer[reg_count++] = ThisAircraft.addr_type;

    floatToRegisters(ThisAircraft.latitude, &txBuffer[reg_count], &txBuffer[reg_count + 1]);
    reg_count += 2;

    floatToRegisters(ThisAircraft.longitude, &txBuffer[reg_count], &txBuffer[reg_count + 1]);
    reg_count += 2;

    // ... добавить остальные поля аналогично sendContainerObject
    // Для краткости показаны только первые поля

    return writeRegistersBlock(0x3000, txBuffer, reg_count);
}
...
bool sendAdditionalData() {
    ...
    // Заголовок
    txBuffer[0] = CMD_SEND_ADDITIONAL;
    txBuffer[1] = 0;
    txBuffer[2] = 40;
    ...
    reg_count = 0;
    txBuffer[reg_count++] = new_flag ? 1 : 0;
    txBuffer[reg_count++] = new_button;
    txBuffer[reg_count++] = setMessageRead ? 1 : 0;
    txBuffer[reg_count++] = MessageRead ? 1 : 0;
    txBuffer[reg_count++] = SOS_Sprite_on_off ? 1 : 0;
    txBuffer[reg_count++] = SOS_View_on_off ? 1 : 0;
    txBuffer[reg_count++] = new_SOS_flag ? 1 : 0;
    txBuffer[reg_count++] = confirm_message ? 1 : 0;
    txBuffer[reg_count++] = isValidGNSS ? 1 : 0;
    txBuffer[reg_count++] = FLYRF_MODE_TEST;

    // msg_resp (60 символов = 30 регистров)
    for (int i = 0; i < 30; i++) {
        uint16_t combined = 0;
        if (i * 2 < 60) combined |= (msg_resp[i * 2] << 8);
        if (i * 2 + 1 < 60) combined |= msg_resp[i * 2 + 1];
        txBuffer[reg_count++] = combined;
    }

    return writeRegistersBlock(0x4000, txBuffer, reg_count);
}

// Передача аналогового сигнала
bool sendAnalogSignal() {
    // Заголовок
    txBuffer[0] = CMD_SEND_ANALOG;
    txBuffer[1] = 0;
    txBuffer[2] = 1;
    ...
    // Аналоговый сигнал
    return writeSingleRegister(0x5000, analog_signal);
}

// Задача передачи данных (выполняется на ядре 0)
void modbusTransmissionTask(void* parameters) {
    tx_command_t command;

    while (true) {
        if (xQueueReceive(modbus_ctrl.tx_queue, &command, pdMS_TO_TICKS(50)) == pdTRUE) {
            // Проверяем, прошло ли достаточно времени с последней передачи
            uint32_t current_time = millis();
            if (current_time - modbus_ctrl.last_transmission_time < TRANSMISSION_INTERVAL) {
                vTaskDelay(pdMS_TO_TICKS(TRANSMISSION_INTERVAL - (current_time - modbus_ctrl.last_transmission_time)));
            }

            if (xSemaphoreTake(modbus_ctrl.rs485_mutex, pdMS_TO_TICKS(1000)) == pdTRUE) {
                ...
                case CMD_SEND_CONTAINER:
                    success = sendContainerObject(command.object_index);
                    break;

                case CMD_SEND_THISAIRCRAFT:
                    success = sendThisAircraft();
                    break;

                case CMD_SEND_ADDITIONAL:
                    success = sendAdditionalData();
                    break;

                case CMD_SEND_ANALOG:
                    success = sendAnalogSignal();
                    break;
                }

                modbus_ctrl.last_transmission_time = millis();

                if (success) {
                    Serial.printf("MODBUS: Command %d sent successfully\n", command.command);
                }
                else {
                    Serial.printf("MODBUS: Command %d failed\n", command.command);
                }
...
void initModbusSource() {
    ...
    modbus_ctrl.tx_queue = xQueueCreate(20, sizeof(tx_command_t));
    modbus_ctrl.transmission_active = false;
    modbus_ctrl.last_transmission_time = 0;
    ...
    Serial.println("MODBUS Source initialized on Core 0");
}

// API функции
bool queueContainerTransmission(uint8_t index) {
    if (index >= MAX_TRACKING_OBJECTS) return false;

    tx_command_t cmd;
    cmd.command = CMD_SEND_CONTAINER;
    cmd.object_index = index;
    cmd.timestamp = millis();

    return xQueueSend(modbus_ctrl.tx_queue, &cmd, pdMS_TO_TICKS(100)) == pdTRUE;
}

bool queueThisAircraftTransmission() {
    tx_command_t cmd;
    cmd.command = CMD_SEND_THISAIRCRAFT;
    cmd.object_index = 0;
    cmd.timestamp = millis();
...
bool queueAdditionalDataTransmission() {
    tx_command_t cmd;
    cmd.command = CMD_SEND_ADDITIONAL;
    cmd.object_index = 0;
    cmd.timestamp = millis();
...
bool queueAnalogSignalTransmission() {
    tx_command_t cmd;
    cmd.command = CMD_SEND_ANALOG;
    cmd.object_index = 0;
    cmd.timestamp = millis();
...
uint32_t getModbusQueueLength() {
    return uxQueueMessagesWaiting(modbus_ctrl.tx_queue);
}
...
void setup()
{
 ...
 /* SoC->Button_setup();*/
  ...
  Serial.println("MODBUS Master ready - transmission every 0.5 seconds");
...
void loop()
{
 ...
 // case FLYRF_MODE_TXRX_TEST0:
  ...
  // Передача данных каждые 2 секунды (внутри задачи соблюдается интервал 0.5 сек между командами)
  if (millis() - lastTransmission > 2000) {
      for (int i = 0; i < MAX_TRACKING_OBJECTS; i++) {
          queueContainerTransmission(i);
      }
      queueThisAircraftTransmission();
      queueAdditionalDataTransmission();
      queueAnalogSignalTransmission();

      lastTransmission = millis();
  }

  Serial.printf("Queue: %d, Active: %s, Last TX: %d ms ago\n",
      getModbusQueueLength(),
      isModbusTransmissionActive() ? "Yes" : "No",
      millis() - modbus_ctrl.last_transmission_time);
