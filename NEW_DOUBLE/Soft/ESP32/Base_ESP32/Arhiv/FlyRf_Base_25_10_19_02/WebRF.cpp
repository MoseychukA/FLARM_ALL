...
<option %s value='%d'>Тест самолет на месте + в полете</option>\
...
<tr>\
<th align = left>Отправить в трекер</th>\
<td align = right>\
<select name = 'tracker_send'>\
<option % s value = '%d'>Выключить</option>\
<option % s value = '%d'>Одиночный</option>\
<option % s value = '%d'>Периодически</option>\
<option % s value = '%d'>Только новые</option>\
</select>\
</td>\
</tr>"),
...
<tr>\
<th align=left>Приемник 1090 МГц</th>\
<td align=right>\
<select name='d1090'>\
<option %s value='%d'>Прием выключен</option>\
<option %s value='%d'>Только с координатами</option>\
<option %s value='%d'>Полная запись в базу</option>"),
  (settings->d1090 == D1090_RECEIVE_OFF ? "selected" : ""), D1090_RECEIVE_OFF,
  (settings->d1090 == D1090_RECEIVE_COORDINATE ? "selected" : ""), D1090_RECEIVE_COORDINATE,
  (settings->d1090 == D1090_RECEIVE_FULL ? "selected" : ""), D1090_RECEIVE_FULL);
...
void hardwareSettings() //Ручной ввод аппаратных настроек
{
    ...
    <tr>\
    <th align=left>Дисплей</th>\
    <td align=right>\
    <select name='display_set'>\
    <option % s value = '%d'>Нет дисплея</option>\
    <option % s value = '%d'>Встроенный</option>\
    <option % s value = '%d'>Внешний</option>\
    </select>\
    </td>\
    </tr>\
    <tr>\
    ...
    <option % s value = '%d'>Данные с приемника 1090</option>\
    <option % s value = '%d'>Данные на внешний дисплей</option>\
    </select>\
    </td>\
    </tr>\
    <tr>\
    ...
    <tr>\
    <th align=left>Минимальное значение заряда аккумулятора</th>\
    <td align=right>\
    <INPUT type='number' name='akk_min' min='2.6' max='3.7' step='0.1' value='%.1f'>\
    </td>\
    </tr>\
    <tr>\
    <th align=left>Максимальное значение заряда аккумулятора</th>\
    <td align=right>\
    <INPUT type='number' name='akk_max' min='3.3' max='4.5' step='0.1' value='%.1f'>\
    </td>\
    </tr>\
    <tr>\
    <th align=left>Коэффициет пересчета заряда аккумулятора</th>\
    <td align=right>\
    <INPUT type='number' name='akk_koef' min='0.5' max='20.0'  step='0.01' value='%.02f'>\
    </td>\
    </tr>"),
    ...
    (settings->display_set == DISPLAY_OFF ? "selected" : ""), DISPLAY_OFF,
    (settings->display_set == DISPLAY_BUILT_IN ? "selected" : ""), DISPLAY_BUILT_IN,
    (settings->display_set == DISPLAY_EXTERNAL ? "selected" : ""), DISPLAY_EXTERNAL,
    (settings->serial_out == SEND_SERIAL_OFF ? "selected" : ""), SEND_SERIAL_OFF,
    (settings->serial_out == SEND_SERIAL_1090 ? "selected" : ""), SEND_SERIAL_1090,
...
void handleInput() // Ввод основных настроек
{
  ...
  for ( uint8_t i = 0; i < server.args(); i++ ) 
  {
    ...
    <tr><th align=left>Отправлять в трекер</th><td align=right>%d</td></tr>\
    ...
    <hr>\
     <p align=center><h2 align=center>Выполняется перезагрузка... Пожалуйста, подождите!</h2></p>\
      ...
      settings->mode,
      settings->aircraft_type, settings->tracker_send, settings->bluetooth,
...
void hardwareInput() // Отображение аппаратных настроек
{
    ...
    for (uint8_t i = 0; i < server.args(); i++)
    {
        ...
        } else if (server.argName(i).equals("akk_min")) {
            settings->akk_min = server.arg(i).toFloat();
        } else if (server.argName(i).equals("akk_max")) {
            settings->akk_max = server.arg(i).toFloat();
        } else if (server.argName(i).equals("akk_koef")) {
            settings->akk_koef = server.arg(i).toFloat();
        }
    }
    ...
    <tr><th align=left>Минимальное значение заряда аккумулятора</th><td align=right>%.1f</td></tr>\
    <tr><th align=left>Максимальное значение заряда аккумулятора</th><td align=right>%.1f</td></tr>\
    <tr><th align=left>Коэффициет пересчета заряда аккумулятора</th><td align=right>%.2f</td></tr>\
    </table>\
    <hr>\
     <p align=center><h2 align=center>Выполняется перезагрузка... Пожалуйста, подождите!</h2></p>\
    </body>\
