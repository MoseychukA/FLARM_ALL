...
typedef struct Settings {
    ...
    uint8_t  d1090:3;   //Управление приемом данных с приемника 1090
    uint8_t  json:2;
    ...
    bool     akk_view : 2;
    bool     sos_view : 2;
    float    akk_min;                // Минимальное значение заряда аккумулятора
    float    akk_max;                // Максимальное значение заряда аккумулятора
    float    akk_koef;               // Коэффициет пересчета заряда аккумулятора
    ...
    bool     out_of_sync : 2;       // Полет тестовых самолетов синхронно или не синхронно
    uint8_t  display_set;
    uint8_t  serial_out : 3;
