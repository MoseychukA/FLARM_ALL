...
// Типы пакетов для отправки на внешний дисплей
enum : uint8_t {
    PKT_CONTAINER = 0x01,
    PKT_THISAC    = 0x02,
    PKT_ANALOG    = 0x03,
    ...
    for (;;)
    {
        ...
        if (hdr.type == PKT_ACK || hdr.type == PKT_NACK) 
        {
            ...
            if (ok && hdr.length == 4) 
            {
                ...
                {
                    strncpy(CommandHandler.msg_tmp_all, "\0", strlen(CommandHandler.msg_tmp_all));
                    strncpy(AuxFlags.msg_resp_M, "\0", 150/*strlen(AuxFlags.msg_resp_M)*/);
                    AuxFlags.msg_resp_M[0] = 0;
                    CommandHandler.msg_tmp_all[0] = 0;
                }
    ...
    for (;;)
    {
        if (xQueueReceive(txQueue, &item, portMAX_DELAY) == pdTRUE) 
        {
            ...
            for (int attempt = 0; attempt < MAX_RETRY && !sentOk; ++attempt) 
            {
                ...
                {
                    rs485SetTX(true);
                    RS485_SERIAL.write((uint8_t*)&hdr, sizeof(hdr)); // Отправить преамбулу во внешний дисплей
                    RS485_SERIAL.write(item.payload, item.length);   // Отправить пакет во внешний дисплей
                    RS485_SERIAL.write((uint8_t*)&crc, sizeof(crc)); // Отправить контрольную сумму во внешний дисплей
                    RS485_SERIAL.flush();
                    delayMicroseconds(200);
                    rs485SetTX(false);
                    xSemaphoreGive(serialMutex);
...
void DataGenTask(void* arg) 
{
    ...
    for (;;) 
    {
        ...
        // Доп.флаги
        AuxFlags.new_flag_M = service.getNewMessageFlag();                                 // Получить признак нового сообщения 
        //AuxFlags.new_buttton_M = (uint8_t)((ms / 1000) % 4);
        AuxFlags.setMessageRead_M = service.getMessageRead();
        //AuxFlags.MessageRead_M = ((ms / 5000) % 2);
        AuxFlags.Time_Hour_M = service.get_time_hour();
        AuxFlags.Time_Minute_M = service.get_time_minute();
        AuxFlags.new_SOS_flag_M = digitalRead(SOC_GPIO_PIN_SOS);   // Получить признак состояния кнопки SOS;
        AuxFlags.confirm_message_M = service.get_confirm_message();       // Разрешить отправить подтверждение прочтения сообщения.
        if (strlen(CommandHandler.msg_tmp_all) > 0)
        {
            strncpy(AuxFlags.msg_resp_M, CommandHandler.msg_tmp_all, strlen(CommandHandler.msg_tmp_all));
        }
        //snprintf(AuxFlags.msg_resp_M, sizeof(AuxFlags.msg_resp_M), "Test msg t=%lu", (unsigned long)(ms / 1000));
       // snprintf(AuxFlags.msg_resp_M, sizeof(AuxFlags.msg_resp_M), CommandHandler.msg_tmp_all, (unsigned long)(ms / 1000));
        AuxFlags.isValidGNSS_M = (bool)service.get_GNSS_on_off();
...
void DeltaCheckTask(void* arg) 
{
    ...
    for (;;) 
    {
        ...
        for (int i = 0; i < MAX_TRACKING_OBJECTS; ++i)
        {
            ...
            //{
            //    Serial.printf("[RX] Container[%u]:$%06X:%d:%8s:%.1f:%d:%d:%.6f:%.6f\r\n",
            //        i,
            ...
            //   //// Serial.print(":"); Serial.print(Container[i].seen_time);
            //   // Serial.println();
            //    Serial.flush();
            //}
        }
        ...
        if (rxIndex >= PACKET_SIZE)
        {
            if (rxBuffer[PACKET_SIZE - 3] == 0xFF && rxBuffer[PACKET_SIZE - 2] == 0xFF && rxBuffer[PACKET_SIZE - 1] == 0xFF)
            {
                ...
                if (settings->serial_out == SEND_SERIAL_1090 && settings->nmea_out != NMEA_UART)
                {
                    ...
                    Serial.print(":"); Serial.print(packet.seen_time);
                    Serial.println();
                    Serial.flush();
                }

                //if (settings->d1090 == D1090_RECEIVE_COORDINATE && packet.lat_msg != 0 && packet.lon_msg != 0) // запись в базу только с координатами
                //{
                //    fo.addr = packet.addr;
                ...
                //    fo.vert_rate = 0;
                //    fo.signal_source = 2;
                //    // fo.timestamp = packet.seen_time; // 
                //    fo.course = packet.track;
                ...
                //    Traffic_Add(&fo);
                //}
                //else if (settings->d1090 == D1090_RECEIVE_FULL)     // запись в базу всю информацию
                //{
                    fo.addr = packet.addr;
                    fo.squawk = packet.squawk;
                    memcpy((char*)fo.flight, packet.flight, strlen(packet.flight));
...
void loop()
{
    ...
    if (settings->d1090 != D1090_RECEIVE_OFF)
    {
