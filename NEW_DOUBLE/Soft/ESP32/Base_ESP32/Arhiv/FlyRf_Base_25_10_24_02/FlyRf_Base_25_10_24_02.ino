...
//void sendPacket_RS485(const full_packet_net_t* pkt) {
//    const size_t plen = sizeof(full_packet_net_t);
//    // Буфер глобален внутри функции - не переполняет стек задачи!
//    static uint8_t buf[sizeof(full_packet_net_t)];
//
//    memcpy(buf, pkt, plen);
//    uint16_t crc = crc16_ccitt(buf, plen);
//
//    // Контроль отправки для отладки
//    Serial.print("Отправка пакета, полный размер: ");
//    Serial.println(plen + 8); // 4(header)+data+2(crc)+4(footer)
//    //Serial.print("HEADER: ");
...
//    digitalWrite(RS485_DE_PIN, HIGH);
//    digitalWrite(LED, LOW);
//    delay(15); // Задержка! (важна на MAX скорости)
//    RS485_SERIAL.write((uint8_t*)&PACKET_HEADER, sizeof(PACKET_HEADER));
//    RS485_SERIAL.write(buf, plen);
//    RS485_SERIAL.write((uint8_t*)&crc, sizeof(crc));
//    RS485_SERIAL.write((uint8_t*)&PACKET_FOOTER, sizeof(PACKET_FOOTER));
//
//    RS485_SERIAL.flush();
//    delay(15); // Минимальная задержка (особенно критично на высокой скорости!)
//    digitalWrite(RS485_DE_PIN, LOW);
//    digitalWrite(LED, HIGH);
//    xSemaphoreGive(serialMutex);
//
...
//    while (idx >= sizeof(full_packet_net_t) + 8) // header(4)+packet+crc(2)+footer(4)
//    {
...
//        xSemaphoreTake(containerMutex, portMAX_DELAY);
//
//    // Копируем реальные рабочие данные в пакет!
//        memcpy(&packet.ThisAircraft, &ThisAircraft, sizeof(ufo_t));
//        memcpy(&packet.Container, &Container, sizeof(Container));
//        memcpy(&packet.AuxData, &AuxData, sizeof(aux_t));
//
//        // (или можно попробовать индивидуальные поля для ещё лучшей диагностики)
//        packet.BUTTON1 = count;
//        packet.BUTTON2 = 0x02;
//        count++;
//        esp_task_wdt_reset();
