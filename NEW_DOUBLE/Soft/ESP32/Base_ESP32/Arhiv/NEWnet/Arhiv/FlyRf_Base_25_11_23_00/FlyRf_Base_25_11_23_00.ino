...
{
    size_t hw = uxTaskGetStackHighWaterMark(NULL);
    if (hw < 200) { // 200 слов (~800 байт) — уже тревожный уровень
        Serial.printf("TX stack low water mark:%u words\n", hw);
    }
        ...
        if (rxIndex >= PACKET_SIZE)
        {
            ...
            if (rxBuffer[PACKET_SIZE - 3] == 0xFF && rxBuffer[PACKET_SIZE - 2] == 0xFF && rxBuffer[PACKET_SIZE - 1] == 0xFF)
            {
                ...
                if (settings->serial_out == SEND_SERIAL_1090 && settings->nmea_out != NMEA_UART)
                {
                    ...
                    Serial.print(":"); Serial.print(packet.rssi_rp2040);
                    Serial.println();
                    Serial.flush();
                   // esp_task_wdt_reset();
                }
...
void setup()
{
    ...
    //----------------------------------------------------------------------------------------------
      ...
      //  xTaskCreatePinnedToCore(txTask, "TX", 12288, NULL, 2, &Task1, 0);
      //  xTaskCreatePinnedToCore(rxTask, "RX", 12288, NULL, 1, &Task2, 0);
...
void loop()
{
    ...
    //    lastCheck = millis();
    //    Serial.printf("loopTask stack HW:%u words\n", uxTaskGetStackHighWaterMark(NULL));
    //}
 ...
 // case FLYRF_MODE_TXRX_TEST0:
  ...
  unsigned long currentMillis = millis();
    ...
    if (currentMillis - thresholdMillis >= threshold_interval)
    {
        thresholdMillis = currentMillis;
        values[0] = settings->threshold_level;
        sendPacketToRP2040(values, 1);
    }
    Time_loop();
