...
#define TX_WDT_ITER_LIM 4096    // Для цикла приёма/разбора (см. rxTask)
...
void sendPacket_RS485(const full_packet_net_t* pkt) {
    ...
    delay(8); // рекомендация: обязательно пауза перед началом передачи
    ...
    RS485_SERIAL.flush();
    delay(8); // рекомендация: обязательно пауза после flush
   // delayMicroseconds(200);
    rs485SetTX(false);
    digitalWrite(LED, HIGH);

    xSemaphoreGive(serialMutex);
   // Serial.println("RS485: Пакет отправлен!");
}
...
bool receivePacket_RS485(full_packet_net_t* pkt, uint8_t* btn1, uint8_t* btn2) 
{
    ...
    int safe_counter = TX_WDT_ITER_LIM;
    ...
    while (idx >= sizeof(full_packet_net_t) + 8 && safe_counter-- > 0) 
    {
        ...
        // Сбросить буфер если слишком долго парсим — вероятен мусор/ошибка sync
        if (safe_counter < TX_WDT_ITER_LIM / 2) 
        {
