        ...
        while (pStream->available()>0)              // читаем данные во внутренний буфер
        {
            ...
            *strBuff += ch;
            ...
            if (strBuff->length() >= BUFFER_SIZE+2)   // Если иформации больше чем BUFFER_SIZE - принимать не будем и очистим буфер
            {
                ...
                pStream->println(F("The text is very long. !Maximum 136 characters"));
                clearCommand();
                return true;
            } // if
        } // while
...
bool CommandHandlerClass::setTXT(const char* commandPassed, CommandParser& parser, Stream* pStream, String textString) // Программа приема текстового сообщения
{
    if (commandPassed)
    {
        ...
        if (((minute_all - minute_msg) <= 10 && minute_msg < minute_all) || ((minute_all - minute_msg) == 0))
        {
            ...
            {
                if (tmp_msg_len > 136)  // Проверить максимальную длину сообщения
                {
                    pStream->print(CORE_COMMAND_ANSWER_ERROR);
                    pStream->println(F("The text is very long. !Maximum 136 characters"));
                    msg_tmp[textString.length() + 1] = { 0 };
                    strncpy(msg_tmp, textString.c_str(), 136 + 1);          // Преобразование принятую строку String в массив char для последующей обработки
                }
