...
typedef struct TransmissionTask {
    bool      ready;
    uint8_t   task_type;  // 0=analog, 1=additional, 2=thisaircraft, 3=container
    int       object_index;
...
QueueHandle_t priorityQueue;  // Приоритетная очередь для важных данных
TaskHandle_t transmissionTaskHandle;
SemaphoreHandle_t dataMutex;
SemaphoreHandle_t rs485Mutex;  // Мьютекс для RS485
...
void preTransmission() {
    ...
    delayMicroseconds(200); // Увеличена задержка для стабильности
}
...
// Оптимизированная задача передачи данных на ядре 0
void transmissionTask(void* parameter) 
{
    transmission_task_t task;

    Serial.printf("Оптимизированная задача передачи запущена на ядре: %d\n", xPortGetCoreID());

    while (true) {
        bool taskReceived = false;

        // Сначала проверяем приоритетную очередь
        if (xQueueReceive(priorityQueue, &task, pdMS_TO_TICKS(10)) == pdTRUE) {
            taskReceived = true;
        }
        // Затем обычную очередь
        else if (xQueueReceive(transmissionQueue, &task, pdMS_TO_TICKS(100)) == pdTRUE) {
            taskReceived = true;
        }
        ...
        if (taskReceived) 
        {
            ...
            {
                Serial.printf("[Core 0] Задача устарела, пропускаем (тип: %d)\n", task.task_type);
            ...
            // Получаем мьютекс RS485 перед передачей
            if (xSemaphoreTake(rs485Mutex, pdMS_TO_TICKS(500)) == pdTRUE) {

                switch (task.task_type) {
                case 0: // Аналоговое значение
                    success = transmitAnalogValueWithRetry(task.data_buffer[0]);
                    txStats.analog_packets++;
                    break;

                case 1: // Дополнительные данные
                    success = transmitDataBlocksWithRetry(task.start_register, task.data_buffer, task.data_size, "Additional");
                    txStats.additional_packets++;
                    break;

                case 2: // ThisAircraft
                    success = transmitDataBlocksWithRetry(task.start_register, task.data_buffer, task.data_size, "ThisAircraft");
                    txStats.thisaircraft_packets++;
                    break;

                case 3: // Container объект
                    success = transmitDataBlocksWithRetry(task.start_register, task.data_buffer, task.data_size, "Container");
                ...
                // Адаптивная задержка в зависимости от результата
                if (success) {
                    vTaskDelay(pdMS_TO_TICKS(50));  // Короткая задержка при успехе
                }
                else {
                    vTaskDelay(pdMS_TO_TICKS(200)); // Длинная задержка при ошибке
                }
            }
            else {
                Serial.println("[Core 0] Не удалось получить мьютекс RS485");
                txStats.failed_packets++;
...
bool transmitAnalogValueWithRetry(uint16_t value) 
{
    for (int retry = 0; retry < MAX_RETRIES; retry++) 
    {
        ...
        if (result == node.ku8MBSuccess) 
        {
            if (retry > 0) 
            {
                Serial.printf("[Core 0] Аналоговое значение передано с %d попытки: %d\n", retry + 1, value);
        ...
        else 
        {
            ...
            {
                vTaskDelay(pdMS_TO_TICKS(100 * (retry + 1))); // Экспоненциальная задержка
            }
        }
    }

    Serial.printf("[Core 0] Ошибка передачи аналогового значения после %d попыток\n", MAX_RETRIES);
    return false;
}

bool transmitDataBlocksWithRetry(uint16_t startRegister, uint16_t* dataBuffer, uint16_t dataSize, const char* dataType) 
{
    ...
    for (int block = 0; block < numBlocks; block++) 
    {
        ...
        {
            // Очищаем буфер и заполняем данными
            ...
            else 
            {
                ...
                if (retry < MAX_RETRIES - 1) 
                {
                    vTaskDelay(pdMS_TO_TICKS(50 * (retry + 1))); // Задержка между повторами
                }
            }
        }

        if (!blockSuccess) {
            Serial.printf("[Core 0] Ошибка передачи %s блока %d после %d попыток\n", dataType, block, MAX_RETRIES);
            allSuccess = false;
        }

        // Задержка между блоками для стабильности
        vTaskDelay(pdMS_TO_TICKS(100));
    }
...
{
    switch (errorCode) {
    case 0xE0: // Invalid response
        txStats.response_errors++;
        break;
    case 0xE1: // Timeout
        txStats.timeout_errors++;
        break;
    case 0xE5: // Checksum error
        txStats.checksum_errors++;
        break;
    }
}
...
void sendAnalogValueToQueue(uint8_t priority);
void sendAdditionalDataToQueue(uint8_t priority);
void sendThisAircraftToQueue(uint8_t priority);
void sendContainerObjectToQueue(int objIndex, uint8_t priority);
void monitorQueues();
...
unsigned long previousMillisMOD = 0;        // will store last time LED was updated
...
void setup()
{
 ...
 /* if (settings->input_coordinates == IMPUT_COORD_MANUAL)
  ...
  // Создание очередей с увеличенными размерами
  transmissionQueue = xQueueCreate(20, sizeof(transmission_task_t));  // Увеличена очередь
  priorityQueue = xQueueCreate(5, sizeof(transmission_task_t));       // Приоритетная очередь

  if (transmissionQueue == NULL || priorityQueue == NULL) {
      Serial.println("Ошибка создания очередей!");
      return;
  }

  // Создание задачи передачи на ядре 0 с повышенным приоритетом
  xTaskCreatePinnedToCore(
      transmissionTask,       // Функция задачи
      "OptimizedTransmission",// Имя задачи
      8192,                   // Увеличенный размер стека
      NULL,                   // Параметры
  ...
  Serial.println("Оптимизированный Master ESP32 инициализирован");
  Serial.printf("Размер блока передачи: %d регистров\n", BLOCK_SIZE);
  Serial.printf("Максимальное количество повторов: %d\n", MAX_RETRIES);
  Serial.println("Задача передачи создана на ядре 0 с повышенным приоритетом");
...
void loop()
{
 ...
 // SoC->Display_loop();
  ...
  // Основной цикл выполняется на ядре 1


  unsigned long currentMillis = millis();

  if (currentMillis - previousMillisMOD >= interval) 
  {
      // save the last time you blinked the LED
      previousMillisMOD = currentMillis;
      ...
      // Отправка задач в очередь с приоритетами
      // 1. Передача дополнительных данных (средний приоритет)
      sendAdditionalDataToQueue(1); // Приоритет 1

      // 2. Передача ThisAircraft (средний приоритет)
      sendThisAircraftToQueue(1); // Приоритет 1
      yield();

      // 3. Передача Container объектов (высокий приоритет)
      static int currentObject = 0;
      sendContainerObjectToQueue(currentObject, 0); // Приоритет 0 (высший)
      currentObject = (currentObject + 1) % MAX_TRACKING_OBJECTS;
      yield();

      // 4. Передача аналогового значения (низкий приоритет)
      sendAnalogValueToQueue(2); // Приоритет 2 (низший)
      yield();
      // Вывод статистики
...
void sendAnalogValueToQueue(uint8_t priority) 
{
    transmission_task_t task;
    task.ready = true;
    task.task_type = 0; // Аналоговое значение
    task.object_index = -1;
    ...
    if (xQueueSend(targetQueue, &task, pdMS_TO_TICKS(50)) != pdTRUE) 
    {
        Serial.println("[Core 1] Очередь аналоговых значений переполнена");
    }
}
...
void sendAdditionalDataToQueue(uint8_t priority) 
{
    transmission_task_t task;
    task.ready = true;
    task.task_type = 1; // Дополнительные данные
    task.object_index = -1;
    ...
    if (xSemaphoreTake(dataMutex, pdMS_TO_TICKS(100)) == pdTRUE) 
    {
        ...
        if (xQueueSend(targetQueue, &task, pdMS_TO_TICKS(50)) != pdTRUE) 
        {
            Serial.println("[Core 1] Очередь дополнительных данных переполнена");
        }
    }
}
...
void sendThisAircraftToQueue(uint8_t priority) 
{
    ...
    if (xSemaphoreTake(dataMutex, pdMS_TO_TICKS(100)) == pdTRUE) 
    {
        ufoToModbusData(&ThisAircraft, task.data_buffer);
        xSemaphoreGive(dataMutex);
        ...
        if (xQueueSend(targetQueue, &task, pdMS_TO_TICKS(50)) != pdTRUE) 
        {
            Serial.println("[Core 1] Очередь ThisAircraft переполнена");
        }
    }
}
...
void sendContainerObjectToQueue(int objIndex, uint8_t priority)
{
    transmission_task_t task;
    task.ready = true;
    task.task_type = 3; // Container объект
    ...
    if (xSemaphoreTake(dataMutex, pdMS_TO_TICKS(100)) == pdTRUE) 
    {
        ufoToModbusData(&Container[objIndex], task.data_buffer);
        xSemaphoreGive(dataMutex);

        if (xQueueSend(transmissionQueue, &task, pdMS_TO_TICKS(50)) != pdTRUE) 
        {
            Serial.printf("[Core 1] Очередь Container[%d] переполнена\n", objIndex);
        }
    }
}
...
void updateData() 
{
    // Симуляция изменения дополнительных данных с меньшей частотой
    static uint32_t lastUpdate = 0;

    if (millis() - lastUpdate > 1000) 
    { // Обновляем каждые 8 секунд (реже)

        // Защищаем данные мьютексом при обновлении
        if (xSemaphoreTake(dataMutex, pdMS_TO_TICKS(100)) == pdTRUE) 
        {
            ...
            // Обновляем координаты ThisAircraft
            ThisAircraft.latitude += 0.0001;
            ThisAircraft.longitude += 0.0001;
            ThisAircraft.timestamp = millis() / 1000;

            // Обновляем Container объекты
            for (int i = 0; i < MAX_TRACKING_OBJECTS; i++) 
            {
                Container[i].latitude += 0.0001;
                Container[i].longitude += 0.0001;
                Container[i].altitude += 1;
...
void testConnection() 
{
    ...
    for (int i = 0; i < 3; i++) 
    {
        ...
        {
            Serial.printf("Попытка %d: Ошибка соединения: %d\n", i + 1, result);
            printModbusError(result);
            delay(1000);
        }
    }

    Serial.println("Не удалось установить соединение после 3 попыток");
}
...
void monitorQueues() 
{
    ...
    { // Каждые 5 секунд
        int normalQueue = uxQueueMessagesWaiting(transmissionQueue);
        int prioQueue = uxQueueMessagesWaiting(priorityQueue);

        if (normalQueue > 15 || prioQueue > 3)
        {
            Serial.printf("[ПРЕДУПРЕЖДЕНИЕ] Очереди переполняются: обычная=%d, приоритетная=%d\n", normalQueue, prioQueue);
        }
...
void printTransmissionStats() 
{
    ...
    if (millis() - lastStatsTime > 15000) { // Каждые 15 секунд
        ...
        Serial.println("\n=== РАСШИРЕННАЯ СТАТИСТИКА ПЕРЕДАЧИ ===");
        ...
        Serial.printf("Ошибки таймаута: %u\n", txStats.timeout_errors);
        Serial.printf("Ошибки CRC: %u\n", txStats.checksum_errors);
        Serial.printf("Ошибки ответа: %u\n", txStats.response_errors);
        ...
        // Цветовая индикация качества связи
        if (successRate >= 98.0) {
            Serial.println("🟢 ОТЛИЧНОЕ качество связи");
        }
        else if (successRate >= 95.0) {
            Serial.println("🟡 ХОРОШЕЕ качество связи");
        }
        else if (successRate >= 90.0) {
            Serial.println("🟠 УДОВЛЕТВОРИТЕЛЬНОЕ качество связи");
        }
        else {
            Serial.println("🔴 ПЛОХОЕ качество связи - требуется диагностика");
        }
...
void initTestData() 
{
    ...
    strcpy(AdditionalData.msg_resp, "Optimized dual-core ESP32S3 transmission");
    AdditionalData.isValidGNSS = service.get_isValidGNSS();
    ...
    //ThisAircraft.course = 270;
    //ThisAircraft.speed = 180;
    ...
    Serial.println("Оптимизированные тестовые данные инициализированы");
}
...
//    task.data_size = 1;
//
//    if (xQueueSend(transmissionQueue, &task, pdMS_TO_TICKS(100)) != pdTRUE) {
//        Serial.println("[Core 1] Ошибка отправки аналогового значения в очередь");
//    }
//}
//
//void sendAdditionalDataToQueue() 
//{
//    transmission_task_t task;
//    task.ready = true;
//    task.task_type = 1; // Дополнительные данные
...
//
//        if (xQueueSend(transmissionQueue, &task, pdMS_TO_TICKS(100)) != pdTRUE) {
//            Serial.println("[Core 1] Ошибка отправки дополнительных данных в очередь");
//        }
//    }
//}
//
//void sendThisAircraftToQueue() 
//{
//    transmission_task_t task;
//    task.ready = true;
//    task.task_type = 2; // ThisAircraft
...
//
//        if (xQueueSend(transmissionQueue, &task, pdMS_TO_TICKS(100)) != pdTRUE) {
//            Serial.println("[Core 1] Ошибка отправки ThisAircraft в очередь");
//        }
//    }
//}
//
//void sendContainerObjectToQueue(int objIndex) 
//{
//    transmission_task_t task;
//    task.ready = true;
//    task.task_type = 3; // Container объект
...
//
//        if (xQueueSend(transmissionQueue, &task, pdMS_TO_TICKS(100)) != pdTRUE) {
//            Serial.printf("[Core 1] Ошибка отправки Container[%d] в очередь\n", objIndex);
//        }
//    }
//}
//
...
//    static uint32_t lastUpdate = 0;
//
//    if (millis() - lastUpdate > 5000) { // Обновляем каждые 5 секунд
//
//        // Защищаем данные мьютексом при обновлении
//        if (xSemaphoreTake(dataMutex, pdMS_TO_TICKS(100)) == pdTRUE) {
//            AdditionalData.new_flag = !AdditionalData.new_flag;
...
//
//            // Обновляем сообщение
//            snprintf(AdditionalData.msg_resp, sizeof(AdditionalData.msg_resp),
//                "MSG_%lu", millis() / 1000);
//
//            // Обновляем координаты ThisAircraft
//            ThisAircraft.latitude += 0.0001;
...
//        Serial.printf("Всего пакетов: %u\n", txStats.total_packets);
//        Serial.printf("Успешно: %u (%.1f%%)\n", txStats.successful_packets,
//            (float)txStats.successful_packets * 100.0 / max(txStats.total_packets, 1u));
//        Serial.printf("Ошибок: %u (%.1f%%)\n", txStats.failed_packets,
//            (float)txStats.failed_packets * 100.0 / max(txStats.total_packets, 1u));
//        Serial.printf("Аналоговых: %u\n", txStats.analog_packets);
//        Serial.printf("Дополнительных: %u\n", txStats.additional_packets);
...
//    AdditionalData.new_SOS_flag = false;
//    AdditionalData.confirm_message = true;
//    strcpy(AdditionalData.msg_resp, "Test message from dual-core source ESP32S3");
...
//    }
//
//    Serial.println("Тестовые данные инициализированы для двухъядерной работы");
//}
//
...
//{
//   // Serial.println("Передача дополнительных данных блоками по 25 регистров...");
//
...
//
//    // Передача блоками по 25 регистров (2 блока)
//    bool success = true;
//    for (int block = 0; block < 2; block++) {
//        uint16_t regAddr = 950 + (block * BLOCK_SIZE);
//
//        node.clearTransmitBuffer();
...
//        }
//
//        uint8_t result = node.writeMultipleRegisters(regAddr, BLOCK_SIZE);
//
//        if (result != node.ku8MBSuccess) {
...
//        }
//
//        //Serial.printf("Блок дополнительных данных %d/%d передан (регистры %d-%d)\n",
//        //    block + 1, 2, regAddr, regAddr + BLOCK_SIZE - 1);
//        delay(100); // Увеличенная задержка между блоками
//    }
//
//    if (success) 
//    {
//       // Serial.println("Все дополнительные данные переданы успешно");
//    }
//}
//
//void sendThisAircraft()
//{
//   // Serial.println("Передача ThisAircraft блоками по 25 регистров...");
//
//    // ThisAircraft занимает регистры 900-949 (50 регистров)
//    uint16_t dataBuffer[50];
//    ufoToModbusData(&ThisAircraft, dataBuffer);
//
//    //Serial.printf("ThisAircraft - Addr: 0x%08X, Lat: %.6f, Lon: %.6f, Flight: %s, Alt: %.1f\n",
//    //  ThisAircraft.addr,
...
//    for (int block = 0; block < 2; block++) 
//    {
//        uint16_t regAddr = 900 + (block * BLOCK_SIZE);
//
//        node.clearTransmitBuffer();
//        for (int i = 0; i < BLOCK_SIZE; i++) 
//        {
...
//        }
//
//        //Serial.printf("Блок ThisAircraft %d/%d передан (регистры %d-%d)\n",
//        //    block + 1, 2, regAddr, regAddr + BLOCK_SIZE - 1);
//       // delay(100);
//    }
//
...
//{
//  //  Serial.printf("Передача Container[%d] блоками по 25 регистров...\n", objIndex);
//
...
//        }
//
//        //Serial.printf("Блок Container[%d] %d/%d передан (регистры %d-%d)\n",
//        //    objIndex, block + 1, 2, regAddr, regAddr + BLOCK_SIZE - 1);
//        //delay(100);
//    }
//
//    if (success) 
...
//}
//
//// Функция для передачи всех Container объектов сразу (опциональная)
//void sendAllContainerObjects()
//{
//   // Serial.println("Передача всех Container объектов...");
//
//    for (int objIndex = 0; objIndex < MAX_TRACKING_OBJECTS; objIndex++)
//    {
//        sendContainerObject(objIndex);
//        delay(500); // Пауза между объектами
//    }
//
//   // Serial.println("Все Container объекты переданы");
//}
//
//void additionalDataToModbus(additional_data_t* data, uint16_t* buffer)
//{
...
//    buffer[index++] = lon_union.i & 0xFFFF;
//
//   //!! Serial.printf("***ufo->longitude %6f \r\n", ufo->longitude);
//    // old_latitude - 2 регистра
//    union { float f; uint32_t i; } old_lat_union;
//    old_lat_union.f = ufo->old_latitude;
//    buffer[index++] = (old_lat_union.i >> 16) & 0xFFFF;
...
//    AdditionalData.new_SOS_flag = false;
//    AdditionalData.confirm_message = true;
//    strcpy(AdditionalData.msg_resp, "!!!!Test message from source ESP32S3");
//    AdditionalData.isValidGNSS = service.get_isValidGNSS();
//    AdditionalData.FLYRF_MODE_TEST = 5;
//
//    //// Инициализация ThisAircraft
...
//    //ThisAircraft.addr = 0xABCDEF;
//    //ThisAircraft.addr_type = 1;
//    //ThisAircraft.latitude = 55.93574;
//    //ThisAircraft.longitude = 37.34873;
//    //ThisAircraft.old_latitude = 55.7550;
//    //ThisAircraft.old_longitude = 37.6170;
//    //ThisAircraft.altitude = 1500;
//    //ThisAircraft.pressure_altitude = 1450;
...
//    //    Container[i].addr = 0x123456 + i;
//    //    Container[i].addr_type = i % 3;
//    //    Container[i].latitude = 56.93574 + i * 0.008;
//    //    Container[i].longitude = 38.34873 + i * 0.008;
//    //    Container[i].old_latitude = 56.93576 + i * 0.001;
//    //    Container[i].old_longitude = 38.34875 + i * 0.001;
//    //    Container[i].altitude = 1000 + i/* * 100*/;
//    //    Container[i].pressure_altitude = 950 + i /** 100*/;
//    //    Container[i].course = 90 + i * 10;
//    //    Container[i].speed = 250 + i * 10;
...
//    //}
//
//   // Serial.println("Тестовые данные инициализированы");
//}