...
void transmissionTask(void* parameter) {
    ...
    while (true) 
    {
        ...
        // КРИТИЧНО: Регулярный сброс watchdog
        esp_task_wdt_reset();
        txStats.watchdog_feeds++;
...
void batchTransmissionTask(void* parameter) {
    ...
    esp_task_wdt_reset(); // Сброс watchdog перед передачей
    while (true) 
    {
...
bool transmitDataBlocksFast(uint16_t startRegister, uint16_t* dataBuffer, uint16_t dataSize) 
{
    ...
    esp_task_wdt_reset(); // Сброс watchdog перед началом передачи блоков
    for (int block = 0; block < numBlocks; block++) 
    {
        ...
        for (int retry = 0; retry < MAX_RETRIES && !blockSuccess; retry++) 
        {
            ...
            // Сброс watchdog после каждой попытки
            esp_task_wdt_reset();
        }
...
void setup()
{
    ...
    // Настройка watchdog с увеличенным таймаутом
    esp_task_wdt_init(30, true); // 30 секунд таймаут, паника при срабатывании
    esp_task_wdt_add(NULL); // Добавляем главную задачу
 ...
 /* SoC->Button_setup();*/
  ...
  // Увеличена скорость RS485 до 57600
  ...
  // Сброс watchdog после инициализации
  esp_task_wdt_reset();
}
...
void loop()
{
 ...
 // Обновление данных с высокой частотой
  ...
  delay(20); // Высокочастотный цикл - каждые 20мс
     ...
     // КРИТИЧНО: Сброс watchdog перед задержкой
  esp_task_wdt_reset();
  Time_loop();
...
void updateDataHighFreq() 
{
    ...
    if (millis() - lastUpdate > 1000) { // Обновляем каждую секунду
        ...
        if (xSemaphoreTake(dataMutex, pdMS_TO_TICKS(10)) == pdTRUE) 
        {
            ...
            ThisAircraft.latitude += 0.00001;
            ThisAircraft.longitude += 0.00001;
            ThisAircraft.timestamp = millis() / 1000;

            // Быстрое обновление Container объектов
            for (int i = 0; i < MAX_TRACKING_OBJECTS; i++) {
                Container[i].latitude += 0.00001;
                Container[i].longitude += 0.00001;
...
void sendContainerBatchToQueue() 
{
    ...
    {
        // Упаковываем все Container объекты в один пакет
        for (int i = 0; i < MAX_TRACKING_OBJECTS; i++) {
            ufoToModbusData(&Container[i], &task.data_buffer[i * 50]);
        }
        xSemaphoreGive(dataMutex);
