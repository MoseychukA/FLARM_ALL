...
void NMEA_Export()
{
    ...
    bool has_Fix = true;//!!isValidFix() || (settings->mode == FLYRF_MODE_TXRX_TEST1) || (settings->mode == FLYRF_MODE_TXRX_TEST2) || (settings->mode == FLYRF_MODE_TXRX_TEST3) || (settings->mode == FLYRF_MODE_TXRX_TEST4) || (settings->mode == FLYRF_MODE_TXRX_TEST5);
 ...
 /*             if (!Container[i].stealth && !ThisAircraft.stealth) 
              ...
              //data_source = (Container[i].protocol == RF_PROTOCOL_ADSB_1090) ? DATA_SOURCE_ADSB : DATA_SOURCE_FLARM;
