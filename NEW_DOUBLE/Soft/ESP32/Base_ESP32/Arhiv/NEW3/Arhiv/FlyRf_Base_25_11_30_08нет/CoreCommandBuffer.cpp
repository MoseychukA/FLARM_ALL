    ...
    if (pStream->available())                       // Определить наличие символа в порту трекера
    {
        while (pStream->available()>0)              // читаем данные во внутренний буфер
        {
            ...
            *strBuff += ch;
            ...
            if (strBuff->length() >= BUFFER_SIZE+2)   // Если иформации больше чем BUFFER_SIZE - принимать не будем и очистим буфер
            {
...
void CommandHandlerClass::setup()
{
    ...
    SERIAL_TRACKER.begin(SERIAL_TRACKER_SPEED, EspSoftwareSerial::SWSERIAL_8N1, SOC_GPIO_PIN_TRACKER_RX, SOC_GPIO_PIN_TRACKER_TX,false,512);
    SERIAL_TRACKER.enableIntTx(false);
    //SERIAL_TRACKER.onReceive(receiveHandler);
}
				...
				if (textString.startsWith(CORE_COMMAND_SET)|| textString.startsWith(CORE_COMMAND_SET_M))          // Если команда "SET=" продолжить разбор
				{
                    ...
                    else if (!strcmp_P(commandName, SET_GSM)|| !strcmp_P(commandName, SET_GSM_M))              // Установить параметры передачи данных по GSM
                    {
                        if (cParser.argsCount() > 2)
                        {
                            ...
                            //const char* paramPtr = cParser.getArg(3);
                            commandHandled = setTRACKER(commandName, cParser, pStream);
                        }
                        else
                        {
                            // недостаточно параметров
...
bool CommandHandlerClass::setTXT(const char* commandPassed, CommandParser& parser, Stream* pStream, String textString) // Программа приема текстового сообщения
{
    if (commandPassed)
    {
      ...
      //  int8_t NumberMessage = atoi(parser.getArg(0));                       // Получить номер сообщения в виде int                
        ...
        static char msg_tmp[BUFFER_SIZE+30] = "";                            // Массив для приема текстовых сообщений
        static char time_msg_tmp[30] = "";                                   // Массив для приема времени текстовых сообщений
        static char msg_tmp_bt[BUFFER_SIZE+30] = "";                         // Массив для отправки текстовых сообщений по Bluetooth

        static char msgOK_Trecker[20] = "#";                                         // Формирование строки для ответного сообщения 
...
void CommandHandlerClass::SendTraffic_Msg()
{
    ...
    if (millis() - tmr_msg > 2000)
    {
        ...
        for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
        {
            ...
            memcpy((char*)fo.callsign, Container_msg[i].callsign, strlen(Container_msg[i].callsign));
...
bool  CommandHandlerClass::setTRACKER(const char* commandPassed, CommandParser& parser, Stream* pStream)      //
{
    if (parser.argsCount() > 2)
    {
        ...
        {
            settings->tracker_send = TRACKER_SEND_OFF;
            delay(1000);
            EEPROM_store();
            RF_Shutdown();
            delay(1000);
        ...
        {
            settings->tracker_send = TRACKER_SEND_SINGLE;
            delay(1000);
            EEPROM_store();
            RF_Shutdown();
            delay(1000);
        ...
        {
            settings->tracker_send = TRACKER_SEND_AUTO;
            delay(1000);
            EEPROM_store();
            RF_Shutdown();
            delay(1000);
        ...
        {
            settings->tracker_send = TRACKER_SEND_MINI;
            delay(1000);
            EEPROM_store();
            RF_Shutdown();
            delay(1000);
...
void CommandHandlerClass::GPS_send_base()
{
    ...
    if ((settings->tracker_send == TRACKER_SEND_AUTO))
    {
    ...
    else if (settings->tracker_send == TRACKER_SEND_SINGLE)
    {
    ...
    else if (settings->tracker_send == TRACKER_SEND_MINI)
    {
        if (gsm_alien_count == 0) return;
        for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
        {
...
void CommandHandlerClass::send_reply()   // отправить ответ
{
