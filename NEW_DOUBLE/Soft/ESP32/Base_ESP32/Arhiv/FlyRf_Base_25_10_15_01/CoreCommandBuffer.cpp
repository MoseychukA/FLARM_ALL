...
bool  CommandHandlerClass::setGSM(const char* commandPassed, CommandParser& parser, Stream* pStream)      //
{
    if (parser.argsCount() > 2)
    {
        ...
        {
            settings->tracker_send = GSM_SEND_OFF;
            delay(1000);
            EEPROM_store();
            RF_Shutdown();
            delay(1000);
        ...
        {
            settings->tracker_send = GSM_SEND_SINGLE;
            delay(1000);
            EEPROM_store();
            RF_Shutdown();
            delay(1000);
        ...
        {
            settings->tracker_send = GSM_SEND_AUTO;
            delay(1000);
            EEPROM_store();
            RF_Shutdown();
            delay(1000);
        ...
        {
            settings->tracker_send = GSM_SEND_MINI;
            delay(1000);
            EEPROM_store();
            RF_Shutdown();
            delay(1000);
...
void CommandHandlerClass::GPS_send_base()
{
    ...
    if ((settings->tracker_send == GSM_SEND_AUTO))
    {
    ...
    else if (settings->tracker_send == GSM_SEND_SINGLE)
    {
    ...
    else if (settings->tracker_send == GSM_SEND_MINI)
    {
        if (gsm_alien_count == 0) return;
        for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
        {
