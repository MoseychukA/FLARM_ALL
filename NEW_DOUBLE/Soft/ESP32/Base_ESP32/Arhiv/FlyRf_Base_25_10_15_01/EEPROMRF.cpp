...
void EEPROM_defaults()
{
  ...
  eeprom_block.field.settings.tracker_send    = GSM_SEND_OFF;
  eeprom_block.field.settings.ram_view    = VIEW_RAM_OFF;
  eeprom_block.field.settings.akk_view    = VIEW_AKK_ON;
  eeprom_block.field.settings.sos_view    = VIEW_SOS_OFF;
  eeprom_block.field.settings.out_of_sync = OUT_OF_SYNC_OFF;
  eeprom_block.field.settings.akk_min     = 3.2;         // Минимальное значение заряда аккумулятора
  eeprom_block.field.settings.akk_max     = 4.2;         // Максимальное значение заряда аккумулятора
  eeprom_block.field.settings.akk_koef    = 3.30;        // Коэффициет пересчета заряда аккумулятора
  ...
  eeprom_block.field.settings.display_set = DISPLAY_BUILT_IN;
 }
