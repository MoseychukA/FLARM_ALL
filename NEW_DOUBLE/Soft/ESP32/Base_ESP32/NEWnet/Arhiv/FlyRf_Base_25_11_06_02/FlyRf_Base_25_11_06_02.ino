...
// Здесь сразу фиксируем быстрый приход значения new_buttton_M
void rxTask(void* param) 
{
    ...
    for (;;) 
    {
        ...
        if (receivePacket_RS485(&recpkt, &btn1, &btn2)) 
        {
            ...
            if (recpkt.AuxData.message_received)
            {
                ...
                service.setMessageRead(false);       // Сбросить флаг о том что новое сообщение получено в выносном дисплее
