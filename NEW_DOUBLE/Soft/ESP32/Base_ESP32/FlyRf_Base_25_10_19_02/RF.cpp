...
byte RF_setup(void)
{
  ...
  if (rf_chip) 
  {
    ...
    switch (settings->rf_protocol)
    {
      ...
      default:                    p = &ogntp_proto_desc;  break;
    }
...
static void sx12xx_setvars()
{
  ...
  {
    LMIC.datarate = 0;// LMIC.protocol->bitrate;
    LMIC.syncword = LMIC.protocol->syncword[0];
    //Serial.print("====== LMIC.datarate: ");
    //Serial.println(LMIC.datarate);
   // Serial.println(LMIC.protocol->bitrate);
  }
  else 
  {
      LMIC.datarate = DR_FSK; //DR_SF9; DR_SF7;
    //Serial.print("====== LMIC.datarate = DR_FSK: ");
    //Serial.println(LMIC.datarate);
  }
...
static void sx12xx_transmit()
{
   // digitalWrite(SOC_GPIO_PIN_LED, LOW);
    ...
    while (sx12xx_transmit_complete == false) 
    {
      ...
      // выполнять запланированные задания и мероприятия
      os_runstep();
