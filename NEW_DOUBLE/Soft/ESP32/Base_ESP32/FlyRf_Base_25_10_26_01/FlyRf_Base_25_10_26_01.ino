...
void unpack_ToDUMP1090(const ToDUMP1090_RAW* in, ToDUMP1090* out)
{
    ...
    memcpy(out->callsign, in->callsign, 8);
...
void txTask(void* param) 
{
   ...
   // static uint8_t count = 0;
    for (;;) 
    {
       ...
       // memcpy(packet.ThisAircraft.callsign, ThisAircraft.callsign, 8);
        //Serial.printf("buffer: ");
        //for (int i = 0; i < 20; ++i) Serial.printf("%02X ", &packet);
        //Serial.println();
    ...
    while (SerialRP2040.available())
    {
        ...
        if (rxIndex >= PACKET_SIZE)
        {
            if (rxBuffer[PACKET_SIZE - 3] == 0xFF && rxBuffer[PACKET_SIZE - 2] == 0xFF && rxBuffer[PACKET_SIZE - 1] == 0xFF)
            {
                ...
                memcpy((char*)fo.callsign, packet.callsign, strlen(packet.callsign));
...
void setup()
{
    ...
    //----------------------------------------------------------------------------------------------
        ...
        containerMutex = xSemaphoreCreateMutex();
        setupRS485();
        xTaskCreatePinnedToCore(txTask, "TX", 8192, NULL, 2, &Task1, 1);
        xTaskCreatePinnedToCore(rxTask, "RX", 4096, NULL, 1, &Task2, 1);
