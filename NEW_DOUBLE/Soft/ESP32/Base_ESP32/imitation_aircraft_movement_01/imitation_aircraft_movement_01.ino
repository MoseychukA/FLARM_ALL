...
void moveAircraft(double distance)
{
    ...
    double dlon = atan2(sin(bearing) * sin(angular_distance) * cos(lat1),
        cos(angular_distance) - sin(lat1) * sin(lat2));

    double lon2 = fmod(lon1 + dlon + 3 * PI, 2 * PI) - PI; // Нормализация долготы
...
////        cos(distance / EARTH_RADIUS) - sin(lat1) * sin(lat1));
////
////    // Обновление координат
////    ThisAircraft.latitude = lat2 * RAD_TO_DEG;
////    ThisAircraft.longitude = lon2 * RAD_TO_DEG;
...
////
////    // Вычисление новых координат по формулам сферической геометрии
////    double lat2 = asin(sin(lat1) * cos(distance / EARTH_RADIUS) + cos(lat1) * sin(distance / EARTH_RADIUS) * cos(bearing));
////
////    double lon2 = lon1 + atan2(sin(bearing) * sin(distance / EARTH_RADIUS) * cos(lat1),  cos(distance / EARTH_RADIUS) - sin(lat1) * sin(lat2));
////
////    // Обновление координат
////    ThisAircraft.latitude = lat2 * RAD_TO_DEG;
////    ThisAircraft.longitude = lon2 * RAD_TO_DEG;
...
//Исправленные ошибки :
//
//Строка 79 - 80 : Добавлены отсутствующие операторы умножения * в формуле расчета новой широты
//Строка 82 - 83 : Добавлены отсутствующие операторы умножения * в формуле расчета новой долготы
//Строка 125 - 128 : Добавлены отсутствующие операторы умножения * в формуле гаверсинуса
//
//Теперь код должен компилироваться без ошибок.Основные исправления касались математических формул, где были пропущены операторы умножения между переменными и функциями.
//*/
//
//
//
//
//////Программа для имитации движения самолета на ESP32:
////
////#include "Arduino.h"
////#include <WiFi.h>
////#include <math.h>
