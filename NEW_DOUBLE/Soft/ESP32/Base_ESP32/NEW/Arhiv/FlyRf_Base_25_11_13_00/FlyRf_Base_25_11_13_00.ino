    ...
    for (;;)
    {
        if (receivePacket_RS485(&recpkt, &btn1, &btn2))
        {
            ...
            if (recpkt.AuxData.new_buttton_M != 0)
            {
              ...
              //  Serial.printf("Принят ответ: BUTTON1=%u BUTTON2=%u new_buttton_M=%u \r\n",  recpkt.BUTTON1, recpkt.BUTTON2, recpkt.AuxData.new_buttton_M);
        ...
        if (rxIndex >= PACKET_SIZE)
        {
            ...
            if (rxBuffer[PACKET_SIZE - 3] == 0xFF && rxBuffer[PACKET_SIZE - 2] == 0xFF && rxBuffer[PACKET_SIZE - 1] == 0xFF)
            {
                ...
                if (settings->serial_out == SEND_SERIAL_1090 && settings->nmea_out != NMEA_UART)
                {
                    ...
                    Serial.print(":"); Serial.print(packet.callsign);
...
void sendPacketToRP2040(uint16_t* data, uint8_t count)
{
    ...
    SerialRP2040.write(count); // Число значений (max 255)
    for (uint8_t i = 0; i < count; ++i)
    {
        // Отправка значения в big-endian
        uint16_t v = ((data[i] >> 8) & 0xFF) | ((data[i] & 0xFF) << 8);
        SerialRP2040.write((uint8_t*)&v, 2);
       // Serial.write((uint8_t*)&v, 2);
    }
    uint16_t mark = 0xAAAA;  // Маркер конца
    uint16_t bmark = ((mark >> 8) & 0xFF) | ((mark & 0xFF) << 8);
    SerialRP2040.write((uint8_t*)&bmark, 2);
    SerialRP2040.flush();
 /*   Serial.write((uint8_t*)&bmark, 2);
    Serial.flush();*/
}

// например, отправить 3 числа
...
{
    Serial.printf("%06X:%d:%8s:%.0f:%.0f:%.0f:%.0f:%d:%.6f:%.6f:%d:%d\r\n",
        ac->addr,                   // Адрес устройства стороннего самолета
        ac->squawk,                 // Номер, назначаемый диспетчером для обмена с локатором. 
        ac->callsign,               // Номер рейса
        ac->altitude,
...
void setup()
{
    ...
    memcpy(ThisAircraft.callsign, &ThisAircraft_callsign, 8);

    //sprintf(ThisAircraft.callsign, "%08X", ThisAircraft.addr);
