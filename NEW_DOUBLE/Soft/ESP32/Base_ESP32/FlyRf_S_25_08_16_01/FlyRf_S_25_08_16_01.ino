...
//} ufo_t;
//
//// Тестовые данные (вместо внешних переменных)
...
bool new_flag_M = true;
uint8_t new_button_M = 5;
bool setMessageRead_M = false;
bool MessageRead_M = true;
bool SOS_Sprite_on_off_M = false;
bool SOS_View_on_off_M = true;
bool new_SOS_flag_M = false;
bool confirm_message_M = true;
char msg_resp_M[60] = "Test message from source";
bool isValidGNSS_M = true;
uint8_t FLYRF_MODE_TEST_M = 3;
uint16_t analog_signal_M = 1234;

ModbusMaster node;

// Структура для управления задачами
typedef struct {
    SemaphoreHandle_t rs485_mutex;
    TaskHandle_t tx_task_handle;
    bool transmission_active;
    uint32_t last_cycle_start;
    uint32_t transmission_stats[5];
    uint32_t successful_transmissions;
    uint32_t failed_transmissions;
} modbus_control_t;

modbus_control_t modbus_ctrl;
...
void preTransmission() {
    ...
    delayMicroseconds(100); // Увеличена задержка для стабильности
}
...
void floatToRegisters(float value, uint16_t* reg1, uint16_t* reg2) {
    ...
    converter.f = value;
    *reg1 = (uint16_t)(converter.i >> 16);
    *reg2 = (uint16_t)(converter.i & 0xFFFF);
}

void timeToRegisters(time_t value, uint16_t* reg1, uint16_t* reg2) {
    *reg1 = (uint16_t)(value >> 16);
    *reg2 = (uint16_t)(value & 0xFFFF);
}

// Функция записи одного регистра с проверкой результата
bool writeSingleRegisterSafe(uint16_t address, uint16_t value) {
    uint8_t result = node.writeSingleRegister(address, value);
    if (result == node.ku8MBSuccess) {
        return true;
    } else {
        Serial.printf("Write failed at 0x%04X, error: %d\n", address, result);
        return false;
    }
}

// Упаковка одного объекта UFO в буфер
uint16_t packUFOToBuffer(const ufo_t* obj, uint16_t* buffer) {
    uint16_t pos = 0;
    
    // timestamp (2 регистра)
    timeToRegisters(obj->timestamp, &buffer[pos], &buffer[pos+1]);
    pos += 2;
    
    // addr (3 регистра)
    buffer[pos++] = (uint16_t)(obj->addr >> 16);
    buffer[pos++] = (uint16_t)(obj->addr & 0xFFFF);
    buffer[pos++] = obj->addr_type;
    
    // координаты (8 регистров)
    floatToRegisters(obj->latitude, &buffer[pos], &buffer[pos+1]); pos += 2;
    floatToRegisters(obj->longitude, &buffer[pos], &buffer[pos+1]); pos += 2;
    floatToRegisters(obj->old_latitude, &buffer[pos], &buffer[pos+1]); pos += 2;
    floatToRegisters(obj->old_longitude, &buffer[pos], &buffer[pos+1]); pos += 2;
    
    // высоты и параметры полета (10 регистров)
    floatToRegisters(obj->altitude, &buffer[pos], &buffer[pos+1]); pos += 2;
    floatToRegisters(obj->pressure_altitude, &buffer[pos], &buffer[pos+1]); pos += 2;
    floatToRegisters(obj->course, &buffer[pos], &buffer[pos+1]); pos += 2;
    floatToRegisters(obj->speed, &buffer[pos], &buffer[pos+1]); pos += 2;
    floatToRegisters(obj->vs, &buffer[pos], &buffer[pos+1]); pos += 2;
    
    buffer[pos++] = obj->aircraft_type;
    
    // flight (8 регистров)
    for(int i = 0; i < 8; i++) {
        uint16_t combined = 0;
        if (i*2 < 16) combined |= (obj->flight[i*2] << 8);
        if (i*2+1 < 16) combined |= obj->flight[i*2+1];
        buffer[pos++] = combined;
    }
    
    // vert_rate и Squawk (4 регистра)
    buffer[pos++] = (uint16_t)(obj->vert_rate >> 16);
    buffer[pos++] = (uint16_t)(obj->vert_rate & 0xFFFF);
    buffer[pos++] = (uint16_t)(obj->Squawk >> 16);
    buffer[pos++] = (uint16_t)(obj->Squawk & 0xFFFF);
    
    // timemsg (2 регистра)
    timeToRegisters(obj->timemsg, &buffer[pos], &buffer[pos+1]);
    pos += 2;
    
    // geoid_separation, distance, bearing (6 регистров)
    floatToRegisters(obj->geoid_separation, &buffer[pos], &buffer[pos+1]); pos += 2;
    floatToRegisters(obj->distance, &buffer[pos], &buffer[pos+1]); pos += 2;
    floatToRegisters(obj->bearing, &buffer[pos], &buffer[pos+1]); pos += 2;
    
    // остальные параметры
    buffer[pos++] = obj->hdop;
    buffer[pos++] = (uint16_t)obj->rssi;
    buffer[pos++] = obj->signal_source;
    
    // seen (2 регистра)
    timeToRegisters(obj->seen, &buffer[pos], &buffer[pos+1]);
    pos += 2;
    
    buffer[pos++] = obj->hour_msg;
    buffer[pos++] = obj->min_msg;
    buffer[pos++] = obj->delay_time_msg;
    
    // callsign (4 регистра)
    for(int i = 0; i < 4; i++) {
        uint16_t combined = 0;
        if (i*2 < 8) combined |= (obj->callsign[i*2] << 8);
        if (i*2+1 < 8) combined |= obj->callsign[i*2+1];
        buffer[pos++] = combined;
    }
...
// Отправка заголовка команды
bool sendCommandHeader(uint8_t command, uint8_t object_index, uint16_t data_length) {
    bool success = true;
    success &= writeSingleRegisterSafe(0x1000, command);
    success &= writeSingleRegisterSafe(0x1001, object_index);
    success &= writeSingleRegisterSafe(0x1002, data_length);
    
    if (success) {
        Serial.printf("Header sent: cmd=%d, idx=%d, len=%d\n", command, object_index, data_length);
    }
...
bool sendDataBuffer(uint16_t start_address, uint16_t* buffer, uint16_t length) {
    Serial.printf("Sending %d registers starting at 0x%04X\n", length, start_address);
    
    for (uint16_t i = 0; i < length; i++) {
        if (!writeSingleRegisterSafe(start_address + i, buffer[i])) {
            Serial.printf("Failed at register %d (0x%04X)\n", i, start_address + i);
            return false;
        }
        
        // Небольшая задержка между регистрами для стабильности
        if (i % 10 == 9) {
            delay(5);
        }
    }
    
    Serial.printf("Successfully sent %d registers\n", length);
    return true;
}

// Быстрая передача Container объекта
bool sendContainerObjectFast(uint8_t index) {
    if (index >= MAX_TRACKING_OBJECTS) return false;
    
    uint16_t buffer[80];
    uint16_t data_size = packUFOToBuffer(&Container[index], buffer);
    
    // Отправляем заголовок
    if (!sendCommandHeader(CMD_SEND_CONTAINER, index, data_size)) {
        return false;
    }
    ...
    // Отправляем данные
    bool result = sendDataBuffer(0x2000 + (index * 100), buffer, data_size);
    
    if (result) {
        Serial.printf("Container[%d] sent successfully (%d registers)\n", index, data_size);
    }
    
    return result;
}

// Быстрая передача ThisAircraft
bool sendThisAircraftFast() {
    uint16_t buffer[80];
    uint16_t data_size = packUFOToBuffer(&ThisAircraft, buffer);
    
    if (!sendCommandHeader(CMD_SEND_THISAIRCRAFT, 0, data_size)) {
        return false;
    }
    
    delay(INTER_COMMAND_DELAY);
    
    bool result = sendDataBuffer(0x3000, buffer, data_size);
    
    if (result) {
        Serial.printf("ThisAircraft sent successfully (%d registers)\n", data_size);
    }
    
    return result;
}

// Быстрая передача дополнительных данных
bool sendAdditionalDataFast() 
{
    uint16_t buffer[50];
    uint16_t pos = 0;
    
    // Дополнительные данные
    buffer[pos++] = new_flag_M ? 1 : 0;
    buffer[pos++] = new_button_M;
    buffer[pos++] = setMessageRead_M ? 1 : 0;
    buffer[pos++] = MessageRead_M ? 1 : 0;
    buffer[pos++] = SOS_Sprite_on_off_M ? 1 : 0;
    buffer[pos++] = SOS_View_on_off_M ? 1 : 0;
    buffer[pos++] = new_SOS_flag_M ? 1 : 0;
    buffer[pos++] = confirm_message_M ? 1 : 0;
    buffer[pos++] = isValidGNSS_M ? 1 : 0;
    buffer[pos++] = FLYRF_MODE_TEST_M;
    
    // msg_resp (60 символов = 30 регистров)
    for(int i = 0; i < 30; i++) 
    {
        uint16_t combined = 0;
        if (i*2 < 60) combined |= (msg_resp_M[i*2] << 8);
        if (i*2+1 < 60) combined |= msg_resp_M[i*2+1];
        buffer[pos++] = combined;
    }
    
    if (!sendCommandHeader(CMD_SEND_ADDITIONAL, 0, pos)) {
        return false;
    }
    
    delay(INTER_COMMAND_DELAY);
    
    bool result = sendDataBuffer(0x4000, buffer, pos);
    
    if (result) {
        Serial.printf("Additional data sent successfully (%d registers)\n", pos);
    }
    
    return result;
}

// Быстрая передача аналогового сигнала
bool sendAnalogSignalFast() {
    if (!sendCommandHeader(CMD_SEND_ANALOG, 0, 1)) {
        return false;
    }
    
    delay(INTER_COMMAND_DELAY);
    
    bool result = writeSingleRegisterSafe(0x5000, analog_signal_M);
    
    if (result) {
        Serial.printf("Analog signal sent: %d\n", analog_signal_M);
    }
...
void initTestData() {
    ...
    for (int i = 0; i < MAX_TRACKING_OBJECTS; i++) {
        ...
        Container[i].addr = 0x100000 + i;
        Container[i].addr_type = 1;
        Container[i].latitude = 55.7558 + i * 0.001;
        Container[i].longitude = 37.6176 + i * 0.001;
        Container[i].old_latitude = Container[i].latitude - 0.0001;
        Container[i].old_longitude = Container[i].longitude - 0.0001;
        Container[i].altitude = 1000 + i * 100;
        Container[i].pressure_altitude = Container[i].altitude + 50;
        Container[i].course = i * 45;
        Container[i].speed = 100 + i * 10;
        Container[i].aircraft_type = 1;
        snprintf(Container[i].flight, 16, "TEST%03d", i);
        Container[i].vert_rate = i * 100 - 400;
        Container[i].Squawk = 1200 + i;
        Container[i].timemsg = Container[i].timestamp;
        Container[i].vs = Container[i].vert_rate;
        Container[i].geoid_separation = 45.5;
        Container[i].hdop = 100 + i * 10;
        Container[i].rssi = -50 - i * 5;
        Container[i].distance = 1000 + i * 500;
        Container[i].bearing = i * 30;
        Container[i].signal_source = 1;
        Container[i].seen = Container[i].timestamp;
        Container[i].hour_msg = 12;
        Container[i].min_msg = 30 + i;
        Container[i].delay_time_msg = i * 100;
        snprintf((char*)Container[i].callsign, 8, "CS%02d", i);
    }
    
    // Инициализация ThisAircraft
    ThisAircraft.timestamp = millis() / 1000;
    ThisAircraft.addr = 0x200000;
    ThisAircraft.addr_type = 2;
    ThisAircraft.latitude = 55.7558;
    ThisAircraft.longitude = 37.6176;
    ThisAircraft.old_latitude = 55.7557;
    ThisAircraft.old_longitude = 37.6175;
    ThisAircraft.altitude = 1500;
    ThisAircraft.pressure_altitude = 1550;
    ThisAircraft.course = 90;
    ThisAircraft.speed = 250;
    ThisAircraft.aircraft_type = 2;
    strcpy(ThisAircraft.flight, "OWN001");
    ThisAircraft.vert_rate = 0;
    ThisAircraft.Squawk = 7000;
    ThisAircraft.timemsg = ThisAircraft.timestamp;
    ThisAircraft.vs = 0;
    ThisAircraft.geoid_separation = 45.5;
    ThisAircraft.hdop = 95;
    ThisAircraft.rssi = -45;
    ThisAircraft.distance = 0;
    ThisAircraft.bearing = 0;
    ThisAircraft.signal_source = 0;
    ThisAircraft.seen = ThisAircraft.timestamp;
    ThisAircraft.hour_msg = 12;
    ThisAircraft.min_msg = 30;
    ThisAircraft.delay_time_msg = 0;
    strcpy((char*)ThisAircraft.callsign, "OWNSHIP");
    
    Serial.println("Test data initialized");
}

// Основная задача передачи данных (выполняется на ядре 0)
void modbusTransmissionTask(void* parameters) {
    while (true) {
        uint32_t cycle_start = millis();
        
        if (xSemaphoreTake(modbus_ctrl.rs485_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
            modbus_ctrl.transmission_active = true;
            ...
            // Передача всех Container объектов
            for (int i = 0; i < MAX_TRACKING_OBJECTS; i++) {
                if (sendContainerObjectFast(i)) {
                    successful_commands++;
                } else {
                    all_success = false;
                }
            ...
            if (sendAnalogSignalFast()) {
                successful_commands++;
            } else {
                all_success = false;
            }
            
            uint32_t transmission_time = millis() - start_time;
            
            // Обновление статистики
            if (all_success) {
                modbus_ctrl.successful_transmissions++;
                Serial.printf("=== Cycle completed successfully in %d ms (%d commands) ===\n", 
                             transmission_time, successful_commands);
            } else {
                modbus_ctrl.failed_transmissions++;
                Serial.printf("=== Cycle completed with errors in %d ms (%d/%d commands) ===\n", 
                             transmission_time, successful_commands, MAX_TRACKING_OBJECTS + 3);
            }
            
            // Сохранение статистики
            static uint8_t stat_index = 0;
            modbus_ctrl.transmission_stats[stat_index] = transmission_time;
            stat_index = (stat_index + 1) % 5;
            
            modbus_ctrl.transmission_active = false;
            xSemaphoreGive(modbus_ctrl.rs485_mutex);
        }
        
        // Обновление тестовых данных для следующего цикла
        analog_signal_M++;
        ThisAircraft.altitude += 10;
        for (int i = 0; i < MAX_TRACKING_OBJECTS; i++) {
            Container[i].latitude += 0.0001;
            Container[i].longitude += 0.0001;
        }
        
        // Ожидание до следующего цикла
        uint32_t cycle_time = millis() - cycle_start;
        if (cycle_time < TRANSMISSION_INTERVAL) {
            vTaskDelay(pdMS_TO_TICKS(TRANSMISSION_INTERVAL - cycle_time));
        }
        
        modbus_ctrl.last_cycle_start = cycle_start;
    }
}

// Инициализация MODBUS источника
void initModbusSource() {
    ...
    RS485Serial.setTxBufferSize(2048); // Увеличен буфер передачи
    
    node.begin(SLAVE_ID, RS485Serial);
    node.preTransmission(preTransmission);
    node.postTransmission(postTransmission);
    
    modbus_ctrl.rs485_mutex = xSemaphoreCreateMutex();
    modbus_ctrl.transmission_active = false;
    modbus_ctrl.last_cycle_start = 0;
    modbus_ctrl.successful_transmissions = 0;
    modbus_ctrl.failed_transmissions = 0;
    memset(modbus_ctrl.transmission_stats, 0, sizeof(modbus_ctrl.transmission_stats));
    
    xTaskCreatePinnedToCore(
        modbusTransmissionTask,
        "MODBUS_TX",
        8192,
        NULL,
        10, // Высокий приоритет
        &modbus_ctrl.tx_task_handle,
        0  // Ядро 0
    );
    
    Serial.printf("MODBUS Source initialized on Core 0\n");
    Serial.printf("Baud rate: %d, cycle time: %d ms\n", BAUD_RATE, TRANSMISSION_INTERVAL);
    Serial.printf("Slave ID: %d, Max block size: %d registers\n", SLAVE_ID, MAX_MODBUS_BLOCK);
}

// API функции для статистики
bool isModbusTransmissionActive() {
    return modbus_ctrl.transmission_active;
}

uint32_t getLastCycleTime() {
    return millis() - modbus_ctrl.last_cycle_start;
}

uint32_t getAverageTransmissionTime() {
    uint32_t total = 0;
    for (int i = 0; i < 5; i++) {
        total += modbus_ctrl.transmission_stats[i];
    }
    return total / 5;
}

void printModbusStats() {
    Serial.printf("MODBUS Source Stats:\n");
    Serial.printf("  Active: %s\n", isModbusTransmissionActive() ? "Yes" : "No");
    Serial.printf("  Last cycle: %d ms ago\n", getLastCycleTime());
    Serial.printf("  Avg time: %d ms\n", getAverageTransmissionTime());
    Serial.printf("  Successful: %d, Failed: %d\n", 
                  modbus_ctrl.successful_transmissions, 
                  modbus_ctrl.failed_transmissions);
    Serial.printf("  Success rate: %.1f%%\n", 
                  100.0 * modbus_ctrl.successful_transmissions / 
                  (modbus_ctrl.successful_transmissions + modbus_ctrl.failed_transmissions + 1));
}
...
void setup()
{
 ...
 /* SoC->Button_setup();*/
  ...
  //----------------------------------------------------------------------------------------------
    ...
    Serial.println("MODBUS Master ready - Full data exchange every 0.5 seconds");
    Serial.println("Sending test data with detailed logging");
