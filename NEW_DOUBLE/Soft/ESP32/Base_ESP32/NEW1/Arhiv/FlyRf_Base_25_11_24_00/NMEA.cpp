        ...
        if (Container[i].addr && (this_moment - Container[i].timestamp) <= EXPORT_EXPIRATION_TIME) 
        {
          ...
          if (settings->nmea_l) 
          {
            ...
            if (distance < ALARM_ZONE_NONE) 
            {
              ...
              /*
               * When flight is available - send it to a NMEA client.
               * If it is not - generate a flight substitute,
               * based upon a protocol ID and the ICAO address
               */
              memset((void *) NMEA_Callsign, 0, sizeof(NMEA_Callsign));

              if (strnlen((char *) Container[i].flight, sizeof(Container[i].flight)) > 0) 
              {
                memcpy(NMEA_Callsign, Container[i].flight, sizeof(Container[i].flight));
