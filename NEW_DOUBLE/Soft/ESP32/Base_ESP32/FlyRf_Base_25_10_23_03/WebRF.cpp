...
void hardwareSettings() //Ручной ввод аппаратных настроек
{
    ...
    (settings->display_settings == DISPLAY_OFF ? "selected" : ""), DISPLAY_OFF,
    (settings->display_settings == DISPLAY_BUILT_IN ? "selected" : ""), DISPLAY_BUILT_IN,
    (settings->display_settings == DISPLAY_EXTERNAL ? "selected" : ""), DISPLAY_EXTERNAL,
...
void hardwareInput() // Отображение аппаратных настроек
{
    ...
    for (uint8_t i = 0; i < server.args(); i++)
    {
        ...
        } else if (server.argName(i).equals("display_settings")) {
            settings->display_settings = server.arg(i).toInt();
