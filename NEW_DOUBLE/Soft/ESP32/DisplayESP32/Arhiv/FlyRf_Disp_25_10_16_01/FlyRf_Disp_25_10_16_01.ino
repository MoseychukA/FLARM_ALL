        ...
        switch (hdr.type) 
        {
            case PKT_CONTAINER: 
            {
                ...
                if (hdr.index < MAX_TRACKING_OBJECTS && hdr.length == sizeof(ufo_t)) 
                {
                    ...
                    Serial.printf("Container[%u]:$%06X:%d:%8s:%.1f:%d:%d:%d:%.6f:%.6f:%d:%d\r\n",
                        hdr.index,
                        Container[hdr.index].addr,                   // Адрес устройства стороннего самолета
                        Container[hdr.index].squawk,                 // Номер, назначаемый диспетчером для обмена с локатором.
                        Container[hdr.index].flight,                 // Номер рейса
                        Container[hdr.index].altitude,
                        Container[hdr.index].speed,
                        Container[hdr.index].course,
                        Container[hdr.index].vert_rate,
                        Container[hdr.index].latitude,
                        Container[hdr.index].longitude,
                        Container[hdr.index].last_message_signal_strength_dbm,
                        Container[hdr.index].last_message_signal_quality_db
                    );
