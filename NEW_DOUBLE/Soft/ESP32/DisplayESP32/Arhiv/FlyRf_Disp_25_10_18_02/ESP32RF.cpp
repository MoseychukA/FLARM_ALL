...
#define INFO_HEIGHT 32 // Высота одного спрайта, например: 480/8 = 60
...
float battery_read()
{
    ...
    voltage = (float)service.get_analog_value();
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
static void  ESP32_syncContainerDisplay()
{
    int shown = 0;
    for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
    {
        ...
        if (Container[i].addr)
        {
            Serial.println(Container[i].addr);
            //if (!airplane_info[i])
            //{
            ...
            //}
            // 
            // rows_Message.setTextSize(1);
            //// Всегда используем ТЕКУЩИЕ поля структуры!
            //airplane_info[i]->fillSprite(TFT_BLACK);
            ...
            //airplane_info[i]->drawString(String("ICAO: ") + Container[i].addr, 4, 22, 2);
            //// airplane_info[i]->drawString(String("Altitude: ") + String(Container[i].altitude), 4, 22, 2);
            // // ... и другие поля

            //airplane_info[i]->pushSprite(0, shown * INFO_HEIGHT);
            shown++;
        }
        else
        {
    ...
    for (int k = shown; k < MAX_TRACKING_OBJECTS; k++)
    {
        //tft.fillRect(0, k * INFO_HEIGHT, WIDTH, INFO_HEIGHT, TFT_BLACK);
    }
}
...
static void ESP32_Display_loop()
{
    ...
    { /* TFT_display_frontpage  Основная программа отображения воздушной обстановки*/ 
        ...
        if (millis() - tmr > DATA_MEASURE_THRESHOLD)
        {
            ...
            //Serial.print("**(bool)AdditionalData.isValidGNSS ");
            //Serial.println((bool)AdditionalData.isValidGNSS);
            //if ((bool)AdditionalData.isValidGNSS == true)
            //{
            //    tmr1_GNSS = millis();
            //    text_call = false;
            //    fix_tmp = true;
           ...
           // backsprite.setPivot(160, 160);                // Назначаем центр вращения спрайта воздушной обстановки
            ...
            if (settings->akk_view == VIEW_AKK_ON)
            {
                uint16_t akk = service.get_analog_value();//battery_read();
