...
//            airplane_info[i]->drawString(String("callsign: ") + Container[i].callsign, 4, 2, 2);
...
static void* ESP32_getResetInfoPtr()
{
  switch (rtc_get_reset_reason(0))
  {
    ...
    default                     : reset_info.reason = REASON_DEFAULT_RST;; break;
...
static byte ESP32_Display_setup()
{
  ...
  if (settings->power_view == VIEW_AKK_ON)
  {
    ...
    back.createSprite(WIDTH, HEIGHT);
    ...
    backsprite.createSprite(WIDTH, HEIGHT);
    ...
    { 
        ...
        if (millis() - tmr > DATA_MEASURE_THRESHOLD) 
        {
            ...
            if (view_alien_count >= 1)
            {
                ...
                //    ThisAircraft.addr,                   // Адрес устройства стороннего самолета
                //    ThisAircraft.squawk,                 // Номер, назначаемый диспетчером для обмена с локатором. 
                //    ThisAircraft.callsign,               // Номер рейса
                ...
                //    ThisAircraft.rssi,
                //    ThisAircraft.last_message_signal_strength_dbm,
                //    ThisAircraft.last_message_signal_quality_db);
