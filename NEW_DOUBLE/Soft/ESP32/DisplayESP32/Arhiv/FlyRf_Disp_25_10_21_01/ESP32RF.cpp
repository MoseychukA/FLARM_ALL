    ...
    { 
        ...
        if (millis() - tmr > DATA_MEASURE_THRESHOLD) 
        {
            ...
            if (settings->power_view == VIEW_AKK_ON)
            {
                ...
                voltage_percent = round((voltage - minV) * 100.0 / (maxV - minV));
           
                int val_x = map(voltage_percent, 0, 100, 0, 26);
               
                char str_Vcc[4];
                String str_pr = "%";
                dtostrf(voltage_percent, 3, 0, str_Vcc);

                power1.setTextDatum(CL_DATUM);
                power1.setFreeFont(&FreeSerif9pt7b);
                int power_x = 398;
                /*Рисуем заряд аккумулятора*/
                if (voltage_percent > 40.0)
                {
                ...
                else if (voltage_percent <= 40.0 && voltage_percent >= 15.1)
                {
                    power1.fillSprite(TFT_BLACK);
                    power1.fillRect(2, 2, 26, 12, TFT_BLACK);
            ...
            if (settings->voltage_view == VOLTAGE_VIEW_ON)
            {
                ...
                char str_Vcc[4];
                String str_V = "V";
                dtostrf(current, 3, 0, str_Vcc);
                char str_current[4];
                String str_Cur = "mA";


                voltage1.createSprite(74, 20);
                voltage1.fillSprite(TFT_BLACK);
                voltage1.setTextColor(TFT_WHITE, TFT_BLACK);
                voltage1.drawString(str_Vcc + str_V, 33, 23);
                voltage1.pushToSprite(&back, power_x, 4, TFT_BLACK);
               
                current1.createSprite(74, 20);
                current1.fillSprite(TFT_BLACK);
                current1.setTextColor(TFT_WHITE, TFT_BLACK);
                current1.drawString(current + str_Cur, 33, 39);
                current1.pushToSprite(&back, power_x, 4, TFT_BLACK);
            }
            esp_task_wdt_reset();
