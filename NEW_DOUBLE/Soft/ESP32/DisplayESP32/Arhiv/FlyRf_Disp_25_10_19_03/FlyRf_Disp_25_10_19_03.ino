...
void RxTask(void* arg) 
{
    ...
    uint16_t rxCrc;
   // delay(1000);
    trackerPowerOn(5);                         // Включение питания трекера
    for (;;) 
    {
...
//uint32_t powerOffTimer = 0;                 // Таймер отключения питания прибора
bool power_ON = false;
//powerOffTimer = millis();                   // Таймер отключения питания прибора

void modulePowerOn()                          // Включение питания молуля
{
    int i = 0;
    int time_i = 25;                          // время нажатия на кнопку включения питания.
    do
    {
        delay(100);
...
void trackerPowerOn(int time_OnOff)                      // Включение питания трекера
{
    int i = 0;
    int time_i = time_OnOff * 10;                        // время формирования импульса включения или отключения трекера
    digitalWrite(POWER_TRACKER, HIGH);
    ...
    digitalWrite(POWER_TRACKER, LOW);
