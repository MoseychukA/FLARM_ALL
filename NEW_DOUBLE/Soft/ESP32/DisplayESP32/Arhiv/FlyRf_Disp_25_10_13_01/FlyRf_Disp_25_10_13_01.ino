        ...
        switch (hdr.type) 
        {
            case PKT_CONTAINER: 
            {
                if (hdr.index < MAX_TRACKING_OBJECTS && hdr.length == sizeof(ufo_t)) 
                {
                    ...
                    Serial.printf("[RX] Container[%u]: % 06X flight=%s\r\n",
                        hdr.index,
                        Container[hdr.index].addr,
...
void set_packet()
{
   ...
   // service.setNewMessageFlag(AuxFlags.new_flag_M);
   // service.setMessageRead(AuxFlags.setMessageRead_M);
