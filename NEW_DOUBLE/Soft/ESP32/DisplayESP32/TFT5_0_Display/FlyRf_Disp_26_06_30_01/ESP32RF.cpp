...
static void ESP32_checkTargetModule()
{
  uint32_t flash_size = spi_flash_get_chip_size();
  size_t psram_size = ESP.getPsramSize();

  Serial.print(F("[HW] Target module: "));
  Serial.println(F(PROJECT_ESP32_MODULE_NAME));
  Serial.print(F("[HW] Flash bytes: "));
  Serial.println(flash_size);
  Serial.print(F("[HW] PSRAM bytes: "));
  Serial.println(psram_size);

  if (flash_size < PROJECT_FLASH_SIZE_BYTES) {
    Serial.println(F("[HW] WARNING: expected 16 MB Flash for ESP32-S3-WROOM-1-N16R8."));
  }
  if (psram_size < PROJECT_PSRAM_SIZE_BYTES) {
    Serial.println(F("[HW] WARNING: expected 8 MB PSRAM for ESP32-S3-WROOM-1-N16R8."));
  }
}
...
static byte ESP32_Display_setup()
{
    ...
    if (GL050001C0_40_setup()) {
        GL050001C0_40_showStartup(service.getVer());
    }
    return rval;
#endif
...
{
#if PROJECT_USE_GL050001C0_40
    GL050001C0_40_showStatus(rx_packets_counter, tx_packets_counter);
    GL050001C0_40_loop();
    return;
#endif
    ...
    { 
        ...
        if (millis() - tmr > DATA_MEASURE_THRESHOLD) 
        {
            ...
            if (view_alien_count >= 1)          // Сторонние самолеты есть в базе
            {
                esp_task_wdt_reset();
              ...
              //=====================================================================================
                ...
                for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
                {
                    if (Container[i].latitude != 0 && Container[i].longitude != 0 && Container[i].distance < min_distance)
                    {
                        min_distance = Container[i].distance;
                ...
                for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
                {
                    if (Container[i].addr)  // Если есть данные стороннего самолета
                    {
                        ...
                        */
                        ...
                        if (Container[i].addr && Container[i].latitude != 0.0 && Container[i].longitude != 0.0)
                        {
                           // esp_task_wdt_reset();
                            // --------------------------------------------------------------------------------
