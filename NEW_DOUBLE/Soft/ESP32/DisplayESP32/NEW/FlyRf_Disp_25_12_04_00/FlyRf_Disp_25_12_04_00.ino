...
//===============================================================================================
// Пин,по которому снаружи подаётся низкий уровень (триггер)
const int TRIGGER_PIN = 18; // GPIO18 (указать ваш реальный пин)
...
void loop()
{
    ...
    if (!wasDisabled && digitalRead(TRIGGER_PIN) == LOW) 
    {
        ...
        if (digitalRead(TRIGGER_PIN) == LOW) 
        {
            ...
            rtc_gpio_deinit(PULSE_PIN); // на всякий случай
            rtc_gpio_init(PULSE_PIN);
            rtc_gpio_set_direction(PULSE_PIN, RTC_GPIO_MODE_OUTPUT_ONLY);

            // Устанавливаем HIGH
            rtc_gpio_set_level(PULSE_PIN, 1);

            // Включаем удержание уровня в deep sleep
            rtc_gpio_hold_en(PULSE_PIN);
            // Спим 6 секунд
            esp_sleep_enable_timer_wakeup(6ULL * 1000000ULL);
            esp_light_sleep_start();
            rtc_gpio_set_level(PULSE_PIN, 0);
            delay(DELAY_AFTER_LOW);
            esp_deep_sleep_start(); // отсюда программа уже не вернётся
        }
    }
