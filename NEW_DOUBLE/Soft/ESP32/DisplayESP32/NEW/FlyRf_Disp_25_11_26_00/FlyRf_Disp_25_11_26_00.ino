...
const unsigned long MIN_ON_TIME_MS = 30000; //30 сек задержка выключения

bool deviceOn = false;
bool btnPrevState = HIGH;
unsigned long btnPressStart = 0;
unsigned long deviceOnTime = 0;      // метка времени включения
bool holdProcessed = false;
bool allowPowerOff = false;          // разрешение выключения
...
void power_Off() 
{
    ...
    if (deviceOn && !allowPowerOff) 
    {
        // Блокируем выключение в течение первой минуты
        if ((now - deviceOnTime >= MIN_ON_TIME_MS) && btnPrevState == HIGH) 
        {
            allowPowerOff = true;
        }
    }
    ...
    case Idle:
        ...
        if (btnState == LOW) 
        {
            if (!holdProcessed && deviceOn && allowPowerOff && (now - btnPressStart >= HOLD_TIME_MS)) 
            {
                holdProcessed = true;
                 SoC->View_powerOff(); // вызов функции-индикатора
...
void setup()
{
    ...
    digitalWrite(SOC_GPIO_PIN_TFT_LED, HIGH);
     ...
     // Проверим, зажата ли кнопка при старте (3 сек)
     if (digitalRead(PIN_BTN) == LOW) 
     {
         ...
         while (digitalRead(PIN_BTN) == LOW)
         {
             if (millis() - tStart >= HOLD_TIME_MS) 
             {
                 ...
                 deviceOn = true;
                 deviceOnTime = millis(); // записываем время включения
                 allowPowerOff = false; // Сброс блокировки выключения
                 digitalWrite(PIN_PULSE, HIGH);
                 delay(PULSE_ON_MS);
                 digitalWrite(PIN_PULSE, LOW);
                 break;
