...
void clearPlaneDisplay()
{
    ...
    //// 3. Очистить область вывода полосами (используем ту же сетку и смещения)
    //for (int l = 0; l < MAX_TRACKING_OBJECTS; l++)
    //{
...
static void clearMSG()
{
    ...
    rows_Message.pushToSprite(&back, 0, 100, TFT_BLACK);
    back.pushSprite(0, 0);
    vTaskDelay(3000);
    rows_Message.deleteSprite();
    // back.pushSprite(0, 0);
            ...
            case 1:
                ...
                if (setMessageRead)
                {
                    ...
                    if (confirm_message)                                                              // Получен специальный код признака, означает что подтверждение не отправлено. Нужно отправить
                    {
                        ...
                        service.setViewTime(false);                                                   // Отключить отсчет времени удаления сообщения через 10 минут
                        service.setAllow_flashing(false);                                             // Запретить мигание сообщения
                       // service.set_confirm_message(false);                                           // 
