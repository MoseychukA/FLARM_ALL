...
//#define AA_FONT_TIME zTimesNRItalic36
...
TFT_eSprite location_Message = TFT_eSprite(&tft);   // Спрайт отображения текста о состоянии GPS
    ...
    for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
    {
        if (Container[i].addr != 0)
        {
            ...
            if (need_draw)
            {
                ...
                airplane_alient_info[i]->drawString(addrStr, 5, 2, 2);
...
static void waiting_txt() // Вывод текста ожидания определения координат
{
   // clearPlaneDisplay();
    ...
    location_Message.loadFont(AA_FONT_TIME28I);     // Must load the font first
...
static void ESP32_Display_loop()
{
    ...
    { 
        ...
        if (millis() - tmr > DATA_MEASURE_THRESHOLD) 
        {
            ...
            if (!base_con_)
            {
                base_connection.createSprite(300, 26);
                base_connection.fillSprite(TFT_BLACK);
                base_connection.setTextDatum(CC_DATUM);
                base_connection.loadFont(AA_FONT_TIME24);     //  
                base_connection.setTextColor(TFT_RED, backColor);
                base_connection.drawString("НЕТ СВЯЗИ С БАЗОЙ",150, 14);
                base_connection.pushToSprite(&back, 90, 3);        // 
            }
            else if (base_con_)
            {
            ...
            {
                service.setViewTime(true);                                                 // Начинаем отсчет времени удаления сообщения через 10 минут
                service.setNewMessageFlag(false);                                        // Сбросить флаг нового сообщения. Программа извещена и приступила к обработке нового сообщения.
                ...
                memset(msg_mem_tmp, 0, sizeof(msg_mem_tmp));                              // Очистить временный буфер
                strncpy(msg_mem_tmp, service.msg_tmp_all, strlen(service.msg_tmp_all));   // Записать новое сообщение во временный буфер
                memset(service.msg_tmp_all, 0, sizeof(service.msg_tmp_all));              // Очистить буфер сообщения
            ...
            if (SOS_View_on_off)
            {
                ...
                state_SOS_button.pushToSprite(&back, 197, 160, TFT_BLACK);
            }
            else
            {
            ...
            bool flags_TimeOn = service.getViewTime();

            if (flags_TimeOn)
            {
                static uint32_t mail_off = millis();
                if (millis() - mail_off > MESSAGE_OFF_DELAY)                                  // Убрать отображение сообщения почты
                {
                    mail_off = millis();
                    service.setViewTime(true);                                              // Время 10 минут закончилось
                    service.setAllow_flashing(false);                                       // Запретить мигание сообщения
                    rows_Message.fillSprite(TFT_BLACK);
