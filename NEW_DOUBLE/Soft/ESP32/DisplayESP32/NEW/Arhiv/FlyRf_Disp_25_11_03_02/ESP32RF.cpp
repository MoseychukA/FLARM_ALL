...
static byte ESP32_Display_setup()
{
  ...
  if (settings->power_view == VIEW_AKK_ON)
  {
    ...
    //info_header.createSprite(480, 25);
...
int line_Sprite_Mini = 0; // текущая линия для вывода (строк без дыр)
int line_Sprite = 0;      // текущая линия для вывода (строк без дыр)
// Глобально или static:
...
void displayAllPlanes()
{
    ...
    if (has_data_to_display)
    {
        if (!info_header.created())
        {
            ...
            info_header.drawString("   ICAO  | FLIGHT |  ALT  | SPEED |COURSE| LATITUDE | LONGITUDE |  SIG", 0, 1, 2);
    ...
    for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
    {
        if (Container[i].addr != 0)
        {
            ...
            if (need_draw)
            {
                ...
                // Заполняем данными
                airplane_alient_info[i]->fillSprite(backColor);
                airplane_alient_info[i]->drawString(String(Container[i].addr, HEX), 5, 2, 2);
                airplane_alient_info[i]->drawString(Container[i].callsign, 60, 2, 2);
                airplane_alient_info[i]->drawString(String((int)Container[i].altitude), 122, 2, 2);
                airplane_alient_info[i]->drawString(String((int)Container[i].speed), 180, 2, 2);
                airplane_alient_info[i]->drawString(String((int)Container[i].course), 234, 2, 2);
                airplane_alient_info[i]->drawString(String(Container[i].latitude, 5), 280, 2, 2);
                airplane_alient_info[i]->drawString(String(Container[i].longitude, 5), 358, 2, 2);

                if (Container[i].signal_source == 0)
                {
                    if (Container[i].rssi == 0)
                        airplane_alient_info[i]->drawString(String(Container[i].rssi), 450, 2, 2);
                    else
                        airplane_alient_info[i]->drawString(String(Container[i].rssi), 440, 2, 2);
                }
                else if (Container[i].signal_source == 1)
                {
                    if (Container[i].last_message_signal_strength_dbm == 0)
                        airplane_alient_info[i]->drawString(String(Container[i].last_message_signal_strength_dbm), 450, 2, 2);
                    else
                        airplane_alient_info[i]->drawString(String(Container[i].last_message_signal_strength_dbm), 440, 2, 2);
                }
...
//    else
//    {
//        // Если данных для показа нет — удаляем заголовок если он был
//        if (info_header.created())
//        {
//            info_header.deleteSprite();
//        }
...
//    for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
//    {
//        // Есть активный самолёт?
//        if (Container[i].addr != 0)
//        {
//            bool need_draw = false;
//            // Для режима "только с координатой"
//            if (settings->display_set == INFO_DISPLAY_COORDINATE)
//            {
//                if (Container[i].latitude != 0 && Container[i].longitude != 0)
//                    need_draw = true;
...
//                airplane_alient_info[i]->drawString(String(Container[i].addr, HEX), 5, 2, 2);
//                airplane_alient_info[i]->drawString(Container[i].callsign, 60, 2, 2);
//                airplane_alient_info[i]->drawString(String((int)Container[i].altitude), 126, 2, 2);
//                airplane_alient_info[i]->drawString(String((int)Container[i].speed), 180, 2, 2);
//                airplane_alient_info[i]->drawString(String((int)Container[i].course), 234, 2, 2);
//                airplane_alient_info[i]->drawString(String(Container[i].latitude, 5), 280, 2, 2);
//                airplane_alient_info[i]->drawString(String(Container[i].longitude, 5), 358, 2, 2);
...
//                {
//                    if (Container[i].rssi == 0)
//                        airplane_alient_info[i]->drawString(String(Container[i].rssi), 455, 2, 2);
//                    else
//                        airplane_alient_info[i]->drawString(String(Container[i].rssi), 440, 2, 2);
//                }
//                else if (Container[i].signal_source == 1)
//                {
//                    if (Container[i].last_message_signal_strength_dbm == 0)
//                        airplane_alient_info[i]->drawString(String(Container[i].last_message_signal_strength_dbm), 455, 2, 2);
//                    else
//                        airplane_alient_info[i]->drawString(String(Container[i].last_message_signal_strength_dbm), 440, 2, 2);
//                }
//
//                // Отрисовываем спрайт строки (позиция зависит от номера строки)
//                airplane_alient_info[i]->pushToSprite(&back, 0, 41 + (line_Sprite * INFO_HEIGHT));
//
//                line_Sprite_Mini++;
//                line_Sprite++;
...
//    }
//
//    //// 4. Очищаем, если самолетов стало меньше
//    //for (int l = line_Sprite; l < MAX_TRACKING_OBJECTS; l++)
//    //{
...
static void ESP32_Display_loop()
{
    ...
    { 
        ...
        if (millis() - tmr > DATA_MEASURE_THRESHOLD) 
        {
            ...
            //Serial.println(thisAircraft_altitude_tmr);

            //Serial.print("ThisAircraft :");
            //Serial.print(" latitude:");
            //Serial.print(ThisAircraft.latitude, 5);
            //Serial.print(" thisAircraft longitude:");
            ...
            //        displayAllPlanes();
            //    }
            //    else if(line_Sprite_Mini == 0 || view_alien_count == 0)
            //    {
            //        displayAllPlanes();
            //        if (info_header.created())
            //        {
 ...
 //           if (view_alien_count >= 1 && settings->display_set == INFO_DISPLAY_COORDINATE)
 //           {
 //               info_header.createSprite(480, 20);
 //               info_header.setTextDatum(TL_DATUM);
 //               info_header.setTextColor(TFT_WHITE, backColor);
 ...
 //               back.setFreeFont(&FreeSerif9pt7b);
 //               back.drawString(" ICAO | FLIGHT |  ALT  |SPEED|COUR|LATITUDE|LONGITUDE|SIG", 0,25,2);
 //               back.drawLine(0, 39, 479, 39, TFT_WHITE);*/

 //               displayAllPlanes();
 //           }
 //           else if (view_alien_count >= 1 && settings->display_set == INFO_DISPLAY_MAXI)
 //           {
 //               info_header.createSprite(480, 20);
 //               info_header.setTextDatum(TL_DATUM);
 //               info_header.setTextColor(TFT_WHITE, backColor);
 ...
 //
            ...
            if (view_alien_count >= 1)
            {
                ...
                for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
                {
                    ...
                    /* вычисляем минимальное значение дистанции для переключения диапазона просмотра */
                        if (Container[i].latitude != 0 && Container[i].longitude != 0) // Расчет возможен если получены координаты стороннего самолета)
                        {
                            ...
                            //Serial.println((int)Container[i].distance);
               ...
               //// SoC->syncContainerDisplay();
                ...
                for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
                {
                    ...
                    //    //{
                    //    //    airplane_alient_info[i] = new TFT_eSprite(&tft);
                    //    //    airplane_alient_info[i]->createSprite(350, INFO_HEIGHT);

                    //    //    // Заполняем спрайт актуальной информацией
                    //    //    airplane_alient_info[i]->fillSprite(backColor);
                    //    //    airplane_alient_info[i]->setTextColor(TFT_WHITE, backColor);
                    //    //    airplane_alient_info[i]->setTextDatum(TL_DATUM);
                    //    //    airplane_alient_info[i]->drawRect(0, 0, 350, INFO_HEIGHT, TFT_WHITE);
                    ...
                    //    //    //airplane_alient_info[i]->drawString(String("Flight: ") + Container[i].callsign, 4, 2, 2);
                    //    //    //airplane_alient_info[i]->drawString(String("ICAO: ") + String(Container[i].addr, HEX), 4, 22, 2);
                    //    //    //airplane_alient_info[i]->drawString(String("Altitude: ") + String(Container[i].altitude), 160, 2, 2);
                    //    //    //airplane_alient_info[i]->drawString(String("Speed: ") + String(Container[i].speed), 160, 22, 2);
                    //    //    //// ... добавьте остальные поля при необходимости

                    //    //    // Показываем спрайт на экране в нужной строчке
                    //    //    airplane_alient_info[i]->pushSprite(0, 40 + (line * INFO_HEIGHT));
                    //    //}
                    //    line_Sprite++;
                    //}
                    //else
            ...
            bool base_con_ = service.get_connection_base();

            if (!base_con_)
            {
                base_connection.createSprite(300, 25);
                base_connection.setFreeFont(&FreeSerif12pt7b);
                base_connection.fillSprite(TFT_BLACK);
                base_connection.setTextDatum(CC_DATUM);
                base_connection.setTextColor(TFT_RED, backColor);
                base_connection.drawString("NO CONNECTION BASE",150, 11);
                base_connection.pushToSprite(&back, 90, 1);        // 
            }
            else if (base_con_)
            {
                if (base_connection.created())
                {
                    base_connection.deleteSprite();
                }
            }
            ...
            if ((settings->view_test_coord == VIEW_COORD_ON)) // 
            {
                back.setTextDatum(0);
                back.setTextColor(TFT_DARKGREY, TFT_BLACK);
                back.drawString("Lat: " + String(ThisAircraft.latitude, 5), 1, 287);
                back.drawString("Lon: " + String(ThisAircraft.longitude, 5), 1, 303);
            ...
            if (settings->voltage_view == VOLTAGE_VIEW_ON)
            {
                ...
                voltage1.createSprite(80, 20);
                voltage1.fillSprite(TFT_BLACK);
                voltage1.setTextColor(TFT_BLUE, TFT_BLACK);
                voltage1.setTextDatum(CC_DATUM);
                voltage1.drawString(str_Vcc + str_V, 40, 10);
                voltage1.pushToSprite(&back, power_x+25, 277, TFT_BLACK);
 

                current1.createSprite(110, 20);
                current1.fillSprite(TFT_BLACK);
                current1.setTextColor(TFT_BLUE, TFT_BLACK);
                current1.setTextDatum(CC_DATUM);
                current1.drawString(current + str_Cur, 55, 10);
                current1.pushToSprite(&back, power_x, 295, TFT_BLACK);
            }
            esp_task_wdt_reset();
