...
*description: 选择输入按键类型(I2C KEY、ADC KEY)。
...
BOOL POS_SysTimeDriver(void)
{
   ...
   if(currenTime3Cont > preTime3Cont)
   {
        ...
        if(tempValBuf >= 0x5b91)                   // 1s ->0x5b91 理论值
        {
            tempValBuf =tempValBuf - 0x5b91;
            currenTime3Cont -= tempValBuf;
            preTime3Cont =  currenTime3Cont;    //更新
            //这里就要对时钟加一秒
...
*input:   ADchanel    ADC 通道选择  
*output:  返回当前选中通道的ADC值   
*update:  2011-12-07
*state:   try out

*description:  返回当前选中通道的ADC值   
...
*description:
      MCU: 表示MCU控制视频模块,ARKchip 为Mater模式.
      IIC: 表示IIC控制，ARKchip为Slave模式。
...
#define ZERO_H_WITH    	0X08
    ...
    #if IR_TYPE == NEC
	//系统设置，用户一般不需要修改一下寄存器值。
	XBYTE[0XFF82] = 0X01;//设置协议类型，数据极性，脉冲极性。
    XBYTE[0XFF62] = 0x3f;//时钟分频 从27M分频。
	XBYTE[0XFF63] = 0x10;//毛起滤波
	XBYTE[0XFF64] = 0x04;//选择时钟有效位数。
    XBYTE[0XFF6B] = 0x00;//
    XBYTE[0XFF6C] = 0x00;//
    XBYTE[0XFF6D] = 0x00;//
    XBYTE[0XFF6E] = 0x00;//
	XBYTE[0XFF79] = 0x01;//设置中断号向量
    XBYTE[0XFF7A] = 0x73;//设置红外解码模式 ，包括是否判断客户码
    XBYTE[0XFF7B] = 0xff;//
    XBYTE[0XFF7C] = 0x02;// 
    XBYTE[0XFF77] = 0x59;//释放时间计数器
	XBYTE[0XFF78] = 0x00;
    ...
    XBYTE[0XFF61] = IR_Delta_DATA;//delta :此数值为除 repeat 很长的数据的delta.	
    XBYTE[0XFF7D] = IR_Delta_96ms;//delta :专门提供给repeat最后的一个长脉宽用的。
	#endif
	#endif

	#if IR_DECODE_MODE == IR_SOFTWARE
	XBYTE[0XFF82] = 0X01;//设置协议类型，数据极性，脉冲极性。
    XBYTE[0XFF62] = 0x3f;//时钟分频 从24M分频。
	XBYTE[0XFF63] = 0x10;//毛起滤波
	XBYTE[0XFF64] = 0x04;//选择时钟有效位数。
	XBYTE[0XFF79] = 0x01;//设置中断号向量
    XBYTE[0XFF7A] = 0x01;//设置为软件解码模式。   
	#endif
