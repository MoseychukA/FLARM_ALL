    ...
    { 
        ...
        if (millis() - tmr > DATA_MEASURE_THRESHOLD) 
        {
            ...
            if (settings->voltage_view == VOLTAGE_VIEW_ON)
            {
                ...
                voltage1.setTextColor(TFT_GREEN, TFT_BLACK);
                voltage1.setTextDatum(CC_DATUM);
                voltage1.drawString(str_Vcc + str_V, 50, 10);
                voltage1.pushToSprite(&back, power_x+50, 270, TFT_BLACK);
               
                current1.createSprite(139, 20);
                current1.fillSprite(TFT_BLACK);
                current1.setTextColor(TFT_GREEN, TFT_BLACK);
                voltage1.setTextDatum(CC_DATUM);
                current1.drawString(current + str_Cur, 70, 10);
                current1.pushToSprite(&back, power_x, 290, TFT_BLACK);
            }
            esp_task_wdt_reset();
