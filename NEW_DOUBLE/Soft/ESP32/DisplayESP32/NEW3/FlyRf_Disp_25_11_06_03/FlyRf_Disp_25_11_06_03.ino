...
void buttonsTask(void* param)
{
    ...
    {
        esp_task_wdt_reset();
        BUTTON1 = service.get_num_buttton();   //digitalRead(BTN1_PIN);
...
// Параметры кнопки
const int BUTTON_PIN = 45; // подберите под ваш пин
const unsigned long DEBOUNCE_MS = 20; // дребезг
const unsigned long DOUBLE_CLICK_MS = 400; // окно двойного клика
const unsigned long LONG_PRESS_MS = 3000; // порог длинного нажатия

// Состояние кнопки в ISR
...
// Флаги событий (для обработки в task)
volatile bool evtShort = false;
volatile bool evtDouble = false;
volatile bool evtLong = false;

// Новый флаг для надёжной обработки окна двойного клика
volatile bool pendingShort = false;
volatile unsigned long shortEndTime = 0;
        ...
        if (buttonPressed)
        {
            ...
            if (duration >= LONG_PRESS_MS) {
                ...
                if (waitingForSecondClick && shortClickCount == 1) {
                    // второе короткое за окно — двойной клик
                    evtDouble = true;
                    waitingForSecondClick = false;
                    shortClickCount = 0;
                    pendingShort = false;
                    ...
                    // первое короткое нажатие — ждём второе
                    shortClickCount = 1;
                    waitingForSecondClick = true;
                    // запускаем окно двойного клика
                    windowEndTime = t + DOUBLE_CLICK_MS;
                    pendingShort = true;
                    shortEndTime = t + DOUBLE_CLICK_MS;
                }
            }
        }
    }
...
// Задача обработки кнопок на ядре 0
static void ButtonTask(void* pvParameters)
{
    // Настройка пина и прерывания
    pinMode(BUTTON_PIN, INPUT_PULLUP);
    attachInterrupt(digitalPinToInterrupt(BUTTON_PIN), onButtonChange, CHANGE);

    for (;;) {
        // Обработка флагов и вывод
        if (evtLong) {
            evtLong = false;
            Serial.println("LONG");
            service.set_num_buttton(3); // замените на ваш вызов
        }
        ...
        // Таймаут окна двойного клика:если прошло DOUBLE_CLICK_MS и второй клик не был
        // обработан в ISR,зафиксируем одиночный короткий
        if (pendingShort && (millis() >= shortEndTime)) 
        {
            evtShort = true;
            pendingShort = false;
            waitingForSecondClick = false;
...
void setup()
{
  ...
  //============================================================================
    ...
    // Запуск четырех задач на Core 0
