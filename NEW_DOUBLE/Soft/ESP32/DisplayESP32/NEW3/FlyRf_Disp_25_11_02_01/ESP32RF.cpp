        ...
        if (Container[i].addr != 0)
        {
            if (settings->display_set == INFO_DISPLAY_MINI && Container[i].latitude != 0 && Container[i].longitude != 0)
            {
                ...
                if (!airplane_alient_info[i]->created())
                {
                    ...
                    // airplane_alient_info[i]->drawRect(0, 0, 380, INFO_HEIGHT, TFT_WHITE);
                }
                // Выводим основные поля (пример)
                airplane_alient_info[i]->fillSprite(backColor);
                airplane_alient_info[i]->drawString(String(Container[i].addr, HEX), 3, 2, 2);
                airplane_alient_info[i]->drawString(Container[i].callsign, 55, 2, 2);
                airplane_alient_info[i]->drawString(String((int)Container[i].altitude), 125, 2, 2);
                airplane_alient_info[i]->drawString(String((int)Container[i].speed), 180, 2, 2);
                airplane_alient_info[i]->drawString(String((int)Container[i].course), 230, 2, 2);
                airplane_alient_info[i]->drawString(String(Container[i].latitude, 5), 280, 2, 2);
                airplane_alient_info[i]->drawString(String(Container[i].longitude, 5), 365, 2, 2);
                if (Container[i].signal_source == 0)
                {
                    ...
                    {
                        airplane_alient_info[i]->drawString(String(Container[i].rssi), 442, 2, 2);
                    }
                }
                else if (Container[i].signal_source == 1)
                {
                    ...
                    {
                        airplane_alient_info[i]->drawString(String(Container[i].last_message_signal_strength_dbm), 442, 2, 2);
                    }
                   
                }

                // Показываем спрайт на экране в нужной строчке
                airplane_alient_info[i]->pushToSprite(&back, 0, 40 + (line_Sprite * INFO_HEIGHT));
...
static void ESP32_Display_loop()
{
    ...
    { 
        ...
        if (millis() - tmr > DATA_MEASURE_THRESHOLD) 
        {
            ...
            if (view_alien_count >= 1 && settings->display_set != INFO_DISTLAY_OFF)
            {
