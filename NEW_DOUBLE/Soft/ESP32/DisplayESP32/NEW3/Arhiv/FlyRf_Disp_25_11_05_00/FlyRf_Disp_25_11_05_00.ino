...
// ESP32S3 - обработка 3 видов нажатий через прерывание(CHANGE), вывод в Serial в loop
...
const unsigned long LONG_PRESS_MS = 2000; // порог длинного нажатия
...
IRAM_ATTR void onButtonChange() 
{
    ...
    // Таймер окна двойного клика:если прошло окно и второе нажатие не было
    if (waitingForSecondClick && t > windowEndTime) {
        evtShort = true; // одно короткое нажатие зафиксировано
        waitingForSecondClick = false;
        shortClickCount = 0;
    }
}
...
void button_processing()
{
    ...
    // Обработка флагов,вывод через service в loop
    if (evtLong) {
        evtLong = false;
        Serial.println("LONG");
...
void loop()
{
    ...
    if (strlen(AuxData.msg_resp_M) > 0)
    {
      ...
      //  Serial.println(AuxData.msg_resp_M);
        strncpy(service.msg_tmp_all, AuxData.msg_resp_M, strlen(AuxData.msg_resp_M));
      //  Serial.println(AuxData.msg_resp_M);
        strncpy(AuxData.msg_resp_M, "", strlen(AuxData.msg_resp_M));
        AuxData.msg_resp_M[0] = 0;
    }
