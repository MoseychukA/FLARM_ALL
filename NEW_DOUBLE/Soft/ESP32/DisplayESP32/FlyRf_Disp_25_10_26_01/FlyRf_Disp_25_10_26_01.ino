...
bool receivePacket_RS485(full_packet_net_t* pkt, uint8_t* btn1, uint8_t* btn2)
{
    ...
    while (idx >= sizeof(full_packet_net_t) + 8)
    {
        ...
        //for (int i = 0; i < 30; ++i) Serial.printf("%02X ", buffer[i]);
        //Serial.println();
...
void rxTask(void* param)
{
    ...
    for (;;) 
    {
        ...
        {
            digitalWrite(LED, LOW);
            hasActiveLink = true; // <<< связь есть
            viewActiveLink = true; // <<< связь есть
            net_to_ufo(&recpkt.ThisAircraft, &ThisAircraft);
            memcpy(ThisAircraft.callsign, &recpkt.ThisAircraft.callsign, 8);
...
void linkWatchdogTask(void* param) 
{
    ...
    for (;;) 
    {
        ...
        if (millis() - last > 2000)
        {
            if (viewActiveLink)
            {
                Serial.println("Нет связи с базой");
