...
Serial2.print(','); Serial2.print(src); Serial2.print(','); Serial2.printf("%06X", icao & 0xFFFFFF); Serial2.print(','); Serial2.print(callsign); 
Serial2.print(','); if(has_pos){ Serial2.print(lat,6); Serial2.print(','); Serial2.print(lon,6); } else { Serial2.print(','); Serial2.print(','); }
Serial2.print(','); Serial2.print(alt); Serial2.print(','); if(has_vel){ Serial2.print(gs,1); Serial2.print(','); Serial2.print(trk,1);} else { Serial2.print(',');
Serial2.print(','); } Serial2.print(','); if (vs!=INT32_MIN){ Serial2.print(vs);} Serial2.print(','); if (bg!=INT32_MIN){ Serial2.print(bg);} Serial2.print(',');
Serial2.print(rssi_u); Serial2.print(','); Serial2.print(crc_ok?"Y":"N"); Serial2.print(""); }; switch(g_out_mode){ case OUT_RAW: if (g_log_csv_dup) print_csv(); 
break; case OUT_CSV: print_csv(); break; case OUT_NMEA: { String s="PADSB,"; char icaohex[7]; snprintf(icaohex,sizeof(icaohex),"%06X",icao&0xFFFFFF);
                   s+=icaohex; s+=','; s+=src; s+=','; s+=callsign; s+=','; if(has_pos){ s+=String(lat,6); s+=','; s+=String(lon,6);} else { s+=","; s+=","; } s+=',';
                   s+=String(alt); s+=','; if(has_vel){ s+=String(gs,1); s+=','; s+=String(trk,1);} else { s+=","; s+=","; } s+=','; if(vs!=INT32_MIN) s+=String(vs); s+=','; 
                   if(bg!=INT32_MIN) s+=String(bg); s+=','; s+=(crc_ok?"Y":"N"); nmea_write_sentence(s); if (g_log_csv_dup) print_csv(); break; } case OUT_JSON: { Serial2.print('{');
                   Serial2.print("ts:"); Serial2.print((uint32_t)millis()); Serial2.print(','); Serial2.print("ch:"); Serial2.print(ch+1); Serial2.print(',');
                   Serial2.print("df:"); Serial2.print(df); Serial2.print(','); Serial2.print("src:"); Serial2.print(src); Serial2.print(",");
                   Serial2.print("icao:"); Serial2.printf("%06X",icao&0xFFFFFF); Serial2.print(","); Serial2.print("callsign:"); Serial2.print(callsign);
                   Serial2.print(","); Serial2.print("crc_ok:"); Serial2.print(crc_ok?"true":"false"); Serial2.print(','); Serial2.print("rssi:");
                   Serial2.print(rssi_u); Serial2.print(','); if(has_pos){ Serial2.print("lat:"); Serial2.print(lat,6); Serial2.print(','); Serial2.print("lon:"); Serial2.print(lon,6);
                   Serial2.print(','); } Serial2.print("alt:"); Serial2.print(alt); Serial2.print(','); if(has_vel){ Serial2.print("gs:"); Serial2.print(gs,1); 
                   Serial2.print(','); Serial2.print("trk:"); Serial2.print(trk,1); Serial2.print(','); } if(vs!=INT32_MIN){ Serial2.print("vs:"); Serial2.print(vs);
                   Serial2.print(','); } if(bg!=INT32_MIN){ Serial2.print("bg:"); Serial2.print(bg); Serial2.print(','); } Serial2.print("mode:json}"); Serial2.print("");
...
static void proc_task(void* arg){
  ...
  while(1){
    ...
    if(xQueueReceive(q_packets,&pkt,pdMS_TO_TICKS(10))==pdPASS){ decode_and_send(pkt);} if(xQueueReceive(q_fixed_packets,&pkt,0)==pdPASS)
    { if(g_out_mode==OUT_RAW){ Serial2.print("FIXED_RAW="); for(uint8_t i=0;i<(pkt.nbits+7)/8;i++){ if(pkt.data[i]<16) Serial2.print('0'); Serial2.print(pkt.data[i],HEX);} Serial2.print(""); } }
