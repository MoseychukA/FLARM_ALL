...
static void nmea_write_sentence(const String& payload){ uint8_t cs=0; for(size_t i=0;i<payload.length();++i) cs ^= (uint8_t)payload[i]; auto hex1=[](uint8_t x)->char{ return (x<10)?('0'+x):('A'+x-10); }; Serial2.print('$'); Serial2.print(payload); Serial2.print('*'); Serial2.print(hex1((cs>>4)&0xF)); Serial2.print(hex1(cs&0xF)); Serial2.print("
"); }
...
static void decode_and_send(AdsbPacket& p){ const char* src_tag="ADSB"; uint8_t df=(p.data[0]>>3)&0x1F; if(df==18){ uint8_t cf=(p.data[0]&0x07); src_tag=classify_df18_src(cf); }
  ...
  if((df==17 || df==18) && p.nbits==112){ uint8_t tc=(me[0]>>3)&0x1F; if(tc>=1&&tc<=4){ decode_callsign(me,callsign);} else if(tc>=9&&tc<=18){ bool odd=(me[0]&0x04)!=0; static IcaoCprState ICAO_CPR_LOCAL[ICAO_MAP_SIZE]; uint8_t h=(uint8_t)icao; if(ICAO_CPR_LOCAL[h].icao!=icao){ memset(&ICAO_CPR_LOCAL[h],0,sizeof(IcaoCprState)); ICAO_CPR_LOCAL[h].icao=icao; } if(odd){ memcpy(ICAO_CPR_LOCAL[h].odd_me,me,7); ICAO_CPR_LOCAL[h].odd_time_ms=millis(); ICAO_CPR_LOCAL[h].has_odd=true; } else { memcpy(ICAO_CPR_LOCAL[h].even_me,me,7); ICAO_CPR_LOCAL[h].even_time_ms=millis(); ICAO_CPR_LOCAL[h].has_even=true; } if(ICAO_CPR_LOCAL[h].has_even && ICAO_CPR_LOCAL[h].has_odd && (uint32_t)abs((int)(ICAO_CPR_LOCAL[h].even_time_ms - ICAO_CPR_LOCAL[h].odd_time_ms)) <= 10000){ cpr_global_decode(lat,lon,ICAO_CPR_LOCAL[h].even_me,ICAO_CPR_LOCAL[h].odd_me);} else { cpr_local_decode(lat,lon,me,CPR_REF);} alt=decode_altitude(me);} else if(tc==19){ decode_velocity_tc19(me,gs,trk,vs,baro_geo);} }
...
static void rx_task(void* arg){
  ...
  TickType_t lastBlink=xTaskGetTickCount(); adc_init(); adc_gpio_init(PIN_RSSI); adc_select_input(0);
