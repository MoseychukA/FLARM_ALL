...
static void output_record(uint8_t ch, uint8_t df, const char* src, uint32_t icao, const char* callsign, bool has_pos, double lat, double lon, int alt, bool has_vel, double gs, double trk, int vs, int bg, uint32_t rssi_u, bool crc_ok){ auto print_csv = [&](void){ Serial2.print("CSV,"); Serial2.print((uint32_t)millis()); Serial2.print(','); Serial2.print(ch+1); Serial2.print(','); Serial2.print(df); Serial2.print(','); Serial2.print(src); Serial2.print(','); Serial2.printf("%06X", icao & 0xFFFFFF); Serial2.print(','); Serial2.print(callsign); Serial2.print(','); if(has_pos){ Serial2.print(lat,6); Serial2.print(','); Serial2.print(lon,6); } else { Serial2.print(','); Serial2.print(','); } Serial2.print(','); Serial2.print(alt); Serial2.print(','); if(has_vel){ Serial2.print(gs,1); Serial2.print(','); Serial2.print(trk,1);} else { Serial2.print(','); Serial2.print(','); } Serial2.print(','); if (vs!=INT32_MIN){ Serial2.print(vs);} Serial2.print(','); if (bg!=INT32_MIN){ Serial2.print(bg);} Serial2.print(','); Serial2.print(rssi_u); Serial2.print(','); Serial2.print(crc_ok?"Y":"N"); Serial2.print("
...
static volatile int g_out_mode = 3; // OUT_JSON
...
static void proc_task(void* arg){ Serial2.setTX(PIN_UART2_TX); Serial2.setRX(PIN_UART2_RX); Serial2.begin(921600); storage_init(); apply_profile(PROF_NORMAL); g_out_mode=3; g_log_csv_dup=false; // defaults
  Settings set; if(storage_load(set)){ if(set.mode.length()){ String m=set.mode; m.toUpperCase(); if(m=="CSV") g_out_mode=1; else if(m=="NMEA") g_out_mode=2; else if(m=="JSON") g_out_mode=3; else if(m=="UBX") g_out_mode=4; else if(m=="RAW") g_out_mode=0; } if(set.profile.length()) apply_profile(*profile_from_string(set.profile)); g_log_csv_dup=set.log_on; CPR_REF.ref_lat=set.ref_lat; CPR_REF.ref_lon=set.ref_lon; }
...
'){ String s=line; s.trim(); String up=s; up.toUpperCase(); if(up.startsWith("MODE ")){ String m=up.substring(5); if(m=="CSV") g_out_mode=1; else if(m=="NMEA") g_out_mode=2; else if(m=="JSON") g_out_mode=3; else if(m=="UBX") g_out_mode=4; else if(m=="RAW") g_out_mode=0; Serial2.print("OK MODE "); Serial2.println(m); } else if(up.startsWith("PROFILE ")){ String p=up.substring(8); apply_profile(*profile_from_string(p)); Serial2.print("OK PROFILE "); Serial2.println(p); } else if(up.startsWith("LOG ")){ String v=up.substring(4); g_log_csv_dup=(v=="ON"); Serial2.print("OK LOG "); Serial2.println(v); } else if(up.startsWith("REF ")){ double la=CPR_REF.ref_lat, lo=CPR_REF.ref_lon; sscanf(s.c_str()+4, "%lf %lf", &la, &lo); CPR_REF.ref_lat=la; CPR_REF.ref_lon=lo; Serial2.print("OK REF "); Serial2.print(la,6); Serial2.print(' '); Serial2.println(lo,6); } else { Serial2.println("OK"); } line=""; } else if(line.length()<128){ line+=ch; } }
    if(xQueueReceive(q_packets,&pkt,pdMS_TO_TICKS(10))==pdPASS){ decode_and_send(pkt);} if(xQueueReceive(q_fixed_packets,&pkt,0)==pdPASS){ if(g_out_mode==0){ Serial2.print("FIXED_RAW="); for(uint8_t i=0;i<(pkt.nbits+7)/8;i++){ if(pkt.data[i]<16) Serial2.print('0'); Serial2.print(pkt.data[i],HEX);} Serial2.print("
"); } }
