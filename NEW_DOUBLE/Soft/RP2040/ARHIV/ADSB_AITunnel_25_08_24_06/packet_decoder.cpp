...
bool PacketDecoder::UpdateDecoderLoop() 
{
    ...
    for (uint16_t i = 0; i < num_packets_to_process; i++)
    {
        ...
        if (decoded_packet.IsValid() || decoded_packet.GetDownlinkFormat() == 4 || decoded_packet.GetDownlinkFormat() == 5)
        {
            ...
            //rx_packet1_[i].mlat_48mhz_64bit_counts = (uint64_t)time_us_64() * 48ULL;
            //ADSB_OnRawPacket(&decoded_packet[i]);
        ...
        else if (config_.enable_1090_error_correction && decoded_packet.GetBufferLenBits() == Raw1090Packet::kExtendedSquitterPacketLenBits)
        {
            ...
            if (bit_flip_index > 0)
            {
                ...
                //rx_packet1_[i].mlat_48mhz_64bit_counts = (uint64_t)time_us_64() * 48ULL;
                //ADSB_OnRawPacket(&rx_packet1_[i]);
        ...
        if (decoded_packet.GetDownlinkFormat() == 17 || decoded_packet.GetDownlinkFormat() == 5)
        {
