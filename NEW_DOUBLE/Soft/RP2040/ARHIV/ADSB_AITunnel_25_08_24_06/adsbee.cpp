...
bool ADSBee::Update() 
{
    uint32_t timestamp_ms = millis(); //!! get_time_since_boot_ms();
    ...
    timestamp_ms = millis();//!! get_time_since_boot_ms();
...
void ADSBee::FlashStatusLED(uint32_t led_on_ms) {
    SetStatusLED(true);
    led_on_timestamp_ms_ = millis();//!! get_time_since_boot_ms();
}
               ...
               // rx_packet_[sm_index].buffer[i] >>= 1;
                ...
                switch (packet_num_words) {
                    case Raw1090Packet::kSquitterPacketNumWords32:
                        ...
                        // Ставим метку времени MLAT 48 Мгц (если нет вашего счетчика)
