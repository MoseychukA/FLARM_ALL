...
static void process_packet(const PacketBuf& pb) {
    ...
    bool used_alt = false;
    if (!ok) { used_alt = true; ok = check_crc_auto(bytes_alt, nbits); if (ok) memcpy(bytes, bytes_alt, sizeof(bytes)); }
...
static void periodic_housekeeping() {
    ...
    if (now - last_ser >= SERIALIZE_PERIOD_MS) {
        for (int i = 0; i < MAX_AIRCRAFT; ++i)
            if (g_aircraft[i].icao != 0) send_status_json(&g_aircraft[i]);
        last_ser = now;
    }
}
...
void loop() {
    ...
    else { periodic_housekeeping(); delay(5); }
