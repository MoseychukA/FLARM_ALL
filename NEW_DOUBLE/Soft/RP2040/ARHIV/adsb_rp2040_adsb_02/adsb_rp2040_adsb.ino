...
{ 
    ...
    Serial2.print('*'); 
    auto hex=[&](uint8_t x){ char h=x<10?('0'+x):('A'+x-10); Serial2.print(h); }; 
    hex((cs>>4)&0xF); hex(cs&0xF); Serial2.print(""); 
}
...
{ 
    auto print_csv = [&](void)
    {
        ...
        Serial2.print(','); Serial2.print(ch+1);
        Serial2.print(','); Serial2.print(df); 
        Serial2.print(','); Serial2.print(src);
        Serial2.print(','); Serial2.printf("%06X", icao & 0xFFFFFF);
        Serial2.print(','); Serial2.print(callsign); Serial2.print(',');
        if(has_pos)
        { Serial2.print(lat,6);
            Serial2.print(',');
            Serial2.print(lon,6); 
...
{ 
    Serial2.setTX(PIN_UART2_TX); Serial2.setRX(PIN_UART2_RX); Serial2.begin(921600); storage_init(); apply_profile(PROF_NORMAL); g_out_mode=3; g_log_csv_dup=false; // defaults
  ...
  { 
      ...
      { 
          ...
          if(ch==' ')
          {
              ...
              { 
                  ...
                  if(m=="CSV") g_out_mode=1; 
                  else if(m=="NMEA") g_out_mode=2; 
                  else if(m=="JSON") g_out_mode=3; 
                  else if(m=="UBX") g_out_mode=4; 
                  else if(m=="RAW") g_out_mode=0; 
              ...
              else if(up.startsWith("PROFILE "))
              {
                  ...
                  Serial2.print("OK PROFILE "); Serial2.println(p); 
              } 
              ...
              {
                  double la=CPR_REF.ref_lat, lo=CPR_REF.ref_lon; sscanf(s.c_str()+4, "%lf %lf", &la, &lo);
                  CPR_REF.ref_lat=la;
                  CPR_REF.ref_lon=lo; 
    ...
    { 
        if(g_out_mode==0){ Serial2.print("FIXED_RAW="); 
...
{
    q_packets=xQueueCreate(256,sizeof(AdsbPacket)); q_fixed_packets=xQueueCreate(64,sizeof(AdsbPacket)); TaskHandle_t hRx=nullptr,hProc=nullptr; 
    xTaskCreate(rx_task,"rx",16384,NULL,configMAX_PRIORITIES-1,&hRx); xTaskCreate(proc_task,"proc",12288,NULL,configMAX_PRIORITIES-2,&hProc); 
