...
// Ёта скорость определ€ет максимальное рассто€ние, на которое может перепрыгнуть самолет между обновлени€ми положени€
//  без необходимости в повторном образце дл€ подтверждени€ его нового положени€. Ёто позвол€ет перехватывать отчеты о
//  положении, которые возвращаютс€ как действительные из декодера CPR, но относ€тс€ к €вно неверному положению.
...
{
    // TODO: «десь может быть условие, при котором мы временно тер€ем пакеты, когда счетчик MLAT циклически обнул€етс€ и
    // все временные метки пакетов отображаютс€ как 0 мс, но это, веро€тно, не имеет большого значени€.
 ...
 /*       CONSOLE_WARNING("Aircraft1090::DecodePosition",
                        ...
                        icao_address);*/
        return false;  // нужны как четные, так и нечетные пакеты, чтобы иметь возможность декодировать позицию
    }
    ...
    {
        // ќтклонить пары пакетов —Ћ–, которые слишком сильно разнесены по времени.
        WriteBitFlag(BitFlag::kBitFlagPositionValid, false);  // ќтклонить пару пакетов SLR, которые слишком сильно изменились со временем.
        //CONSOLE_WARNING("Aircraft1090::DecodePosition",
...
{
    // ¬Ќ»ћјЌ»≈: «десь задействованы две отдельные временные базы! ¬ременна€ база дл€ временных меток пакетов CPR Ч это MLAT
    // временна€ база, в то врем€ как информаци€ словар€ самолетов более высокого уровн€ находитс€ в системном времени.

    if (!CanDecodePosition()) 
    {
        // Ќевозможно попытатьс€ принудительно декодировать и проверить, €вл€етс€ ли это допустимым,
        //поскольку в итоге мы будем использовать устаревший четный или нечетный пакет
        return false;
    }
    ...
    if (!result_valid) 
    {
        ...
        return false;  // —амолет пересек две широтные зоны, не удаетс€ декодировать эту пару пакетов.
    }
...
bool AircraftDictionary::IngestDecoded1090Packet(Decoded1090Packet &packet) 
{
    ...
    switch (packet.GetBufferLenBits()) 
    {
        case Raw1090Packet::kSquitterPacketLenBits:
            ...
            if (!packet.IsValid()) 
            {
                //  адр сквиттера не смог проверить себ€ или не смог пройти проверку по данным ICAO в словаре.
                return false;
            }
...
bool AircraftDictionary::ApplyAirbornePositionMessage(Aircraft1090 &aircraft, ADSBPacket packet) {
    ...
    // ME[7] - ƒополнение NIC B (ранее флаг одной антенны)
...
bool AircraftDictionary::ApplyAirborneVelocitiesMessage(Aircraft1090 &aircraft, ADSBPacket packet) {
    ...
    if (vertical_rate_magnitude_fpm == 0) {
        ...
        //CONSOLE_WARNING("AircraftDictionary::ApplyAirborneVelocitiesMessage", "Vertical rate not available for ICAO 0x%lx.", aircraft.icao_address);
        decode_successful = false;
    } else {
