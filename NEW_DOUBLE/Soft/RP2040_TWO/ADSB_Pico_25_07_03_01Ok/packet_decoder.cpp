    ...
    for (uint16_t i = 0; i < num_messages; i++) 
    {
        ...
        switch (message.log_level) 
        {
            ...
            case SettingsManager::LogLevel::kWarnings:
                comms_manager.console_printf("PacketDecoder2..  ", "%s", message.message);
                break;
            case SettingsManager::LogLevel::kErrors:
                comms_manager.console_printf("PacketDecoderp3..  " , "%s", message.message);
                break;
            default:
...
bool PacketDecoder::UpdateDecoderLoop() 
{
    ...
    //else
    //{
    //    comms_manager.console_printf("num_packets_to_process.  %d \r\n", num_packets_to_process);
