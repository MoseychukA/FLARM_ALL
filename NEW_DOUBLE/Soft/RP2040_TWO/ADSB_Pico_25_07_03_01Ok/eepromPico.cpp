...
#include "hal.h"           // For timing.
#include "hardware/gpio.h"  // For optional I2C initialization.
#include "hardware/i2c.h"

void EEPROMPICO::BeginPostWriteDelay() { last_write_timestamp_us_ = get_time_since_boot_us(); }

void EEPROMPICO::WaitForSafeReadWriteTime() 
{
...
int EEPROMPICO::WriteByte(const uint16_t reg, const uint8_t byte)
{
...
int EEPROMPICO::ReadByte(const uint16_t reg, uint8_t &byte)
{
...
int EEPROMPICO::WriteBuf(const uint16_t reg, uint8_t *buf, const uint16_t num_bytes)
{
...
int EEPROMPICO::ReadBuf(const uint16_t reg, uint8_t *buf, const uint16_t num_bytes)
{
