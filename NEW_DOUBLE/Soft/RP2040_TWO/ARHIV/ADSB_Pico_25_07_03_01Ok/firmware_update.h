...
class FirmwareUpdateManager 
{
    ...
    */
    ...
    // Установите это значение достаточно большим, чтобы обеспечить эффективность, 
    //но достаточно малым, чтобы программы не истекали из-за тайм-аута в ожидании обновления.
    static constexpr uint16_t kMaxSectorsPerErase = 10 * 16384 / 4096; //!! FLASH_BLOCK_SIZE / FLASH_SECTOR_SIZE;
...
#ifdef ON_PICO
    /**
    * Проверяет, находится ли счетчик программ в указанном разделе флэш-памяти.
    * @param[in] раздел Индекс раздела. Должен быть < kNumPartitions
    * @retval True, если в разделе, false, если нет или раздел не существует.
    */
    ...
    /**
    * Удалить содержимое раздела флэш-памяти. По умолчанию стирается весь раздел, включая заголовок и
    * приложение.
    * @param[in] partition Индекс раздела для стирания. Должен быть < kNumPartitions.
    * @param[in] offset Смещение от начала раздела, с которого начинается стирание. Должен быть кратным
    * размеру сектора.
    * @param[in] len_bytes Длина содержимого раздела, начиная со смещения для стирания. Если len_bytes не кратно
    * размеру сектора, будет стерт весь сектор, содержащий последний байт.
    * @retval True, если стирание прошло успешно, false, если возникла ошибка.
    */
                                          ...
                                          uint32_t len_bytes = kFlashHeaderLenBytes + kFlashAppLenBytes) 
    {
        ...
        for (uint16_t sector = offset / FLASH_SECTOR_SIZE; remaining_sectors_to_erase > 0;sector += num_sectors_to_erase) 
        {
    ...
    /**
    * Преобразует из адресов во флэш-памяти во смещения флэш-памяти, требуемые функциями флэш-памяти Pico SDK.
    * @param[in] flash_addr Абсолютный адрес во флэш-памяти.
    * @retval Смещение от начала флэш-памяти.
    */
    static inline uint32_t FlashAddrToOffset(uint32_t flash_addr) { return flash_addr - kFlashBlStartAddr; }

    /**
    * Преобразует состояние раздела флэш-памяти в строку.
    * @param[in] status Состояние раздела флэш-памяти.
    * @param[out] buf Буфер для записи. Должен быть длиной не менее kFlashPartitionStatusStrMaxLen.
    */
     ...
     */
    static inline bool PartialReadFlashPartition(uint16_t partition, uint32_t offset, uint32_t len_bytes, uint8_t *buf) 
    {
     ...
     */
    static inline bool PartialWriteFlashPartition(uint16_t partition, uint32_t offset, uint32_t len_bytes, const uint8_t *buf) 
    {
        if (partition >= kNumPartitions) 
        {
   ...
   private:
    ...
    static inline void RestoreInterrupts(void) { restore_interrupts(stored_interrupts_); }
     ...
     /**
      * Изменяет слово состояния заголовка раздела флэш-памяти, перезаписывая полный заголовок. Обратите внимание, что не все
      * значения возможны для слова состояния, так как биты могут быть перевернуты только с 1 на 0, а не наоборот,
      * без стирания всего сектора.
      * @param[in] partition Индекс раздела для изменения заголовка.
      * @param[in] status Новое слово состояния для записи.
      */
    ...
    static uint32_t stored_interrupts_ = 0;
