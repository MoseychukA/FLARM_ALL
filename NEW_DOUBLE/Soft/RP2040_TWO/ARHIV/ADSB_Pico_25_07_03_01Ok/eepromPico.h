#ifndef EEPROMPICO_H_
#define EEPROMPICO_H_
...
class EEPROMPICO 
{
   public:
     ...
     */
    EEPROMPICO(EEPROMConfig config_in) : config_(config_in) {};
    ...
    /**
    * Загрузка экземпляра объекта из EEPROM, начиная с адреса start_addr.
    * @param[out] data_to_load Ссылка на объект для загрузки данных.
    * @param[in] start_reg Необязательный адрес регистра для начала объекта. По умолчанию 0x0.
    * @retval True, если загрузка прошла успешно, в противном случае false.
    */
    template <typename T>  // Реализовано здесь, поскольку реализация шаблонной функции в файле .cpp — это проблема.
