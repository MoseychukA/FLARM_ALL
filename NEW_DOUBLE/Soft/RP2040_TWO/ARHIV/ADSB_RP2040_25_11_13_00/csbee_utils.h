...
void sendToDUMP1090_UART1(const ToDUMP1090& src) 
{
   ...
   // out.squawk = src.squawk;
    out.squawk = toBigEndian16(src.squawk);
    ...
    out.last_message_signal_strength_dbm = toBigEndian16(src.last_message_signal_strength_dbm);
    out.last_message_signal_quality_db = toBigEndian16(src.last_message_signal_quality_db);
...
uint16_t squawkStrToUint16(const char src[5], bool* ok = nullptr)
{
    // 1️⃣ Проверяем, что строка действительно завершается нулём.
    if (src[4] != '\0') 
    {
        if (ok) *ok = false;
        return 0;
    ...
    // 2️⃣ Проверяем, что каждый из первых четырёх символов – цифра.
    ...
    // 3️⃣ Преобразуем вручную (без вызова strtoul – экономим код и RAM).
    uint32_t value = 0;
    for (uint8_t i = 0; i < 4; ++i) 
    {
    ...
    // 4️⃣ Проверяем, что получилось в диапазоне uint16_t.
    if (value > UINT16_MAX) 
    {
        if (ok) *ok = false;
        return 0;
...
inline int16_t WriteCSBeeAircraftMessageStr(char message_buf[], const Aircraft1090 &aircraft) 
{
    ...
    packet.squawk = squawkStrToUint16(squawk_buf);                           // Flight number SQUAWK
    ...
    packet.last_message_signal_strength_dbm = aircraft.last_message_signal_strength_dbm;  // SIGS
    packet.last_message_signal_quality_db = aircraft.last_message_signal_quality_db;      // SIGQ
 
   // Serial.print(packet.squawk); Serial.print(" : "); Serial.print(aircraft.squawk); Serial.print(" : "); Serial.println(squawk_buf);
   // Serial.print(packet.squawk);
...
inline int16_t WriteCSBeeSerialMessageStr(char message_buf[], const Aircraft1090& aircraft)
{
    // #A:ICAO,FLAGS,CALL,SQ,LAT,LON,ALT_BARO,TRACK,VELH,VELV,SIGS,SIGQ,FPS,NICNAC,ALT_GEO,ECAT,CRC\r\n
    ...
    int16_t num_chars =  // Вывести все, кроме CRC, в строковый буфер.
        snprintf(message_buf, kCSBeeMessageStrMaxLen - kCRCMaxNumChars - 1,
            ...
            aircraft.last_message_signal_strength_dbm,        // SIGS aircraft.last_message_signal_strength_dbm, 
            aircraft.last_message_signal_quality_db,          // SIGQ
            aircraft.metrics.valid_squitter_frames,           // SFPS допустимые кадры сквиттера
            aircraft.metrics.valid_extended_squitter_frames  // ESFPS допустимые расширенные кадры скв
         );
