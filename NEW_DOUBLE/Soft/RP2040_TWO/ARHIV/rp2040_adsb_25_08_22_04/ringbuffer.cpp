...
{
	CorrCfg c; 
	c.preamble_len_samp=160; 
	c.bit_samp=20;
	c.halfbit_samp=10;
	c.short_min=cfg.short_min_samp;
	c.long_max=cfg.long_max_samp;
	return c; 
}
...
static inline uint8_t sget(int ch, uint32_t idx)
{
	return g_samp[ch].s[idx & (SAMPLE_FIFO_SIZE-1)]; 
}
...
static int correlate_preamble(int ch, uint32_t cur, const CorrCfg &ccfg, const struct Config &cfg, float &score, int &t0_offset)
{
	...
	{
		int t0=(int)cur - ccfg.preamble_len_samp + shift; 
		if(t0<0) continue;
		float posHits=0.0f; 
		int penalties=0;
		...
		bool perwin_ok=true; 
		int cpos[6]; 
		for(int k=0;k<6;k++) cpos[k]=t0+centers[k]; 
		if (abs((cpos[1]-cpos[0]) - 20) > cfg.pre_strict_tol) continue;
		if (abs((cpos[2]-cpos[1]) - 50) > cfg.pre_strict_tol) continue; 
		if (abs((cpos[3]-cpos[2]) - 20) > cfg.pre_strict_tol) continue; 
		for(int k=0;k<6;k++)
		{
			int c=cpos[k]; 
			int hits=0;
			for(int d=-win; d<=win; ++d) hits += sget(ch, c+d);
			...
			{ int mid = t0 + (centers[k]+centers[k+1])/2;
			   int off=0;
			  for(int d=-win; d<=win; ++d) off += sget(ch, mid+d); penalties += off; 
			}
		} 
		if(!perwin_ok) continue;
		int run=0; 
		for(int s=0;s<ccfg.preamble_len_samp;s++)
		{ 
			if(sget(ch,t0+s))
			{ 
				run++;
				if(run>ccfg.long_max)
				{ 
					longPulse=true;
					break;
				} 
...
{ 
	...
	{ 
		...
		for(int d=0; d<ccfg.halfbit_samp; ++d) bh += sget(ch, bit_base+ccfg.halfbit_samp+d); bits[b] = (a>bh)?1:0; 
	}
  int msg_bits=112, msg_bytes=14;
  int energy_after_56=0; 
  for(int i=56;i<112;i++) energy_after_56 += bits[i]; if(energy_after_56 < 3)
  {
...
{ 
	...
	static float ema_noise[3]={0,0,0};
	CorrCfg ccfg=cfg_from(cfg);
	uint32_t widx=g_samp[ch].widx; 
	if ((int32_t)(widx - last_t0_samp[ch]) < (int32_t)(cfg.dead_time_us * 20)) return 0; 
	float score=0;
	int t0=0;
	int ok=correlate_preamble(ch, widx, ccfg, cfg, score, t0);
	if(!ok)
	{ 
		ema_noise[ch] = (1.0f-cfg.ema_alpha)*ema_noise[ch] + cfg.ema_alpha*1.0f;
		return 0;
	}
	float dyn_thr = cfg.base_thr + cfg.dyn_k * ema_noise[ch];
	if(score < dyn_thr)
	{ 
		ema_noise[ch] = (1.0f-cfg.ema_alpha)*ema_noise[ch] + cfg.ema_alpha*score;
	    return 0;
	}
	RawMessage rm;
	rm.channel=ch;
