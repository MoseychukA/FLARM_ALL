...
uint32_t crc24_syndrome(const uint8_t *buffer, uint16_t buffer_len_bytes, uint32_t initial_value) 
{
    ...
    uint32_t buffer_crc =  (buffer[buffer_len_bytes - 3] << 16) | (buffer[buffer_len_bytes - 2] << 8) | buffer[buffer_len_bytes - 1];
...
int16_t crc24_find_single_bit_error(uint32_t syndrome, uint16_t message_len_bits) 
{
    ...
    {
        case 112:
            // 112-битные пакеты Extended Squitter обрабатываются так, как они получены, с 24-битным CRC в конце.
            syndrome_table = crc24_single_bit_syndrome_112;
            break;
        case 56:
            // 56-битные пакеты Squitter имеют 24-битный CRC XOR с адресом ICAO. Таким образом, синдром — это ICAO.
