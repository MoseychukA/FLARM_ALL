...
bool CommsManager::UpdateReporting() 
{
    ...
    // ѕродолжить обновление и записать временную метку.
...
{
    // «аписать сообщение CSBee Aircraft дл€ каждого самолета в словаре самолетов.
    for (auto &itr : adsbee.aircraft_dictionary.dict) 
    {
        ...
        SendESP32(message, message_len_bytes);
        SendBuf(iface, message, message_len_bytes); 
    }

    // Ќаписать сообщение статистики CSBee.
    char message[kCSBeeMessageStrMaxLen];
    int16_t message_len_bytes =        WriteCSBeeStatisticsMessageStr(message,                                                            // Buffer to write into.
