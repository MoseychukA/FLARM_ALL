...
bool Aircraft1090::DecodePosition(bool filter_cpr_position) 
{
    ...
    if (!result_valid) 
    {
        //!!comms_manager.console_printf("CONSOLE_WARNINGAircraft1090::DecodePosition Can't decode position from packet pair that spans different latitude zones.\r\n");
    ...
    if (filter_cpr_position) 
    {
        ...
        if (most_recent_received_timestamp_ms <= last_filter_received_timestamp_ms_) 
        {
            ...
            //!!comms_manager.console_printf("!WARNING Received CPR position update for ICAO 0x%lx, but timestamp %lu ms is not newer than last filter received timestamp %lu ms.\r\n", icao_address, most_recent_received_timestamp_ms, last_filter_received_timestamp_ms_);
            return false;
        }
...
bool AircraftDictionary::IngestDecoded1090Packet(Decoded1090Packet& packet) 
{
    ...
    //    aircrafts[idx].squawk = aircraft.squawk;
    //    strncpy(aircrafts[idx].callsign, aircraft.callsign, 8);
    //    aircrafts[idx].baro_altitude_ft = aircraft.gnss_altitude_ft;
    //    //aircrafts[idx].velocity_kts;
    //    //aircrafts[idx].direction_deg;
    //    //aircrafts[idx].vertical_rate_fpm;
    //    //aircrafts[idx].lat,
    //    //aircrafts[idx].lon;
...
bool AircraftDictionary::ApplyAirbornePositionMessage(Aircraft1090& aircraft, ADSBPacket packet)  // Сообщение о местоположении в воздухе
{
   ...
   // aircraft.SetCPRLatLon(latitudeBits, longitudeBits, odd, timestamp);
    //!!comms_manager.console_printf("\r\n\r\n*** odd = %d latitude %d longitude %d timestamp %d\r\n\r\n", odd, latitudeBits, longitudeBits, timestamp);
