...
#include "comms.h"  // For debug logging.
...
// Эта скорость определяет максимальное расстояние, которое может преодолеть самолёт между обновлениями местоположения без необходимости
// повторного замера для подтверждения нового местоположения. Это позволяет перехватывать отчёты о местоположении, которые возвращаются декодером CPR как достоверные, но
// соответствуют заведомо неверному местоположению.
const uint32_t kCPRPositionFilterVelocityMps = 1000;  // This is faster than the SR-71, so we're just catching really big jumps for now.
...
{
     memset(callsign, '\0', kCallSignMaxNumChars + 1);  // очистить строку позывного, включая дополнительный символ EOS
}
...
//================================================================================
#define AIRCRAFT_MAX 20
#define TIMEOUT_MS 20000    // 20 секунд

struct AircraftData
{
    uint32_t icao;             // ICAO-адрес
    int squawk;
    char callsign[9];          // Позывной (макс. 8 + 0)
    int baro_altitude_ft;
    int velocity_kts;
    float direction_deg;
    int vertical_rate_fpm;
    float lat, lon;
    unsigned long first_seen;  // millis() первого пакета
    unsigned long last_seen;   // millis() последнего пакета
    bool valid;                // Используется ли слот массива
};

AircraftData aircrafts[AIRCRAFT_MAX];


// Поиск самолёта по ICAO, возвращает индекс, если найден, иначе -1
int AircraftDictionary::findAircraft(uint32_t icao)
{
    for (int i = 0; i < AIRCRAFT_MAX; i++)
        if (aircrafts[i].valid && aircrafts[i].icao == icao)
            return i;
    return -1;
}

// Поиск свободного слота
int AircraftDictionary::findFreeSlot()
{
    for (int i = 0; i < AIRCRAFT_MAX; i++)
        if (!aircrafts[i].valid)
            return i;
    return -1;
}


// Очистка неактуальных самолётов
void AircraftDictionary::cleanOldAircrafts()
{
    unsigned long now = millis();
    for (int i = 0; i < AIRCRAFT_MAX; i++)
    {
        if (aircrafts[i].valid && (now - aircrafts[i].last_seen > TIMEOUT_MS))
        {
            aircrafts[i].valid = false;
        }
    }
}

// Отправить данные по Serial2 (например, CSV)
void AircraftDictionary::transmitAllAircrafts()
{
    for (int i = 0; i < AIRCRAFT_MAX; i++)
    {
        if (aircrafts[i].valid)
        {
            comms_manager.console_printf(
                ...
                "%.5f,"                                           // LON, e.g. 17.59554
                "%d\r\n",
                aircrafts[i].icao,                                // ICAO
                aircrafts[i].squawk,                              // SQUAWK
                aircrafts[i].callsign,                                // CALL
                aircrafts[i].baro_altitude_ft,                        // ALT_BARO
                aircrafts[i].velocity_kts,                            // VELH
                aircrafts[i].direction_deg,                           // TRACK
                aircrafts[i].vertical_rate_fpm,                        // VELV
                aircrafts[i].lat,                            // LAT
                aircrafts[i].lon,                           // LON
                (millis() - aircrafts[i].last_seen) / 1000
            );
        }
    }
}

void AircraftDictionary::transmitBaseAllAircrafts()
{
    // 2. Очищаем неактивных каждые 1-2 секунды
    static unsigned long last_clean = 0;
    if (millis() - last_clean > 1000)
    {
        cleanOldAircrafts();
        last_clean = millis();
    }

    // 3. Передаем массив (например, каждую секунду)
    static unsigned long last_tx = 0;
    if (millis() - last_tx > 1000)
    {
        transmitAllAircrafts();
        last_tx = millis();
    }
}
...
{
    // TODO: Здесь может возникнуть ситуация, при которой мы временно потеряем пакеты, когда счётчик MLAT зацикливается и
   // все временные метки пакетов отображаются как 0 мс, но это, вероятно, не имеет большого значения.
    if (!(last_odd_packet_.received_timestamp_ms > 0 && last_even_packet_.received_timestamp_ms > 0)) 
    {
       //!! comms_manager.console_printf("!WARNING Unable to decode position without receiving an odd and even packet pair for ICAO 0x%lx.\r\n",  icao_address); // Невозможно декодировать местоположение без получения пары четных и нечетных пакетов для ICAO
        return false;  // нужны как четные, так и нечетные пакеты, чтобы иметь возможность декодировать положение
    }
    uint32_t cpr_interval_ms = MIN(last_odd_packet_.received_timestamp_ms - last_even_packet_.received_timestamp_ms,  last_even_packet_.received_timestamp_ms - last_odd_packet_.received_timestamp_ms);
    if (cpr_interval_ms > GetMaxAllowedCPRIntervalMs()) 
    {
        // Отклонить пары пакетов CPR, которые слишком сильно разнесены во времени.
        WriteBitFlag(BitFlag::kBitFlagPositionValid,    false);  // keep last known good coordinates, but mark as invalid
        //!!comms_manager.console_printf("!WARNING CPR packet pair too far apart in time (%lu ms). Can't decode position.\r\n", cpr_interval_ms); // Пара пакетов СЛР слишком далека во времени
        return false;
    }
    return true;
}
...
{
    // ВНИМАНИЕ: Здесь задействованы две отдельные временные базы! Временная база для пакетов CPR — это MLAT,
    // а информация о словаре воздушного судна более высокого уровня — это системное время.

    if (!CanDecodePosition()) 
    {
        // Невозможно попытаться принудительно декодировать и проверить, является ли оно допустимым, поскольку в итоге мы будем использовать устаревшую четную или нечетную пару пакетов.
        return false;
    }

    uint32_t most_recent_received_timestamp_ms =  MAX1(last_even_packet_.received_timestamp_ms, last_odd_packet_.received_timestamp_ms);
    ...
    if (!result_valid) 
    {
       return false;  // Самолет пересек две широтные зоны, не может декодировать эту пару пакетов.
    }
    ...
    if (filter_cpr_position) 
    {
        ...
        {
            // Не допускайте повторного вызова DecodePosition() без нового пакета для переопределения фильтра.
            return false;
        }
        ...
        // Примечание: Игнорируйте проверку местоположения во время первого обновления, чтобы записать местоположение воздушного судна после получения первого пакета.
        if (HasBitFlag(BitFlag::kBitFlagPositionValid) && distance_meters > max_distance_meters) 
        {
            //comms_manager.console_printf("!WARNING Filtered CPR position update for ICAO 0x%lx, distance %lu m exceeds max %lu m.\r\n", icao_address, distance_meters, max_distance_meters);
...
{
    // Перебираем каждую пару «ключ-значение» в unordered_map. Удаляем, если устарели, обновляем метрики, если они ещё актуальны.
    for (auto it = dict.begin(); it != dict.end(); /* No increment here */) 
    {
        if (timestamp_ms - it->second.last_message_timestamp_ms > config_.aircraft_prune_interval_ms) // 60 секунд 
        {
    ...
    metrics = metrics_counter_;    // Заменить значения счетчиков на публично видимые значения.
    metrics_counter_ = Metrics();  // Используйте конструктор по умолчанию для очистки всех значений.
}
...
{
    // Проверка корректности и запись статистики.
    int16_t source = packet.GetRaw().source;
    switch (packet.GetBufferLenBits()) 
    {
        case Raw1090Packet::kSquitterPacketLenBits: 
            // Проверить пакет на соответствие адресам ICAO в словаре или разрешить его, если это ответ на все вызовы с DF=11
            // пакет, который сам себя проверил (например, это ответ на спонтанный сигнал обнаружения с запросчика
            // ID=0, что позволяет использовать контрольную сумму).
            if (packet.GetDownlinkFormat() != Decoded1090Packet::kDownlinkFormatAllCallReply &&  ContainsAircraft(packet.GetICAOAddress())) 
            {
                // DF=0,4-5 (DF=11 не подходит, так как идентификатор запросчика может быть перекрыт адресом ICAO
                // — мы ожидаем, что спонтанно обнаруженные DF=11 будут помечены как действительные).
                // Пакет представляет собой 56-битный пакет Squitter, который не может пройти проверку, и его CRC был
                // проверен по адресам ICAO в словаре воздушного судна.
                packet.ForceValid();
            }

            if (!packet.IsValid()) 
            {
                // Кадр сквиттера не смог проверить себя или не смог пройти проверку по данным ICAO в словаре.
                return false;
            }

            // Записываем действительный пакет сквиттера.
        ...
        case Raw1090Packet::kExtendedSquitterPacketLenBits:
            ...
            {
                return false;  // Расширенный сквиттер-кадр не прошел CRC.
            }
            break;
        default:
            return false;
    ...
    case Decoded1090Packet::DownlinkFormat::kDownlinkFormatAltitudeReply:      // DF = 4
        ingest_ret = IngestAltitudeReplyPacket(AltitudeReplyPacket(packet));
        break;
        // Identity Reply Packet.
    case Decoded1090Packet::DownlinkFormat::kDownlinkFormatIdentityReply:      // DF = 5
        //!!comms_manager.console_printf("\r\n ****IngestIdentityReplyPacket ***\r\n");
    ...
    //comms_manager.console_printf("*** ingest_ret %d  \r\n", ingest_ret);

    return ingest_ret;
}

bool AircraftDictionary::IngestIdentityReplyPacket(IdentityReplyPacket packet)  //Принять пакет ответа на идентификацию
{
    ...
    {
        //!!comms_manager.console_printf("CONSOLE_WARNING AircraftDictionary::IngestIdentityReplyPacket Unable to find or create new aircraft with ICAO address 0x%lx in dictionary.\r\n", icao_address);
    ...
    //unsigned long now = millis();
    //int idx = findAircraft(icao_address);


    //if (idx == -1)
    //{ // Новый самолет
    //    idx = findFreeSlot();
    //    //!!  if (idx == -1) return; // Нет места
    //    aircrafts[idx].squawk = aircraft_ptr->squawk;
    //    aircrafts[idx].first_seen = now;
    //    aircrafts[idx].last_seen = now;
    //    aircrafts[idx].valid = true;
    //}
    //else
    //{ // Уже наблюдается
    //  // Обновить только если данные изменились
    //    aircrafts[idx].squawk = aircraft_ptr->squawk;
    //    aircrafts[idx].last_seen = now;
    //}
...
bool AircraftDictionary::IngestAltitudeReplyPacket(AltitudeReplyPacket packet) 
{
    ...
    if (aircraft_ptr == nullptr)
    {
        //!!comms_manager.console_printf("CONSOLE_WARNING AircraftDictionary::IngestAltitudeReplyPacket Unable to find or create new aircraft with ICAO address 0x%lx in dictionary.\r\n", icao_address);
        return false;  // unable to find or create new aircraft in dictionary
    }
    aircraft_ptr->WriteBitFlag(Aircraft1090::BitFlag::kBitFlagIsAirborne, packet.IsAirborne());
    aircraft_ptr->WriteBitFlag(Aircraft1090::BitFlag::kBitFlagAlert, packet.HasAlert());
    ...
    //    idx = findFreeSlot();
    //    //!!  if (idx == -1) return; // Нет места
    //    aircrafts[idx].baro_altitude_ft = aircraft_ptr->baro_altitude_ft;
    //    aircrafts[idx].first_seen = now;
    //    aircrafts[idx].last_seen = now;
    //    aircrafts[idx].valid = true;
    //}
   ...
   // comms_manager.console_printf("ICAO %06X, baro_altitude_ft1 %d \r\n", icao_address, aircraft_ptr->baro_altitude_ft);
...
bool AircraftDictionary::IngestAllCallReplyPacket(AllCallReplyPacket packet) 
{
    ...
    if (aircraft_ptr == nullptr)
    {
        //!!comms_manager.console_printf("CONSOLE_WARNING AircraftDictionary::IngestAllCallReplyPacket Unable to find or create new aircraft with ICAO address 0x%lx in dictionary.\r\n", icao_address);
        return false;  // unable to find or create new aircraft in dictionary
    }
    aircraft_ptr->last_message_timestamp_ms = millis(); 

    // Обновить возможности транспондера самолета.
...
bool AircraftDictionary::IngestADSBPacket(ADSBPacket packet) 
{
 ...
 /*   comms_manager.console_printf("*** packet DF17 %d  \r\n", packet);*/
    ...
    aircraft_ptr->last_message_timestamp_ms = millis(); //!!get_time_since_boot_ms();
    bool ret = false;
    ...
    if (ret) 
    {
        aircraft_ptr->IncrementNumFramesReceived(true);  // Подсчитайте полученный кадр режима S.
    }
     ...
     //    idx = findFreeSlot();
     //    //!!  if (idx == -1) return; // Нет места
     //    aircrafts[idx].icao = icao_address;
     //    // memcpy(aircrafts[idx].squawk, aircraft.squawk, sizeof(aircraft.squawk));
     //    //strncpy(aircrafts[idx].callsign, aircraft.callsign, 8);
     //    //aircrafts[idx].velocity_kts = aircraft.velocity_kts;
     //    //aircrafts[idx].direction_deg = aircraft.direction_deg;
     //    //aircrafts[idx].vertical_rate_fpm  = aircraft.vertical_rate_fpm;
     //    //aircrafts[idx].lat,
     //    //aircrafts[idx].lon;
     //    aircrafts[idx].first_seen = now;
     //    aircrafts[idx].last_seen = now;
     ...
     //    //aircrafts[idx].direction_deg = aircraft.direction_deg;
     //    //aircrafts[idx].vertical_rate_fpm = aircraft.vertical_rate_fpm;
     //    //aircrafts[idx].velocity_kts = aircraft.vertical_rate_fpm;
     //    //aircrafts[idx].lat,
     //    //aircrafts[idx].lon;
     //    aircrafts[idx].last_seen = now;
     //}
...
bool AircraftDictionary::InsertAircraft(const Aircraft1090& aircraft) 
{
    ...
    if (itr != dict.end()) 
    {
        // Перезапись существующего самолета
        itr->second = aircraft;
        return true;
    }
    ...
    {
        //!!comms_manager.console_printf("CONSOLE_INFO AIrcraftDictionary::InsertAircraft Не удалось добавить самолет в словарь, максимальное количество самолетов %d.\r\n", kMaxNumAircraft);
...
bool Aircraft1090::SetCPRLatLon(uint32_t n_lat_cpr, uint32_t n_lon_cpr, bool odd, uint32_t received_timestamp_ms) {
    if (n_lat_cpr > kCPRLatLonMaxCount || n_lon_cpr > kCPRLatLonMaxCount) {
        //!!comms_manager.console_printf("CONSOLE_ERROR Aircraft1090::SetCPRLatLon Received CPR packet with out of bounds lat/lon values (%lu, %lu).\r\n", n_lat_cpr, n_lon_cpr);
...
bool AircraftDictionary::ApplyAircraftIDMessage(Aircraft1090& aircraft, ADSBPacket packet) 
{
    ...
    ////=====================================================================
    //unsigned long now = millis();
    //int idx = findAircraft(aircraft.icao_address);

    //if (idx == -1)
    //{ // Новый самолет
    //    idx = findFreeSlot();
    //    //!!  if (idx == -1) return; // Нет места
    //    aircrafts[idx].icao = aircraft.icao_address;
    //    memcpy(aircrafts[idx].callsign, aircraft.callsign, sizeof(aircraft.callsign));
    //    aircrafts[idx].first_seen = now;
    //    aircrafts[idx].last_seen = now;
    //    aircrafts[idx].valid = true;
    //}
...
bool AircraftDictionary::ApplySurfacePositionMessage(Aircraft1090& aircraft, ADSBPacket packet) {
    ...
    if (aircraft.NICBitIsValid(Aircraft1090::NICBit::kNICBitA) && aircraft.NICBitIsValid(Aircraft1090::NICBit::kNICBitC)) 
    {
        ...
        uint32_t icao_address = packet.GetICAOAddress();
        //comms_manager.console_printf("ICAO, %06X Miles %06X, %f\r\n", icao_address, aircraft.navigation_integrity_category);
    }
    return false;
}

bool AircraftDictionary::ApplyAirbornePositionMessage(Aircraft1090& aircraft, ADSBPacket packet)  // Сообщение о местоположении в воздухе
{
    aircraft.WriteBitFlag(Aircraft1090::BitFlag::kBitFlagIsAirborne, true);
    uint16_t typecode = packet.GetTypeCode();

    bool decode_successful = true;
    // ME[5-6] - Статус наблюдения
    ...
    case 0:  // No condition.
        ...
        break;
        // ПРИМЕЧАНИЕ: Возможно одновременное получение и оповещения, и идентификатора, но это невозможно передать
        // в одном сообщении о местоположении в воздухе.
    case 1:  // Постоянная тревога.
    case 2:  // Временное оповещение.
       // Относиться к постоянным и временным оповещениям одинаково.
        aircraft.WriteBitFlag(Aircraft1090::BitFlag::kBitFlagAlert, true);
        break;
    case 3:  // Состояние SPI.
        aircraft.WriteBitFlag(Aircraft1090::BitFlag::kBitFlagIdent, true);
        break;
    }

    // ME[7] - Дополнение NIC B (ранее флаг Single Antenna)
    ...
    {
        // Назначить сетевую карту на основе дополнительных битов сетевой карты A и B и полученного кода типа.
        ...
        case (18 << 3) | 0b000:
            ...
            break;
        default:
            // Проверьте наличие TypeCodes, которые могут определить сетевую карту без необходимости обращения к дополнительным битам сетевой карты.
            switch (typecode) {
            case 20:
                aircraft.navigation_integrity_category =
    ...
    case ADSBPacket::TypeCode::kTypeCodeAirbornePositionBaroAlt: 
    {
        ...
        if (encoded_altitude_ft_with_q_bit == 0) 
        {
            ...
            //!!comms_manager.console_printf("CONSOLE_WARNING AircraftDictionary::ApplyAirbornePositionMessage Altitude information not available for ICAO 0x%lx.\r\n", aircraft.icao_address);
        ...
        else 
        {
            ...
            // FIXME: В настоящее время не поддерживает барометрические высоты выше 50175 футов. Что-нибудь о кодах Грея?
    ...
    //    idx = findFreeSlot();
    //    //!!  if (idx == -1) return; // Нет места
    //    aircrafts[idx].baro_altitude_ft = aircraft.baro_altitude_ft;
    //    aircrafts[idx].first_seen = now;
    //    aircrafts[idx].last_seen = now;
    //    aircrafts[idx].valid = true;
    //}
   ...
   // comms_manager.console_printf("ICAO %06X, baro_altitude_ft %d \r\n", icao_address, aircraft.baro_altitude_ft);
  

    // ME[20] - Time
    // TODO: выяснить, нужно ли нам это

    // ME[21] - CPR Format
    bool odd = packet.GetNBitWordFromMessage(1, 21);  // Флаг четности пакета данных координат

    // ME[32-?]

    //// Извлекаем данные из пакета (проверка)
    //uint32_t latitudeBits = packet.GetNBitWordFromMessage(17, 22);
    //uint32_t longitudeBits = packet.GetNBitWordFromMessage(17, 39);
    //uint64_t timestamp = packet.GetTimestampMs();

   // aircraft.SetCPRLatLon(latitudeBits, longitudeBits, odd, timestamp);
 
    aircraft.SetCPRLatLon(packet.GetNBitWordFromMessage(17, 22), packet.GetNBitWordFromMessage(17, 39), odd, packet.GetTimestampMs());

    if (!aircraft.CanDecodePosition()) 
    {
        // Не удаётся декодировать местоположение самолёта, но это не ошибка. Возвращаем decode_successful, если были другие ошибки.
...
bool AircraftDictionary::ApplyAirborneVelocitiesMessage(Aircraft1090& aircraft, ADSBPacket packet) 
{
    ...
    ADSBPacket::AirborneVelocitiesSubtype subtype =  static_cast<ADSBPacket::AirborneVelocitiesSubtype>(packet.GetNBitWordFromMessage(3, 5));
    ...
    case ADSBPacket::AirborneVelocitiesSubtype::kAirborneVelocitiesGroundSpeedSubsonic: 
    {
        ...
        if (v_ew_kts_plus_1 == 0 || v_ns_kts_plus_1 == 0) {
            ...
            //!!comms_manager.console_printf("CONSOLE_WARNING AircraftDictionary::ApplyAirborneVelocitiesMessage Ground speed not available for ICAO 0x%lx.\r\n", aircraft.icao_address);
            decode_successful = false;
        }
        else 
        {
            ...
            //comms_manager.console_printf("ICAO %06X, velocity_kts %d \r\n", icao_address, aircraft.velocity_kts);
    ...
    {
        //!!comms_manager.console_printf("CONSOLE_WARNING AircraftDictionary::ApplyAirborneVelocitiesMessage Difference between GNSS and baro altitude not available for aircraft 0x%lx.\r\n", aircraft.icao_address);
        // Не устанавливайте decode_successful на false, чтобы игнорировать отсутствующую информацию о высоте GNSS/барометра.
    }
    else 
    {
        int gnss_alt_baro_alt_difference_ft =  (encoded_gnss_alt_baro_alt_difference_ft - 1) * 25 * (gnss_alt_below_baro_alt ? -1 : 1);
    ...
    if (aircraft.baro_altitude_ft != 0)
    {
       ...
       // comms_manager.console_printf("ICAO %06X, baro_altitude_ft2 %d \r\n", icao_address, aircraft.baro_altitude_ft);
    }
...
bool AircraftDictionary::ApplyAircraftOperationStatusMessage(Aircraft1090& aircraft, ADSBPacket packet) {
    ...
    aircraft.system_design_assurance =  static_cast<Aircraft1090::SystemDesignAssurance>(packet.GetNBitWordFromMessage(2, 30));
    ...
    case ADSBPacket::OperationStatusSubtype::kOperationStatusSubtypeSurface:  // ST = 1
    {
        ...
        // ME[16-18] - NACv
        aircraft.navigation_accuracy_category_velocity =  static_cast<Aircraft1090::NACHorizontalVelocityError>(packet.GetNBitWordFromMessage(3, 16));
    ...
    default:
        //!!comms_manager.console_printf("CONSOLE_ERROR AircraftDictionary::ApplyAircraftOperationStatusMessage Received unsupported Operation Status (TC=31) message subtype %d. Expected 0 or 1.\r\n", subtype);
        return false;
    }
    return true;
}
