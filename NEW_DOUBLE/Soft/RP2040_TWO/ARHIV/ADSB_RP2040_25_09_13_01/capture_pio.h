...
void preamble_detector_program_init(
    ...
    // Устанавливает конечный автомат и обертывает цель. Эта функция автоматически енерируется в preamble_detector.pio.h.
    ...
    pio_gpio_init(pio, demod_pin);                                 // Разрешить PIO управлять GPIO demod_pin (как вывод для отладки)
   ...
   // Подключите demod_pin к SET и SIDESET
    ...
    sm_config_set_clkdiv(&c, div);                                 // Устанавливаем делитель часов для конечного автомата
...
)
{
    ...
    pio_gpio_init(pio, recovered_clk_pin);                                    // Разрешить PIO управлять recovery_clk_pin (как вывод для отладки).
