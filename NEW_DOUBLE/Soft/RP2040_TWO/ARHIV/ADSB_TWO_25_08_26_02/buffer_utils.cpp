...
uint32_t GetNBitWordFromBuffer(uint16_t n, uint32_t first_bit_index, const uint32_t buffer[]) 
{
    ...
    {
       // printf("GetNBitWordFromBuffer: Tried to get %d bit word from buffer, but word bitlength must be between 1 and 32.\r\n", n);
        return 0;
    }
...
void SetNBitWordInBuffer(uint16_t n, uint32_t word, uint32_t first_bit_index, uint32_t buffer[]) 
{
    if (n > WORD_32_NUM_BITS || n < 1) 
    {
       // printf( "SetNBitWordInBuffer: Tried to set %d-bit word in buffer, but word bitlength must be between 1 and 32.\r\n", n);
        return;
    }
