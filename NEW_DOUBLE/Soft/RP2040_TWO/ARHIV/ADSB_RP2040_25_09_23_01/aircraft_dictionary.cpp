...
bool AircraftDictionary::IngestDecoded1090Packet(Decoded1090Packet& packet) 
{
    ...
    case Raw1090Packet::kExtendedSquitterPacketLenBits:
        ...
        break;
    default:
       //!!comms_manager.console_printf("CONSOLE_ERROR AircraftDictionary::IngestDecoded1090Packet Received packet with unrecognized bitlength %d, expected %d (Squitter) or %d (Extended Squitter).\r\n", packet.GetBufferLenBits(), Raw1090Packet::kSquitterPacketLenBits,
    ...
    default:
        //!!comms_manager.console_printf("CONSOLE_WARNING AircraftDictionary::IngestDecoded1090Packet Encountered unexpected downlink format %d for ICAO 0x%lx.\r\n", downlink_format,      packet.GetICAOAddress());
        ingest_ret = false;
        break;
    }
    return ingest_ret;
...
bool AircraftDictionary::IngestADSBPacket(ADSBPacket packet) 
{
    ...
    if (aircraft_ptr == nullptr) 
    {
        //!!comms_manager.console_printf("CONSOLE_WARNING AircraftDictionary::IngestADSBPacket Unable to find or create new aircraft with ICAO address 0x%lx in dictionary.\r\n",icao_address);
        return false;  // unable to find or create new aircraft in dictionary
    }
    ...
    case ADSBPacket::kTypeCodeAircraftOperationStatus:  // TC = 31 (Aircraft operation status)
        ...
        break;
    default:
        //!!comms_manager.console_printf("CONSOLE_WARNING AircraftDictionary::IngestADSBPacket Received ADSB message with unsupported typecode %d for ICAO 0x%lx.\r\n", typecode, packet.GetICAOAddress());
    ...
    {
        //!!comms_manager.console_printf(" CONSOLE_WARNING AircraftDictionary::IngestADSBPacket Failed to apply ADSB message with typecode %d to ICAO 0x%lx.\r\n", typecode, packet.GetICAOAddress());
    }
    return ret;
}
...
bool AircraftDictionary::InsertAircraft(const Aircraft1090& aircraft) {
    ...
    if (dict.size() >= kMaxNumAircraft) {
        //!!comms_manager.console_printf("CONSOLE_INFO AIrcraftDictionary::InsertAircraft Failed to add aircraft to dictionary, max number of aircraft is %d.\r\n", kMaxNumAircraft);
...
bool AircraftDictionary::ApplySurfacePositionMessage(Aircraft1090& aircraft, ADSBPacket packet) {
    ...
    if (aircraft.NICBitIsValid(Aircraft1090::NICBit::kNICBitA) &&
        ...
        default:
            break;
            //!!comms_manager.console_printf("CONSOLE_WARNING AircraftDictionary::ApplySurfacePositionMessage Unable to assign NIC with typecode %d and nic_bits %d for ICAO 0x%lx.\r\n", packet.GetTypeCode(), aircraft.nic_bits, aircraft.icao_address);
        }
    }

    //!!comms_manager.console_printf("CONSOLE_WARNING AircraftDictionary::ApplySurfacePositionMessage Surface position messages not yet supported. Received message with ICAO 0x%lx\r\n", aircraft.icao_address);
    return false;
}
...
bool AircraftDictionary::ApplyAirbornePositionMessage(Aircraft1090& aircraft, ADSBPacket packet)  // Сообщение о местоположении в воздухе
{
    ...
    if (aircraft.NICBitIsValid(Aircraft1090::NICBit::kNICBitA) &&  aircraft.NICBitIsValid(Aircraft1090::NICBit::kNICBitB)) 
    {
        ...
        default:
            ...
            default:
                break;
                //!!//!!comms_manager.console_printf("CONSOLE_WARNING AircraftDictionary::ApplyAirbornePositionMessage Unable to assign NIC with typecode %d and nic_bits %d for ICAO 0x%lx.\r\n", typecode, aircraft.nic_bits, aircraft.icao_address);
            }
        }
    }
    ...
    default:
        //!!comms_manager.console_printf("CONSOLE_WARNING AircraftDictionary::ApplyAirbornePositionMessage Received packet with unsupported typecode %d with ICAO 0x%lx.\r\n", packet.GetTypeCode(),aircraft.icao_address);
        return false;
    }
   ...
   // aircraft.SetCPRLatLon(latitudeBits, longitudeBits, odd, timestamp);
    ...
    if (aircraft.DecodePosition(config_.enable_cpr_position_filter)) 
    {
        ...
        decode_successful = false;
        //!!comms_manager.console_printf("CONSOLE_WARNING AircraftDictionary::ApplyAirbornePositionMessage Had valid packets, but aircraft position decode failed for ICAO 0x%lx.\r\n", aircraft.icao_address);
    }
...
bool AircraftDictionary::ApplyAirborneVelocitiesMessage(Aircraft1090& aircraft, ADSBPacket packet) 
{
    ...
    case ADSBPacket::AirborneVelocitiesSubtype::kAirborneVelocitiesAirspeedSubsonic: {
        ...
        if (airspeed_kts_plus_1 == 0) {
            //!!comms_manager.console_printf("CONSOLE_WARNING AircraftDictionary::ApplyAirborneVelocitiesMessage Airspeed not available for ICAO 0x%lx.\r\n", aircraft.icao_address);
            decode_successful = false;
        }
        else 
        {
    ...
    default:
        //!!comms_manager.console_printf("CONSOLE_ERROR AircraftDictionary::ApplyAirborneVelocitiesMessage Encountered invalid airborne velocities message subtype %d (valid values are 1-4).\r\n", subtype);
    ...
    if (vertical_rate_magnitude_fpm == 0) {
        ...
        //!!comms_manager.console_printf("CONSOLE_WARNING AircraftDictionary::ApplyAirborneVelocitiesMessage Vertical rate not available for ICAO 0x%lx.\r\n", aircraft.icao_address);
        decode_successful = false;
    }
    else 
    {
