...
bool AircraftDictionary::IngestDecoded1090Packet(Decoded1090Packet& packet) 
{
 ...
 //==========================================================================================
    ...
    uint16_t sm_index = static_cast<uint16_t>(source); if (sm_index < kMaxSmSlots)
    { 
    ...
    // Отправка в Serial 
        char buf[128]; 
        int n = snprintf(buf,sizeof(buf), "RX[%u]:sigs_dbm=%d,sigq_db=%d,source=%u\r\n", static_cast<unsigned int>(sm_index), rx_packet_[sm_index].sigs_dbm, rx_packet_[sm_index].sigq_db, rx_packet_[sm_index].source);
        if (n > 0) 
