...
ADSBee::ADSBee(ADSBeeConfig config_in) 
{
    ...
    tl_pwm_slice_ = pwm_gpio_to_slice_num(config_.tl_pwm_pin); // Pin 9 for Trigger Level PWM output.
    tl_pwm_chan_ = pwm_gpio_to_channel(config_.tl_pwm_pin);    // Определить модуль ШИМ, подключенный к указанному GPIO
}
...
bool ADSBee::Update() 
{
    ...
    if (timestamp_ms - last_aircraft_dictionary_update_timestamp_ms_ > config_.aircraft_dictionary_update_interval_ms) 
    {
        ...
        // Добавить новые значения метрик в стопку, используемую для обучения TL.
       // В случае обучения добавить количество полученных корректных пакетов в стопку, используемую для обучения на уровне триггера.
        if (tl_learning_temperature_mv_ > 0)
        {
            tl_learning_num_valid_packets_ += (aircraft_dictionary.metrics.valid_squitter_frames + aircraft_dictionary.metrics.valid_extended_squitter_frames);
        }
    ...
    if (tl_learning_temperature_mv_ > 0 &&  timestamp_ms - tl_learning_cycle_start_timestamp_ms_ > kTLLearningIntervalMs) 
    {
        ...
        float valid_packet_ratio = (tl_learning_num_valid_packets_ - tl_learning_prev_num_valid_packets_) / MAX(tl_learning_prev_num_valid_packets_, 1);  // Avoid divide by zero.
        float random_weight =  static_cast<int16_t>(get_rand_32()) * kInt16MaxRecip * 0.25;  // Random value from [-0.25,0.25].
...
void ADSBee::OnDemodComplete() 
{
    for (uint16_t sm_index = 0; sm_index < bsp.r1090_num_demod_state_machines; sm_index++) 
    {
        ...
        rx_packet_[sm_index].sigq_db = 666; // rx_packet_[sm_index].sigs_dbm - GetNoiseFloordBm();
