...
static inline void WordBufferToByteBuffer(const uint32_t word_buffer[], uint8_t byte_buffer[], uint16_t num_bytes) {
    uint16_t num_words = num_bytes / 4 + (num_bytes % 4 ? 1 : 0);
    for (uint16_t i = 0; i < num_words; i++) {
        uint16_t bytes_remaining = num_bytes - i * 4;

        byte_buffer[i * 4] = word_buffer[i] >> 24;
        if (--bytes_remaining == 0) break;
        byte_buffer[i * 4 + 1] = (word_buffer[i] >> 16) & 0xFF;
        if (--bytes_remaining == 0) break;
        byte_buffer[i * 4 + 2] = (word_buffer[i] >> 8) & 0xFF;
        if (--bytes_remaining == 0) break;
        byte_buffer[i * 4 + 3] = word_buffer[i] & 0xFF;
    }
}
...
bool adsb_check_crc(const Raw1090Packet& pkt) {
    if (pkt.buffer_len_bits < 56) return false;

    uint16_t packet_len_bytes = pkt.buffer_len_bits / 8;
    uint8_t raw_buffer[16]; // максимум 112 бит = 14 байт
    WordBufferToByteBuffer(pkt.buffer, raw_buffer, packet_len_bytes);

    uint32_t crc_calc = crc24(raw_buffer, packet_len_bytes - 3);
    uint32_t crc_recv = ((uint32_t)raw_buffer[packet_len_bytes - 3] << 16) |
        ((uint32_t)raw_buffer[packet_len_bytes - 2] << 8) |
        (uint32_t)raw_buffer[packet_len_bytes - 1];
...
DecodedAdsb adsb_decode(const Raw1090Packet& pkt) {
    DecodedAdsb out;
    if (pkt.buffer_len_bits < 56) return out;
    ...
    // DF = первые 5 бит
    out.df = (raw_buffer[0] >> 3) & 0x1F;

    // ICAO адрес (24 бита: байты 1,2,3)
    out.icao = ((uint32_t)raw_buffer[1] << 16) |
        ((uint32_t)raw_buffer[2] << 8) |
        raw_buffer[3];

    // Type Code (TC) = первые 5 бит 4-го байта
    out.tc = (raw_buffer[4] >> 3) & 0x1F;

    // Декодируем позывной (если сообщение типа 1–4, DF17 TC1–4)
    if (out.df == 17 && out.tc >= 1 && out.tc <= 4) {
        static const char charset[] = "#ABCDEFGHIJKLMNOPQRSTUVWXYZ#####_###############0123456789######";
        for (int i = 0; i < 8; i++) {
            uint8_t sixbits;
            if (i % 2 == 0) {
                sixbits = (raw_buffer[5 + (i / 2) * 3] & 0xFC) >> 2;
            }
            else {
                sixbits = ((raw_buffer[5 + (i / 2) * 3 - 1] & 0x03) << 4) | ((raw_buffer[5 + (i / 2) * 3] & 0xF0) >> 4);
            }
            if (sixbits < sizeof(charset) - 1) {
                out.callsign[i] = charset[sixbits];
            }
            else {
                out.callsign[i] = '_';
            }
    ...
    // Высота (TC 9–18: Airborne position)
    if (out.df == 17 && out.tc >= 9 && out.tc <= 18) {
        int raw_alt = ((raw_buffer[5] & 0x07) << 8) | raw_buffer[6];
