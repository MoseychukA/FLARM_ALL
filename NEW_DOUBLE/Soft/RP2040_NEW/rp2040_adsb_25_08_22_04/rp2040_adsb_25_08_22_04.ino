...
static constexpr uint8_t PIN_RX1 = 18;
static constexpr uint8_t PIN_RX2 = 19;
static constexpr uint8_t PIN_RX3 = 22;
...
void setup()
{
  ...
  while (!Serial && !Serial.dtr() && (millis() - t0) < 2000)
    delay(2000);
  ...
  PIO p0 = pio0;
  PIO p1 = pio1;
  int off0 = pio_add_program(p0, &adsb_sampler_program);
  int off1 = pio_add_program(p1, &adsb_sampler_program);
...
void core1_entry()
{
  ...
  while (true)
  {
    ...
    rssi_ema = 0.9f * rssi_ema + 0.1f * (float)rssi_adc_last;
    for (int i = 0; i < 3; i++)
    {
      uint32_t waddr = dma_hw->ch[chn[i].dma_chan].write_addr;
      uint32_t baseaddr = (uint32_t)(uintptr_t)chn[i].ring;
      uint32_t widx = ((waddr - baseaddr) >> 2) & (DMA_RING_WORDS - 1);
      chn[i].widx = widx;
      while (chn[i].ridx != chn[i].widx)
      {
        uint32_t word = chn[i].ring[chn[i].ridx];
        chn[i].ridx = (chn[i].ridx + 1) & (DMA_RING_WORDS - 1);
        // push_samples_filtered(i, word);
        push_samples(i, word);
      }
      int got = run_correlator_and_slice(i, g_cfg, pktq[i]);
      if (got > 0)
      {
        sio_hw->gpio_set = (1u << chn[i].pre_pin);
      }
      static uint32_t pre_decay[3] = {0, 0, 0};
      if (sio_hw->gpio_out & (1u << chn[i].pre_pin))
      {
        if ((t - pre_decay[i]) > 100)
        {
          sio_hw->gpio_clr = (1u << chn[i].pre_pin);
          pre_decay[i] = t;
        }
      } else pre_decay[i] = t;
    }
