...
static volatile uint16_t rssi_adc_last = 0; static float rssi_ema = 0.0f;
...
static void setup_pio_dma_channel(PioDmaChannel &c, PIO pio, uint sm, uint pin_in, uint pre_pin, int8_t pio_prog_offset){
  ...
  adsb_sampler_program_init(pio, sm, pio_prog_offset, pin_in, SAMPLE_FREQ_HZ);
...
static void handle_usb_command(const String &cmd){ String u=cmd; u.trim(); u.toUpperCase();
  if(u=="HELP"){ Serial.println("Commands: HELP, SHOW, SAVE, LOAD, PROFILE <NORMAL|HIGH-EMI|URBAN|REMOTE>, RAW ON|OFF, FORMAT <LOG|CSV|JSON|NMEA|UBX>, REF <lat> <lon>"); return; }
  ...
  if(u.startsWith("PROFILE ")){ String p=u.substring(8); p.trim(); if(p=="NORMAL") g_cfg.profile=Profile::Normal; else if(p=="HIGH-EMI") g_cfg.profile=Profile::HighEMI; else if(p=="URBAN") g_cfg.profile=Profile::Urban; else if(p=="REMOTE") g_cfg.profile=Profile::Remote; else { Serial.println("Bad profile"); return;} Serial.print("Profile set: "); Serial.println(profile_name(g_cfg.profile)); return; }
  ...
  if(u.startsWith("FORMAT ")){ String f=u.substring(7); f.trim(); int fmt=0; if(f=="LOG") fmt=0; else if(f=="CSV") fmt=1; else if(f=="JSON") fmt=2; else if(f=="NMEA") fmt=3; else if(f=="UBX") fmt=4; else { Serial.println("Bad format"); return;} g_cfg.out_format=fmt; Serial.print("Format set: "); Serial.println(f); return; }
  if(u.startsWith("REF ")){ float a,b; int c = sscanf(u.c_str()+4, "%f %f", &a, &b); if(c==2){ g_cfg.ref_lat=a; g_cfg.ref_lon=b; Serial.print("REF set: "); Serial.print(a,6); Serial.print(", "); Serial.println(b,6); } else Serial.println("REF usage: REF <lat> <lon>"); return; }
...
static void print_by_format(const DecodedFrame &df, const FlightOutput &fo, int rssi){
  switch(g_cfg.out_format){
    ...
    case 2: Serial.printf("{"icao":"%06X","src":%d,"cs":"%s","lat":%.6f,"lon":%.6f,"alt":%d,"spd":%d,"vs":%d,"trk":%d,"rssi":%d}
", df.icao, df.src_type, df.callsign, df.lat, df.lon, fo.altitude, fo.speed, fo.vert_rate, fo.course, rssi); break;
    case 3: Serial.printf("$PADSB,%06X,%s,%.6f,%.6f,%d,%d,%d,%d*
", df.icao, df.callsign, df.lat, df.lon, fo.altitude, fo.speed, fo.vert_rate, fo.course); break;
    case 4: Serial.println("UBX not implemented"); break;
  }
}

static void process_packets_loop(){ RawMessage rm; DecodedFrame df; while (pktq[0].pop(rm) || pktq[1].pop(rm) || pktq[2].pop(rm)){
    if (g_cfg.output_raw) emit_debug_raw(rm);
    CrcStatus cs = check_and_fix_crc(rm); if(cs==CRC_FIXED1) g_cfg.stat_crc_fix++; else if(cs==CRC_FAIL) g_cfg.stat_crc_fail++;
    if (decode_adsb_frame(rm, df, g_cfg)){
      g_cfg.stat_msgs++;
      FlightOutput fo={0}; fo.addr=df.icao; fo.Squawk=df.squawk; memcpy(fo.flight, df.callsign, sizeof(fo.flight)); fo.altitude=df.altitude_m_geo; fo.pressure_altitude=df.altitude_m_baro; fo.speed=df.speed_kmh; fo.course=df.track_deg; fo.vert_rate=df.vert_rate_mpm; fo.latitude=df.lat; fo.longitude=df.lon; fo.seen=df.seen_time_ms; fo.timestamp=millis(); fo.signal_source=1; fo.aircraft_type=df.aircraft_type;
...
void setup(){
  ...
  Serial.println("RP2040 ADS-B RX v2 starting...");
  LittleFS.begin(); load_config(g_cfg);
  if (fabs(g_cfg.ref_lat) < 0.0001f && fabs(g_cfg.ref_lon) < 0.0001f){ g_cfg.ref_lat = 55.93574f; g_cfg.ref_lon = 37.34873f; }
...
'||c==''){ if(cmd_line.length()){ handle_usb_command(cmd_line); cmd_line=""; } } else { cmd_line += c; if(cmd_line.length()>200) cmd_line.remove(0, cmd_line.length()-200); } } process_packets_loop(); }

void core1_entry(){ for(int i=0;i<3;i++) dma_channel_start(chn[i].dma_chan); uint32_t last_blink=time_us_32(); CorrState corr[3]; for(int i=0;i<3;i++) init_corr_state(corr[i], g_cfg);
  while(true){ uint32_t t=time_us_32(); if((t-last_blink)>=500000){ last_blink=t; sio_hw->gpio_togl=(1u<<PIN_BLINK_CORE1);} rssi_adc_last=analogRead(PIN_RSSI_ADC); rssi_ema = 0.9f*rssi_ema + 0.1f*(float)rssi_adc_last;
    for(int i=0;i<3;i++){ uint32_t waddr=dma_hw->ch[chn[i].dma_chan].write_addr; uint32_t baseaddr=(uint32_t)(uintptr_t)chn[i].ring; uint32_t widx=((waddr-baseaddr)>>2) & (DMA_RING_WORDS-1); chn[i].widx=widx; while(chn[i].ridx!=chn[i].widx){ uint32_t word=chn[i].ring[chn[i].ridx]; chn[i].ridx=(chn[i].ridx+1)&(DMA_RING_WORDS-1); push_samples_filtered(i, word); } int got = run_correlator_and_slice(i, g_cfg, pktq[i]); if(got>0){ sio_hw->gpio_set=(1u<<chn[i].pre_pin);} static uint32_t pre_decay[3]={0,0,0}; if(sio_hw->gpio_out & (1u<<chn[i].pre_pin)){ if((t-pre_decay[i])>100){ sio_hw->gpio_clr=(1u<<chn[i].pre_pin); pre_decay[i]=t; } } else pre_decay[i]=t; }
  }
}
