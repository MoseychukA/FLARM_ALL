	...
	if(LittleFS.exists("/config.txt"))
	{
		...
		{ 
			...
			{ 
				...
				else if(k=="out_format") cfg.out_format=v.toInt();
			} f.close();
		} 
	} 
}
...
{ 
	...
	if(!f) return;
	f.printf("profile=%s", profile_name(cfg.profile)); 
	f.printf("base_thr=%f", cfg.base_thr); 
	f.printf("k_ema=%f", cfg.k_ema); 
	f.printf("dead_time_us=%d", cfg.dead_time_us);
	f.printf("min_pos_hits=%d", cfg.min_pos_hits); 
	f.printf("penalty_w=%f", cfg.penalty_w);
	f.printf("output_raw=%d", cfg.output_raw?1:0);
	f.printf("ref_lat=%fref_lon=%f", cfg.ref_lat, cfg.ref_lon);
	f.printf("ema_alpha=%f", cfg.ema_alpha);
	f.printf("dyn_k=%f", cfg.dyn_k); 
	f.printf("min_pos_hits_per_win=%d", cfg.min_pos_hits_per_win);
	f.printf("pre_strict_tol=%d", cfg.pre_strict_tol);
	f.printf("short_min_samp=%d long_max_samp=%d", cfg.short_min_samp, cfg.long_max_samp); 
	f.printf("out_format=%d", cfg.out_format);
	f.close(); 
}

void print_config(const struct Config &cfg)
{
	Serial.printf("Profile=%s base_thr=%.1f k_ema=%.2f dead=%dus minHits=%d pen=%.2f RAW=%d ref=(%.5f,%.5f) outfmt=%d", profile_name(cfg.profile), cfg.base_thr, cfg.k_ema, cfg.dead_time_us, cfg.min_pos_hits, cfg.penalty_w, cfg.output_raw?1:0, cfg.ref_lat, cfg.ref_lon, cfg.out_format); 
}
