...
struct Config{
  ...
  bool output_raw; int out_format;
  // correlator thresholds (microsecond-based)
  int dead_time_us;      // 6..8
  int tol_us;            // timing tolerance (±)
  int min_pulses;        // minimal matching pulses in preamble sequence
  int min_win_hits;      // minimal edge hits per expected window
  float base_thr;        // base score threshold
  float ema_alpha;       // EMA for noise
  float dyn_k;           // weight for dynamic threshold
  // input options per channel
  bool invert[3];
};
