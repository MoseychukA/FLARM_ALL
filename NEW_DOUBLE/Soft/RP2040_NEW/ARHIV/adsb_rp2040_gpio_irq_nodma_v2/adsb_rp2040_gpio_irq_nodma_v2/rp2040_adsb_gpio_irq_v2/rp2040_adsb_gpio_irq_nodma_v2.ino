// rp2040_adsb_gpio_irq_nodma_v2.ino
#include <Arduino.h>
#include <LittleFS.h>
...
static void apply_profile(Config &c){
  switch(c.profile){
    case 1: // High-EMI
      c.dead_time_us=8; c.tol_us=2; c.min_pulses=5; c.min_win_hits=1; c.base_thr=6.0f; c.ema_alpha=0.1f; c.dyn_k=1.0f; break;
    case 2: // Urban
      c.dead_time_us=7; c.tol_us=2; c.min_pulses=5; c.min_win_hits=1; c.base_thr=6.0f; c.ema_alpha=0.15f; c.dyn_k=1.2f; break;
    case 3: // Remote
      c.dead_time_us=6; c.tol_us=3; c.min_pulses=4; c.min_win_hits=1; c.base_thr=4.0f; c.ema_alpha=0.08f; c.dyn_k=0.8f; break;
    default: // Normal
      c.dead_time_us=7; c.tol_us=2; c.min_pulses=5; c.min_win_hits=1; c.base_thr=5.0f; c.ema_alpha=0.1f; c.dyn_k=1.0f; break;
  }
}

static void handle_cmd(String u){ u.trim(); u.toUpperCase();
  if(u=="HELP"){ Serial.println("Commands: HELP, SHOW, SAVE, LOAD, RAW ON|OFF, PROFILE <NORMAL|HIGH-EMI|URBAN|REMOTE>, INV <CH1|CH2|CH3> <ON|OFF>"); return; }
  if(u=="SHOW"){ Serial.print("Profile="); Serial.print(profile_name(g_cfg.profile)); Serial.print(" RAW="); Serial.print(g_cfg.output_raw?1:0); Serial.print(" INV="); for(int i=0;i<3;i++){ Serial.print(g_cfg.invert[i]?"1":"0"); if(i<2) Serial.print(','); } Serial.println(); return; }
  if(u=="SAVE"){ File f=LittleFS.open("/config.txt","w"); if(f){ f.printf("profile=%d
raw=%d
inv=%d,%d,%d
", g_cfg.profile, g_cfg.output_raw?1:0, g_cfg.invert[0]?1:0, g_cfg.invert[1]?1:0, g_cfg.invert[2]?1:0); f.close(); Serial.println("Saved"); } return; }
...
'); line.trim(); if(!line.length()||line[0]=='#') continue; int eq=line.indexOf('='); if(eq<0) continue; String k=line.substring(0,eq), v=line.substring(eq+1); k.trim(); v.trim(); if(k=="profile") g_cfg.profile=v.toInt(); else if(k=="raw") g_cfg.output_raw = (v=="1"); else if(k=="inv"){ int a,b,c; if(sscanf(v.c_str(), "%d,%d,%d", &a,&b,&c)==3){ g_cfg.invert[0]=a; g_cfg.invert[1]=b; g_cfg.invert[2]=c; } } } f.close(); } } apply_profile(g_cfg); Serial.println("Loaded"); return; }
  ...
  if(u.startsWith("PROFILE ")){ String p=u.substring(8); p.trim(); if(p=="NORMAL") g_cfg.profile=0; else if(p=="HIGH-EMI") g_cfg.profile=1; else if(p=="URBAN") g_cfg.profile=2; else if(p=="REMOTE") g_cfg.profile=3; else { Serial.println("Bad profile"); return; } apply_profile(g_cfg); Serial.print("Profile set: "); Serial.println(profile_name(g_cfg.profile)); return; }
  if(u.startsWith("INV ")){ String rest=u.substring(4); rest.trim(); int ch=-1; if(rest.startsWith("CH1")) ch=0; else if(rest.startsWith("CH2")) ch=1; else if(rest.startsWith("CH3")) ch=2; int sp=rest.indexOf(' '); if(ch<0||sp<0){ Serial.println("INV usage: INV CH1|CH2|CH3 ON|OFF"); return;} String st=rest.substring(sp+1); st.trim(); bool on = (st=="ON"); g_cfg.invert[ch]=on; Serial.print("Invert CH"); Serial.print(ch+1); Serial.print("="); Serial.println(on?"ON":"OFF"); return; }
  Serial.println("Unknown");
}

void setup(){ Serial.begin(115200); unsigned long t0=millis(); while(!Serial && !Serial.dtr() && (millis()-t0)<8000) delay(10); Serial.println("GPIO IRQ no-DMA v2 start");
  LittleFS.begin(); g_cfg.profile=0; g_cfg.output_raw=true; g_cfg.out_format=0; g_cfg.invert[0]=g_cfg.invert[1]=g_cfg.invert[2]=false; apply_profile(g_cfg);
  ...
  // Inputs with pull-down (change to pull-up if active-low)
...
void loop(){ uint32_t now=millis(); if(now-last_blink0>1000){ last_blink0=now; sio_hw->gpio_togl=(1u<<PIN_BLINK_CORE0);} for (int i=0;i<3;i++) run_edge_correlator_and_slice(i, g_cfg, pktq[i]); process_packets(); while(Serial.available()){ char c=Serial.read(); if(c=='
