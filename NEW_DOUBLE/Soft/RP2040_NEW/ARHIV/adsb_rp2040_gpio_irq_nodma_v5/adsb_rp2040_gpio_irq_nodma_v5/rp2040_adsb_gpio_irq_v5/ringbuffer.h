...
struct Config{
  int profile; bool output_raw; int out_format; // 0 LOG, 1 CSV, 2 JSON
  int dead_time_us; int tol_us; int min_pulses; int min_win_hits; float base_thr; float ema_alpha; float dyn_k;
  bool invert[3]; float ref_lat; float ref_lon;
  float w_short_us; float w_long_us; // width filter
  uint8_t chmask; // bitmask CH1=1, CH2=2, CH3=4
  float rssi_alpha; float rssi_k; // RSSI EMA and weight
};
