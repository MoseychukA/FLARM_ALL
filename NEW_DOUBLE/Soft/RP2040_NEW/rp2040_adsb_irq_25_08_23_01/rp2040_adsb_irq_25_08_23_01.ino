...
void core1_entry()
{
    ...
    { 
        ...
        if(t-last>500000)
        { 
            last=t; sio_hw->gpio_togl=(1u<<PIN_BLINK_CORE1);
        } // nothing else, IRQs feed FIFOs
    tight_loop_contents(); 
    }
}
...
{ 
    ...
    if(u=="HELP")
    { 
        Serial.println("Commands: HELP, SHOW, SAVE, LOAD, RAW ON|OFF"); 
        return;
    } 
    if(u=="SHOW")
    {
...
static void pio0_irq_handler()
{
    uint32_t status = pio0->irq; // any source
...
static void setup_pio_irq_channel(Channel& c, PIO pio, uint sm, uint pin, int prog_off) {
    c.pio = pio; c.sm = sm; c.pin = pin; adsb_sampler_program_init(pio, sm, prog_off, pin, SAMPLE_FREQ_HZ);
...
static void process_packets_loop() {
    RawMessage rm; DecodedFrame df; if (pktq[0].pop(rm) || pktq[1].pop(rm) || pktq[2].pop(rm)) {
...
void setup()
{
    ...
    delay(2000);
    Serial.println("IRQ no-DMA variant start");
  //LittleFS.begin(); 
  //load_config(g_cfg);
