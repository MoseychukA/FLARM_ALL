// rp2040_adsb_receiver_v2_fix.ino
#include <Arduino.h>
#include <LittleFS.h>
...
static void setup_pio_dma_channel(PioDmaChannel &c, PIO pio, uint sm, uint pin_in, uint pre_pin, int8_t prog_offset){
  ...
  // Ring wrap on write address: 1024 words * 4 bytes -> 4096 bytes ring => 12 low bits
...
void setup(){
  ...
  Serial.println("RP2040 ADS-B RX v2 FIX starting...");
...
void core1_entry(){ // DMA уже запущены в dma_channel_configure(..., start=true)
