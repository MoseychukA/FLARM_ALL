...
void loop(){ uint32_t now=millis(); if(now-last_blink0>=1000){ last_blink0=now; sio_hw->gpio_togl=(1u<<PIN_BLINK_CORE0);} while(Serial.available()){ char c=Serial.read(); if(c==' '||c==' '){ if(cmd_line.length()){ handle_usb_command(cmd_line); cmd_line=""; } } else { cmd_line += c; if(cmd_line.length()>200) cmd_line.remove(0, cmd_line.length()-200); } } process_packets_loop(); }
