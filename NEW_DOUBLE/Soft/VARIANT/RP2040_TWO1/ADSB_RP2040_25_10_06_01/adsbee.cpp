...
void ADSBee::MLATCounterInit() 
{
    ...
    for (uint16_t sm_index = 0; sm_index < bsp.r1090_num_demod_state_machines; sm_index++) 
	{
        mlat_jitter_dma_channel_[sm_index] = dma_claim_unused_channel(true);  // Claim a DMA channel for each state machine.
...
void ADSBee::UpdateTLLearning() 
{
    ...
    if (tl_learning_temperature_mv_ > 0 &&
        ...
        uint16_t new_tl_offset_mv = tl_offset_mv_ + static_cast<int16_t>(get_rand_32()) * tl_learning_temperature_mv_ / INT16_MAX;
