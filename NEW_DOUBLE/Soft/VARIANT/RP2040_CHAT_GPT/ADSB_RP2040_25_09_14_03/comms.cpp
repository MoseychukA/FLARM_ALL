...
bool CommsManager::Init() 
{
    ...
    uart_set_translate_crlf(comms_uart_handle, true);     //Если true, преобразовать перевод строки в возврат каретки при передаче
    uart_init(comms_uart_handle, 115200);
...
int CommsManager::console_printf(const char *format, ...) 
{
    ...
    int res = iface_vprintf(SettingsManager::SerialInterface::Serial, format, args);
   //11 int res = iface_vprintf(SettingsManager::SerialInterface::kCommsUART, format, args);
    va_end(args);
    return res;
}
...
bool CommsManager::iface_putc(SettingsManager::SerialInterface iface, char c) 
{
    switch (iface) {
        case SettingsManager::kCommsUART:
            uart_putc_raw(comms_uart_handle, c);
...
bool CommsManager::iface_getc(SettingsManager::SerialInterface iface, char &c) 
{
    switch (iface) {
        case SettingsManager::kCommsUART:
            if (uart_is_readable_within_us(comms_uart_handle, /*config_.*/uart_timeout_us)) 
            {
                c = uart_getc(comms_uart_handle);
                return true;
            }
            return false;  // No chars to read.
            break;
        case SettingsManager::kConsole: {
            int ret = getchar_timeout_us(uart_timeout_us);
            if (ret >= 0) {
                c = (char)ret;
                return true;
            }
...
bool CommsManager::iface_puts(SettingsManager::SerialInterface iface, const char *buf) 
{
    switch (iface) {
        case SettingsManager::kCommsUART:
            uart_puts(comms_uart_handle, buf);
            return true;  // Function is void so we won't know if it succeeds.
            break;
        case SettingsManager::kConsole:
