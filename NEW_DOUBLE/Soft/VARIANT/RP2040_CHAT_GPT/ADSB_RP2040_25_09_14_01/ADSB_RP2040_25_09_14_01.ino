...
//
//static AircraftState* getAircraft1(uint32_t icao) 
//{
...
//    if (!rf.valid) return;
//    AircraftState* ac = getAircraft1(rf.icao);
//    ac->seen = millis();
//
//    // callsign
...
//    AircraftState* ac = getAircraft1(icao);
//    ac->squawk = squawk;
//    ac->seen = millis();
//}
//
...
#include "dump1090_adapter.h"   // наша структура ToDUMP1090 и конвертер

// Размер очереди подбирается — например, на 256 пакетов
static Decoded1090Packet decoded_buf[256];

PFBQueue<Decoded1090Packet> decoded_1090_packet_out_queue(
    { .buf_len_num_elements = 256,
      .buffer = decoded_buf,
...
// Подключение к внешней очереди, в которую пушатся декодированные пакеты
extern PFBQueue<Decoded1090Packet> decoded_1090_packet_out_queue;

// Словарь всех самолётов
static AircraftDictionary aircraft_dict;
...
void loop() 
{
  ...
  //  comms_manager.Update();    // Вывод расшифрованных пакетов.
    ...
    uint8_t msg[14];
    int len = 0;
    Decoded1090Packet pkt;

    // Читаем все пакеты из очереди
    while (decoded_1090_packet_out_queue.Pop(pkt)) {
        aircraft_dict.IngestDecoded1090Packet(pkt);
    }

    // Обновляем словарь (протухшие записи выкидывает сам)
    aircraft_dict.Update((uint32_t)(millis() * 1000UL));

    // Каждые EMIT_INTERVAL_MS мс печатаем состояние
    uint32_t now = millis();
    if (now - last_emit_ms >= EMIT_INTERVAL_MS) {
        last_emit_ms = now;

        for (auto& kv : aircraft_dict.dict) {
            const Aircraft1090& ac = kv.second;
            if (ac.icao_address == 0) continue;

            // Преобразуем в формат ToDUMP1090
            ToDUMP1090 dump = ConvertToDump1090(ac);

            // Вывод для отладки в USB Serial
            Serial.printf("ICAO:%06X FLT:%-8s SQ:%04o ALT:%d SPD:%d TRK:%d VRT:%d LAT:%.6f LON:%.6f\n",
                dump.addr,
                dump.flight[0] ? dump.flight : "----",
                dump.squawk,
                dump.altitude,
                dump.speed,
                dump.track,
            ...
            // Бинарно отправляем в ESP32S3 через Serial2
            Serial2.write((uint8_t*)&dump, sizeof(dump));
        }
    }
