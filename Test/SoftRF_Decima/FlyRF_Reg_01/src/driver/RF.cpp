...
byte RF_setup(void)
{
  ...
  if (rf_chip == NULL) 
  {
    ...
    if (sx1276_ops.probe())  // Проверили - sx1276 подключен
    {
      rf_chip = &sx1276_ops; // Назначить rf_chip sx1276
 #else
    if (false) 
    {
#endif
  ...
  if (rf_chip)  // Определили наличие модуля LoRa. Выполняем настройку
  {
     rf_chip->setup();
...
static bool sx1276_probe()   // Проверить, подключен ли модуль LoRa
{
  u1_t v, v_reset;
  ...
  Serial.print("RegPaConfig5 = "); Serial.println(sx1276_readReg(RegPaConfig), HEX);
  Serial.print("SX1276_RegPaDac5 = "); Serial.println(sx1276_readReg(RegPaDac), HEX);
  Serial.print("RegOcp5 = ");  Serial.println(sx1276_readReg(RegOcp), HEX);
...
static void sx12xx_setup()  // Настройка LoRa
{
  SoC->SPI_begin();
  //Serial.print("RegPaConfig5 = "); Serial.println(sx1276_readReg(RegPaConfig), HEX);
  //Serial.print("SX1276_RegPaDac5 = "); Serial.println(sx1276_readReg(RegPaDac), HEX);
  ...
  LMIC_reset();
  //Serial.print("RegPaConfig6 = "); Serial.println(sx1276_readReg(RegPaConfig), HEX);
  //Serial.print("SX1276_RegPaDac6 = "); Serial.println(sx1276_readReg(RegPaDac), HEX);
  //Serial.print("RegOcp6 = ");  Serial.println(sx1276_readReg(RegOcp), HEX);
  switch (settings->rf_protocol)
  {
  ...
  case RF_TX_POWER_FULL:
    ...
    {
      /* SX1276 не способен выдать более 20 дБм.*/
      if (LMIC.txpow > 20)
        LMIC.txpow = 20;
    }
...
#if 1
    /*
      * Применять ограничение мощности передачи до подтверждения
