...
void TFT_setup() 
{
  ...
  tbw1 = tftDC->textWidth(Current_version);
  x_tft = (tftDC->width() - tbw1) - 4;
  y_tft = tftDC->height() - tftDC->fontHeight() + 10;
  tftDC->setCursor(x_tft, y_tft);
  tftDC->print(Current_version);
...
void TFT_loop()
{
    ...
    if (!TFT_display_frontpage)
    {
      ...
      //  time_info.pushToSprite(&back, 259, 1);        // Отображаем табло времени
       // rssi_info.pushRotated(90);
       // rssi_info.pushSprite(5,5);
       // rssi_info.pushToSprite(&back, 1, 295);          // Отображаем табло уровня сигнала LoRa
       ...
       // our_plane.pushRotated(-90);
       // our_plane.pushSprite(width_air, height_air);
        our_plane.pushRotated(&back,  270, TFT_BLACK);
       // our_plane.pushToSprite(&back, width_air, height_air, TFT_BLACK);
      
       // our_plane.pushRotated(&back, -90, TFT_BLACK);
        // 
