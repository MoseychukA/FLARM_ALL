...
void Dio0_INT_IRQHandler(void)
{
	__HAL_GPIO_EXTI_CLEAR_IT(SX1276DIO0_Pin);
   DioIrqHanlderProcess( 0 ); 
}

void Dio1_INT_IRQHandler( void )
{
	__HAL_GPIO_EXTI_CLEAR_IT(SX1276DIO1_Pin);
  DioIrqHanlderProcess( 1 );
}

//static void Dio2IrqHandler( void )
//{
//	 __HAL_GPIO_EXTI_CLEAR_IT(SX1276DIO2_Pin);
//    DioIrqHanlderProcess( 2 );
//}

//static void Dio3IrqHandler( void )
//{
//	 __HAL_GPIO_EXTI_CLEAR_IT(SX1276DIO3_Pin);
    ...
    if( SX1276.Settings.Modem == MODEM_FSK )
    {
        ...
        if( rxContinuous == false )
        {
            ...
            // TimerStart( &RxTimeoutSyncWord );
						SX1276SyncWordTimeoutTimerStart(timeout);
        }
    }
    else
    {
...
static void SX1276SetTx( uint32_t timeout )
{
    ...
    //TimerStart( &TxTimeoutTimer );
		SX1276TxTimeoutTimerStart(timeout);
...
void SX1276_SPIWrite( uint8_t Data )
{
	ClrSX1276Nss();				
...
uint8_t SX1262_SPIWriteRead( uint8_t dataTX )
{
	uint8_t	dataRX;

	ClrSX1276Nss();
 	dataRX=0;
  dataRX = spi1_read_write_byte(dataTX);
...
void SX1276WriteBuffer( uint32_t addr, uint8_t *buffer, uint8_t size )
{
    ...
    SX1276_SPIWrite( addr | 0x80 );
    for( i = 0; i < size; i++ )
    {
        SX1276_SPIWrite( buffer[i] );
    }
...
void SX1276ReadBuffer( uint32_t addr, uint8_t *buffer, uint8_t size )
{
    ...
    SX1276_SPIWrite( addr & 0x7F );

    for( i = 0; i < size; i++ )
    {
        buffer[i] = SX1262_SPIWriteRead( 0 );
    }
	  SetSX1276Nss();
}
