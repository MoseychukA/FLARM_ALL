...
BITTIM	EQU	206  ; the Bit-time for selected Baudrate (in CYCLES * (1/(48000000/4))
...
; subroutine timeloop. delays for exactly 40 machine cycles times count
; parameter: R0 = timevalue/count
; we need to cover CYCLES amount of machine-cycles per stepcount in R0

timeloop:
; the first 40 cycles is done manually to include the acall and ret and MOV insrtructions
; acall = 3 + ret = 4  + MOV Rn, #data = 2   = 9
	...
	nop
	DJNZ R0, time1 ; 3
	sjmp time2  ; already zero...return
	 ; loop
time1:
	mul ab ; 5
	mul ab ; 5
