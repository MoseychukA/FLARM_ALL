...
<tr>\
<th align=left>latitude(минуты 6 знаков) </th>\
<td align=right>\
<INPUT type='number' name='test_latitudeM' min='0' max='999999' value='%d'>\
</td>\
</tr>\
<tr>\
...
<tr>\
<th align=left>longitude(минуты 6 знаков)</th>\
<td align=right>\
<INPUT type='number' name='test_longitudeM' min='0' max='999999' value='%d'>\
</td>\
</tr>\
"),
settings->freq_corr,
...
void hardwareInput() 
{
    ...
    for (uint8_t i = 0; i < server.args(); i++) 
    {
        if (server.argName(i).equals("rfc")) {
            ...
            settings->test_latitudeM = server.arg(i).toDouble();
        }
            ...
            settings->test_longitudeM = server.arg(i).toDouble();
        }
    }
