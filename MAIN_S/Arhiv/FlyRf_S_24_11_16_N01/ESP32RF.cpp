...
static void ESP32_setup()
{
 ...
 //    ESP32_BUTTON_Interrupt_handler, FALLING);
  ...
  if ((settings->mode == FLYRF_MODE_TXRX_TEST1) || (settings->mode == FLYRF_MODE_TXRX_TEST2) || (settings->mode == FLYRF_MODE_TXRX_TEST3) || (settings->mode == FLYRF_MODE_TXRX_TEST4) || (settings->mode == FLYRF_MODE_TXRX_TEST5)) //  в тестовом режиме
  {
...
static void waiting_txt() // Вывод текста "ОПРЕДЕЛЕНИЕ МЕСТОПОЛОЖЕНИЯ"
{
   ...
   // digitalWrite(SOC_GPIO_PIN_TFT_LED, HIGH);
    if (!TFT_display_frontpage) 
    {
        ...
        if (!fix_tmp && (settings->mode != FLYRF_MODE_TXRX_TEST1) && (settings->mode != FLYRF_MODE_TXRX_TEST2) && (settings->mode != FLYRF_MODE_TXRX_TEST3) && (settings->mode != FLYRF_MODE_TXRX_TEST4) && (settings->mode != FLYRF_MODE_TXRX_TEST5))
        {
            ...
            } while ((uint8_t)isValidGNSSFix());


            TFT_display_frontpage = true;

            //if (!TFT_LED_MESSAGE && !TFT_LED_BASE && !TFT_LED_GPS && !TFT_LED_TEST)
            //{
            //    digitalWrite(SOC_GPIO_PIN_TFT_LED, LOW);
            //}
            //vTaskDelay(2000);
    ...
    { /* TFT_display_frontpage  Основная программа отображения воздушной обстановки*/ 
        ...
        if (millis() - tmr > DATA_MEASURE_THRESHOLD)
        {
            ...
            uint8_t fix_tmp1 = (uint8_t)isValidGNSSFix();

            if (!fix_tmp1 && (settings->mode != FLYRF_MODE_TXRX_TEST1) && (settings->mode != FLYRF_MODE_TXRX_TEST2) && (settings->mode != FLYRF_MODE_TXRX_TEST3) && (settings->mode != FLYRF_MODE_TXRX_TEST4) && (settings->mode != FLYRF_MODE_TXRX_TEST5)) // Эта проверка не проводится в тестовом режиме
            {
                digitalWrite(SOC_GPIO_PIN_TFT_LED, HIGH);

                //bool TFT_LED_MESSAGE = false; // Управление подсветкой дисплея при приходе сообщения
                //bool TFT_LED_BASE = false; // Управление подсветкой дисплея при налмчии информации в базе
                TFT_LED_GPS = true;          // Управление подсветкой дисплея при наличии определения координат
             ...
             /*   if (!TFT_LED_MESSAGE && !TFT_LED_BASE && !TFT_LED_GPS && !TFT_LED_TEST)
               ...
               /* =========== Фильтр скорости нашего самолета. ================== */
                ...
                if (view_alien_count >= 1)
                {
                    esp_task_wdt_reset();
                    digitalWrite(SOC_GPIO_PIN_TFT_LED, HIGH);
                    TFT_LED_BASE = true; // Управление подсветкой дисплея при налмчии информации в базе
                ...
                else
                {
                    ...
                    TFT_LED_BASE = false; // Управление подсветкой дисплея при налмчии информации в базе
                    /*if (!TFT_LED_MESSAGE && !TFT_LED_BASE && !TFT_LED_GPS && !TFT_LED_TEST)
                    {
                        digitalWrite(SOC_GPIO_PIN_TFT_LED, LOW);
                    }*/
                ...
                {
                    digitalWrite(SOC_GPIO_PIN_TFT_LED, HIGH);
                    TFT_LED_MESSAGE = true;                                                  // Управление подсветкой дисплея 
