...
static int esp32_board = ESP32_S3_DEVKIT;
...
static void ESP32_post_init()
{
  ...
  //if (hw_info.model == FLYRF_MODEL_PRIME_MK3)
  //{
    Serial.println();
...
//    }
    ...
    /* AUTO and RU RF bands are deprecated since Release v1.3 */
    /*if (settings->band == RF_BAND_AUTO || settings->band == RF_BAND_RU) 
    {
...
static byte ESP32_Display_setup()
{
  byte rval = DISPLAY_NONE;
    ...
    if (u8x8) 
    {
      ...
      u8x8->draw2x2String( 2, 2 - shift_y, FlyRF_text1);

      if (shift_y) 
      {
        u8x8->drawString   ( 6, 3, FlyRF_text2);
        u8x8->draw2x2String( 2, 4, FlyRF_text3);
      }
...
#error "This ESP32 family build variant is not supported!"
 ...
 //!! &ESP32_Bluetooth_ops,
  ...
  ESP32_Baro_setup,
   NULL,//ESP32_UATSerial_begin,
   NULL,//ESP32_UATModule_restart,
  ESP32_WDT_setup,
  ESP32_WDT_fini,
  ESP32_Button_setup,
  ESP32_Button_loop,
