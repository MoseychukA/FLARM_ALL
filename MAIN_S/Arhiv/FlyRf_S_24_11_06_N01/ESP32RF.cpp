...
static void ESP32_setup()
{
 ...
 //    ESP32_BUTTON_Interrupt_handler, FALLING);
  ...
  SERIAL_TRACKER.begin(SERIAL_TRACKER_SPEED, SERIAL_IN_BITS, SOC_GPIO_PIN_TRACKER_RX, SOC_GPIO_PIN_TRACKER_TX);
...
{
    service.saveMailOn(false);               // Отключить отсчет времени удаления сообщения через 10 минут
    service.saveAllow_flashing(false);       // Запретить мигание сообщения
