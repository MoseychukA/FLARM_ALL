...
void EEPROM_defaults()
{
  ...
  eeprom_block.field.settings.rf_protocol   = hw_info.model == FLYRF_MODEL_BRACELET ?
                                              RF_PROTOCOL_FANET :
                                              hw_info.model == FLYRF_MODEL_ES ?
                                              RF_PROTOCOL_ADSB_1090 :
                                              hw_info.model == FLYRF_MODEL_HAM ?
                                              RF_PROTOCOL_APRS : RF_PROTOCOL_OGNTP;
  eeprom_block.field.settings.band          = RF_BAND_EU;
  eeprom_block.field.settings.aircraft_type = hw_info.model == FLYRF_MODEL_BRACELET ?
                                              AIRCRAFT_TYPE_STATIC :
                                              AIRCRAFT_TYPE_GLIDER;
  eeprom_block.field.settings.txpower       = hw_info.model == FLYRF_MODEL_ES ?
                                              RF_TX_POWER_OFF :
                                              hw_info.model == FLYRF_MODEL_HAM ?
  ...
  if (hw_info.model == FLYRF_MODEL_STANDALONE) {
...
#else
  eeprom_block.field.settings.nmea_out   = hw_info.model == FLYRF_MODEL_BADGE     ?
                                           NMEA_BLUETOOTH :
                                           hw_info.model == FLYRF_MODEL_ES        ?
                                           NMEA_OFF :
                                           hw_info.model == FLYRF_MODEL_ACADEMY  ||
                                           hw_info.model == FLYRF_MODEL_LEGO      ?
                                           NMEA_USB : NMEA_UART;
#endif
  ...
  eeprom_block.field.settings.d1090_new  = D1090_OFF_ON;
  eeprom_block.field.settings.json       = JSON_OFF;
  eeprom_block.field.settings.stealth    = false;
  eeprom_block.field.settings.no_track   = false;
  eeprom_block.field.settings.power_save = hw_info.model == FLYRF_MODEL_BRACELET ||
                                           hw_info.model == FLYRF_MODEL_HAM      ?
  ...
  eeprom_block.field.settings.CountNotReadMessage = 0;   // —четчик количества непрочитанных сообщений
  eeprom_block.field.settings.CurrentCountMessage = 0;  // —четчик количества сообщений
