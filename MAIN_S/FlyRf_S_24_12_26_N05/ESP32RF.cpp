...
bool SOSflashing_on_off;                     //Режим включен или потушен
...
#define ONHIGH_WE 0x0D
  */
  ...
  //pinMode(SOC_GPIO_PIN__BUTTON, INPUT /* INPUT_PULLUP */);
  //attachInterrupt(digitalPinToInterrupt(SOC_GPIO_PIN__BUTTON), ESP32_BUTTON_Interrupt_handler, FALLING); // прерывание вызывается только при смене значения на порту с HIGH на LOW
...
static void execution_state_SOS_button()
{
    if (settings->sos_view == VIEW_SOS_ON)
    {
        ...
        bool new_SOS_flag = digitalRead(SOC_GPIO_PIN_SOS);                               // Получить признак состояния кнопки SOS
        previousMillis_SOS = SOSflashing_on_off ? SOS_FLASHING_ON : SOS_FLASHING_OFF;     //

        if (new_SOS_flag)
        {
            static uint32_t tmr_flashing_SOS = millis();
            if (millis() - tmr_flashing_SOS >= previousMillis_SOS)
            {
                tmr_flashing_SOS = millis();
                if (SOSflashing_on_off == true)
                {
                    SOSflashing_on_off = false;
                    state_SOS_button.deleteSprite();
                }
                else
                {
                    SOSflashing_on_off = true;
                    display_state_SOS_button();                                                 // вызвать программу отображения состояния кнопки SOS
                }
            }

            if (SOSflashing_on_off)
            {
                display_state_SOS_button();                                                     // вызвать программу отображения состояния кнопки SOS
            }
        }
...
float battery_read()
{
   ...
   // voltage = roundf(voltage * 100) / 100;
    voltage = roundf(voltage * 100);
     ...
     /* Фильтр */
    dimension_array[array_count] = (int)voltage;
    array_count++;
    int val_voltage = 0;
    ...
    else
    {
        ...
        {
            sum_filtre += dimension_array[array_count - 1];
        }
    ...
    { /* TFT_display_frontpage  Основная программа отображения воздушной обстановки*/ 
        ...
        if (millis() - tmr > DATA_MEASURE_THRESHOLD)
        {
            ...
            else
            {
                ...
                {
                    text_call = false;                             // Готов к выводу нового сообщения 
                    location_Message.deleteSprite();               // Удаляем сообщение с экрана
                }
            }
                ...
                for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
                {
                    ...
                    if (settings->rssi_view == VIEW_RSSI_ON)
                    {
                        ...
                        if (Container[i].rssi < 0 && Container[i].signal_source == 0)
                        {
                            ...
                            back.drawString(String(Container[i].rssi) + " db", 20, 210);
                            rssi_off = true;
                        }
                   
                        if (!rssi_off)
                        {
                            back.drawString("      ", 20, 210);
                        }
                    }
                }
            }
