...
static int8_t Alarm_Distance(ufo_t *this_aircraft, ufo_t *fop)
{
  ...
  { /* нет предупреждений, если слишком высокий или слишком низкий */
...
void Traffic_Update(ufo_t *fop)
{
  ...
  //***********************
  Serial.print(" lat ");
  Serial.print(fop->latitude);
  Serial.print(" lon ");
  Serial.println(fop->longitude);
...
bool Traffic_Add(ufo_t *fop)
{
  ...
  //***********************
  Serial.print(" max_dist_ndx ");
  Serial.println(Container[max_dist_ndx].distance);
  //Serial.print(" lon ");
  //Serial.println(Container[i].longitude);
...
void ParseData()
{
    ...
    Serial.print((unsigned long)now()); StdOut.print(F(","));
    Serial.print(Bin2Hex(fo.raw, rx_size)); StdOut.print(F(","));
