...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::setup(TFTMenu* menuManager)
 {
   ...
   for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
   {
     ...
     //  Air_txt_Sprite[i]->fillSprite(backColor);      // Закрасим поле 
      // Air_txt_Sprite[i]->fillRect(0, 0, 50, 52, backColor);


      /* Air_txt_Sprite[i]->drawRect(0,0,50, 52, TFT_RED);*/
      ...
      // little_airplane[i]->fillRect(0, 0, 21, 16, backColor);
 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
   ...
   angle = 360 - 350;// test_curse;                     // Определяем направление нашего самолета и вращаем в другую сторону шкару
   ...
   for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
   {
       Air_txt_Sprite[i]->fillSprite(TFT_BLACK);       // Закрасим поле 
       little_airplane[i]->fillSprite(TFT_BLACK);      // Закрасим поле 
   }
   up_arrow.fillSprite(TFT_BLACK);              // Закрасим поле 
   arrow_down.fillSprite(TFT_BLACK);            // Закрасим поле 
...
/***************    TFT_Draw_Radar              *******************/
   ...
   // TFT_zoom = ZOOM_HIGH;
    ...
    int val=Container[0].distance;
   //  for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
    for (int i = 0; i < sizeof(Container[MAX_TRACKING_OBJECTS].distance) / sizeof(Container[0].distance); i++)
    {

         if(val<Container[i].distance)val=Container[i].distance;
     
         if(val > 4900)
         {
            TFT_zoom = ZOOM_LOWEST;
         }
         else if(val <= 4900 && val> 1900)
         {
            TFT_zoom = ZOOM_LOW;
         }
         else if(val <= 1900 && val> 900)
         {
   ...
   if (settings->m.units == UNITS_METRIC)
   {
           ...
           data_KM.drawString(TFT_zoom == ZOOM_LOWEST ? "20" :
 ...
 /*Рисуем новую картинку*/
    ...
    for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
    {
       ...
       if (Container[i].addr && (now() - Container[i].timestamp) <= TFT_EXPIRATION_TIME)
       {
           ...
           {
               alien_curse = bearing_calc(alien_latitude_old[i], alien_longitude_old[i], Container[i].latitude, Container[i].longitude);
...
float alien_longitude_old[MAX_TRACKING_OBJECTS];
 ...
 /*          Serial.print(" dbearing - ");
          ...
          /* if (altitude != Container[i].altitude_old)
           {
               Serial.print(" altitude_old - ");
               Serial.print(Container[i].altitude_old);
          ...
          //  Air_txt_Sprite[i]->drawString(String(int(altitude)), 12, 25, 1);
            Air_txt_Sprite[i]->drawString(String(int(speed)), 2, 37, 1);
            ...
            static uint32_t tmr_altitude = millis();

            if (millis() - tmr_altitude > 20000) 
            {
                tmr_altitude = millis();

                if (altitude_old[i] == altitude_old_tmr[i])
                {
                    up_down = 0;

                }

                altitude_old_tmr[i] = altitude_old[i];
            }
            ...
            case 0:
                ...
                break;
            case 1:
                up_arrow.drawLine(4, 0, 4, 10, TFT_YELLOW);
                up_arrow.drawLine(0, 4, 4, 0, TFT_YELLOW);
                up_arrow.drawLine(4, 0, 8, 4, TFT_YELLOW);
                up_arrow.pushToSprite(*Air_txt_Sprite, 0, 25, TFT_BLACK);
                Air_txt_Sprite[i]->drawString(String(int(altitude)), 12, 25, 1);
                break;
            case 2:
                arrow_down.drawLine(4, 0, 4, 10, TFT_YELLOW);
                arrow_down.drawLine(0, 6, 4, 10, TFT_YELLOW);
                arrow_down.drawLine(4, 10, 8, 6, TFT_YELLOW);
      ...
      ///*             backsprite.fillTriangle(radar_center_x + x1 - 4, radar_center_y - y1 + 3,
      //                 radar_center_x + x1, radar_center_y - y1 - 5,
      //                 radar_center_x + x1 + 4, radar_center_y - y1 + 3,
      //                 TFT_WHITE);*/
      ...
      //     {
           ...
           little_airplane[i]->drawLine(12, 1, 12, 16, little_air_color);

           little_airplane[i]->drawLine(6, 5, 15, 5, little_air_color);
           little_airplane[i]->drawLine(3, 6, 18, 6, little_air_color);
           little_airplane[i]->drawLine(0, 7, 21, 7, little_air_color);


           little_airplane[i]->drawLine(6, 15, 14, 15, little_air_color);
           little_airplane[i]->drawLine(7, 16, 15, 16, little_air_color);
  ...
  /*          Serial.print("alien_curse ");
            ...
            little_airplane[i]->pushRotated(*Air_txt_Sprite, alien_curse, TFT_BLACK);
