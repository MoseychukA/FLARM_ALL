...
TFT_eSprite mail       = TFT_eSprite(&tft);         // Спрайт отображения заряда аккумулятора 
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::setup(TFTMenu* menuManager)
 {
   ...
   for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
   {
       ...
       Air_txt_Sprite[i]->createSprite(56, 16);
       Air_txt_Sprite[i]->setPivot(28, 7);
 ...
 /*       int16_t new_angle;*/
        ...
        if(!fix && (settings->mode != SOFTRF_MODE_TXRX_TEST1) && (settings->mode != SOFTRF_MODE_TXRX_TEST2)) // Эта проверка не проводится в тестовом режиме
        {
        ...
        else
        {
                    ...
                    */
                    ...
                    if (set_table_alien[i].lat != 0 && set_table_alien[i].lon != 0)   
                    {
                        ...
                        if (new_x >= 0)  // Зона правая сторона?
                        {
                            ...
                            {
                                Air_txt_x = 30;            // Расположение текста при направлении полета от 0 до 180 градусов 
                                Air_txt_left[i] = false;
                                if (new_y <= -62) //  
                                {
                                    form_x = new_x -65; // Спрайт текста находтся слева от самолета
                                    form_y = new_y + 10; //
                                }
                                else
                                {
                            ...
                            {
                                Air_txt_x = 28;  // Расположение текста при направлении полета от 180 до 360 градусов 
                                Air_txt_left[i] = false;
                                if (new_y <= -62) //
                                {
                                    form_x = new_x + 10; // Спрайт текста  находтся справа самолета
                                    form_y = new_y + 15; //
                                }
                                else
                                {
                       ...
                       // Air_txt_Sprite[i]->drawSmoothRoundRect(0, 0, 1, 1, 55, 15, TFT_RED); //!! Только для теста
          ...
          //============================== Формируем неподвижное базовое изображение на экране =========================================== 
            ...
            mail.pushToSprite(&back, 240, 1);        // Отображаем табло курса стороннего самолета
                ...
                if (isTeam_all[i] == true)
                {
                    ...
                    if (set_table_alien[i].lat != 0 && set_table_alien[i].lon != 0)
                    {
                       ...
                       // area_airplane[i]->pushToSprite(&back, radar_center_x + Container_alien_X[i] - 15, radar_center_y - Container_alien_Y[i] - 15, TFT_BLACK);
                        arrow[i]->pushToSprite(Air_txt_Sprite[i], 1, 3, TFT_BLACK);  // Рисуем стрелку в спрайте текста
 ...
 void TFTMenuScreen::draw(TFTMenu* menuManager)
 {
   ...
   data_az.pushToSprite(&back, 185, 1);
   ...
   data_speed.pushToSprite(&back, 85, 1);
   ...
   Airplane.pushToSprite(&back, 148, 150, TFT_BLACK);
    ...
    if (!fix && (settings->mode != SOFTRF_MODE_TXRX_TEST1) && (settings->mode != SOFTRF_MODE_TXRX_TEST2))
    {
            if (!text_call)
            {
