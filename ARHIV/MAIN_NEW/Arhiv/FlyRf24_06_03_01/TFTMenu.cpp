...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
      ...
      if (new_flag)                                                                 // если новое сообщение
      {
          ...
          int cout_mail = 1;
          //for (int k = 0; k < Max_Count_Block_Message; k++)
          //{
              for (int i = 4; i < 128; i++)
              {
                  mail_tmp[cout_mail][i-4] = settings->mail[cout_mail][i];
              }
              for (int n = 0; n < 124; n++)
              {
                  Serial.print(settings->mail[cout_mail][n]);
                  //Serial.print( mail_tmp[cout_mail][n]);
              }
              //cout_mail++;
          //}
