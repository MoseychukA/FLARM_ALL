...
//#if defined(ENABLE_D1090_INPUT) ||  defined(ENABLE_RTLSDR) || defined(ENABLE_HACKRF) || defined(ENABLE_MIRISDR)
...
void D1090_Import(uint8_t *msg)
{
  ...
  struct mode_s_msg mm;
   ...
   if (state.check_crc == 0 || mm.crcok) 
   {
     ...
     */
    //printf("%d %d %02d %03d %02x%02x%02x\r\n", mm.altitude, mm.velocity, mm.msgtype, mm.msgbits, mm.aa1, mm.aa2, mm.aa3);
      ...
      int altitude = 0;
      if (mm.msgtype == 0 || mm.msgtype == 04 || mm.msgtype == 20)
      {
          altitude = mm.altitude / _GPS_FEET_PER_METER;
         // printf("%d %02d %03d %02x%02x%02x\r\n", altitude, mm.msgtype, mm.msgbits, mm.aa1, mm.aa2, mm.aa3);
      }

      for (a = state.aircrafts; a; a = a->next)
      {
          //if (/*a->even_cprtime && a->odd_cprtime &&*/ abs((long)(a->even_cprtime - a->odd_cprtime)) <= MODE_S_INTERACTIVE_TTL * 1000)
          //{
             // printf("%d %02d %03d %02x%02x%02x\r\n", altitude, mm.msgtype, mm.msgbits, mm.aa1, mm.aa2, mm.aa3);
              if (es1090_decode(a, &ThisAircraft, &fo))
              {
