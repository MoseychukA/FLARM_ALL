...
<body>\
 ...
 <table width=100%%>\
  ...
  <tr><th align=left>Версия ПО</th><td align=right>%s&nbsp;&nbsp;%s</td></tr>"
 ...
 <table width=100%%>\
  <tr>\
    ...
    <td align=center><input type=button onClick=\"location.href='/about'\" value='Авторы'></td>"), 
    ThisAircraft.addr, ver_soft1  //SOFTRF_FIRMWARE_VERSION
#if defined(USE_USB_HOST)
    "H"
#endif /* USE_USB_HOST */
...
{
    /*ver_soft1 = __FILE__;
    int val_srt = ver_soft1.lastIndexOf('\\');
    ver_soft1.remove(val_srt-2);
    val_srt = ver_soft1.lastIndexOf('\\');
    ver_soft1.remove(0, val_srt + 1); 
    val_srt = ver_soft1.lastIndexOf('.');
    ver_soft1.remove(val_srt);
    Serial.print("********");
    Serial.println(ver_soft1);*/
    
 /*   ver_soft1.reserve(ver_soft1.length() + 5);
    ver_soft1 +=MainScreen->Current_version;
    */
    


   // Serial.println(MainScreen->Current_version);
   // Serial.println(ver_soft1);
 // versionFlyRf = MainScreen->getVer();
