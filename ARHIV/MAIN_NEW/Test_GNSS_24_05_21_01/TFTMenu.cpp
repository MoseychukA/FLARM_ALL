...
TFT_eSprite Line_Spr    = TFT_eSprite(&tft);         // Спрайт  
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::setup(TFTMenu* menuManager)
 {
    ...
    tft.setRotation(3);
     ...
     txt_Heading.createSprite(319, 40);         // Спрайт  
     ...
     txt_GNSS_Spr.createSprite(170, 23);         // Спрайт  
...
//===============================================================================
     ...
     Line_Spr.createSprite(300, 10);         // Спрайт  
     Line_Spr.drawLine(0, 5, 299, 5, TFT_WHITE);
     Line_Spr.setTextColor(TFT_WHITE, backColor);
     Line_Spr.loadFont(NotoSansBold15);          // Загружаем шрифты символов направления света
     Line_Spr.setSwapBytes(true);
     Line_Spr.setTextDatum(4);

     GNSS_Spr.createSprite(40, 23);          // Спрайт  
     GNSS_Spr.drawRect(0, 0, 39, 22, TFT_DARKGREY);
     ...
     GNSS_Spr.drawString("U6", 20, 12, 0);                // Спрайт 
     ...
     txt_Heading.drawString("Test GNSS", 150, 20, 0);
     txt_GNSS_Spr.drawString("GNSS", 2, 12, 0);                     // Спрайт  
 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
   ...
   if (millis() - tmr > DATA_MEASURE_THRESHOLD)
   {
       ...
       float alt = ThisAircraft.altitude; 

       long latitude_t = Settings.getCurrentLatitude();
       float latitude_f = ThisAircraft.latitude; 

       long longitude_t = Settings.getLCurrentLongitude();
       float longitude_f = ThisAircraft.longitude; 
       ...
       const char* txt_GNSS = GNSS_name[hw_info.gnss];

       int y = 30;
       y_tft = 44;

       Line_Spr.fillSprite(backColor);                                 // Закрасим поле 
       Line_Spr.drawLine(0, 5, 299,5, TFT_WHITE);
       Line_Spr.pushToSprite(&back, 10, y_tft, TFT_BLACK);
 
       y_tft += y;

       GNSS_Spr.fillSprite(backColor);                                 // Закрасим поле 
       GNSS_Spr.drawRect(0, 0, 39, 22, TFT_DARKGREY);
       GNSS_Spr.drawString(String(txt_GNSS), 20, 12, 0);               // Спрайт  
       GNSS_Spr.pushToSprite(&back, 320 - 10 - 40, y_tft, TFT_BLACK);
       y_tft += y;
 ...
 void TFTMenuScreen::draw(TFTMenu* menuManager)
 {
     ...
     const char EPD_SoftRF_text3[] = "Test GNSS";
