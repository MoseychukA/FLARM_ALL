  ...
  if (mm->msgtype == 17) {
    ...
    else if (mm->metype == 19 && mm->mesub >= 1 && mm->mesub <= 4) 
    {
      ...
      if (mm->mesub == 1 || mm->mesub == 2) 
      {
        ...
        //Вычислите скорость и угол по двум компонентам скорости.
        mm->velocity = sqrt(mm->ns_velocity*mm->ns_velocity + mm->ew_velocity*mm->ew_velocity);
        if (mm->velocity) 
        {
...
/* Возвращаем новую структуру самолета для связанного списка интерактивного режима самолетов. */
...
struct mode_s_aircraft *interactiveReceiveData(mode_s_t *self, struct mode_s_msg *mm) 
{
    ...
    /* Найдите наш самолет или создайте новый. */
    ...
    {
        /* Если это уже известный самолет, переместим его вперед
        * поэтому мы сохраняем порядок самолетов по времени получения сообщения.
        *
        * Однако перемещайте его по голове только по истечении хотя бы одной секунды.
        * поскольку самолет, который в данный момент находится в движении, отправил сообщение,
        * в противном случае с несколькими самолетами одновременно у нас есть
        * бесполезное перемешивание позиций на экране. */
        if (0 && self->aircrafts != a && (time(NULL) - a->seen) >= 1) 
        {
            aux = self->aircrafts;
            while(aux->next != a) aux = aux->next;
            /* Теперь мы находимся перед узлом перед удалением самолета. */
    ...
    else if (mm->msgtype == 17) 
    {
        ...
        else if (mm->metype >= 9 && mm->metype <= 18)
        {
            ...
            /* Если разница между двумя данными составляет менее 10 секунд, вычислите положение. */
