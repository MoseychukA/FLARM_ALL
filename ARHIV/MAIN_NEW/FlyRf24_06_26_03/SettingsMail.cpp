...
{
	// Получить текущий номер сообщения
	uint8_t number_from_message = MemRead(Number_from_Message_ADDRESS); // получить состояние текущего счетчика
...
{
	// Сохранить текущий номер сообщения
	MemWrite(Number_from_Message_ADDRESS, count_cur);  // сохранить текущее состояние счетчика
	//MemCommit();
}
