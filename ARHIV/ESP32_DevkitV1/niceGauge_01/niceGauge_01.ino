...
#define THROTTLE 33
#define BRAKE 32
#define LEFT  35  //direction pointer
#define RIGHT 18  //direction ponter
#define SHORT 25  //headlights
#define LONG  27 //headlights light
#define GEARUP 12
#define GEARDOWN 13
#define HORN 26
...
#define right_pointer 39
#define head_lights  22
#define buzzer 22
  ...
  for(int i=0;i<360;i++)
    {
       ...
       px[i] = ((r - 14) * cos(rad * a)) + cx;  //Длина линии внутрення точка
       py[i] = ((r - 14) * sin(rad * a)) + cy;  //Длина линии внутрення точка
       px1[i] = ((r - 8) * cos(rad * a)) + cx;  //Длина линии внутрення точка
       py1[i] = ((r - 8) * sin(rad * a)) + cy;  //Длина линии внутрення точка
       lx[i]=((r-28)*cos(rad*a))+cx;  //Положение символов по кругу
       ly[i]=((r-28)*sin(rad*a))+cy;  //Положение символов по кругу
   ...
   //.....................................................Метки
     for(int i=0;i<36;i++)
     {
         ...
         color2 = TFT_WHITE;
          ...
          if(i%3==0) 
          {
             sprite.drawWedgeLine(x[i * 10], y[i * 10], px[i * 10], py[i * 10], 1, 1, color2);
             sprite.setTextColor(color2,backColor);
             sprite.drawString(String(i*10),lx[i*10],ly[i*10]);
          }
          else
          {
              sprite.drawWedgeLine(x[i * 10], y[i * 10], px1[i * 10], py1[i * 10], 1, 1, color2);
          }
          
     }

  tft.drawString("TEST", 20, 200);
