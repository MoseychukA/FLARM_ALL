...
static void tx_func (osjob_t* job) 
{
  ...
  // перепланировать задание каждые TX_INTERVAL (плюс немного случайных для предотвращения
  // систематических коллизий), если пакеты не получены, то rx_func
  // перепланирует на половину этого времени.
