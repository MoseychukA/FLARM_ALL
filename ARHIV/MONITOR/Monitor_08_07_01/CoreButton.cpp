...
// обновляем внутреннее состояние
    ...
    if (curDoubleClick != state.lastDoubleClick)        // состояние даблклика с момента последней проверки изменилось
    {
        ...
        if (state.lastDoubleClick)                      // проверяем - если кнопка не нажата, то сбрасываем счетчик нажатий 
            state.clickCounter = 0;
    }
    ...
    if (curBounce != state.lastBounce)                  // состояние проверки дребезга изменилось
    {
        ...
        if (isButtonPressed && curBounce)               // если кнопка была нажата в момент последнего замера и сейчас - значит, дребезг прошел и мы можем сохранять состояние
        {
            state.click_down = true;                    // выставляем флаг, что кнопка нажата

            ++state.clickCounter;                       // увеличиваем счетчик кликов

            if (state.clickCounter == 2)                // если кликнули два раза
            {
                state.clickCounter = 0;                 // сбрасываем счетчик кликов
