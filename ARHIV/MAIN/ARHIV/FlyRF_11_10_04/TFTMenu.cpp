...
TFT_eSprite* Alien_location[MAX_TRACKING_OBJECTS];  // Местоположение чужого самолета
TFT_eSprite* New_location[MAX_TRACKING_OBJECTS];    // Местоположение чужого самолета
TFT_eSprite* New_txt[MAX_TRACKING_OBJECTS];         // Местоположение чужого самолета
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::setup(TFTMenu* menuManager)
 {
   ...
   for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
   {
       ...
       //Alien_location[i] = new TFT_eSprite(&tft);     // Спрайт информации стороннего воздушного объекта
   /*    Alien_location[i]->createSprite(20, 160);
       Alien_location[i]->setPivot(10, 0);*/
 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
  ...
  /*                  Serial.print(i);
                ...
                //new_form_x = radar_center_x + cos(new_angle) * distance_tmr[i];
                //new_form_y = radar_center_y + sin(new_angle) * distance_tmr[i];

                ///* Расчет координат формуляра стороннего самолета */
                //new_form_x = constrain((distance_tmr[i]-50)* sin(radians(new_angle)), -32768, 32767);
                //new_form_y = constrain((distance_tmr[i]-50)* cos(radians(new_angle)), -32768, 32767);
               ...
               /* Определяем разность высот и устанавливаем цвет предупреждения*/
                ...
                New_location[i] = new TFT_eSprite(&tft);     // Спрайт информации стороннего воздушного объекта
                New_txt[i] = new TFT_eSprite(&tft);          // Спрайт информации 
                New_txt[i]->createSprite(20, 20);

                New_location[i]->createSprite(20, 160);
                if (New_location[i]->created())
                {
                    New_location[i]->setPivot(10, 0);
                    New_location[i]->fillSprite(TFT_BLACK);                  // Закрасим поле соообщений
                    New_location[i]->fillRect(0, 0, 20, 160, TFT_BLACK);
                    //New_location[i]->drawCircle(10, 150, 5, TFT_GREEN);    // Рисуем кружок символа градуса
                    //New_location[i]->drawRect(0, 0, 20, 160, TFT_WHITE);
                    New_txt[i]->setPivot(10, 10);
                    New_txt[i]->drawCircle(10, 10, 5, TFT_GREEN);           // Рисуем кружок символа градуса
                }
  /*              Serial.print("dist ");
                Serial.println(distance_tmr[i]);*/
            }
        }
...
/*
    ...
    */          // backsprite.pushRotated(&back, angle, TFT_BLACK);
               ...
               //New_location[i]->drawRect(0, 0, 20, 160, TFT_WHITE);
                ...
                little_airplane[i]->pushRotated(New_location[i], alien_curse[i], TFT_BLACK);
   /*            New_txt[i]->pushRotated(New_location[i], 0, TFT_BLACK);
               New_txt[i]->pushToSprite(New_location[i], 0, 40, TFT_BLACK);*/
               New_location[i]->pushRotated(&back, 180+new_angle, TFT_BLACK);
               New_location[i]->pushToSprite(&back, radar_center_x, radar_center_y-10, TFT_BLACK);
               // Alien_location[i]->pushRotated(&back, 180 + new_angle, TFT_BLACK);
                //Serial.print(i);
