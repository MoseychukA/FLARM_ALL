 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
        ...
        for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
        {
            if (Container[i].addr && (now() - Container[i].timestamp) <= TFT_EXPIRATION_TIME)
            {
                ...
                /* вычисляем минимальное значение дистанции для переключения диапазона просмотра */
                    ...
                    Serial.print(Container[i].distance);
  ...
  /*                isTeam_all[i] = true;
        ...
        //}
           ...
           // angle = (360 - test_curse) % 360;                       // Определяем направление нашего самолета и вращаем в другую сторону шкару
            ...
            if (settings->units == UNITS_METRIC)
            {
                data_KM.drawString(TFT_zoom == ZOOM_LOWEST ? "15" :
                    ...
                    TFT_zoom == ZOOM_HIGH ? "1 " : "", 28, 14);
            }
            else
            {
                data_KM.drawString(TFT_zoom == ZOOM_LOWEST ? "30" :
                    TFT_zoom == ZOOM_LOW ? "5 " :
                    TFT_zoom == ZOOM_MEDIUM ? "2 " : "", 28, 14);
            }
            ...
            for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
            {
                if (isThere_plane[i] == true)
                {
                    ...
                    little_airplane[i]->deleteSprite();
                    Air_txt_Sprite[i]->deleteSprite();
                    up_arrow[i]->deleteSprite();              // Спрайт отображения стрелка вверх
                    arrow_down[i]->deleteSprite();            // Спрайт отображения стрелка вниз
//                  isTeam_all[i] = false;
                    isThere_plane[i] = false;
                    esp_task_wdt_reset();
                }
