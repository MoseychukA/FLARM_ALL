...
int alien_curse_in[MAX_TRACKING_OBJECTS];
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::setup(TFTMenu* menuManager)
 {
   ...
   for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
   {
       ...
       area_txt_airplane[i]->createSprite(56, 28);
       area_txt_airplane[i]->setPivot(28, 14);
 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
   ...
   static uint32_t tmr = millis();
    ...
    if (millis() - tmr > DATA_MEASURE_THRESHOLD)
    {
        ...
        if (ThisAircraft.speed >= 0 && ThisAircraft.speed < 10)
        {
        ...
        for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
        {
            if (Container[i].addr)
            {
              ...
              //  bearing_tmr1[i] = (int)Container[i].bearing;   // угол в градусах между нашим самолетом и сторонним
                distance_tmr[i] = (int)Container[i].distance;    // дистанция между нашим самолетом и сторонним

                /* вычисляем угол в градусах между нашим самолетом и сторонним */
                //if (settings->pointer == DIRECTION_TRACK_UP)
                //{
                  //  bearing_tmr[i] = (360 + bearing_tmr[i] - (int)ThisAircraft.course) % 360; // % 360 - Остаток от деления.
                //}
                ...
                //{
                //    alien_curse_in[i] = bearing_calc(alien_latitude_old[i], alien_longitude_old[i], Container[i].latitude, Container[i].longitude);

                //    alien_latitude_old[i] = Container[i].latitude;
                //    alien_longitude_old[i] = Container[i].longitude;

                //    //Serial.print("alien_curse_in ");
                //    //Serial.print(alien_curse_in[i]);

                //    /* Корректируем курс (градусы) согласно углу поворота картинки*/


                //    if (speed_tmr[i] >= 0 && speed_tmr[i] < 10)      // if(bearing_tmr[i])
                //    {
                //        alien_curse[i] = bearing_tmr[i] + 180;
                //    }


                //    alien_curse[i] = angle + alien_curse_in[i];
                ...
                Serial.print(" Container[i].course ");
                Serial.print(Container[i].course);

                if (speed_tmr[i] >= 0 && speed_tmr[i] < 10)         // if(bearing_tmr[i])
                {

                   // Container[i].course = bearing_tmr[i];
                    alien_curse[i] = (360 - angle + bearing_tmr[i]-180) % 360;
                }
                else
                {

                    alien_curse[i] = (360 - angle + (int)Container[i].course) % 360;
                }
                    //if (alien_curse[i] > 360)
                    //{
                    //    alien_curse[i] -= 360;
                ...
                Serial.print(" alien_curse[i] ");
                Serial.println(alien_curse[i]);
                ...
                int delta_dist = distance_tmr[i] / 100 * 5;  // Увеличим дистанцию на 5% для вывода формуляра
               ...
               /* Определяем разность высот и устанавливаем цвет предупреждения*/
                ...
                {
                    Air_txt_Sprite[i]->drawString("+" + String(int(Height_difference[i])), 7, 0, 1);
                }
                else
                {
                    Air_txt_Sprite[i]->drawString(String(int(Height_difference[i])), 7, 0, 1);
                ...
                case 1:
                    ...
                    up_arrow[i]->pushToSprite(Air_txt_Sprite[i], 0, 0, TFT_BLACK);
                    break;
                case 2:
                    ...
                    arrow_down[i]->pushToSprite(Air_txt_Sprite[i], 0, 0, TFT_BLACK);
                    break;
                default:
                    break;
                }
        ...
        if (arr_min != 32767)
        {
            ...
            dist_info.pushToSprite(&back,Text_location, 218, TFT_BLACK);
        }
        else
        {
