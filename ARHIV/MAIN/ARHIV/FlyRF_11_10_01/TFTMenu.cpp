...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::setup(TFTMenu* menuManager)
 {
   ...
   for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
   {
       ...
       Alien_location[i]->createSprite(24, 18);
       Alien_location[i]->setPivot(12, 9);
 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
   ...
   static uint32_t tmr = millis();
    ...
    if (millis() - tmr > DATA_MEASURE_THRESHOLD)
    {
      ...
      //  data_KM.pushToSprite(&back, 3, 1); // Записываем информацию в спрайт
        ...
        if(*isThere_plane == true)
        {
            ...
            for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
            {
                ...
                /*Функция проверяет и если надо задает новое значение, так чтобы оно была в области допустимых значений, заданной параметрами.*/
                int16_t new_angle = angle + bearing_tmr[i];
                if (new_angle > 360)
                {
                    new_angle -= 360;
                }
                ...
                Container_logbook_X[i] = new_x;  // Сохранить координаты формуляра стороннего самолета
                Container_logbook_Y[i] = new_y;

                /* Расчет координат формуляра стороннего самолета */
                //new_form_x = constrain((distance_tmr[i])* sin(radians(new_angle)), -32768, 32767);
                //new_form_y = constrain((distance_tmr[i])* cos(radians(new_angle)), -32768, 32767);
               ...
               /* Определяем разность высот и устанавливаем цвет предупреждения*/
                ...
                Alien_location[i]->fillSprite(TFT_BLACK);                        // Закрасим поле соообщений
        ...
        {
           arr_min = 32767; // первоначально будем сравнивать
        }
       ...
       // angle = (360 - test_curse) % 360;                       // Определяем направление нашего самолета и вращаем в другую сторону шкалу
        ...
        for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
        {
            if (isTeam_all[i] == true)
            {
               ...
               // Alien_location
                little_airplane[i]->pushRotated(Alien_location[i], alien_curse[i], TFT_BLACK);


                Alien_location[i]->pushToSprite(&back, radar_center_x + Container_alien_X[i] - 2, radar_center_y - Container_alien_Y[i] - 2, TFT_BLACK);
               ...
               /* Air_txt_Sprite[i]->pushRotated(Alien_location[i], alien_curse[i], TFT_BLACK);*/
