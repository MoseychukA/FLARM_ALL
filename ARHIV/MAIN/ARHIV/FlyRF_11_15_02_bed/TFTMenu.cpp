...
TFT_eSprite* area_txt_base[MAX_TRACKING_OBJECTS];      // Этот спрайт, площадка в котором будет располагатся формуляр стороннего самолета
...
int agle_txt[MAX_TRACKING_OBJECTS];
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::setup(TFTMenu* menuManager)
 {
   ...
   for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
   {
       ...
       Air_txt_Sprite[i]->createSprite(50, 26);
       Air_txt_Sprite[i]->setPivot(25, 13);

       up_arrow[i] = new TFT_eSprite(&tft);           // Спрайт информации стороннего воздушного объекта
       up_arrow[i]->createSprite(8, 8);              // Спрайт отображения стрелка вверх

       arrow_down[i] = new TFT_eSprite(&tft);         // Спрайт информации стороннего воздушного объекта
       arrow_down[i]->createSprite(8, 8);            // Спрайт отображения стрелка вниз
       ...
       area_airplane[i]->createSprite(150, 150);
       area_airplane[i]->setPivot(75, 75);

       area_txt_airplane[i] = new TFT_eSprite(&tft);   // Этот спрайт, площадка в котором будет располагатся формуляр стороннего самолета
       area_txt_airplane[i]->createSprite(60, 120);
       area_txt_airplane[i]->setPivot(30, 0);

       area_txt_base[i] = new TFT_eSprite(&tft);   // Этот спрайт, площадка в котором будет располагатся формуляр стороннего самолета
       area_txt_base[i]->createSprite(60, 60);
       area_txt_base[i]->setPivot(30, 30);
   ...
   static uint32_t tmr = millis();
    ...
    if (millis() - tmr > DATA_MEASURE_THRESHOLD)
    {
      ...
      //  data_KM.setTextDatum(TC_DATUM);
        ...
        else if (divider <= 600 && divider > 150)
        {
          //  data_KM.drawString("200 m", 40, 1);
            divider = 600;
        }
        else if (divider <= 150 )
        {
          //  data_KM.drawString("50 m", 40, 1);
            divider = 150;
        }


        /* При малой скорости нашего самолета поворачиваем экран на отметку 360 */
        if (ThisAircraft.speed >= 0 && ThisAircraft.speed < 4)
        {
        ...
        for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
        {
            ...
            /* При малой скорости смотрим в центр экрана на наш самолет*/
                ...
                agle_txt[i] = (180 + angle + bearing_tmr[i]) % 360;
                ...
                int delta_dist = 0;// (distance_tmr[i]) + (4 * (int32_t)radius / divider);  // Увеличим дистанцию на 10% для вывода формуляра

               // delta_dist = delta_dist * 50;

                Serial.print("delta_dist ");
                Serial.println(delta_dist);


               // int delta_dist = distance_tmr[i] / 100 * 10;  // Увеличим дистанцию на 10% для вывода формуляра
                new_form_x = constrain((distance_tmr[i]+ delta_dist)* sin(radians(new_angle)), -32768, 32767);
                new_form_y = constrain((distance_tmr[i]+ delta_dist)* cos(radians(new_angle)), -32768, 32767);

                form_x = ((int32_t)new_form_x * (int32_t)radius) / divider;
                form_y = ((int32_t)new_form_y * (int32_t)radius) / divider;

                 /* определение места вывода формуляра */

                //if (form_x < 0)  // Зона левая сторона
                //{
                //    if (form_y >= 80)  //
                //    {
                //        form_x = form_x - 15;  //
                //        form_y = form_y + 30;  //
                //    }
                //    else if (form_y <= 80 && form_y > 50) //ok
                //    {
                //        form_x = form_x -20; //1
                //        form_y = form_y + 25;     //
                //    }
                //    else if (form_y <= 50 && form_y> -67) //ok
                //    {
                //        form_x = form_x - 30; //
                //        form_y = form_y ;     //
                //    }
                //    else if(form_y <= -67)   // ok
                //    {
                //        form_x = form_x -30 ; //
                //        form_y = form_y +10; //1
                //    }
                //}
                //else //Зона правая сторона
                //{
                //    if (form_y >= 80)    //
                //    {
                //        form_x = form_x + 10; //
               ...
               /* Определяем разность высот и устанавливаем цвет предупреждения*/
                ...
                {
                    Air_txt_Sprite[i]->drawString("+" + String(int(Height_difference[i])), 8, 0, 1);
                }
                else
                {
                    Air_txt_Sprite[i]->drawString(String(int(Height_difference[i])), 8, 0, 1);
                ...
                /* Фильт скорости стороннего самолета */
                 

                Air_txt_Sprite[i]->drawString(String(speed_tmr[i]), 5, 13, 1);    // Записать скорость в формуляр
                ...
                case 1:
                    ...
                    up_arrow[i]->drawLine(4, 0, 4, 8, little_air_color[i]);
                    up_arrow[i]->drawLine(0, 4, 4, 0, little_air_color[i]);
                    up_arrow[i]->drawLine(4, 0, 8, 4, little_air_color[i]);
                    up_arrow[i]->pushToSprite(Air_txt_Sprite[i], 0, 2, TFT_BLACK);
                    break;
                case 2:

                    /*Рисуем стрелку вниз */
                    arrow_down[i]->drawLine(4, 0, 4, 8, little_air_color[i]);
                    arrow_down[i]->drawLine(0, 6, 4, 8, little_air_color[i]);
                    arrow_down[i]->drawLine(4, 8, 8, 6, little_air_color[i]);
                    arrow_down[i]->pushToSprite(Air_txt_Sprite[i], 0, 2, TFT_BLACK);
                    break;
                default:
                    break;
                }
                ...
                area_txt_base[i]->fillSprite(TFT_BLACK);      // Закрасим поле 
  ...
  //      data_KM.fillSprite(backColor);
        ...
        for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
        {
            if (isTeam_all[i] == true)
            {
              ...
              // area_airplane[i]->drawRect(0, 0, 150, 150, TFT_YELLOW);
              // area_txt_airplane[i]->drawRect(0, 0, 60, 60, TFT_BLUE);
              // area_txt_base[i]->drawRect(0, 0, 60, 60, TFT_ORANGE);
              // Air_txt_Sprite[i]->drawRect(0, 0, 50, 26, TFT_WHITE);
               Air_txt_Sprite[i]->pushRotated(area_txt_base[i], agle_txt[i] - (agle_txt[i] * 2), TFT_BLACK);
               area_txt_base[i]->pushToSprite(area_txt_airplane[i], 0, 5, TFT_BLACK);
               area_txt_airplane[i]->pushRotated(area_airplane[i], agle_txt[i], TFT_BLACK);
               area_airplane[i]->pushToSprite(&back, radar_center_x + Container_alien_X[i] - 75, radar_center_y - Container_alien_Y[i] - 75, TFT_BLACK);
