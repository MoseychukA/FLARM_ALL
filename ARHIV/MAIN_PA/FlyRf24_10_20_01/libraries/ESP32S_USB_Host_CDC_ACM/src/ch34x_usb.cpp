...
#define NANJING_QINHENG_MICROE (0x1A86)
...
namespace esp_usb {
CH34x *CH34x::open_ch34x(uint16_t pid, const cdc_acm_host_device_config_t *dev_config, uint8_t interface_idx)
{
    return new CH34x(pid, dev_config, interface_idx);
}

CH34x::CH34x(uint16_t pid, const cdc_acm_host_device_config_t *dev_config, uint8_t interface_idx)
    : intf(interface_idx)
{
    const esp_err_t err = this->open_vendor_specific(NANJING_QINHENG_MICROE, pid, this->intf, dev_config);
    if (err != ESP_OK) {
        throw(err);
    }
};
...
esp_err_t CH34x::line_coding_set(cdc_acm_line_coding_t *line_coding)
{
    ...
    switch (line_coding->bDataBits) {
        case 5: lcr |= CH34x_LCR_CS5; break;
        case 6: lcr |= CH34x_LCR_CS6; break;
        case 7:	lcr |= CH34x_LCR_CS7; break;
        case 8:	lcr |= CH34x_LCR_CS8; break;
        default: return ESP_ERR_INVALID_ARG;
    }

    switch (line_coding->bParityType) {
        case 0: break;
        case 1: lcr |= CH34x_LCR_ENABLE_PAR; break;
        case 2: lcr |= CH34x_LCR_ENABLE_PAR | CH34x_LCR_PAR_EVEN; break;
        case 3: // Mark
        case 4: lcr |= CH34x_LCR_ENABLE_PAR | CH34x_LCR_MARK_SPACE; break;
        default: return ESP_ERR_INVALID_ARG;
    }

    switch (line_coding->bCharFormat) {
        case 0: break; // 1 Stop bit
        case 2: lcr |= CH34x_LCR_STOP_BITS_2; break;
        default: return ESP_ERR_INVALID_ARG; // 1.5 stop bits not supported
    }
