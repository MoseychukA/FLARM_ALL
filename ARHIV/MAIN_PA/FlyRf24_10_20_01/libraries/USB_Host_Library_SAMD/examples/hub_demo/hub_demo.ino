...
uint8_t getdevdescr( uint8_t addr, uint8_t &num_conf )
{
  ...
  uint8_t rcode;
  rcode = UsbH.getDevDescr( addr, 0, DEV_DESCR_LEN, ( uint8_t *)&buf );
  if ( rcode ) {
    return ( rcode );
  }
...
uint8_t getallstrdescr(uint8_t addr)
{
  uint8_t rcode = 0;
  UsbH.Task();
  if ( UsbH.getUsbTaskState() >= USB_STATE_CONFIGURING ) { // state configuring or higher
    USB_DEVICE_DESCRIPTOR buf;
    rcode = UsbH.getDevDescr( addr, 0, DEV_DESCR_LEN, ( uint8_t *)&buf );
    if ( rcode ) {
      return ( rcode );
    }
    SerialDebug.println("String Descriptors:");
    if ( buf.iManufacturer > 0 ) {
      SerialDebug.print("Manufacturer:\t\t");
...
uint8_t getstrdescr( uint8_t addr, uint8_t idx )
{
  ...
  uint8_t i;
  uint16_t langid;
  rcode = UsbH.getStrDescr( addr, 0, 1, 0, 0, buf );  //get language table length
  if ( rcode ) {
    SerialDebug.println("Error retrieving LangID table length");
    return ( rcode );
  }
  length = buf[ 0 ];      //length is the first byte
  rcode = UsbH.getStrDescr( addr, 0, length, 0, 0, buf );  //get language table
  if ( rcode ) {
    SerialDebug.print("Error retrieving LangID table ");
    return ( rcode );
  }
  langid = (buf[3] << 8) | buf[2];
  rcode = UsbH.getStrDescr( addr, 0, 1, idx, langid, buf );
  if ( rcode ) {
    SerialDebug.print("Error retrieving string length ");
    return ( rcode );
  }
  length = buf[ 0 ];
  rcode = UsbH.getStrDescr( addr, 0, length, idx, langid, buf );
  if ( rcode ) {
    SerialDebug.print("Error retrieving string ");
    return ( rcode );
  }
