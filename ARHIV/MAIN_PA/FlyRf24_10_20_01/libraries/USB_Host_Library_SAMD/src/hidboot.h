	...
	uint32_t rcode = 0;
        ...
        if(bPollEnable && ((long)(millis() - qNextPollTime) >= 0L)) {
                ...
                for(int i = 0; i < epMUL(BOOT_PROTOCOL); i++) {
                        ...
                        uint8_t buf[const_buff_len] __attribute__((aligned(4)));
                        ...
                        USBTRACE3("(hidboot.h) epInfo[epInterruptInIndex + i].bmAttribs=", epInfo[epInterruptInIndex + i].bmAttribs, 0x81);
