...
void XBOXONE::EndpointXtract(uint32_t conf, uint32_t iface, uint32_t alt, uint32_t proto, const USB_ENDPOINT_DESCRIPTOR *pep) {
        bConfNum = conf;
        uint8_t index;
...
#ifdef DEBUG_USB_HOST
                else if(rcode != USB_ERRORFLOW) { // Not a matter of no update to send
