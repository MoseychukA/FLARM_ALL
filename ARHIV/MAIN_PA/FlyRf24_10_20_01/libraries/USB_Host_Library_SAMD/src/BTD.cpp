...
uint32_t BTD::ConfigureDevice(uint32_t parent, uint32_t port, uint32_t lowspeed) {
...
FailHub:
#ifdef DEBUG_USB_HOST
        Notify(PSTR("\r\nPlease create a hub instance in your code: \"USBHub Hub1(&UsbH);\""), 0x80);
#endif
...
void BTD::EndpointXtract(uint32_t conf, uint32_t iface __attribute__((unused)), uint32_t alt, uint32_t proto __attribute__((unused)), const USB_ENDPOINT_DESCRIPTOR *pep) {
