...
{
    HAL_SPI_Transmit(&g_spi1_handler, &txdata, 1,1000);
}
...
void spi1_init(void)
{
    ...
    g_spi1_handler.Init.CLKPolarity = SPI_POLARITY_LOW;               /* 串行同步时钟的空闲状态为高电平 */
...
void SX1276IoInit( void )
{
	...
	spi1_init();                        /* 初始化SPI1 */
	spi1_set_speed(SPI_SPEED_4);        /* SPI1 切换到高速状态 9Mhz */
	
	gpio_init_struct.Pin = SX1276Nss_Pin;                /* Nss引脚 */
	gpio_init_struct.Mode = GPIO_MODE_OUTPUT_PP;         /* 推挽输出 */
	gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;       /* 高速 */
	HAL_GPIO_Init(SX1276_Port, &gpio_init_struct);         
