...
byte OLED_setup() {
  ...
  {
    u8x8 = &u8x8_i2c;
    rval = (hw_info.model == FLYRF_MODEL_MINI     ? DISPLAY_OLED_HELTEC :
            hw_info.model == FLYRF_MODEL_BRACELET ? DISPLAY_OLED_0_49   :
            DISPLAY_OLED_TTGO);
  }
...
static void OLED_text()
{
  ...
  bool hasFix = isValidGNSSFix() || (settings->mode == FLYRF_MODE_TXRX_TEST);
...
#if !defined(EXCLUDE_OLED_049)
    case DISPLAY_OLED_0_49:
      u8x8->draw2x2String(5, 5, reason == FLYRF_SHUTDOWN_LOWBAT ?
                                "BAT" : "OFF");
      delay(2000);
      u8x8->noDisplay();
      break;
    ...
    case DISPLAY_OLED_0_49:
      {
        ...
        const char buf[] = FLYRF_FIRMWARE_VERSION;
