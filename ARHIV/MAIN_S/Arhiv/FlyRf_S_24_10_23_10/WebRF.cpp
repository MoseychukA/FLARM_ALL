...
<option %s value='%d'>UAV</option>\
  ...
  (settings->mode == FLYRF_MODE_TXRX_TEST ? "selected" : ""), FLYRF_MODE_TXRX_TEST,
  (settings->mode == FLYRF_MODE_BRIDGE ? "selected" : ""), FLYRF_MODE_BRIDGE,
  (settings->mode == FLYRF_MODE_UAV ? "selected" : ""), FLYRF_MODE_UAV
/*  (settings->mode == FLYRF_MODE_WATCHOUT ? "selected" : ""), FLYRF_MODE_WATCHOUT, */
  );
...
<td align=right>%s\
    ...
    (settings->rf_protocol == RF_PROTOCOL_ADSB_UAT ? fanet_proto_desc.name : //!!
