...
bool ClearButton1 = false;

bool TFT_LED_MESSAGE = false;         // Управление подсветкой дисплея при приходе сообщения
bool TFT_LED_ALIEN    = false;         // Управление подсветкой дисплея при налмчии информации в базе
...
//static void IRAM_ATTR ESP32_BUTTON_Interrupt_handler() {
//    portENTER_CRITICAL_ISR(&BUTTON_mutex);
//    BUTTON_Irq = true;
//    portEXIT_CRITICAL_ISR(&BUTTON_mutex);
//}
...
static void ESP32_setup()
{
  ...
  //attachInterrupt(digitalPinToInterrupt(38), //Прерывание по кнопке SOC_GPIO_PIN_TBEAM_V08_BUTTON
 //    ESP32_BUTTON_Interrupt_handler, FALLING);
 ...
 // TFT_LED_TEST = false;         // Управление подсветкой дисплея в тестовом режиме
...
{
    if (TFT_LED_TEST == true || TFT_LED_ALIEN == true)
    {
        no_emptyColor = TFT_DARKGREY;
    }
    else
    {
        no_emptyColor = backColor;
    }
    ...
    for (int i = 0; i < 36; i++)
    {
        ...
        if (i % 3 == 0)
        {
            backsprite.drawWedgeLine(fx[i * 10], fy[i * 10], px[i * 10], py[i * 10], 1, 1, no_emptyColor);
            backsprite.setTextColor(no_emptyColor, TFT_BLACK);
            if (i == 0)
            {
                backsprite.drawString("N", lx[i * 10] + 1, ly[i * 10], no_emptyColor);
            }
            if (i == 9)
            {
                backsprite.drawString("E", lx[i * 10], ly[i * 10], no_emptyColor);
            }
            if (i == 18)
            {
                backsprite.drawString("S", lx[i * 10], ly[i * 10], no_emptyColor);
            }
            if (i == 27)
            {
                backsprite.drawString("W", lx[i * 10], ly[i * 10], no_emptyColor);
            }
        }
        else
        {
            backsprite.drawWedgeLine(fx[i * 10], fy[i * 10], px1[i * 10], py1[i * 10], 1, 1, no_emptyColor);
        }
    }
    esp_task_wdt_reset();

    /*Рисуем малый серый круг*/
    backsprite.drawCircle(cx, 160, 80, no_emptyColor);

}

static void waiting_txt() // Вывод текста определения координат
{
    /* Определение местоположения нашего самолета*/
                 //  
    ...
    int textFontWidth = tft.textWidth(data_txt1, 2);

    uint16_t curX = 319;
    uint16_t curY = 239;

    curX = (screenWidth / 2) - (textFontWidth / 2) - 38;          // Координаты вывода 
    curY = 80;                                                    // Координаты вывода текста
    tft.setCursor(curX, curY),                                    // Set cursor for tft.print()
    tft.print(data_txt1);                                         // Отображаем 

    textFontWidth = tft.textWidth(data_txt2, 2);
    curX = (screenWidth / 2) - (textFontWidth / 2) - 50;          // Координаты вывода 
    curY += 30;                                                   // Координаты вывода текста
    tft.setCursor(curX, curY),                                    // Set cursor for tft.print()
    tft.print(data_txt2);                                         // Отображаем 

    textFontWidth = tft.textWidth(data_txt3, 2);
    curX = (screenWidth / 2) - (textFontWidth / 2) - 12;          // Координаты вывода 
    curY += 30;                                                   // Координаты вывода текста
    tft.setCursor(curX, curY),                                    // Set cursor for tft.print()
    tft.print(data_txt3);                                         // Отображаем 
}

static void waiting_txt1() // Вывод текста определения координат. Убираем косяк библиотеки
{
    /* Определение местоположения нашего самолета*/
                 //  
    int screenWidth = tft.width();
...
static void ESP32_Display_loop()
{
    ...
    if (!TFT_display_frontpage) 
    {
        ...
        if((settings->mode == FLYRF_MODE_TXRX_TEST1) || (settings->mode == FLYRF_MODE_TXRX_TEST2) || (settings->mode == FLYRF_MODE_TXRX_TEST3) || (settings->mode == FLYRF_MODE_TXRX_TEST4) || (settings->mode == FLYRF_MODE_TXRX_TEST5))
        {
            TFT_LED_TEST = true;         // Управление подсветкой дисплея в тестовом режиме
        }
    ...
    { /* TFT_display_frontpage  Основная программа отображения воздушной обстановки*/ 
        ...
        bool setMessageRead = service.getMessageRead();
        ...
        // Удалить сообщение
        bool setClearMessage = service.get_ClearMessage();
        if (setClearMessage)
        {
            service.setClearMessage(false);
            clearMSG(); // Удалить все  сообщения
        }

        uint8_t count_buttton1 = service.get_count_buttton();
        if (count_buttton1 != 0)
        {
            ...
            {
                button_tmr = millis();
                service.set_count_buttton(0);
                service.setClearButton(true);
            }
        }
        ...
        if (millis() - tmr > DATA_MEASURE_THRESHOLD)
        {
            ...
            if (!fix_tmp && (settings->mode != FLYRF_MODE_TXRX_TEST1) && (settings->mode != FLYRF_MODE_TXRX_TEST2) && (settings->mode != FLYRF_MODE_TXRX_TEST3) && (settings->mode != FLYRF_MODE_TXRX_TEST4) && (settings->mode != FLYRF_MODE_TXRX_TEST5)) // Эта проверка не проводится в тестовом режиме
            {
                ...
                if (millis() - tmr_GNSS > 15000)
                {
                    ...
                    if (!text_call)
                    {
                        waiting_txt1();  // Вывод сообщения о том что нет данных GPS
            ...
            else
            {
               ...
               /* =========== Фильтр скорости нашего самолета. ================== */
                ...
                if (view_alien_count >= 1)
                {
                    ...
                    uint8_t count_buttton2 = service.get_count_buttton();
                    if (count_buttton2 == 0)
                    {
                ...
                uint8_t count_buttton3 = service.get_count_buttton();
                if (count_buttton3 != 0)
                {
                    switch (count_buttton3)
                    {
                    case 1:
                        divider = 32000;  // 32000
                        divider_num = 1;
                ...
                if (divider <= 32767)
                {
                    ...
                    back.setTextColor(no_emptyColor, TFT_BLACK);
                ...
                back.drawLine(12 + width_air, 0 + height_air, 12 + width_air, 18 + height_air, no_emptyColor);

                /*Рисуем передние крылья*/
                back.drawLine(3 + width_air, 7 + height_air, 20 + width_air, 7 + height_air, no_emptyColor);
                back.drawLine(0 + width_air, 8 + height_air, 23 + width_air, 8 + height_air, no_emptyColor);

                /*Рисуем задние крылья*/
                back.drawLine(7 + width_air, 17 + height_air, 17 + width_air, 17 + height_air, no_emptyColor);
...
AceButton button_1(SOC_GPIO_PIN__BUTTON);

// The event handler for the button.
void handleMainEvent(AceButton* button, uint8_t eventType,  uint8_t buttonState) 
{
  switch (eventType) 
  {
    ...
    case AceButton::kEventDoubleClicked:
        ClearButton1 = service.get_ClearButton();
        if (ClearButton1)
        {
            count_buttton_tmp = 0;
            service.setClearButton(false);
        }
        count_buttton_tmp++;
        if (count_buttton_tmp > 6)
        {
            count_buttton_tmp = 0;
        }
        service.set_count_buttton(count_buttton_tmp);
      break;
    case AceButton::kEventLongPressed:
         service.setClearMessage(true);
      break;
  }
}

void handleAuxEvent(AceButton* button, uint8_t eventType, uint8_t buttonState) 
{
  switch (eventType) 
  {
    case AceButton::kEventClicked:
    case AceButton::kEventReleased:
        //Serial.println("kEventClicked2");
      break;
    case AceButton::kEventDoubleClicked:
        //Serial.println("kEventDoubleClicked2");
      break;
  }
}
...
{
  int button_pin = SOC_GPIO_PIN__BUTTON;
