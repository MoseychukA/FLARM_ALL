...
void parseSettings(JsonObject& root)
{
  ...
  if (mode.success()) {
    ...
    if (!strcmp(mode_s,"NORMAL")) {
      ...
      eeprom_block.field.settings.mode = FLYRF_MODE_BRIDGE;
    } else if (!strcmp(mode_s,"TEST")) {
      eeprom_block.field.settings.mode = FLYRF_MODE_TXRX_TEST;
    } else if (!strcmp(mode_s,"RELAY")) {
      eeprom_block.field.settings.mode = FLYRF_MODE_RELAY;
    }
  }
