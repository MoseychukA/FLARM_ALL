...
<td align=right>\
<INPUT type='number' name='test_latitude' min='0' max='90' value='%d'>\
</td>\
</tr>\
<tr>\
<th align=left>latitude(минуты 5 знаков) </th>\
<td align=right>\
<INPUT type='number' name='test_latitudeM' min='0' max='99999' value='%d'>\
</td>\
</tr>\
<tr>\
...
<td align=right>\
<INPUT type='number' name='test_longitude' size='3' maxlength='3' min='0' max='180' value='%d'>\
</td>\
</tr>\
<tr>\
<th align=left>longitude(минуты 5 знаков)</th>\
<td align=right>\
<INPUT type='number' name='test_longitudeM' min='0' max='99999' value='%d'>\
</td>\
</tr>\
"),
settings->freq_corr,
...
void handleRoot() {
    ...
    dtostrf(ThisAircraft.test_latitude, 8, 6, str_test_lat);
    dtostrf(ThisAircraft.test_longitude, 8, 6, str_test_lon);
 ...
 <table width=100%%>\
  <tr>\
    ...
    <td align=center><input type=button onClick=\"location.href='/about'\" value='Информация'></td>"),
        ...
        str_lat, str_lon, str_test_lat, str_test_lon, str_alt
    );
...
void hardwareInput() 
{
    ...
    for (uint8_t i = 0; i < server.args(); i++) 
    {
        if (server.argName(i).equals("rfc")) {
            ...
            settings->test_latitude = server.arg(i).toInt();
        }
        else if (server.argName(i).equals("test_latitudeM")) {
            settings->test_latitudeM = server.arg(i).toInt();
        }
            ...
            settings->test_longitude = server.arg(i).toInt();
        }
        else if (server.argName(i).equals("test_longitudeM")) {
            settings->test_longitudeM = server.arg(i).toInt();
        }
    }
...
<tr><th align=left>latitude(минуты)</th><td align=right>%d</td></tr>\
...
<tr><th align=left>longitude(минуты)</th><td align=right>%d</td></tr>\
</table>\
<hr>\
 <p align=center><h2 align=center>Выполняется перезагрузка... Пожалуйста, подождите!</h2></p>\
</body>\
