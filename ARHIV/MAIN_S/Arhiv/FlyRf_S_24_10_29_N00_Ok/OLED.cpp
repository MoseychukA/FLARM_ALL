...
const char SoftRF_text3[]  = "Decima";
...
byte OLED_setup() 
{
  ...
  if (u8x8) 
  {
    ...
    u8x8->setFont(u8x8_font_chroma48medium8_r);
      uint8_t shift_y = (hw_info.model == FLYRF_MODEL_DONGLE ? 1 : 0);
      ...
      //u8x8->drawString   (11, 6 + shift_y, "RU");
...
static void OLED_radio()
{
  ...
  Serial.println("*** OLED_radio ");
 // Serial.println(OLED_current_page);
  ...
  /*  if (settings->mode  == FLYRF_MODE_RECEIVER ||  settings->rf_protocol == RF_PROTOCOL_ADSB_UAT || settings->txpower     == RF_TX_POWER_OFF) {
...
static void OLED_text()
{
  ...
  bool hasFix = isValidGNSSFix() || (settings->mode == FLYRF_MODE_TXRX_TEST1) || (settings->mode == FLYRF_MODE_TXRX_TEST2) || (settings->mode == FLYRF_MODE_TXRX_TEST3) || (settings->mode == FLYRF_MODE_TXRX_TEST4) || (settings->mode == FLYRF_MODE_TXRX_TEST5);
...
void OLED_loop()
{
  if (u8x8) 
  {
    ...
    if (isTimeToOLED()) 
    {
        ...
        Serial.print("*** OLED_current_page - ");
        Serial.println(OLED_current_page);
...
void OLED_fini(int reason)
{
  if (u8x8) 
  {
    u8x8->clear();
     ...
     // u8x8->draw2x2String(1, 3, reason == FLYRF_SHUTDOWN_LOWBAT ?
...
#if defined(ENABLE_OLED_TEXT_PAGE)
    if (hw_info.display   == DISPLAY_OLED && OLED_current_page == OLED_PAGE_TEXT && (settings->power_save & POWER_SAVE_NORECEIVE) &&
        (hw_info.rf == RF_IC_SX1276 ||
         hw_info.rf == RF_IC_SX1262))
    {
      OLED_current_page = (OLED_current_page + 1) % page_count;
...
#if !defined(EXCLUDE_OLED_BARO_PAGE)
    if (hw_info.display == DISPLAY_OLED &&
