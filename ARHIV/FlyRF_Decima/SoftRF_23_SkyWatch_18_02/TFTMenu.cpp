...
//TFT_eSprite Air_info = TFT_eSprite(&tft);     // Спрайт информации стороннего воздушного объекта
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::setup(TFTMenu* menuManager)
 {
   ...
   for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
   {
       ...
       Air_txt_Sprite[i]->createSprite(50, 50);
       Air_txt_Sprite[i]->setPivot(13, 9);
       Air_txt_Sprite[i]->fillSprite(backColor);      // Закрасим поле 
      // Air_txt_Sprite[i]->drawRect(0,0,50, 50, TFT_RED);

       little_airplane [i] = new TFT_eSprite(&tft);     // Спрайт информации стороннего воздушного объекта
       little_airplane [i]->createSprite(21, 16);
       little_airplane [i]->fillSprite(backColor);      // Закрасим поле 
      // little_airplane [i]->drawRect(0,0,21, 16, TFT_GREEN);
       little_airplane[i]->setPivot(13,9 );
 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
   ...
   //Air_info.fillSprite(backColor);                 // Фон стороннего объекта
...
/***************    TFT_Draw_Radar              *******************/
    ...
    data_KM.setPivot(159, 160);                  // Определяем местоположение спрайта 
 ...
 /*          Serial.print(" altitude - ");
           ...
           x = ((int32_t)rel_x * (int32_t)radius) / divider;
           y = ((int32_t)rel_y * (int32_t)radius) / divider;

           /*Расчет координат сторонних самолетов на вращаюсемся экране с поправкой на вращение*/
           /*Функция проверяет и если надо задает новое значение, так чтобы оно была в области допустимых значений, заданной параметрами.*/
           ...
           Container[i].alien_Y = new_y;
            ...
            Air_txt_Sprite[i]->setTextColor(TFT_YELLOW, TFT_BLACK);
            Air_txt_Sprite[i]->fillSprite(backColor);                // Закрасим поле 
            Air_txt_Sprite[i]->setTextDatum(0);
            Air_txt_Sprite[i]->loadFont(NotoSansBold15);
            //  Air_txt_Sprite[i]->drawRect(0, 0, 60, 30, TFT_RED);
            Air_txt_Sprite[i]->drawCentreString(String(int(altitude)), 15, 18, 1);
            Air_txt_Sprite[i]->drawCentreString(String(int(speed)), 15, 30, 1);
            isTeam_all[i] = true;
           ...
           if (RelativeVertical > TFT_RADAR_V_THRESHOLD) // 
           {
               if (isTeam)
               {
                   ...
                   backsprite.drawTriangle(radar_center_x + x - 6, radar_center_y - y + 5,
                       ...
                       TFT_WHITE);
                   //backsprite.drawCentreString(String(int(altitude)), radar_center_x + x, radar_center_y - y + 12, 2);
               }
               else
               {
           ...
           little_airplane[i]->drawLine(10, 0, 10, 16, little_air_color);
           little_airplane[i]->drawLine(11, 0, 11, 16, little_air_color);

           little_airplane[i]->drawLine(3, 4, 18, 4, little_air_color);
           little_airplane[i]->drawLine(0, 5, 21, 5, little_air_color);

           little_airplane[i]->drawLine(6, 15, 14, 15, little_air_color);
           little_airplane[i]->drawLine(7, 16, 13, 16, little_air_color);

       }
   }

   /* Загрузили все вращающиеся объекты*/
   backsprite.pushToSprite(&back, 0, 0, TFT_BLACK);
   ...
   Airplane.pushToSprite(&back, 144, 110, TFT_BLACK); 
 
  // Air_info.pushToSprite(&back, radar_center_x + new_x - 5, radar_center_y - new_y + 5, TFT_BLACK);
   ...
   for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
   {
       if (isThere_plane[i] == true)
       {
           ...
           {*/
               little_airplane[i]->pushRotated(*Air_txt_Sprite, /*angle_air*/ Container[i].bearing, TFT_BLACK);
               Air_txt_Sprite[i]->pushToSprite(&back, radar_center_x + Container[i].alien_X - 3, radar_center_y - Container[i].alien_Y + 3, TFT_BLACK);
               //little_airplane[i]->setPivot(radar_center_x + Container[i].alien_X, radar_center_y - Container[i].alien_Y);                // Назначаем центр вращения спрайта маленького самолета
               //little_airplane[i]->pushRotated(&back,angle_air /*Container[i].bearing*/, TFT_BLACK);
               //  little_airplane[i]->pushToSprite(&back, radar_center_x + Container[i].alien_X, radar_center_y - Container[i].alien_Y, TFT_BLACK);
               //little_airplane[i]->pushRotated(&back,angle_air /*Container[i].bearing*/, TFT_BLACK);
           ...
           //else
           //{
           //    Air_txt_Sprite[i]->pushToSprite(&back, radar_center_x + Container[i].alien_X - 5, radar_center_y - Container[i].alien_Y + 16, TFT_BLACK);
           //   // little_airplane[i]->setPivot(radar_center_x + Container[i].alien_X, radar_center_y - Container[i].alien_Y);                // Назначаем центр вращения спрайта маленького самолета
           //    //little_airplane[i]->pushRotated(&back, 0/*Container[i].bearing*/, TFT_BLACK);
  ...
  //    // little_airplane [i]->pushToSprite(&back, 5, 25 + (12 * (i + 1)), TFT_BLACK);
  ///*     if (i == 0)
  //     {*/
  //         little_airplane[i]->setPivot(radar_center_x + Container[i].alien_X, radar_center_y - Container[i].alien_Y);                // Назначаем центр вращения спрайта маленького самолета
  //        // little_airplane[0]->pushToSprite(&back, 5, 25 + (12 * (i + 1)), TFT_BLACK);
  //         little_airplane[i]->pushRotated(&back, 0/*Container[i].bearing*/, TFT_BLACK);
  //     //}
