...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::setup(TFTMenu* menuManager)
 {
    ...
    data.createSprite(48, 25);
    data.setTextColor(TFT_YELLOW, backColor);
 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
   ...
   backsprite.drawLine(160, 308, 170, 318, TFT_BLUE);
    ...
    data.setPivot(24, 160);                  // Определяем местоположения данных
    data.fillRect(0, 0, 48, 25, backColor);
    data.drawRect(0, 0, 48, 25, TFT_WHITE);
    data.drawCircle(40, 8, 3, TFT_YELLOW);

    if (360 - angle_air < 10)
    {
        data.drawString(String(360 - angle_air), 23, 3);
    }
    else if ((360 - angle_air > 9) && ((360 - angle_air < 100)))
    {
        data.drawString(String(360 - angle_air), 13, 3);
    }
    else if (360 - angle_air > 99)
    {
        data.drawString(String(360 - angle_air), 3, 3);
    }


    data.pushRotated(&back, 0, backColor);
  ...
  /*Выполняем поворот по азимуту*/
    backsprite.pushRotated(&back, angle_air, TFT_BLACK);
   /*рисуем все спрайты*/
    back.pushSprite(0, 0);
