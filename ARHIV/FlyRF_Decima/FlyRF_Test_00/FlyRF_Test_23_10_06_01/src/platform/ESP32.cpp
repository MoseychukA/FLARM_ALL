...
static int EPD_zoom = 2;// ZOOM_MEDIUM;
...
static bool TFT_tft_frontpage = false;
...
static void ESP32_post_init()
{
  ...
  if (hw_info.model == SOFTRF_MODEL_PRIME_MK3)
  {
    ...
    Serial.flush();
    Serial.print(F("tft  : "));
    Serial.println(hw_info.tft == tft_OLED_1_3 ? F("PASS") : F("FAIL"));
    Serial.flush();
    Serial.print(F("RTC      : "));
...
static byte ESP32_OLED_ident(TwoWire *bus)
{
  ...
  if (bus->available()) {
    ...
    if (r == 0x08 || r == 0x00 || r == 0x0C) {
        rval = tft_OLED_1_3;  // SH1106
    } else if (r == 0x03 || r == 0x04 || r == 0x06 || r == 0x07) {
        rval = tft_OLED_TTGO; // SSD1306
    }
  }
...
#if LV_HOR_RES == 135
    rval = tft_TFT_TTGO_135;
#elif LV_HOR_RES == 80
    rval = tft_TFT_HELTEC_80;
#else
    rval = tft_TFT_TTGO_240;
#endif /* LV_HOR_RES */
#endif /* USE_TFT */
  }
...
#if defined(USE_TFT)
#if LV_HOR_RES == 240
  case tft_TFT_TTGO_240:
    if (tft) 
    {
      if (!TFT_tft_frontpage) 
      {
        tft->fillScreen(TFT_NAVY);
...
//                if (isTeam) 
//                {
//                    tft->drawTriangle(radar_center_x + x - 5, radar_center_y - y + 4,
//                        radar_center_x + x, radar_center_y - y - 6,
//                        radar_center_x + x + 5, radar_center_y - y + 4,
//                        TFT_BLACK);
//                    tft->drawTriangle(radar_center_x + x - 6, radar_center_y - y + 5,
...
//
//        /* little airplane */
//        tft->drawFastVLine(radar_center_x, radar_center_y - 4, 14, TFT_BLACK);
//        tft->drawFastVLine(radar_center_x + 1, radar_center_y - 4, 14, TFT_BLACK);
//
//        tft->drawFastHLine(radar_center_x - 8, radar_center_y, 18, TFT_BLACK);
//        tft->drawFastHLine(radar_center_x - 10, radar_center_y + 1, 22, TFT_BLACK);
//
//        tft->drawFastHLine(radar_center_x - 3, radar_center_y + 8, 8, TFT_BLACK);
//        tft->drawFastHLine(radar_center_x - 2, radar_center_y + 9, 6, TFT_BLACK);
//
//    }
//    else 
//    {
...
//    tft->getTextBounds("0", 0, 0, &tbx, &tby, &tbw, &tbh);
//
//    x = radar_x + tbw / 2;
//    y = radar_y + radar_w - tbh;
//    tft->setCursor(x, y);
//
//    tft->print(Traffic_Count());
//
//   // tft->setFont(&FreeMonoBold9pt7b);
//   // tft->setFont(&Picopixel);
//    tft->getTextBounds("ACFTS", 0, 0, &tbx, &tby, &tbw, &tbh);
//    y += tbh; y += tbh;
//    tft->setCursor(x, y);
//    tft->print("ACFTS");
//
//   // tft->setFont(&FreeMonoBold12pt7b);
//    tft->getTextBounds("00 ", 0, 0, &tbx, &tby, &tbw, &tbh);
//
...
//    if (ui->units == UNITS_METRIC || ui->units == UNITS_MIXED) 
//    {
//        tft->print(EPD_zoom == ZOOM_LOWEST ? "60" :
...
//    else 
//    {
//        tft->print(EPD_zoom == ZOOM_LOWEST ? "30" :
//            EPD_zoom == ZOOM_LOW ? "5 " :
...
//   // tft->setFont(&FreeMonoBold9pt7b);
//   // tft->setFont(&Picopixel);
//    tft->getTextBounds("KM", 0, 0, &tbx, &tby, &tbw, &tbh);
//    x += tbw;
//    y += tbh; y += tbh;
//    tft->setCursor(x, y);
//
//    tft->print(ui->units == UNITS_METRIC || ui->units == UNITS_MIXED ? "KM" : "NM");
// 
//           // tft->tft(true);
//
//}
...
static void ESP32_tft_fini(int reason)
{
  switch (hw_info.tft)
  {
#if defined(USE_OLED)
