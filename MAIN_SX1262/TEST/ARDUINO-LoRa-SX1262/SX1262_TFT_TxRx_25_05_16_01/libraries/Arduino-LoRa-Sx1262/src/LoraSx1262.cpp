...
int LoraSx1262::lora_receive_async(byte* buff, int buffMaxLen) {
 ...
 // (Необязательно) Прочитайте информацию о состоянии пакета с радио.
 // Это такие вещи, как сила радиосигнала, шум и т. д.
 // См. таблицу данных 13.5.3 для получения дополнительной информации
 // Это предоставляет отладочную информацию о пакете, который мы получили
  digitalWrite(SX1262_NSS,0); //Enable radio chip-select
  ...
  rssi       = -((int)spiBuff[2]) / 2;      //"Среднее значение RSSI за последний полученный пакет. Фактическая мощность сигнала –RssiPkt/2 (дБм)"
  snr        =  ((int8_t)spiBuff[3]) / 4;   //SNR возвращается как байт со знаком, поэтому сначала нам нужно выполнить некоторое преобразование
...
int LoraSx1262::lora_receive_rssi() 
{
     return rssi;  //"Среднее значение RSSI за последний полученный пакет. Фактическая мощность сигнала –RssiPkt/2 (дБм)"
}
int LoraSx1262::lora_receive_snr()
{
    return snr;  //SNR (соотношение сигнал/шум), SNR возвращается как байт со знаком, поэтому сначала нам нужно выполнить некоторое преобразование
}
int LoraSx1262::lora_receive_signalRssi()
{
    return signalRssi;  //Return how many bytes we actually read
}
