...
static void *msgHeartbeat()
{
  ...
  /* Status Byte 1 */
  HeartBeat.gnss_pos_valid = 0;//!! isValidFix();
...
static void *msgOwnershipGeometricAltitude(ufo_t *aircraft)
{
   ...
   */
  uint16_t altitude = (int16_t)((aircraft->altitude + aircraft->geoid_separation) *_GPS_FEET_PER_METER / 5);
#else
  /*
...
void GDL90_Export()
{
  ...
  if (settings->gdl90 != GDL90_OFF) {
    ...
    for (int i=0; i < MAX_TRACKING_OBJECTS; i++) 
    {
      ...
      //  if ((ThisAircraft.latitude == 0 && ThisAircraft.longitude == 0) || Container[i].distance < ALARM_ZONE_NONE) 
      //  {
