...
void WiFi_setup()
{
  // Set Hostname.
    ...
    char ssidTest[23];
    snprintf(ssidTest, 23, "Chip ID - %llX", ESP.getEfuseMac());
    Serial.println(ssidTest);

    host_name += String((Settings.ESP32_getChipId() & 0xFFFFFF), HEX);
  ...
  if (WiFi.status() != WL_CONNECTED) 
  {
    ...
    Serial.println(WiFi.softAP(host_name.c_str(), ap_default_psk) ? F("Ready") : F("Failed!"));
