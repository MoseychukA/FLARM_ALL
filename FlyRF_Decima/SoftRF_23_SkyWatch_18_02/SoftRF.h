...
typedef struct UFO {
	...
	uint32_t  ID;                 // address
	uint16_t  Track;              // трек или заголовок
	int16_t   TurnRate;
	uint16_t  GroundSpeed;        // horizontalVelocity Скорость движения по земле
	float     ClimbRate;          // verticalVelocity вертикальная скорость
    ...
    float     course;     /* CoG */
    float     speed;      /* путевая скорость в узлах */
...
extern const uint8_t egm96s_dem[16200] PROGMEM;
