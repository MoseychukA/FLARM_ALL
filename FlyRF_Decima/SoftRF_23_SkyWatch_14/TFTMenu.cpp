...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
  ...
  // needle.drawRect(0, 0, 60, 35, TFT_RED);
   ...
   // if (test_curse/*360 - angle_air*/ < 10)
   // {
   //     data_az.drawString(String(test_curse/*360 - angle_air*/), 23, 3);
   // }
   // else if ((test_curse/*360 - angle_air*/ > 9) && ((test_curse/*360 - angle_air*/ < 100)))
   // {
   //     data_az.drawString(String(test_curse/*360 - angle_air*/), 13, 3);
   // }
   // else if (test_curse/*360 - angle_air*/ > 99)
   // {
   //     data_az.drawString(String(test_curse/*360 - angle_air*/), 3, 3);
   // }
    ...
    if (angle < 10)
    {
        data_az.drawString(String(angle), 23, 3);
    }
    else if ((angle > 9) && ((angle < 100)))
    {
        data_az.drawString(String(angle), 13, 3);
    }
    else if (angle > 99)
    {
        data_az.drawString(String(angle), 3, 3);
    }
 ...
 /*          Serial.print(" altitude - ");
           ...
           else
           {
               ...
               /*    backsprite.drawCircle(radar_center_x + x,
                  ...
                  // backsprite.drawRect(radar_center_x + x, radar_center_y - y, 10, 5, TFT_RED);
                   ...
                   //NotoSansMonoSCB20 NotoSansBold15
                  // backsprite.setFreeFont(MYFONT32);                   // Select the font
                  ...
                  // backsprite.drawCentreString( String(int(altitude)), radar_center_x + x, radar_center_y - y + 12, 2);
                   //backsprite.drawChar(myChar, radar_center_x + x, radar_center_y - y + 12, 1);

                  // backsprite.drawCentreString(String(int(speed)), radar_center_x + x, radar_center_y - y + 25, 2);
                  // backsprite.pushToSprite(&needle, radar_center_x + x, radar_center_y - y + 35);
                  // backsprite.pushToSprite(&needle, 0, 0);


                  /* needle.setPivot(160, 160);
                   needle.drawRect(0,0, 50, 30, TFT_RED); */
       ...
       /*            backsprite.fillTriangle(radar_center_x + x - 4, radar_center_y - y - 3,
           ...
           if (i == 0)
           {
               ...
               int  needleX = ((radar_center_x + x - 160) - (radar_center_x + x - 160) * 2); /*+((x*2) - (x * 4));  */ // (rel_x / 2) + 160;
               int  needleY = ((radar_center_y - y - 160) - (radar_center_y - y - 160) * 2); /*+((y*2) - (y * 4));*/   //(rel_y/2)+160;


               //Serial.print("needleX ");
               //Serial.println(needleX);

               /*Serial.print("  160-needleX ");
               Serial.println(needleX);*/

               //Serial.print("needleY ");
               //Serial.println(needleY);

               //Serial.print(" altitude - ");
               //Serial.println(altitude);

               //int new_x = radar_center_x + (distance/100 /*divider*/) * cos(angle);
               //int new_y = radar_center_y + (distance /100 /*divider*/) * sin(angle);
               ...
               new_rel_x = constrain(distance * sin(radians(angle)), -32768, 32767);
               new_rel_y = constrain(distance * cos(radians(angle)), -32768, 32767);

               int16_t new_x = ((int32_t)new_rel_x * (int32_t)radius) / divider;
               int16_t new_y = ((int32_t)new_rel_y * (int32_t)radius) / divider;
              ...
              // needle.setPivot(radar_center_x + new_x, radar_center_y - new_y+15);

              // needle.setPivot(new_x, new_y);

               needle.setPivot(needleX, needleY);
               needle.setTextDatum(0);
               needle.loadFont(NotoSansBold15);
               needle.setTextColor(TFT_YELLOW, TFT_BLACK);
               needle.drawRect(0,0, 50, 30, TFT_RED);

               needle.drawString(String(int(altitude)),10, 2, 1);
              // needle.drawRect(needleX, needleY, 50, 30, TFT_RED);
               needle.pushRotated(&back, (angle - (angle*2)/2), TFT_BLACK);
           }
       }
   }
