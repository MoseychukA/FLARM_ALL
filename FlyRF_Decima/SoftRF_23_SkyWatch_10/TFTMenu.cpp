...
//TFT_eSprite row1 = TFT_eSprite(&tft);
//TFT_eSprite row2 = TFT_eSprite(&tft);
//TFT_eSprite row3 = TFT_eSprite(&tft);
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
   ...
   //needle.loadFont(NotoSansBold15);            // Загружаем шрифты символов направления света
       ...
       if (Container[i].addr && (now() - Container[i].timestamp) <= TFT_EXPIRATION_TIME)
       {
           ...
           else
           {
...

               {
                  ...
                  // backsprite.pushToSprite(&needle, 50, 35);
                  // back.pushToSprite(&needle, 10, 10);
