...
const char SoftRF_text3[] = "www.decima.ru";
const char SoftRF_text2[] = "flyrf.ru";
...
byte TFT_setup()
{
...

  {
    ...
    tft->setTextFont(2);
    tft->setTextSize(1);
    tbw = tft->textWidth(SoftRF_text2);
    tft->setCursor((tft->width() - tbw-30) / 2, (tft->height() + (tbh*2)) / 2);
    tft->println(SoftRF_text2);
    tbw = tft->textWidth(SoftRF_text3);
    tft->setCursor((tft->width() - tbw-20) / 2, (tft->height() + (tbh * 3)) / 2);
    tft->println(SoftRF_text3);
