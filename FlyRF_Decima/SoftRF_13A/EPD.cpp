...
const char EPD_SoftRF_text1[] = "FlytRF";
...
void EPD_setup()
{
 ...
 // tft_epd->setRotation((3 + ui->rotate) & 0x3); /* 270 deg. is default angle */
  tft_epd->setTextColor(TFT_BLACK);
  tft_epd->setTextWrap(false);

  //tft_epd->setFreeFont(&FreeMonoBold24pt7b);
  tft_epd->print(EPD_SoftRF_text1/*, 0, 0,*//*, &tbx1, &tby1, &tbw1, &tbh1*/);
  //!!tft_epd->getTextBounds(EPD_SoftRF_text3, 0, 0, &tbx3, &tby3, &tbw3, &tbh3);

  //!!tft_epd->setFullWindow();

  //tft_epd->fillScreen(TFT_WHITE);

 // if (hw_info.model == SOFTRF_MODEL_BADGE) {

 //   x = (tft_epd->width()  - tbw1) / 2;
 //   y = (tft_epd->height() + tbh1) / 2 - tbh3;
 ...
 //   x = (tft_epd->width()  - tbw2) / 2;
 //   y = (tft_epd->height() + tbh2) / 2;
 //   tft_epd->setCursor(x, y);
 //   tft_epd->print(EPD_SoftRF_text2);

 //   tft_epd->setFreeFont(&FreeMonoBold24pt7b);

 //   x = (tft_epd->width()  - tbw3) / 2;
 //   y = (tft_epd->height() + tbh3) / 2 + tbh3;
 //   tft_epd->setCursor(x, y);
 ...
 //   //!!tft_epd->getTextBounds(buf, 0, 0, &tbx4, &tby4, &tbw4, &tbh4);
 //   x = (tft_epd->width() - tbw4) / 2;
 //   y = tft_epd->height() - tbh4;
 //   tft_epd->setCursor(x, y);
 //   tft_epd->print(buf);

 // } else {
 //   x = (tft_epd->width()  - tbw1) / 2;
 //   y = (tft_epd->height() + tbh1) / 2;
 //   tft_epd->setCursor(x, y);
 //   tft_epd->print(EPD_SoftRF_text1);
 // }
