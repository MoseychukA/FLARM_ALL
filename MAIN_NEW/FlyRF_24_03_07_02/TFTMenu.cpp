                ...
                if (Container[i].addr)  // Если есть данные стороннего самолета
                {
                 ...
                 //-------------------------------------------------------------------------------------------------
                    ...
                    */
                    ...
                    if (set_table_alien[i].lat != 0 && set_table_alien[i].lon != 0)    /* Если координаты стороннего самолета определены */
                    {
                        ...
                        /* Определяем цвет текстов предупреждения об опастности !! Нужно еще проработать */
                        ...
                        /*  
                            ...
                            Напоминание - alien_altitude_array_countMax[i] = true;  // Флаг готовности данные о высоте стороннего самолета
                        */
                        ...
                        if (alien_altitude_array_countMax[i] && ((int)Container[i].distance < 3000))  //Разрешить выдавать данные о высоте если дистанция ближе 3000 м.
                        {
                            ...
                            if (millis() - tmr_array > 300)  //Исключить мигание стрелки вверх/вниз
                            {
                                tmr_array = millis();
                    ...
                    //============================= Конец обработки данных всех самолетов =============================================


                    /* Настраиваем вывод текста в формуляр  слева на экране скорость и высоту неподвижного стороннего самолета
                       Действительно только для данных, полученных без координат
                       Выводим слева на экране*/
                    if (set_table_alien[i].lat == 0 && set_table_alien[i].lon == 0)      /* Если координаты стороннего самолета не определены */
                    {
                        /* Определение цвета вывода текста */
                        /*Напоминание: VerticalSet - Данные разности высот со знаком +/-*/
                        ...
                        //}
                        //else if (VerticalSet > alarm_height_set && arrow_up_down[i] == 2)   //  Чужой самолет выше расстояния информирования и стрелка вниз
                        //{
                        ...
                        DUMP1090_Sprite[i]->loadFont(NotoSansBold15);                        // Установить шрифт формуляра
                                               ...
                                               /*
                            Действительно для самолетов с неизвестными координатами.
                            Определяем наличие и направление вывода стрелок.
                            Напоминание - alien_altitude_array_countMax[i] = true;  // Флаг готовности данные о высоте стороннего самолета
                        */
                ...
                if (isTeam_all[i] == true)
                {
                    ...
                    if (set_table_alien[i].lat == 0 && set_table_alien[i].lon == 0)
                    {
                      //  arrow[i]->pushToSprite(DUMP1090_Sprite[i], 5, 6, TFT_BLACK);
                        ...
                        if (set_table_alien[i].altitude != 0)
                        {
