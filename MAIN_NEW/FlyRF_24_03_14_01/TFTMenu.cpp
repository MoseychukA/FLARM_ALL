...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 /*       int16_t new_angle;*/
       ...
       // Serial.print("getVbusVoltage:"); Serial.print(PMU.getVbusVoltage()); Serial.println("mV");
        ...
        //float b_voltage = Battery_voltage();
        //Serial.println(b_voltage,2);
        ...
        else
        {
            ...
            for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
            {
                ...
                else
                {
                    ...
                    isTeam_all[i] = false;     // Сторонние самолеты не определены и не зарегистрированы в базе
                }
                esp_task_wdt_reset();
            }
            ...
            /*    for (int i = 0; i < view_alien_count; i++)
                {
                    ...
                    Serial.print(set_table_alien[i].altitude);
                    Serial.print(" speed ");
                    Serial.print(set_table_alien[i].speed);
                    Serial.print(" latitude ");
                    Serial.print(set_table_alien[i].lat);
                    Serial.print(" longitude ");
                    Serial.print(set_table_alien[i].lon);
                    Serial.print(" heading ");
                    Serial.print(set_table_alien[i].heading);
                    Serial.print(" pSignal ");
                    Serial.print(Container[i].pSignal);
                ...
                if (Container[i].addr)  // Если есть данные стороннего самолета
                {
                 ...
                 //================================== Фильтр высоты стороннего самолета =============================================
                    ...
                    */
                    ...
                    {
                        if (millis() - tmr_array[i] > 100 + (DATA_MEASURE_THRESHOLD*4))  //Исключить мигание стрелки вверх/вниз. Необходимо немного времени для изменения высоты самолета
                        {
                            tmr_array[i] = millis();
                    ...
                    if (set_table_alien[i].lat != 0 && set_table_alien[i].lon != 0)   
                    {
                        ...
                        /* Определяем рвсположение формуляра на экране слева или справа*/
                        ...
                        */
                            ...
                            // Формируем изображение летящего объекта с учетом с какого источника были получены данные о координатах
                        ...
                        else if (set_table_alien[i].signal_source == 2) // С учетом данных, полученных с приемника LoRa868
                        {
                            ...
                            LoRa_airplane[i]->drawTriangle(5, 17, 10, 0, 15, 17, little_air_color[i]);
                            area_airplane[i]->fillSprite(TFT_BLACK);      // Закрасим поле 
                        }
          ...
          //============================== Формируем неподвижное базовое изображение на экране =========================================== 
            ...
            /*  отображаем на табло вверху курс в градусах*/
            esp_task_wdt_reset();
            ...
            if (min_distance != 32767)
            {
                ...
                data_az.drawString(String(alient_course0), 37, 14);
            }
            else
            {
                dist_info.drawRect(0, 0, 80, 25, TFT_DARKGREY);
                ...
                data_az.drawString("---", 37, 14);
            }
            esp_task_wdt_reset();

            dist_info.pushToSprite(&back, 1, 1);        // Отображаем табло дистанции
            data_az.pushToSprite(&back, 90, 1);         // Отображаем табло курса
            ...
            for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
            {
                ...
                if (isTeam_all[i] == true)
                {
                    ...
                    if (set_table_alien[i].lat == 0 && set_table_alien[i].lon == 0)
                    {
                        ...
                        //  arrow[i]->pushToSprite(DUMP1090_Sprite[i], 5, 6, TFT_BLACK);
                        ...
                        if (set_table_alien[i].altitude != 0)  // Отобразить информацию если есть данные по высоте
                        {
                            ...
                            if (!screen_side)
                            {
                                DUMP1090_Sprite[i]->pushToSprite(&back, 1, 35 + (25 * i), TFT_BLACK);
                            }
                            else
                            {
                                DUMP1090_Sprite[i]->pushToSprite(&back, 220, 35 + (25 * i), TFT_BLACK);
                            }
                        }
                    }
                    ...
                    if (set_table_alien[i].lat != 0 && set_table_alien[i].lon != 0)
                    {
                        ...
                        if (Air_txt_left[i])  // Рисуем треугольники формуляра, указывающие на чужой самолет
                        {
 ...
 void TFTMenuScreen::draw(TFTMenu* menuManager)
 {
    ...
    dist_info.pushToSprite(&back, 1, 1);
   ...
   data_az.setTextDatum(CR_DATUM);
   data_az.fillSprite(TFT_BLACK);
   data_az.drawRect(0, 0, 48, 25, TFT_DARKGREY);
   data_az.drawCircle(40, 8, 3, TFT_GREEN);    // Рисуем кружок символа градуса
   data_az.setTextColor(TFT_GREEN, backColor);
   data_az.drawString("---", 35, 14);
   data_az.pushToSprite(&back, 90, 1);
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 float TFTMenuScreen::getPowerVoltage() // Контроль напряжения питания .
 {
     return voltage_USB;
 }
 void TFTMenuScreen::setPowerVoltageAkk(float voltage) // Контроль напряжения питания.
 {
     voltage_USB = voltage;
     Serial.print("getVbusVoltage2:"); Serial.print(voltage_USB/1000,2); Serial.println("V");
 }
