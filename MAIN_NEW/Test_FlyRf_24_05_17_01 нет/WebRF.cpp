...
void handleRoot() 
{
    ...
    float vdd = 5.0;// Battery_voltage();
    bool low_voltage = false;// (Battery_voltage() <= Battery_threshold());
  ...
  char str_Vcc[8];
  char str_nav[1];
  char str_nav1 = Settings.getnavSystem();
...
<body>\
 ...
 <table width=100%%>\
  <tr><th align=left>N=GNSS,P=GPS,L=GLONASS,A=Galileo</th><td align=right>%s</td></tr>\
 ...
 <table width=100%%>\
  <tr>\
    <td align=center><input type=button onClick=\"location.href='/about'\" value='Информация'></td>"),
      Settings.ESP32_getChipId() & 0x00FFFFFF, FLYRF_FIRMWARE_VERSION,
       "0",sats, str_lat, str_lon, str_alt
  );
 ...
 <p align=center><h2 align=center>Внимание! Для обновления данных, обновите WEB страницу </h2></p>\
</body>\
</html>")
);
