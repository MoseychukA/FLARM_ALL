...
void NMEA_Export()
{
    ...
    bool has_Fix       = isValidFix() || (settings->mode == SOFTRF_MODE_TXRX_TEST1) || (settings->mode == SOFTRF_MODE_TXRX_TEST2) || (settings->mode == SOFTRF_MODE_TXRX_TEST3);
        ...
        if (Container[i].addr && (this_moment - Container[i].timestamp) <= EXPORT_EXPIRATION_TIME) 
        {
          ...
          if (settings->nmea_l) 
          {
            ...
            if (distance < ALARM_ZONE_NONE) // Отображаем только если дистанция меньше 25 киллометров
            {
              ...
              delay(30);
              snprintf_P(NMEABuffer, sizeof(NMEABuffer),
                  PSTR("$FLYRF,%06X,%d,%8s,%d,%d,%d,%d,%6f,%6f,%d,%X" FLYRF_EXT1_FMT "*"),
                  Container[i].addr,
                  Container[i].Squawk,
                  Container[i].flight,
...
void NMEA_Position()
{
  ...
  if (settings->nmea_g) {
    ...
    if ((settings->mode == SOFTRF_MODE_TXRX_TEST1) || (settings->mode == SOFTRF_MODE_TXRX_TEST2) || (settings->mode == SOFTRF_MODE_TXRX_TEST3))
    {
        /*
