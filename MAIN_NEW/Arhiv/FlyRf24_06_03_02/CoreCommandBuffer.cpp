...
//!!++++++++++++++++++++++++++++++++++++++ Переделать ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      //for (int i = 0; i < 100; i++)
      //{
       //    MAIL_DATA.write(10, 10);
      //}

      //for (int i = 0; i < 100; i++)
      //{
      //    MAIL_DATA.read(i);
      //    uint8_t mail_tmp = MAIL_DATA.read(i);
      ...
      unsigned int cur_adr = (count_message * Number_of_bytes_block);         // получить  адрес текущего сообщения.
      pStream->print(count_message);                                                 //
      delay(50);

      //Сохранить сообщение во внешнюю память
       
      MAIL_DATA.writeByte(cur_adr, 1);                                                             // 1 байт - флаг наличия сообщения. "1" - есть новое сообщение, иначе нет
      MAIL_DATA.writeByte(cur_adr + addr_read_NOT_TRANSMITTED, MESSAGE_NOT_CONFIRMED);             // 1 байт - флаг передачи подтверждения "ОК". "MESSAGE_NOT_CONFIRMED" подтверждение  прочтения НЕ ПЕРЕДАНО      MemWrite(cur_adr + addr_number_this_message, NumberMessage);                             // Записать номер данного сообщения
      MAIL_DATA.writeByte(cur_adr + addr_number_this_message, NumberMessage);                      // Сохранить номер сообщения из центра
      MAIL_DATA.writeBytes(cur_adr + addr_time_this_message, time_msg_tmp, sizeof(time_msg_tmp));  // Записать время соббщения в память по текущему адресу 
      MAIL_DATA.writeBytes(cur_adr + addr_current_message, msg_tmp, sizeof(msg_tmp));              // Записать соббщение в память по текущему адресу 
