...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::setup(TFTMenu* menuManager)
 {
   ...
   for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
   {
       ...
       DUMP1090_Sprite[i]->createSprite(90, 20);
       DUMP1090_Sprite[i]->setPivot(45, 10);
            ...
            */
            ...
            if (view_alien_count >= 1)
            {
                ...
                for (int i = 0; i < view_alien_count; i++)
                {
                 ...
                 /*   Serial.print("alien_X - ");
                    Serial.print(set_table_alien[i].alien_X, 5);
                    Serial.print(" | alien_Y - ");
                    Serial.print(set_table_alien[i].alien_Y, 5);
                    Serial.println("");*/
                   Serial.println("");
                }
            }
            ...
            for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
            {
                if (Container[i].addr)
                {
                    ...
                    if (set_table_alien[i].lat == 0 && set_table_alien[i].lon == 0)
                    {
                        ...
                        DUMP1090_Sprite[i]->setTextColor(TFT_WHITE, backColor);           // Установить цвет 
                       // DUMP1090_Sprite[i]->setTextColor(little_air_color[i], backColor); // Установить цвет согласно программе предупреждения опастности
                        DUMP1090_Sprite[i]->setTextDatum(CC_DATUM);                       // Определим как будет выводится текс
                        ...
                        if (height_difference[i] > 0) // Чужой самолет выше нашего
                        {
                            DUMP1090_Sprite[i]->drawString("+" + dump1090_info_txt[i], 45, 12, 1);
                        }
                        else
                        {
                            // Чужой самолет ниже нашего
            ...
            for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
            {
                if (isTeam_all[i] == true)
                {
                    ...
                    if (set_table_alien[i].lat == 0 && set_table_alien[i].lon == 0)
                    {
                        ...
                        DUMP1090_Sprite[i]->drawSmoothRoundRect(0, 0, 1, 1, 89, 19, TFT_DARKCYAN);
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::ParsePacket(const byte* packet, int packetSize)
 {
     ...
     //fo.timestamp = receivedPacket.timestamp; // ???
     fo.timestamp = now(); // ???
     fo.seen = receivedPacket.seen;

     /*
     SerialOutput.print("\n\nflight=");
     SerialOutput.print(receivedPacket.flight);

     SerialOutput.print("\naddr=");
     SerialOutput.print(receivedPacket.addr,HEX);

     SerialOutput.print("\nlat=");
     SerialOutput.print(receivedPacket.lat);

     SerialOutput.print("\nlon=");
     SerialOutput.print(receivedPacket.lon);

     SerialOutput.print("\naltitude=");
     SerialOutput.print(receivedPacket.altitude);

     SerialOutput.print("\nspeed=");
     SerialOutput.print(receivedPacket.speed);

     SerialOutput.print("\nsignal_source=");
     SerialOutput.print(receivedPacket.signal_source);

     SerialOutput.print("\ntimestamp=");
     SerialOutput.print(receivedPacket.timestamp);

     SerialOutput.print("\nseen=");
     SerialOutput.print(receivedPacket.seen);
