...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::setup(TFTMenu* menuManager)
 {
    ...
    for (int i = 0; i < 3; i++)
    {
        rows[i] = new  TFT_eSprite(&tft);
        rows[i]->createSprite(320, 15);
    }
    ...
    if (millis() - tmr > DATA_MEASURE_THRESHOLD)
    {
        ...
        else
        {
          ...
          //============================== Формируем неподвижное базовое изображение на экране =========================================== 
            ...
            rows[0]->pushToSprite(&back, 1, 29 + (16 * 0), TFT_BLACK);
 ...
 void TFTMenuScreen::drawMessage(TFTMenu* menuManager, int count_message, int count_view)
 {
     ...
     */
     ...
     char str_count_message[70];
     sprintf(str_count_message, "%d %s %s", count_message, time_msg, msg);

     int n = 0;
     rows[n]->loadFont(NotoSansBold15);
     rows[n]->fillSprite(backColor);
     rows[n]->setTextColor(TFT_WHITE, backColor);
     rows[n]->setTextDatum(CL_DATUM);
    // rows[n]->drawRect(0, 0, 319, 14, TFT_DARKGREY);
     rows[n]->drawString(str_count_message, 1, 8);
   //  rows[n]->pushToSprite(&back, 1, 29 + (16 * n), TFT_BLACK);
