...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      if (pressed_button_Retention != -1)
      {
          ...
          if (pressed_button_Retention == BUTTON_MAIL)
          {
            ...
            //  Serial.println("Mem cleared!");
              ...
              for (int i = 0; i < 3; i++)
              {
                 rows[i]->fillSprite(backColor);
                 rows[i]->pushToSprite(&back, 1, 29 + (18 * 0), TFT_BLACK);
              }
              back.pushSprite(0, 0);
 ...
 void TFTMenuScreen::drawMessage(TFTMenu* menuManager, int count_message, int count_view)
 {
    ...
    // sprintf(str_count_message, "%d Test ПРОБА 0123456789 0123456789", count_message);
     sprintf(str_count_message, "%d %s ", count_message, msg);
     int n = 0;
     rows[n]->loadFont(FONT_18);
     rows[n]->fillSprite(backColor);
     rows[n]->setTextColor(TFT_WHITE, backColor);
     rows[n]->setTextDatum(TL_DATUM);
    // rows[n]->drawRect(0, 0, 319, 14, TFT_DARKGREY);
     rows[n]->drawString(str_count_message, 0, 0);
