...
//TFT_eSprite Time_display = TFT_eSprite(&tft);         // Спрайт отображения времени на экране
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::setup(TFTMenu* menuManager)
 {
    ...
    ***************************************************************************************/
    ...
    //Time_display.createSprite(60, 25);
    //Time_display.setTextColor(TFT_WHITE, backColor);
 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
      ...
      if (flags.MailOn)
      {
          ...
          if (millis() - TFTScreen->idleTimer > MAIL_OFF_DELAY) // Убрать отображение сообщения почты
          {
              Allow_flashing = true;                                                    // Разрешить мигание сообщения
              flags.MailOn = false;

              //Time_display.loadFont(NotoSansMonoSCB20);
              //Time_display.fillSprite(TFT_BLACK);
              //Time_display.drawRect(0, 0, 60, 25, TFT_DARKGREY);
              //Time_display.setTextColor(txt_color, backColor);
              //Time_display.setTextDatum(CC_DATUM);
              //Time_display.drawString("-----", 30, 12);   // Рисуем текущее время
              //Time_display.pushToSprite(&back, 260, 1);   // Отображаем текущее время
      ...
      else if (released_button != -1) // Короткое нажатие кнопки
      {
             ...
             // Serial.println("released_button");
              ...
              if (confirmation_OK == MESSAGE_CONFIRMED)                                         // Получен специальный код признака "MESSAGE_CONFIRMED" - означает что подтверждение не отправлено
              {
                 ...
                 // MemWriteChars(Response_message_block_ADDRESS, msg_resp, sizeof(msg_resp));    // Сохраняем в памяти вновь сформированную строку с подтверждениями о прочтении
                  ...
                  rows_mail.fillSprite(backColor);                                               // Удаляем сообщение с экрана
                  rows_mail.pushToSprite(&back, 0, 29, TFT_BLACK);                               // Удаляем сообщение с экрана
   ...
   static uint32_t tmr = millis();
    ...
    if (millis() - tmr > DATA_MEASURE_THRESHOLD)
    {
        ...
        //{
        //    Serial.print("hour - ");
        //    Serial.println(CommandHandler.info.utc.hour);
        //    Serial.print("min - ");
        //    Serial.println(CommandHandler.info.utc.min);

        //}

        if(!fix && (settings->mode != SOFTRF_MODE_TXRX_TEST1) && (settings->mode != SOFTRF_MODE_TXRX_TEST2) && (settings->mode != SOFTRF_MODE_TXRX_TEST3) && (settings->mode != SOFTRF_MODE_TXRX_TEST4)) // Эта проверка не проводится в тестовом режиме
        {
        ...
        else
        {
            ...
            for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
            {
                ...
                else
                {
                    ...
                    //if (Container[i].signal_source != 3)
                    //{
            ...
            if (view_alien_count >= 1)
            {
                ...
                for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
                {
                    if (Container[i].addr)
                    {
                        ...
                        set_table_alien[base_index_tmp].altitude_type = (int)Container[i].altitude_type;
                    ...
                    */
                    ...
                    if (set_table_alien[i].lat != 0.0 && set_table_alien[i].lon != 0.0)   
                    {
                        ...
                        */
                        ...
                        else if (set_table_alien[i].signal_source == 3)  // С учетом данных, полученных с текстовой строки
                        {
                            /*Рисуем маленький самолетик */
          ...
          //============================== Формируем неподвижное базовое изображение на экране =========================================== 
            ...
            //    mail.setTextColor(TFT_WHITE, backColor);
            //    char str_num_mail[3];
            //    sprintf(str_num_mail, "%d ", count_message);
            //    mail.drawString(str_num_mail, 23, 12);                          // Рисуем курс стороннего самолета
            //    mail.pushToSprite(&back, 240, 0);
            //}
            //else
            //{
            //    mail.fillSprite(backColor);
            //    mail.pushToSprite(&back, 240, 0);
            //}
 ...
 void TFTMenuScreen::draw(TFTMenu* menuManager)
 {
    ...
    //power1.pushToSprite(&back, 285, 4, TFT_BLACK);
           ...
           /* Отображаем время прихода сообщения */
   ...
   Airplane.pushToSprite(&back, 148, 150, TFT_BLACK);
    ...
    if (!fix && (settings->mode != SOFTRF_MODE_TXRX_TEST1) && (settings->mode != SOFTRF_MODE_TXRX_TEST2) && (settings->mode != SOFTRF_MODE_TXRX_TEST3) && (settings->mode != SOFTRF_MODE_TXRX_TEST4))
    {
            if (!text_call)
            {
 ...
 void TFTMenuScreen::drawMessage(TFTMenu* menuManager, int count_message, int count_view)
 {
     ...
     */
     ...
     //    Time_display.setTextColor(txt_color, backColor);
     //    Time_display.setTextDatum(CC_DATUM);
     //    Time_display.drawString(String(time_msg), 30, 12);   // Рисуем текущее время
     //    Time_display.pushToSprite(&back, 260, 1);   // Отображаем текущее время
     ...
     //    rows_mail.drawString(msg, 0, 0);
     //}
     //else if (len_Width1 > 32 && len_Width1 <= 66)
     //{
     //    String mystring(msg);
     //    String myString0 = mystring.substring(0, 32);
     //    rows_mail.drawString(myString0, 0, 0);
     //    Serial.println(myString0);
     //    String myString1 = mystring.substring(32, 65);
     //    rows_mail.drawString(myString1, 0, 16);
     //    Serial.println(myString1);
     //}
     //else if (len_Width1 > 66)
     //{
     //    String mystring(msg);
     //    String myString0 = mystring.substring(0, 33);
     //    rows_mail.drawString(myString0, 0, 0);
     //    Serial.println(myString0);
     //    String myString1 = mystring.substring(33, 66);
     //    rows_mail.drawString(myString1, 0, 16);
     //    Serial.println(myString1);
     //    String myString2 = mystring.substring(66, 98);
     //    rows_mail.drawString(myString2, 0, 32);
