        ...
        if (start_setup)
        {
            ...
            if (myButton.isSingleClick())
            {
              //  Serial.println("single");
                MainScreen->setMessageRead = true;
            }
            if (myButton.isDoubleClick())
            {
                if (MainScreen->setClearButton)
                {
                    count_buttton_tmp = 0;
                    MainScreen->setClearButton = false;
                }
                count_buttton_tmp++;
                ...
                MainScreen->count_buttton = count_buttton_tmp;
 /*               Serial.println(MainScreen->count_buttton);*/
            }
            if (myButton.isLongClick())
            {
               // Serial.println("long");
                MainScreen->setClearMessage = true;
            }

            //if (myButton.isClick())
            //{
            //    // Serial.println("click");
            //    SettingsMain.SetButtonOn(true);
            //}
...
void setup()
{
  ...
  case SOFTRF_MODE_TXRX_TEST3:
      ...
      Time_setup();
      break;
  case SOFTRF_MODE_TXRX_TEST4:
      set_air = 4;
      Time_setup();
      break;
  case SOFTRF_MODE_TXRX_TEST5:
      set_air = 5;
      Time_setup();
      break;
  ...
  MemInit();
  SettingsMain.setup();
  SettingsMain.saveVer(ver_soft);  // Сохранить строку с текущей версией.
...
void loop()
{
  ...
  // Сначала выполните общие действия с радиочастотами
  RF_loop();                       // Проверить прием пакета от LoRa
...
void normal()
{
  ...
  if (isValidFix()) 
  {
    ...
    ThisAircraft.speed =  gnss.speed.kmph();
    ...
    if (TxPosUpdMarker == 0 || (millis() - TxPosUpdMarker) > 2000)
    {
        ...
        case 1:
           ...
           //55.958388, 37.243838
            ...
            //    if(test_filtre > 360)
            //        test_filtre = 0;
            //    Serial.print("***** test_filtre ");
            //    Serial.println(test_filtre);
            //test_curse0 += 1.0;
            //if (test_curse0 == 360.0)
            //{
            //    test_curse0 = 0.0;
            //}
            //if (test_filtre == 20)
