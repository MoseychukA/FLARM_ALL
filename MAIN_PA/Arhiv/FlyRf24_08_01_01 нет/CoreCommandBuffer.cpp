...
void CommandHandlerClass::SendTraffic_Msg()
{
    ...
    if (millis() - tmr_msg > 1500)
    {
        ...
        {
           test2coordinat((double)fo.latitude, (double)fo.longitude, (int)1000, (int)fo.course);
        }
    }
 ...
 /*   char buf[1024];
    ...
    //    Serial.println(buf);
       ...
       // printf(buf, "%f\t%f\t%f\n", Degrees(pt2[0]), Degrees(pt2[1]), Degrees(azi2));
        ...
        Serial.println("--------");
        Serial.println(Degrees(pt2[0]),4);
        Serial.println(Degrees(pt2[1]),4);
...
void CommandHandlerClass::test2coordinat(double lat1, double lon1, int d, int brng)
{
  ...
  /*  Serial.print("DDMMSS lon ");
    ...
    lat2 = asin(sin(radians(lat1)) * cos(d / R) + cos(radians(lat1)) * sin(d / R) * cos(radians(brng)));
    // lon2 = lon + atan2(sin(bearing) * sin(dr) * cos(lat), cos(dr) - sin(lat) * sin(lat2))
    lon2 = lon1 + atan2(sin(radians(brng)) * sin(d / R) * cos(radians(lat1)), cos(d / R) - sin(radians(lat1)) * sin(radians(lat2)));
