...
/*
 * Простое назначение уровня тревоги на основе расстояния.
 */
static int8_t Alarm_Distance(ufo_t *this_aircraft, ufo_t *fop)
{
  ...
  { /* no warnings if too high or too low */
...
/*
 * Метод «Наследие» основан на короткой истории двумерных векторов скорости (NS/EW).
 */
...
void ParseData()
{
   ...
   // Serial.println("ParseData");
    ...
    //Serial.print("protocol_decode =");
    //Serial.println((int)protocol_decode,HEX);
