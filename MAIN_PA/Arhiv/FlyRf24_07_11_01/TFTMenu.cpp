...
if (released_button != -1) // Короткое нажатие кнопки очищаем текущее сообщение и отправляем подтверждение о прочтении
{
 ...
 /*                info.utc.hour = gnss.time.hour();
                 ...
                 time_info.drawString(String(gnss.time.hour()) +":"+ String(gnss.time.minute()), 29, 4);
             }
             else
             {
             ...
             //String time_txt = String(min_distance);
             time_info.pushToSprite(&back, 259, 1);        // Отображаем табло дистанции
             ...
             for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
             {
                 ...
                 if (isTeam_all[i] == true)
                 {
                     ...
                     if (set_table_alien[i].lat == 0.0 && set_table_alien[i].lon == 0.0 && set_table_alien[i].altitude != 0 && set_table_alien[i].signal_source == 1)
                     {
