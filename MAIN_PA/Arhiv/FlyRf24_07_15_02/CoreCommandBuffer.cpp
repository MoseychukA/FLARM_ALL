...
void CommandHandlerClass::processCommand(const String& command, Stream* pStream) // выполнение входящей команды
{
  ...
  //                          for (int i = 0; i < strlen(paramPtr); i++)
  //                          {
  //                              SettingsMain.air_msg[i][0]= paramPtr[i];  // Массив сохранения текстовых данных о чужом самолете

  //                             // SettingsMain.air_msg[Number_of_bytes_msg][MAX_TRACKING_OBJECTS];  // Массив сохранения текстовых данных о чужом самолете

  ///*                              Serial.print(paramPtr[i]);
  //                              Serial.print("|");*/
  //                          }

  //                          air_msg_count++;
  //                          if (air_msg_count > MAX_TRACKING_OBJECTS)
  //                              air_msg_count = 0;
  //                         Serial.println(strlen(paramPtr));
...
bool CommandHandlerClass::setFly(const char* param)
{
    ...
    *writePtr = 0;
    ...
    fo.addr = strtoul(writePtr, NULL, 16); // Извлечение адреса из строки
    ...
    *writePtr = 0;
    ...
    strncpy(fo.flight, writePtr, strlen(writePtr));  //strlenПреобразование принятую строку
 //    Serial.println(flight);
    ...
    *writePtr = 0;
    ...
    fo.pressure_altitude = atof(writePtr);
    ...
    *writePtr = 0;
    ...
    fo.aircraft_type = atoi(workBuff); 
    ...
    *writePtr = 0;
    writePtr = workBuff;
    fo.hour_msg = atoi(workBuff);  // Получить часы
   // Serial.println(fo.hour_msg);
    ...
    *writePtr = 0;
    ...
    fo.min_msg = atoi(workBuff);   // Получить минуты
   // Serial.println(fo.min_msg);
    ...
    Traffic_Add(&fo);
   // Пример строки
   //#4#SET#FLY#3BC7C9,1234,SMD6406,2100,2100.0,810.0,20.0,50,56.033575,37.289899,2,10,23
   //#4#SET#FLY#3BC7C7,1234,SMD6406,1500,990.0,810.0,20.0,50,55.933575,37.589899,7,23,01
   //#4#SET#FLY#3BC7C1,1234,SMD6406,340,340.0,210.0,20.0,50,55.833575,37.189899,4,15,46
   //#4#SET#FLY#3BC7C1,1234,SMD6406,340,340.0,210.0,20.0,50,56.088352,36.779844,4,15,46
   //#4#SET#FLY#3BC7C7,1234,SMD6406,1500,990.0,810.0,20.0,50,56.200326,36.473858,7,23,01
   //56.088352, 36.779844 32km
   //56.200326, 36.473858 55km 
    return true;
...
////--------------------------------------------------------------------------------------------------------------------------------------
////repeat message
//void CommandHandlerClass::repeat_air_message()
//{
//    /*const */char* paramPtr = { 0 };
//
//    for (int i = 0; i < Number_of_bytes_msg; i++)
//    {
//       // SettingsMain.air_msg[i][0] = paramPtr[i];  // Массив сохранения текстовых данных о чужом самолете
//
//        paramPtr[i] = SettingsMain.air_msg[i][0]; // Массив сохранения текстовых данных о чужом самолете
//
//
//        Serial.print(paramPtr[i]);
//        Serial.print("|");
