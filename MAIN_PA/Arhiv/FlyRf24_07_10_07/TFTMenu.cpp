...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
    ...
    //============================== Блок работы с текстовыми сообщениями ===============================================================
     static uint32_t tmr_mail = millis();

     /* Проверяем наличие новой информации */
     if (millis() - tmr_mail > DATA_MAIL_THRESHOLD)  // через сколько миллисекунд проверять почту
     {
         tmr_mail = millis();
    ...
    if (released_button != -1) // Короткое нажатие кнопки очищаем текущее сообщение и отправляем подтверждение о прочтении
    {
        if (released_button == BUTTON_MAIL)
        {
            ...
            if (confirmation_OK == MESSAGE_CONFIRMED)                                         // Получен специальный код признака "MESSAGE_CONFIRMED" - означает что подтверждение не отправлено. Нужно отправить
            {
                ...
                rows_mail.pushToSprite(&back, 0, 0, TFT_BLACK);                                // Удаляем сообщение с экрана
    ...
    if (millis() - tmr > DATA_MEASURE_THRESHOLD)  // Период проверки информации
    {
            tmr = millis();

            int Air_txt_x = 41;              // Расположение текста в формуляре стороннего самолета 
 
        /* Проверяем есть ли данные GPS. Ждем 800 миллисекунд */
        ...
        if(!fix && (settings->mode != SOFTRF_MODE_TXRX_TEST1) && (settings->mode != SOFTRF_MODE_TXRX_TEST2) && (settings->mode != SOFTRF_MODE_TXRX_TEST3) && (settings->mode != SOFTRF_MODE_TXRX_TEST4) && (settings->mode != SOFTRF_MODE_TXRX_TEST5)) // Эта проверка не проводится в тестовом режиме
        {
            ...
            if (millis() - tmr_GNSS > 20000)
            {
                ...
                if (!text_call)
                {
                    ...
                    text_call = true;          // Запретить повторный вывод нового сообщения  о том что нет данных GPS
                }
            }
        }
        else
        {
            text_call = false;                  // Разрешить повторный вывод на экран нового сообщения  о том что нет данных GPS
            ...
            if (view_alien_count >= 1)              // Сторонние самолеты есть, записать их данные
            {
                /*============================== Определим минамальное расстояние до ближайшего стороннего самолета =============================*/
                int min_distance = 32767;    // Запишем максимальное число дистанции для сравнения . Первоначально будем сравнивать

                for (int i = 0; i < MAX_TRACKING_OBJECTS; i++) // Сканируем базу сторонних самолетов для определения минимальной дистанции
                {
                    ...
                    if (Container[i].addr && (now() - Container[i].timestamp) <= TFT_EXPIRATION_TIME)  // Если есть самолет в базе и подошло время обновления данных
                    {
                        ...
                        isTeam_all[i] = true;     // Сторонние самолеты определены и зарегистрированы в базе

                    /* вычисляем минимальное значение дистанции для переключения диапазона просмотра  !!Переделать*/
                    ...
                    else
                    {
                        ...
                        if (Container[i].signal_source != 2) // Не сбрасывать изображение стороннего самолета если  данные введенны со строки.
                        {
                            isTeam_all[i] = false;           // Сторонних самолетов  нет в базе 
                        }
                        else
                        {
                            isTeam_all[i] = true;           // Не сбрасывать изображение стороннего самолета при данных, введенных со строки.
                        }
                   ...
                   /* ============== Сначала определим масштаб вывода изображения ==================================*/
                ...
                //======================== Сканируем базу сторонних самолетов ====================================
                ...
                for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
                {
                    if (Container[i].addr)  // Если есть данные стороннего самолета
                    {
                     ...
                     //=============================== Конец фильтра скорости стороннего самолета ==========================================
                     ...
                     //================================== Фильтр высоты стороннего самолета =============================================
                        ...
                        //================================== Конец фильтра высоты стороннего самолета =========================================================================================
                        ...
                        */
                        ...
                        if (set_table_alien[i].lat != 0.0 && set_table_alien[i].lon != 0.0)
                        {
                            ...
                            */
                            ...
                            if (set_table_alien[i].signal_source == 0) // С учетом данных, полученных с приемника LoRa868
                            {
                            ...
                            else if (set_table_alien[i].signal_source == 1)  // С учетом данных, полученных с приемника DUMP1090
                            {
                            ...
                            else if (set_table_alien[i].signal_source == 2)  // С учетом данных, полученных с текстовой строки
                            {
                                /*Рисуем маленький самолетик */
                        ...
                        if (set_table_alien[i].lat == 0.0 && set_table_alien[i].lon == 0.0 && set_table_alien[i].altitude != 0 && set_table_alien[i].signal_source == 1/* && set_table_alien[i].signal_source != 0 && set_table_alien[i].signal_source != 2*/)
                        {
                            ...
                            */
                            ...
                            if (VerticalSet >= settings->alarm_height)      //  Чужой самолет дальше расстояния информирования
                            {
                                DUMP1090_air_color[i] = TFT_WHITE;
                            }
                            else if (VerticalSet < settings->alarm_height)   //  Чужой самолет ближе расстояния информирования 
                            {
                ...
                if (divider <= 32767)
                {
                    ...
                    {
                    case 1:
                        back.drawString("16000 m", data_KM_x - 3, data_KM_y);
                        break;
                    case 2:
                        back.drawString("10000 m", data_KM_x - 3, data_KM_y);
                        break;
                    case 3:
                        back.drawString("5000 m", data_KM_x - 2, data_KM_y);
                        break;
                    case 4:
                        back.drawString("2000 m", data_KM_x - 2, data_KM_y);
                        break;
                    case 5:
                        back.drawString("1000 m", data_KM_x - 2, data_KM_y);
                        break;
                    case 6:
                    ...
                    case 7:
                        ...
                        break;
                    case 8:
                        back.drawString("100 m", data_KM_x - 1, data_KM_y);
                        break;
                    case 9:
                        back.drawString("50 m", data_KM_x, data_KM_y);
                        break;
                    default:
                        back.drawString("16000 m", data_KM_x - 3, data_KM_y);
                        break;
                ...
                for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
                {
                    ...
                    if (isTeam_all[i] == true)
                    {
                        ...
                        if (set_table_alien[i].lat != 0.0 && set_table_alien[i].lon != 0.0)
                        {
                            ...
                            switch (set_table_alien[i].signal_source) 
                            {
                            case 0:
            ...
            else
            {
              ...
              //  still_image(menuManager, angle, 0); // Формируем неподвижное базовое изображение на экране
                if (rows_mail.created())
                {
                    rows_mail.pushToSprite(&back, 0, 0, TFT_BLACK);
                }
 ...
 //------------------------------------------------------------------------------------------------------------------------------------------------------------------------
     ...
     //============================== Формируем неподвижное базовое изображение на экране =========================================== 
 void TFTMenuScreen::still_image(TFTMenu* menuManager, int angle, int divider_num)
 {
     back.fillSprite(backColor);                   // Закрасим поле 
     backsprite.fillSprite(TFT_BLACK);             // 
     backsprite.setPivot(160, 160);                // Назначаем центр вращения спрайта воздушной обстановки
