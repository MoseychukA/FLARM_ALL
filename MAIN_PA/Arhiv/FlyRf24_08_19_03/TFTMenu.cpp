...
//#include <PinButton.h>
//
//PinButton myButton(BUTTON_MAIL);
 ...
 /* for (size_t i = 0; i < hardwareButtons.size(); i++)
  {
    ...
    //        //}
    //    }
    //    else if (hardwareButtons[i]->isPressed()&& !hardwareButtons[i]->isDoubleClicked())
    //    {
    //        // кликнута кнопка на пине pin
    //        uint8_t pin = hardwareButtons[i]->pinNumber();
    ...
    //    }
    //    else if (hardwareButtons[i]->isClicked() && !hardwareButtons[i]->isDoubleClicked())
    //    {
    //        // кликнута кнопка на пине pin
    //        uint8_t pin = hardwareButtons[i]->pinNumber();
...
// 
    ...
    //        button_Retention_on = !button_Retention_on;
    //    }
    //}
     ...
     if (millis() - tmr > DATA_MEASURE_THRESHOLD)
     {
         ...
         else
         {
            ...
            /* Фильтр скорости нашего самолета.    Нужно выяснить откуда этот пример взят Предполагаю, данные со смещением по массиву*/
             ...
             if (thisAircraft_speed_tmr >= 0 && thisAircraft_speed_tmr < 10)
             {
