...
void Module1090::ParsePacket(const byte* packet, int packetSize)
{
 ...
 /*       Serial.print("sizeof ToDUMP1090 ");
    ...
    char latitude_msg[8];
    char longitude_msg[9];

    //fo.latitude = atof(receivedPacket.lat);               // Широта
    //fo.longitude = receivedPacket.lon;              // Долгота

    memcpy(latitude_msg, receivedPacket.strLat_msg, sizeof(receivedPacket.strLat_msg));
    memcpy(longitude_msg, receivedPacket.strLon_msg, sizeof(receivedPacket.strLon_msg));

  /*  memcpy(fo.latitude, receivedPacket.lat, sizeof(receivedPacket.lat));
    memcpy(fo.longitude, receivedPacket.flight, sizeof(receivedPacket.flight));*/
    fo.latitude = atof(latitude_msg);          // receivedPacket.lat;               // Широта
    fo.longitude = atof(longitude_msg);        // receivedPacket.lon;              // Долгота
