    ...
    if (millis() - tmr > DATA_MEASURE_THRESHOLD)  // Период проверки информации
    {
        ...
        else
        {
            ...
            if (view_alien_count >= 1)      // Сторонние самолеты есть, записать их данные
            {
                        ...
                        */
                        ...
                        if (set_table_alien[i].lat != 0.0 && set_table_alien[i].lon != 0.0)   
                        {
                            ...
                            */
                            ...
                            else if (set_table_alien[i].signal_source == 0) // С учетом данных, полученных с приемника LoRa868
                            {
                        ...
                        if (set_table_alien[i].lat == 0.0 && set_table_alien[i].lon == 0.0 && set_table_alien[i].altitude != 0 && set_table_alien[i].signal_source == 0 && set_table_alien[i].signal_source != 1 && set_table_alien[i].signal_source != 2)
                        {
                ...
                still_image(menuManager, angle, divider_num); // Формируем неподвижное базовое изображение на экране
            }
            else
            {
