...
void CommandHandlerClass::SendTraffic_Msg()
{
    ...
    if (millis() - tmr_msg > 1500)
    {
        ...
        {
           test3coordinat(fo.latitude, fo.longitude, (float)1000.0, fo.course);
        }
    }
}
...
void CommandHandlerClass::test3coordinat(float lat1, float lon1, float dist, float brng)
{
    CurLon = lon1;   //37.243040;
    CurLat = lat1;   // 56.097114;
    Bearing = brng;  // Bearing of travel
    Distance = dist;    // km per update 
