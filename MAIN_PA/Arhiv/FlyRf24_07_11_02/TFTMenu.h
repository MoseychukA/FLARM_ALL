...
typedef struct
{
   ...
   // uint8_t Container_i;                     // Номер самолета в контейнере
    ...
    uint8_t base_alien[MAX_TRACKING_OBJECTS];    // Перечень в базе
    //uint8_t base_index;                      // Порядковый номер в базе
    uint16_t alien_X;                        // Координата X
    uint16_t alien_Y;                        // Координата Y
    uint16_t pressure_altitude;              // Вариант измерения высоты барометр, dump1090
    uint16_t altitude;                       // Высота  геоид
    float lat;                               // 
    float lon;                               // 
