...
void Module1090::ParsePacket(const byte* packet, int packetSize)
{
 ...
 /*       Serial.print("sizeof ToDUMP1090 ");
    ...
    fo.latitude = atof(receivedPacket.strLat_msg);  // Широта
    fo.longitude = atof(receivedPacket.strLon_msg); // Долгота
    ...
    Serial.println("Hex , Squawk, Flight, alt , pres alt, speed, course, vert_rate,   lat   ,    lon   , air_type");
    Serial.println("--------------------------------------------------------------------------------------------");
    snprintf_P(DUMP1090Buffer, sizeof(DUMP1090Buffer),
        PSTR("%06X,%d, %8s,%d,   %d,   %d,    %d,   %d,      %8f, %9f, %d"),
