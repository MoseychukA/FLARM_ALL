...
void NMEA_Export()
{
    ...
    /* Объект с высоким приоритетом (наиболее релевантная цель)*/
    int HP_bearing     = 0;
    int HP_alt_diff    = 0;
    ...
    bool has_Fix       = isValidFix() || (settings->mode == SOFTRF_MODE_TXRX_TEST1) || (settings->mode == SOFTRF_MODE_TXRX_TEST2) || (settings->mode == SOFTRF_MODE_TXRX_TEST3) || (settings->mode == SOFTRF_MODE_TXRX_TEST4) || (settings->mode == SOFTRF_MODE_TXRX_TEST5);
      ...
      for (int i=0; i < MAX_TRACKING_OBJECTS; i++) 
      {
        if (Container[i].addr && (this_moment - Container[i].timestamp) <= EXPORT_EXPIRATION_TIME) 
        {
          ...
          if (settings->nmea_l) 
          {
            ...
            if (distance < ALARM_ZONE_NONE) // Отображаем только если дистанция меньше 25 киллометров
            {
              ...
              /*
                * Если доступен позывной - отправьте его клиенту NMEA.
                * Если нет - сгенерируйте замену позывного,
                * на основе идентификатора протокола и адреса ICAO
               */
              ...
              data_source = (Container[i].protocol == RF_PROTOCOL_ADSB_UAT || Container[i].protocol == RF_PROTOCOL_ADSB_1090) ?
              ...
              delay(30);
              snprintf_P(NMEABuffer, sizeof(NMEABuffer),
                  PSTR("$FLYRF,%06X,%d,%8s,%d,%d,%d,%d,%d,%6f,%6f,%d,%X" FLYRF_EXT1_FMT "*"),
                    ...
                    (int)(Container[i].speed * _GPS_MPS_PER_KNOT), // Скорость
                    (int)Container[i].course,            // Курс в градусах
                    Container[i].vert_rate,              // Скорость подъема или снижения
    ...
    /* Одно предложение PFLAU NMEA является обязательным независимо от статуса приема трафика. */
    if (settings->nmea_l) 
    {
...
void NMEA_Position()
{
  ...
  if (settings->nmea_g) {
    ...
    if ((settings->mode == SOFTRF_MODE_TXRX_TEST1) || (settings->mode == SOFTRF_MODE_TXRX_TEST2) || (settings->mode == SOFTRF_MODE_TXRX_TEST3) || (settings->mode == SOFTRF_MODE_TXRX_TEST4) || (settings->mode == SOFTRF_MODE_TXRX_TEST5))
    {
        /*
        ...
        */
        ...
        info.latitude = ((double)55.958388) * 100.0;
        info.longitude = ((double)37.243838) * 100.0;
    }
    else
    {
