...
void CommandHandlerClass::SendTraffic_Msg()
{
    ...
    if (millis() - tmr_msg > 1500)
    {
        ...
        {
          // test1coordinat(fo.latitude, fo.longitude, (double)1000, (double)fo.course);
           test3coordinat((double)fo.latitude, (double)fo.longitude, (double)1, (double)fo.course);
        }
    }
}
...
int CommandHandlerClass::test1coordinat(float lat1, float lon1, double azi1, double dist)
//int CommandHandlerClass::test1coordinat()
{
    ...
    //DD_DDDDDtoDDMMSS((double)lat1, &DD, &MM, &SS );

    //double lat4 = (DD * 100) + MM + (SS/100);
    //Serial.print("lat4 ");
    //Serial.println(lat4, 4);
    //Serial.println("-----------------------");


    //DD_DDDDDtoDDMMSS(lon1, &DD, &MM, &SS);

    //double lon4 = (DD * 100) + MM + (SS / 100);
    //Serial.print("lon4 ");
    //Serial.println(lon4, 4);
    //Serial.println("-----------------------");
    
        pt1[0] = Radians(lat12);
        pt1[1] = Radians(lon12);
        SphereDirect(pt1, Radians(azi12), dist / A_E, pt2);	    // Решение прямой задачи
       ...
       // printf(buf, "%f\t%f\t%f\n", Degrees(pt2[0]), Degrees(pt2[1]), Degrees(azi2));
        ...
        Serial.println("--------");
        Serial.print(Degrees(pt2[0]),6);
        Serial.print(", ");
        Serial.println(Degrees(pt2[1]),6);
...
void CommandHandlerClass::test2coordinat(double lat1, double lon1, double d, double brng)
{
    dr = d / R;
    Serial.print("lat1 - ");
    Serial.print(lat1, 5);
    Serial.print(" lon1 - ");
    Serial.println(lon1, 4);
    ...
    lat2 = asin(sin(radians(lat4)) * cos(d / R) + cos(radians(lat4)) * sin(d / R) * cos(radians(((int)brng + 180) % 360)));

    // lon2 = lon + atan2(sin(bearing) * sin(dr) * cos(lat), cos(dr) - sin(lat) * sin(lat2))
    lon2 = lon1 + atan2(sin(radians(brng)) * sin(d / R) * cos(radians(lat4)), cos(d / R) - sin(radians(lat4)) * sin(radians(lat2)));

   // lon2 = lon4 + atan2(cos(d / R) - sin(lat4) * sin(lat2), sin(radians((brng + 180) % 360)) * sin(d / R) * cos(lat4));
    //lon2 = lon4 + atan2(cos(d / R) - sin(radians(lat4)) * sin(radians(lat2)), sin(radians((brng + 180) % 360)) * sin(d / R) * cos(radians(lat4)));


   // 
   //// lat2 = asin(sin(lat4) * cos(d / R) + cos(lat4) * sin(d / R) * cos(brng));
   // // lat2 = asin(sin(lat11) * cos(d / R) + cos(lat11) * sin(d / R) * cos((brng + 180) % 360));
   // lat2 = asin(sin(radians(lat4)) * cos(d / R) + cos(radians(lat4)) * sin(d / R) * cos(radians(brng)));
   //// lon2 = lon1 + atan2(cos(d / R) - sin(lat1) * sin(lat2), sin(radians((brng + 180) % 360)) * sin(d / R) * cos(lat1));
   // lon2 = lon1 + atan2(sin(radians(brng)) * sin(d / R) * cos(lat4), cos(d / R) - sin(lat4) * sin(lat2));

 /*   lat2 = asin(sin(lat4) * cos(dr) + cos(lat4) * sin(dr) * cos(bearing))
      lon2 = lon + atan2(sin(bearing) * sin(dr) * cos(lat4), cos(dr) - sin(lat4) * sin(lat2))*/
   ...
   // 56.098675, 37.227161
    ...
    //Serial.print("lat2 rad ");
    //Serial.println(lat2*180/PI, 5);

    Serial.print("lon2 ");
    Serial.println(lon2,5);
    Serial.print("lon2 rad ");
    Serial.println((lon2 * 180 / PI), 5);
 }

void CommandHandlerClass::test3coordinat(double lat1, double lon1, double d, double brng)
{
    CurLon = (float)lon1;   //37.243040;
    CurLat = (float)lat1;   // 56.097114;
    //CurLon = 37.243040;
    //CurLat = 56.097114;
    Bearing = (float)brng;  // Bearing of travel
    Distance = (float)d;    // km per update 
  ...
  //System.out.printf("starting at a Longitude of %f and a Latitude of %f ",CurLon,CurLat); 
    ...
    Serial.println("--------");
    Serial.print(degrees(DestLat), 6);
    Serial.print(", ");
    Serial.println(degrees(DestLon), 6);
...
/*
* 
* Средняя точка 55.958388, 37.243838
...
Для окончательного направления просто возьмите начальное направление от конечной точки
до начальной точки и измените его на обратное с помощью (brng+180)%360.

Формула перевода радианов в градусы: x ⋅ 180° π. Т.е. чтобы перевести угол из радианов в градусы,
нужно значение угла в радианах умножить на 180 и разделить на пи.
Формула перевода из градусов в радианы: x°= x° ⋅ π 180° радиан. Т.е. чтобы перевести угол из градусов в радианы,
нужно значение угла в градусах умножить на пи и разделить на 180.
