...
byte OLED_setup() {
  ...
  if (u8x8) {
    ...
    default:
      ...
      u8x8->drawString   ( 3, 6 + shift_y, FLYTRF_FIRMWARE_VERSION);
...
static void OLED_text()
{
  ...
  bool hasFix = isValidGNSSFix() || (settings->mode == SOFTRF_MODE_TXRX_TEST) || (settings->mode == SOFTRF_MODE_TXRX_TEST1) || (settings->mode == SOFTRF_MODE_TXRX_TEST2) || (settings->mode == SOFTRF_MODE_TXRX_TEST3) || (settings->mode == SOFTRF_MODE_TXRX_TEST4) || (settings->mode == SOFTRF_MODE_TXRX_TEST5);
    ...
    case DISPLAY_OLED_0_49:
      {
        ...
        const char buf[] = FLYTRF_FIRMWARE_VERSION;
