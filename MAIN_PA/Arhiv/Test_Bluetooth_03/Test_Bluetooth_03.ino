/*
Создайте сервер BLE, который, как только мы получим соединение, будет отправлять периодические уведомления.
Служба рекламирует себя как: 6E400001-B5A3-F393-E0A9-E50E24DCCA9E
Имеет характеристику: 6E400002-B5A3-F393-E0A9-E50E24DCCA9E - используется для получения данных с "WRITE"
Имеет характеристику: 6E400003-B5A3-F393-E0A9-E50E24DCCA9E - используется для отправки данных с "NOTIFY"

Схема создания сервера BLE:
1. Создание сервера BLE
2. Создание службы BLE
3. Создание характеристики BLE для службы
4. Создание дескриптора BLE для характеристики
5. Запуск службы.
6. Запуск рекламы.

В этом примере rxValue — это полученные данные (доступные только внутри этой функции).
А txValue — это данные для отправки, в этом примере просто байт увеличивается каждую секунду.
*/
...
float txValue = 0;
const int readPin = 36; // Use GPIO number. See ESP32 board pinouts
const int LED = 4; // Could be different depending on the dev board. I used the DOIT ESP32 dev board.

//std::string rxValue; // Можно также сделать эту переменную глобальной, чтобы получить к ней доступ в loop()

// Для генерации UUID смотрите следующее:
...
class MyCallbacks : public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic* pCharacteristic) {
        ...
        if (rxValue.length() > 0) {
            ...
            // Выполнять действия на основе команды, полученной от приложения
            if (rxValue.find("A") != -1) {
...
void loop()
{
    if (deviceConnected) 
    {
        // Сейчас сфабрикуем какой-нибудь произвольный хлам...
       // txValue = analogRead(readPin) / 3.456; // This could be an actual sensor reading!

        // Давайте преобразуем значение в массив символов:
        char txString[] = "Test a client connection to notify..."; // убедитесь, что этого достаточно

       // dtostrf(txValue, 1, 2, txString); // float_val, min_width, digits_after_decimal, char_buffer

       // pCharacteristic->setValue(&test_count, 1); // Чтобы отправить целочисленное значение
        pCharacteristic->setValue("Hello!"); // Отправка тестового сообщения
        pCharacteristic->setValue(txString);

        pCharacteristic->notify(); // Отправьте значение в приложение!
        Serial.print("*** Sent Value: ");
        Serial.print(txString);
        Serial.println(" ***");

        // Вместо этого вы можете добавить проверки rxValue здесь
        // если вы установите "rxValue" как глобальную переменную вверху!
        // Обратите внимание, что вам придется удалить объявление "std::string"
        // "rxValue" в функции обратного вызова.
    //    if (rxValue.find("A") != -1) { 
