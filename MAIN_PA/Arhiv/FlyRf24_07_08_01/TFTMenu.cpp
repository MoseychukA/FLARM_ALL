 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
    ...
    if (Allow_flashing)
    {
        ...
        if (millis() - tmr_flashing >= previousMillis_msg)
        {
            ...
            if (flashing_on_off == false)
            {
                ...
                rows_mail.fillSprite(TFT_TRANSPARENT);
                rows_mail.pushToSprite(&back, 0, 0, TFT_TRANSPARENT);
   ...
   static uint32_t tmr = millis();
    ...
    if (millis() - tmr > DATA_MEASURE_THRESHOLD)
    {
        ...
        {
            text_call = false;                  // Готов к выводу нового сообщения  о том что нет данных GPS
           ...
           // Получить установки определения уровней предупреждения. Параметры задаются со смартфона и записываются в EEPROM
            ...
            still_image(menuManager, angle, divider_num);
