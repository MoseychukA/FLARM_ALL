...
<option % s value = '%d'>On</option>\
  ...
  (settings->lora_pa == LORA_PA_OFF ? "selected" : ""), LORA_PA_OFF,
  (settings->lora_pa == LORA_PA_ON ? "selected" : ""), LORA_PA_ON
  );
...
void handleInput() {
  ...
  for ( uint8_t i = 0; i < server.args(); i++ ) {
    ...
    } else if (server.argName(i).equals("lora_pa")) {
      settings->lora_pa = server.arg(i).toInt();
...
<tr><th align=left>LoRa PA</th><td align=right>%d</td></tr>\
