...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::setup(TFTMenu* menuManager)
 {
   ...
   for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
   {
       ...
       Air_txt_Sprite[i]->createSprite(70, 14);
 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
     ...
     //    if (millis() - tmr_flashing > DATA_FLASHING_ON && !flashing_on_off)
     //    {
     //        tmr_flashing = millis();
     //        flashing_on_off = true;

     //        //rows_mail.fillSprite(TFT_BLACK);
     //        //rows_mail.pushToSprite(&back, 0, 29, TFT_BLACK);
     //        //back.pushSprite(0, 0);
     //        rows_mail.deleteSprite();
     //    }
     //    if (millis() - tmr_flashing > DATA_FLASHING_OFF && flashing_on_off)
     //    {
     //        tmr_flashing = millis();
     ...
     //if (millis() - TFTScreen->idleTimer > MAIL_OFF_DELAY && flags.MailOn)       // Убрать отображение сообщения почты
     //{
     //    Allow_flashing = false;                                                 // Запретить мигание сообщения
     ...
     //if (pressed_button_Retention != -1) // Долгое нажатие кнопки
     //{
     ...
     //if (released_button != -1) // Короткое нажатие кнопки
     //{
     ...
     //            //back.pushSprite(0, 0);                                                         // Удаляем сообщение с экрана
     //            rows_mail.deleteSprite();
     //        }
     //    }
     //}
...
if (released_button != -1) // Короткое нажатие кнопки очищаем текущее сообщение и отправляем подтверждение о прочтении
{
    if (released_button == BUTTON_MAIL)
    {
        ...
        isAirDel = false;                                                      // Удалить данные по самолетам полученные по сообщению
        count_message = SettingsMain.getCurrentCountMessage();                 // получить номер текущего сообщения 
        flipping_count_message = SettingsMain.getFlippingCountMessage();       // получить номер листания сообщения 
         ...
         else
         {
            ...
            /* Фильтр скорости нашего самолета.    Нужно выяснить откуда этот пример взят Предполагаю, данные со смещением по массиву*/
             ...
             if (view_alien_count >= 1)
             {
                 ...
                 /* Индексация временной базы не равна основной. Количество может быть меньше но не больше */
                 ...
                 for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
                 {
                     if (Container[i].addr)
                     {
                         ...
                         set_table_alien[base_index_tmp].pressure_altitude = (int)Container[i].pressure_altitude;
                         ...
                         set_table_alien[base_index_tmp].distance = (int)Container[i].distance;    // Сравниваем дистанции для определения наименшего расстояния
                         base_index_tmp++;
                         if (base_index_tmp > MAX_TRACKING_OBJECTS)
                         {
                             base_index_tmp = 0;
                         }
                      ...
                      //================================== Фильтр высоты стороннего самолета =============================================
                         ...
                         */
                         ...
                         if (set_table_alien[i].lat != 0.0 && set_table_alien[i].lon != 0.0)
                         {
                             ...
                             Air_txt_Sprite[i]->drawSmoothRoundRect(0, 0, 1, 1, 70, 15, TFT_RED); //!! Только для теста
                             ...
                             if (height_difference[i] > 0) // Чужой самолет выше нашего
                             {
                                 ...
                                 {
                                     Air_txt_Sprite[i]->drawString("+" + String(int(height_difference[i]))+"/"+ Container[i].hour_msg + ":"+ Container[i].min_msg, 33, 1, 0);
                                 }
                                 else
                                 {
                                     Air_txt_Sprite[i]->drawString("+" + String(int(height_difference[i])), 33, 1, 0);
                                 }
                         ...
                         if (set_table_alien[i].lat == 0.0 && set_table_alien[i].lon == 0.0 && set_table_alien[i].altitude != 0 && set_table_alien[i].signal_source == 1/* && set_table_alien[i].signal_source != 2 && set_table_alien[i].signal_source != 3*/)
                         {
