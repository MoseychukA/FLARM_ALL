        ...
        for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
        {
            ...
            if (Container_msg[i].latitude != 0 && Container_msg[i].longitude != 0) // Проверим на всякий случай.
            {
                ...
                { 
                    ...
                    {
                        hour_gps_tmp = (int)gnss.time.hour() * 60;
                        minute_gps_tmp = (int)gnss.time.minute();
                        minute_all = hour_gps_tmp + minute_gps_tmp; // Текущее GPS время в минутах
                    }
                    else
                    {
                        minute_all = (10 * 60) + 20;             // Текущее тестовое время (10-20) в минутах
                    }

                    // Определить не сколько прошло секунд со времени отправки сообщения.
                    minute_msg = (Container_msg[i].hour_msg * 60) + Container_msg[i].min_msg; // Время отправки сообщения в минутах

                    minute_msg_tmp = (minute_all - minute_msg) * 60;   // Разность времени текущего и времени отправки сообщения в секундах
                    Container_msg[i].delay_time_msg = now();           // Записать в базу стороннего самолета текущее время 
                }
                else
                {
                    minute_msg_tmp = now() - Container_msg[i].delay_time_msg;
                    Container_msg[i].delay_time_msg = now();
                }
                 
               // Serial.println(minute_msg_tmp);

                float speed_tmp = (Container_msg[i].speed * 1000) / 3600 *  minute_msg_tmp;
