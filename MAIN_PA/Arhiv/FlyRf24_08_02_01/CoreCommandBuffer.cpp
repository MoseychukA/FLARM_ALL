...
bool CommandHandlerClass::setTXT(const char* commandPassed, CommandParser& parser, Stream* pStream, String textString) // Программа приема текстового сообщения
{
    if (commandPassed)
    {
        ...
        {
            minute_all = (gnss.time.hour() * 60) + gnss.time.minute();
        }
        else
        {
            minute_all = (10 * 60) + 20;
...
void CommandHandlerClass::SendTraffic_Msg()
{
    ...
    if (millis() - tmr_msg > 2000)
    {
        ...
        for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
        {
            ...
            if (Container_msg[i].latitude != 0 && Container_msg[i].longitude != 0)
            {

                // Serial.print(Container_msg[i].timestamp); Serial.print(" / "); Serial.println(now() - Container_msg[i].timestamp);

                float speed_tmp = (Container_msg[i].speed * 1000) / 3600 * 2.0;
