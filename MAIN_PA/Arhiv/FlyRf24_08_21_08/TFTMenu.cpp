...
//int Aircraft_course_filtre[course_array_size];                     // Фильтр курса нашего самолета
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
    ...
    {
        setClearMessage = false;
        clearMail(menuManager); // Удалить все  сообщения
    }
     ...
     if (millis() - tmr > DATA_MEASURE_THRESHOLD)
     {
         ...
         else
         {
            ...
            /* =========== Фильтр скорости нашего самолета. ================== */
             ...
             //float RawSpeed = ThisAircraft.speed;
             //RunningAverageSpeedBuffer[NextRunningAverageSpeed++] = RawSpeed;
             //if (NextRunningAverageSpeed >= 16)
             //{
             //    NextRunningAverageSpeed = 0;
             //}
             //float RunningAverageSpeed = 0;
             //for (int i = 0; i < 16; ++i)
             //{
             //    RunningAverageSpeed += RunningAverageSpeedBuffer[i];
             //}
             //RunningAverageSpeed /= 16;
             //thisAircraft_speed_tmr = (int)ThisAircraft.speed; //RunningAverageSpeed;                 // Данные по скорости нашего самолета после фильтра
             ...
             /*float RawCourse = ThisAircraft.course;
             RunningAverageCourseBuffer[NextRunningAverageCourse++] = RawCourse;
             if (NextRunningAverageCourse >= 16)
             {
                 NextRunningAverageCourse = 0;
             }
             float RunningAverageCourse = 0;
             for (int i = 0; i < 16; ++i)
             {
                 RunningAverageCourse += RunningAverageCourseBuffer[i];
             }
             RunningAverageCourse /= 16;*/
             //angle = (360 - (int)ThisAircraft.course /*RunningAverageCourse*/) % 360;                   // Данные по курсу нашего самолета после фильтра
             ...
             //float RawAltitude = ThisAircraft.altitude;
             //RunningAverageAltitudeBuffer[NextRunningAverageAltitude++] = RawAltitude;
             //if (NextRunningAverageAltitude >= 16)
             //{
             //    NextRunningAverageAltitude = 0;
             //}
             //float RunningAverageAltitude = 0;
             //for (int i = 0; i < 16; ++i)
             //{
             //    RunningAverageAltitude += RunningAverageAltitudeBuffer[i];
             //}
             //RunningAverageAltitude /= 16;
             thisAircraft_altitude_tmr = val_altitude;// (int)ThisAircraft.altitude; //(int)RunningAverageAltitude;  // Данные по высоте нашего самолета после фильтра
            ...
            // time_info.
             ...
             if (divider <= 32767)
             {
                 ...
                 {
                 case 1:
                     back.drawString("32000 m", data_KM_x - 3, data_KM_y);
                     break;
                 case 2:
                     back.drawString("16000 m", data_KM_x - 3, data_KM_y);
                     break;
                 case 3:
                     back.drawString("8000 m", data_KM_x - 2, data_KM_y);
                     break;
                 case 4:
                     back.drawString("4000 m", data_KM_x - 2, data_KM_y);
                     break;
                 case 5:
                     back.drawString("2000 m", data_KM_x - 2, data_KM_y);
                     break;
                 case 6:
                     back.drawString("1000 m", data_KM_x - 1, data_KM_y);
                     break;
                 case 7:
                 ...
                 if (count_buttton != 0)
                 {
                    ...
                    back.drawRect(data_KM_x - 37, data_KM_y-1, 68, 17, TFT_RED);
             ...
             for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
             {
                 ...
                 if (isTeam_all[i] == true)
                 {
                     ...
                     /*if (Container[i].latitude == 0.0 && Container[i].longitude == 0.0 && Container[i].altitude != 0 && Container[i].signal_source == 1)*/
...
// состоящей из букв английского и русского алфавитов, цифр, общепринятых символов...
 ...
 void TFTMenuScreen::clearMail(TFTMenu* menuManager)
 {
     ...
     rusPrinter->print(data_txt2, 42, 50, backColor, TFT_YELLOW);  // Отображаем 
     delay(3000);
