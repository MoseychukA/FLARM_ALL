...
#define DEVICE_MANUFACTURER "FLYRF"             // Имя "Производителя"
//#define DEVICE_NAME         "3BC7C8_BLE"        // Имя устройства
...
uint8_t test_bufer[] = "Test  ПРоба b String(SoC->getChipId() & 0x00FFFFFFU, HEX) ";
...
void setup()
{
  ...
  BT_name += "-";
  BT_name += String((uint32_t)(ESP.getEfuseMac() >> 24), HEX);
  ...
  BLEDevice::init(BT_name.c_str());  // Инициализация девайса 
  //BLEDevice::init(devName.c_str());  // Инициализация девайса 
  pServer = BLEDevice::createServer(); // Создание сервера
...
void loop()
{
    ...
    if (millis() - tmr > 4000)
    {
        ...
        //ESP32_Bluetooth_write(test_bufer, sizeof(test_bufer));
        //
    }
}
