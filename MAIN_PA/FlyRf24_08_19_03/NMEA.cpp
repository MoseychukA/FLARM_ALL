...
void NMEA_loop()
{
  ...
  if (settings->nmea_s && ThisAircraft.pressure_altitude != 0.0 && isTimeToPGRMZ()) 
  {
    ...
    int altitude = constrain((int) (ThisAircraft.pressure_altitude * _GPS_FEET_PER_METER), -1000, 60000);
...
void NMEA_Export()
{
    ...
    bool has_Fix = isValidFix() || (settings->mode == SOFTRF_MODE_TXRX_TEST) || (settings->mode == SOFTRF_MODE_TXRX_TEST1) || (settings->mode == SOFTRF_MODE_TXRX_TEST2) || (settings->mode == SOFTRF_MODE_TXRX_TEST3) || (settings->mode == SOFTRF_MODE_TXRX_TEST4) || (settings->mode == SOFTRF_MODE_TXRX_TEST5);

    if (has_Fix) 
    {
      for (int i=0; i < MAX_TRACKING_OBJECTS; i++) 
      {
        if ((Container[i].addr && (this_moment - Container[i].timestamp) <= EXPORT_EXPIRATION_TIME)/* || Container[i].signal_source == 2*/)
        {
          ...
          if (settings->nmea_l) 
          {
            ...
            if (distance < ALARM_ZONE_NONE) 
            {
              ...
              uint8_t addr_type = Container[i].addr_type > ADDR_TYPE_ANONYMOUS ? ADDR_TYPE_ANONYMOUS : Container[i].addr_type;
              ...
              {
                dtostrf(constrain(Container[i].vs / (_GPS_FEET_PER_METER * 60.0), -65.5, 65.5), 5, 1, str_climb_rate);
              }
              ...
              {
                memcpy(NMEA_Callsign, NMEA_CallSign_Prefix[Container[i].protocol], strlen(NMEA_CallSign_Prefix[Container[i].protocol]));
              ...
              data_source = DATA_SOURCE_FLARM;

              snprintf_P(NMEABuffer, sizeof(NMEABuffer),
                      PSTR("$PFLAA,%d,%d,%d,%d,%d,%06X!%s,%d,,%d,%s,%X" PFLAA_EXT1_FMT "*"),
                      alarm_level,
                      (int) (distance * cos(radians(bearing))),
                      (int) (distance * sin(radians(bearing))),
                      alt_diff,
                      addr_type,
                      Container[i].addr,
                      NMEA_Callsign,
                      (int) Container[i].course,
                      (int) (Container[i].speed /** _GPS_MPS_PER_KNOT*/),
                      ltrim(str_climb_rate), 
              ...
              /* Наиболее близкий трафик рассматривается как цель с наивысшим приоритетом. */
      ...
      if (C_Version.isUpdated()) {
        if (strncmp(C_Version.value(), "RST", 3) == 0) {
          ...
          snprintf_P(psrfc_buf, sizeof(psrfc_buf),
              ...
              settings->nmea_s,     settings->nmea_out,
