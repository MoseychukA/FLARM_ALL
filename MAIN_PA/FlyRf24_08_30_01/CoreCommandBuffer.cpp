...
const char GET_FLY[]              PROGMEM = "FLY";      // Получить параметры бызы сторонних самолетов
const char GET_FLY_M[]            PROGMEM = "fly";      // Получить параметры бызы сторонних самолетов
    ...
    if (commandPassed)
    {
       ...
       // char msg_tmp_all[128] = "";
        ...
        if ((minute_all - minute_msg) <= 10 && minute_msg < minute_all || (minute_all - minute_msg) == 0)
        {
            ...
            {
               // int tmp_msg_len = textString.length();
               // if (tmp_msg_len > 62) tmp_msg_len = 62;
...
void CommandHandlerClass::sendFly_base()
{
    char FLYBuffer[128];
    ...
    for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
    {
        if (Container[i].addr)  // Если есть самолет в базе
        {
            snprintf_P(FLYBuffer, sizeof(FLYBuffer),
                PSTR("$%06X,%06X,%d,%8s,%d,%d,%d,%d,%d,%6f,%6f,%d,%d,%d,%d""*"),
    ...
    else if (settings->gsm_send == GSM_SEND_MINI)
    {
        ...
        for (int i = 0; i < MAX_TRACKING_OBJECTS; i++)
        {
            ...
            if ((Container[i].addr != addr_old[i]) && (Container[i].addr != 0) && (Container[i].latitude != 0) && (Container[i].addr != ThisAircraft.addr))
            {
                ...
                NMEA_add_checksum(FLYBuffer, sizeof(FLYBuffer) - strlen(FLYBuffer)); // Контрольная сумма пакета
                Serial2.print(FLYBuffer);
            }
