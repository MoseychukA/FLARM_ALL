...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::update(TFTMenu* menuManager)
 {
  ...
  //============================== Основной блок вывода воздушной обстановки на экран ========================================================
     ...
     if (millis() - tmr > DATA_MEASURE_THRESHOLD)
     {
         ...
         if (!fix_tmp && (settings->mode != SOFTRF_MODE_TXRX_TEST1) && (settings->mode != SOFTRF_MODE_TXRX_TEST2) && (settings->mode != SOFTRF_MODE_TXRX_TEST3) && (settings->mode != SOFTRF_MODE_TXRX_TEST4) && (settings->mode != SOFTRF_MODE_TXRX_TEST5)) // Эта проверка не проводится в тестовом режиме
         {
 ...
 void TFTMenuScreen::Draw_circular_scale()
 {
     ...
     for (int i = 0; i < 36; i++)
     {
         ...
         if (i % 3 == 0)
         {
             backsprite.drawWedgeLine(fx[i * 10], fy[i * 10], px[i * 10], py[i * 10], 1, 1, color2);
             backsprite.setTextColor(TFT_WHITE, TFT_BLACK);
             if (i == 0)
             {
         ...
         {
             backsprite.drawWedgeLine(fx[i * 10], fy[i * 10], px1[i * 10], py1[i * 10], 1, 1, color2);
         }
     }
     esp_task_wdt_reset();
...
// состоящей из букв английского и русского алфавитов, цифр, общепринятых символов...
 ...
 /*    for (int i = 0; i < (Max_Count_Block_Message * Number_of_bytes_block) + 400; i++)
     ...
     rusPrinter->print("СООБЩЕНИЯ УДАЛЕНЫ", 42, 50, backColor, TFT_YELLOW);  // Отображаем 
     delay(3000);
 ...
 void TFTMenuScreen::waiting_txt(TFTMenu* menuManager) // Вывод текста "ОПРЕДЕЛЕНИЕ МЕСТОПОЛОЖЕНИЯ"
 {
     ...
     rusPrinter->print(data_txt1, curX, curY, backColor, TFT_YELLOW);       // Отображаем 
 }
